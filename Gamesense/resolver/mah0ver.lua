...
local enable_resolver = ui.new_checkbox("RAGE", "Aimbot", "Enable Advanced Resolver")
...
client.set_event_callback("paint", function()
    renderer.text(10, 10, 255, 0, 0, 255, "+", 0, "Resolver Status: " .. (ui.get(enable_resolver) and "Enabled" or "Disabled"))
    local players = entity.get_players(true)
    if #players > 0 then
        for _, player_index in ipairs(players) do
            if player_index and body_yaw_data[player_index] then
                renderer.text(10, 30, 255, 255, 255, 255, "+", 0, "Player Yaw: " .. tostring(body_yaw_data[player_index]))
                visualize_prediction(player_index)
            else
                renderer.text(10, 30, 255, 255, 255, 255, "+", 0, "Player Yaw: N/A")
            end
        end
    end
end)
