...
local old_font = surface.create_font("Verdana", 26, 700, {0x010, 0x080})
Meta.data = {
	Flip = false,
	Notifys = {},
	SpinYaw = 0,
...
Meta.elements = {
				...
				for index, data in pairs(Meta.data.PlayerState) do
					elements[idx][index] = {
						...
						left = {
							...
							body_yaw_tick = ui.new_slider("AA", "Anti-aimbot angles", ("\n %s %s Body Tick Switch L"):format(index, idx), 2, 64, 8),
						...
						right = {
							...
							body_yaw_tick = ui.new_slider("AA", "Anti-aimbot angles", ("\n %s %s Body Tick Switch R"):format(index, idx), 2, 64, 8),
	...
	Visuals = {
		...
		ManualIndication = ui.new_combobox("AA", "Anti-aimbot angles", "\aFF81F0FF\aFFFFFFCEManual arrows style", {"Off", "❮ ❯", "⮜ ⮞", "⯇ ⯈"}),
		...
		ManualScopeLowest = ui.new_checkbox("AA", "Anti-aimbot angles", "\aFF81F0FF\aFFFFFFCEManual scope lowest"),
		...
		Hitlog = ui.new_combobox("AA", "Anti-aimbot angles", "\aFF81F0FF\aFFFFFFCEHitlog notify style", {"Off", "1", "2"}),
		...
		h = ui.new_checkbox("AA", "Anti-aimbot angles", "\aB9B7F1FF⚠ \aFFFFFFCEPOSE EDITAR"),
		...
		versionText = ui.new_label("AA", "Fake lag", "Script version color"),
		versionColor = ui.new_color_picker("AA", "Fake lag", "\nscript version color", 255, 255, 255, 255),
	...
	Misc = {
		...
		leg_mode = ui.new_combobox("AA", "Anti-aimbot angles", "\aFF81F0FF\aFFFFFFCEMichael \aB9B7F1FFjackson", {"1.0", "2.0"}),
		...
		defensive_aa = ui.new_checkbox("AA", "Other", "Defensive AntiAim"),
		defensive_conditions = ui.new_multiselect("AA", "Other", "Defensive Conditions", {"On Jump", "On Drop", "Only Hidden"}),
		defensive_pitch_mode = ui.new_combobox("AA", "Other", "Defensive Pitch", {"Off", "Tickcount", "Random", "3-Way", "5-Way"}),
		defensive_pitch_ticks = ui.new_slider("AA", "Other", "Defensive Pitch Ticks", 2, 64, 4),
		defensive_pitch_min = ui.new_slider("AA", "Other", "Defensive Pitch Min", - 89, 89, 25),
		defensive_pitch_max = ui.new_slider("AA", "Other", "Defensive Pitch Max", - 89, 89, - 89),
		defensive_yaw_mode = ui.new_combobox("AA", "Other", "Defensive Yaw", {"Off", "Tickcount", "Random", "Spin", "3-Way", "5-Way"}),
		defensive_yaw_ticks = ui.new_slider("AA", "Other", "Defensive Yaw Ticks", 2, 64, 4),
		defensive_yaw_min = ui.new_slider("AA", "Other", "Defensive Yaw Min", - 180, 180, - 25),
		defensive_yaw_max = ui.new_slider("AA", "Other", "Defensive Yaw Max", - 180, 180, 90),
		label = ui.new_label("AA", "Anti-aimbot angles", "check the 'other' panel")
	},
...
Meta.handlereference = function(state)
	for key, data in pairs(Meta.reference) do
		if not Meta.contains({"min_damage_override", "min_damage", "fakelag_limit", "slow_walk", "onshot", "doubletap", "fakelag_amount", "fakelag_variance", "fakelag_reference", "fake_duck", "leg_movement", "force_body", "force_safe"}, key) then
			if type(data) == "table" then
...
Meta.handlemain = function()
	...
	ui.set_visible(Meta.elements.Visuals.BarText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "CrosshairStyle" or ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "Normal")
	ui.set_visible(Meta.elements.Visuals.SafeText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "CrosshairStyle" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "Normal")
	ui.set_visible(Meta.elements.Visuals.DTText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.FDText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.BarColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "Normal" or ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "modern")
	ui.set_visible(Meta.elements.Visuals.OSText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.BaimText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "modern")
	ui.set_visible(Meta.elements.Visuals.SafeColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "modern")
	ui.set_visible(Meta.elements.Visuals.FDColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.DTColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.OSColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.BaimColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "modern")
	ui.set_visible(Meta.elements.Visuals.LegitText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.EdgeText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.NameText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off")
	ui.set_visible(Meta.elements.Visuals.LegitColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.EdgeColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	ui.set_visible(Meta.elements.Visuals.NameColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off")
	ui.set_visible(Meta.elements.Visuals.NameText2, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off")
	ui.set_visible(Meta.elements.Visuals.versionColor, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off")
	ui.set_visible(Meta.elements.Visuals.versionText, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off")
	ui.set_visible(Meta.elements.Visuals.HotkeyAlways, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) == "modern")
	ui.set_visible(Meta.elements.Visuals.NameColor2, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off")
	...
	ui.set_visible(Meta.elements.Visuals.AnimationMode, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Off" and ui.get(Meta.elements.Visuals.CrosshairStyle) ~= "Normal")
	...
	ui.set_visible(Meta.elements.Visuals.ManualScopeLowest, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.ManualIndication) ~= "Off")
	ui.set_visible(Meta.elements.Visuals.ManualLowestSize, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.ManualIndication) ~= "Off" and ui.get(Meta.elements.Visuals.ManualScopeLowest))
	ui.set_visible(Meta.elements.Visuals.ManualScopeSmooth, ui.get(Meta.elements.Group) == "visuals" and ui.get(Meta.elements.Visuals.ManualIndication) ~= "Off" and ui.get(Meta.elements.Visuals.ManualScopeLowest))
	...
	ui.set_visible(Meta.elements.Misc.defensive_aa, ui.get(Meta.elements.Group) == "misc")
	ui.set_visible(Meta.elements.Misc.defensive_conditions, ui.get(Meta.elements.Group) == "misc" and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_pitch_mode, ui.get(Meta.elements.Group) == "misc" and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_yaw_mode, ui.get(Meta.elements.Group) == "misc" and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_pitch_ticks, ui.get(Meta.elements.Group) == "misc" and ui.get(Meta.elements.Misc.defensive_pitch_mode) == "Tickcount" and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_pitch_min, ui.get(Meta.elements.Group) == "misc" and ui.get(Meta.elements.Misc.defensive_pitch_mode) ~= "Off" and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_pitch_max, ui.get(Meta.elements.Group) == "misc" and not Meta.contains({"Off", "3-Way", "5-Way"}, ui.get(Meta.elements.Misc.defensive_pitch_mode)) and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_yaw_ticks, ui.get(Meta.elements.Group) == "misc" and ui.get(Meta.elements.Misc.defensive_yaw_mode) == "Tickcount" and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_yaw_min, ui.get(Meta.elements.Group) == "misc" and ui.get(Meta.elements.Misc.defensive_yaw_mode) ~= "Off" and ui.get(Meta.elements.Misc.defensive_aa))
	ui.set_visible(Meta.elements.Misc.defensive_yaw_max, ui.get(Meta.elements.Group) == "misc" and not Meta.contains({"Off", "Spin", "3-Way", "5-Way"}, ui.get(Meta.elements.Misc.defensive_yaw_mode)) and ui.get(Meta.elements.Misc.defensive_aa))
	...
	ui.set_visible(Meta.elements.AntiAim.OverrideAntiAimBase, ui.get(Meta.elements.Group) == "anti-aim" and ui.get(Meta.elements.AntiAim.Enabled))
	...
	for team, data in pairs(Meta.elements.AntiAim.Custom) do
		for index, data in pairs(data) do
			...
			if ui.get(Meta.elements.AntiAim.OverrideAntiAimBase) then
				ui.set(data.left.yaw, "180")
				ui.set(data.right.yaw, "180")
			end
...
Meta.RunAntiAim = function(e, antiaim_vars)
	local local_player = entity.get_local_player()
	if not local_player or not entity.is_alive(local_player) or not Meta.ShouldAntiAim(e) then
		return
	end
	...
	antiaim_vars.fake_yaw_limit = Meta.clamp(antiaim_vars.fake_yaw_limit, 0, 60)
	if antiaim_vars.pitch == "Down" then
		antiaim_cached.pitch =  89.9
	elseif antiaim_vars.pitch == "Minimal" then
		antiaim_cached.pitch =  89
	elseif antiaim_vars.pitch == "Up" then
		antiaim_cached.pitch = - 89
	elseif antiaim_vars.pitch == "Custom" then
		antiaim_cached.pitch = antiaim_vars.pitch_angles
	elseif antiaim_vars.pitch == "Off" then
		antiaim_cached.pitch = camera_angles.x - 0.00001
	end

	if antiaim_vars.yaw_base == "At targets" and threat_target then
	...
	antiaim_cached.yaw = Meta.normalizedyaw(antiaim_cached.yaw + antiaim_vars.yaw_offset)
	if antiaim_vars.yaw_jitter == "Offset" then
		if Meta.data.Flip then
			antiaim_cached.yaw = Meta.normalizedyaw(antiaim_cached.yaw + antiaim_vars.yaw_jitter_offset)
		end

	elseif antiaim_vars.yaw_jitter == "Center" then
		local jitter_offset = antiaim_vars.yaw_jitter_offset / 2
		antiaim_cached.yaw = Meta.normalizedyaw(antiaim_cached.yaw + (Meta.data.Flip and jitter_offset or - jitter_offset))
	elseif antiaim_vars.yaw_jitter == "Random" then
		local jitter_offset = antiaim_vars.yaw_jitter_offset / 2
		antiaim_cached.yaw = Meta.normalizedyaw(antiaim_cached.yaw + math.random(- jitter_offset, jitter_offset))
	end

	if antiaim_vars.body_yaw == "Opposite" then
		local fake_real_delta = 60
		antiaim_cached.desync_limit = fake_real_delta > 0 and antiaim_vars.fake_yaw_limit or - antiaim_vars.fake_yaw_limit
		antiaim_vars.body_yaw_offset = Meta.clamp(antiaim_vars.body_yaw_offset, - 60, 60)
	elseif antiaim_vars.body_yaw == "Static" then
		antiaim_cached.desync_limit = antiaim_vars.fake_yaw_limit
		antiaim_vars.body_yaw_offset = Meta.clamp(antiaim_vars.body_yaw_offset, - 60, 60)
	elseif antiaim_vars.body_yaw == "Jitter" then
		local jitter_data = Meta.JitterBodyYaw(antiaim_vars.body_yaw_offset, antiaim_vars.fake_yaw_limit)
		antiaim_cached.desync_limit = jitter_data[2]
		antiaim_vars.body_yaw_offset = jitter_data[1]
	end

	if antiaim_vars.body_yaw ~= "Off" then
		local abs_limitation = antiaim_vars.body_yaw_offset > 0 and math.abs(antiaim_cached.desync_limit) or - math.abs(antiaim_cached.desync_limit)
		local abs_desync = (antiaim_vars.body_yaw_offset + abs_limitation) / 2
		Meta.RunDesync(e, antiaim_cached.yaw, abs_desync)
	else
		e.yaw = antiaim_cached.yaw
	end

	e.pitch = antiaim_cached.pitch
	e.roll = antiaim_vars.extended_roll
end
...
Meta.command = function(e)
	...
	elseif ui.get(bindarg.index) == "Single" then
		...
		if bodyyaw_mode == "Jitter" and override_base_antiaim then
		...
		if ui.get(Meta.elements.Misc.defensive_aa) then
			local should_defensive = false
			local origin = vector(entity.get_origin(local_player))
			local defensive_conditions = ui.get(Meta.elements.Misc.defensive_conditions)
			if Meta.contains(defensive_conditions, "On Jump") and Meta.jumping(local_player) then
				should_defensive = true
			end

			if Meta.contains(defensive_conditions, "On Drop") and Meta.data.SyncZOrigin ~= origin.z then
				should_defensive = true
				Meta.data.SyncZOrigin = origin.z
			end

			if Meta.contains(defensive_conditions, "Only Hidden") then
				local m_position = Meta.Extrapolate(vector(entity.hitbox_position(local_player, 1)), 14, local_player)
				for _, ptr in pairs(entity.get_players(true)) do
					if not entity.is_dormant(ptr) and entity.is_alive(ptr) then
						local ent_origin = Meta.GetEyePosition(ptr)
						local _, damage = client.trace_bullet(ptr, ent_origin.x, ent_origin.y, ent_origin.z, m_position.x, m_position.y, m_position.z)
						if damage > 0 then
			...
			if should_defensive then
				...
				local defensive_pitch_mode = ui.get(Meta.elements.Misc.defensive_pitch_mode)
				local defensive_pitch_ticks = ui.get(Meta.elements.Misc.defensive_pitch_ticks)
				local defensive_pitch_min = ui.get(Meta.elements.Misc.defensive_pitch_min)
				local defensive_pitch_max = ui.get(Meta.elements.Misc.defensive_pitch_max)
				local defensive_yaw_mode = ui.get(Meta.elements.Misc.defensive_yaw_mode)
				local defensive_yaw_ticks = ui.get(Meta.elements.Misc.defensive_yaw_ticks)
				local defensive_yaw_min = ui.get(Meta.elements.Misc.defensive_yaw_min)
				local defensive_yaw_max = ui.get(Meta.elements.Misc.defensive_yaw_max)
...
Meta.animation = function()
	...
	if not Meta.data.SwitchLegModified[2] then
		...
		if globals.tickcount() > Meta.data.SwitchLegModified[1] then
			...
			Meta.data.SwitchLegModified[1] = globals.tickcount() + 58
		end
	end
...
Meta.indication = function()
	...
	if ui.get(Meta.elements.Visuals.CrosshairStyle) == "Normal" then
		...
		renderer.text(center.x + scope_size[5], center.y + 19 + (different2 * 17), version_Color[1], version_Color[2], version_Color[3], Meta.clamp(time_pulse, 0, version_Color[4]), "c-", 0, "-DEBUG-")	
		local tickbase_amount = Meta.clamp(tickbase, 0, 11) / 11
		local desync_amount = math.abs(Meta.data.UpdateBodyYaw / 60)
		Meta.textfadeanimation(center.x -1 + scope_size[1], center.y + 22, 1, Meta.tabletocolor(name_color), Meta.tabletocolor(name_color2), "ZYXORIN", true, "", "c-")
		Meta.roundrect(center.x - 13.5 + scope_size[2], center.y + 27, 31, 5, 2, 0, 0, 0, 115)
		if Meta.data.UpdateBodyYaw > 0 then
			Meta.roundrect(center.x - 14 + scope_size[3], center.y + 28, 15, 3, 2, bar_color[1], bar_color[2], bar_color[3], bar_color[4])
		else
			Meta.roundrect(center.x + 1.5 + scope_size[4], center.y + 28, 15, 3, 2, bar_color[1], bar_color[2], bar_color[3], bar_color[4])
		end
	...
	elseif ui.get(Meta.elements.Visuals.CrosshairStyle) == "Modern" then
		...
		renderer.text(center.x + scope_size[7], center.y + 22 + (different * 17), version_Color[1], version_Color[2], version_Color[3], Meta.clamp(time_pulse, 0, version_Color[4]), "c", 0, "debug")
		local hotkeys = {
	...
	if ui.get(Meta.elements.Visuals.ManualIndication) == "❮ ❯" then
		...
		if Meta.data.ManualSide == 1 then
			surface.draw_text(center.x - manual_dist, center.y - 15 + arrows_lowest, manual_color[1], manual_color[2], manual_color[3], Meta.clamp(time_pulse, 0, manual_color[4]), old_font, "❮")
		elseif Meta.data.ManualSide == 2 then
			surface.draw_text(center.x + manual_dist - 13, center.y - 15 + arrows_lowest, manual_color[1], manual_color[2], manual_color[3], Meta.clamp(time_pulse, 0, manual_color[4]), old_font, "❯")
		end
