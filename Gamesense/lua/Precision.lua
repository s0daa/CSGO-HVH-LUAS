...
local on_run = function(cmd)
    ...
    if (curtime - oldtime > 3) then
        for i=1, #players do
            ...
            if (playersdata[player] ~= nil and playersdata[player].hit ~= nil and playersdata[player].angle1 ~= nil and ui.get(interface.angles)) then
            ...
            elseif (playersdata[player] == nil or playersdata[player].hit == nil or playersdata[player].angle1 == nil) then
                plist.set(player, 'Force body yaw', true)
                plist.set(player, "Force body yaw value", newAngle)
                plist.set(player, 'Override safe point', client.random_int(0, 1) == 0 and '-' or 'On')
            end
        end 
...
local on_aim_miss = function(shot)
    ...
    if (ui.get(interface.fallback.enabled)) then
        if (ui.get(interface.fallback.count) >= misseslmao) then
            plist.set(shot.target, 'Force body yaw', false)
            plist.set(shot.target, 'Force body yaw value', 0)
            plist.set(shot.target, 'Override safe point', '-')
        end
...
local on_paint_ui = function()
    ...
    SetTableVisibility({interface.options, interface.fallback.enabled, interface.fallback.count, interface.extrap, interface.angles, interface.flags, interface.flag, interface.clantag, interface.killsay}, ui.get(interface.enabled))
...
local on_setup = function() 
    if not (ui.get(interface.antiaim.FreeStandOnKey)) then
        ui.set(ui.reference("AA", "Anti-aimbot angles", "Freestanding"), {})
    end
