...
math.floor, math.fmod, math.frexp, math.ldexp, string.reverse, table.insert, ipairs, print, string.byte, table.remove,
    type, error, pairs, assert, string.sub, table.concat
...
local function strary_append_int16(n, h)
    ...
    table_insert(strary, tostr(h, math_floor(n / 256), n % 256))
end
local function strary_append_int32(n, h)
    if n < 0 then n = n + 4294967296 end
    table_insert(strary, tostr(h,
        math_floor(n / 16777216),
...
local strary_append_double = function(n)
    ...
    table_insert(strary, tostr(0xcb))
    table_insert(strary, string_reverse(b)) -- reverse: make big endian double precision
end
...
doubleto8bytes = function(x)
    local function grab_byte(v)
        return math_floor(v / 256), tostr(math_fmod(math_floor(v), 256))
    end
    local sign = 0
    if x < 0 then
        sign = 1; x = -x
...
local function bytestobits(ary)
    ...
    for i, v in ipairs(ary) do
        for j = 0, 7, 1 do
            table_insert(out, band(rshift(v, 7 - j), 1))
        end
    end
    return out
end
...
local function bytestodouble(v)
    ...
    local sign = math_floor(v:byte(8) / 128)
    ...
    for i = 1, 4 do table_remove(bits, 1) end
    ...
    local real = math_ldexp(1 + frac, exp)
...
packers["nil"] = function(data)
    table_insert(strary, tostr(0xc0))
end

packers.boolean = function(data)
    if data then -- pack true
        table_insert(strary, tostr(0xc3))
    else      -- pack false
        table_insert(strary, tostr(0xc2))
    end
end

packers.number = function(n)
    if math_floor(n) == n then -- integer
        if n >= 0 then        -- positive integer
            if n < 128 then   -- positive fixnum
                table_insert(strary, tostr(n))
            elseif n < 256 then -- uint8
                table_insert(strary, tostr(0xcc, n))
            ...
            if n >= -32 then    -- negative fixnum
                table_insert(strary, tostr(0xe0 + ((n + 256) % 32)))
            elseif n >= -128 then -- int8
                table_insert(strary, tostr(0xd0, byte_mod(n, 0x100)))
...
packers.string = function(data)
    local n = #data
    if n < 32 then
        table_insert(strary, tostr(0xa0 + n))
    elseif n < 65536 then
...
packers.table = function(data)
    ...
    if is_map then -- pack as map
        if ndata < 16 then
            table_insert(strary, tostr(0x80 + ndata))
        ...
        if nmax < 16 then
            table_insert(strary, tostr(0x90 + nmax))
...
local unpack_number = function(offset, ntype, nlen)
    ...
    if nlen >= 2 then
        b1, b2 = string_byte(strbuf, offset + 1, offset + 2)
    end
    if nlen >= 4 then
        b3, b4 = string_byte(strbuf, offset + 3, offset + 4)
    end
    if nlen >= 8 then
        b5, b6, b7, b8 = string_byte(strbuf, offset + 5, offset + 8)
    end
...
local function unpacker_number(offset)
    local obj_type = type_for(string_byte(strbuf, offset + 1, offset + 1))
...
unpackers.fixnum_posi = function(offset)
    return offset + 1, string_byte(strbuf, offset + 1, offset + 1)
end

unpackers.uint8 = function(offset)
    return offset + 2, string_byte(strbuf, offset + 2, offset + 2)
end
...
unpackers.fixnum_neg = function(offset)
    ...
    local n = string_byte(strbuf, offset + 1, offset + 1)
    local nn = (256 - n) * -1
    return offset + 1, nn
end

unpackers.int8 = function(offset)
    local i = string_byte(strbuf, offset + 2, offset + 2)
    if i > 127 then
        i = (256 - i) * -1
    end
    return offset + 2, i
...
unpackers.fixraw = function(offset)
    local n = byte_mod(string_byte(strbuf, offset + 1, offset + 1), 0x1f + 1)
    ...
    if n > 0 then
        b = string_sub(strbuf, offset + 1 + 1, offset + 1 + 1 + n - 1)
    else
        b = ""
    end
    return offset + n + 1, b
...
unpackers.raw16 = function(offset)
    ...
    local b = string_sub(strbuf, offset + 1 + 1 + 2, offset + 1 + 1 + 2 + n - 1)
    return offset + n + 3, b
end
...
unpackers.raw32 = function(offset)
    ...
    local b = string_sub(strbuf, offset + 1 + 1 + 4, offset + 1 + 1 + 4 + n - 1)
    return offset + n + 5, b
end

unpackers.fixarray = function(offset)
    return unpack_array(offset + 1, byte_mod(string_byte(strbuf, offset + 1, offset + 1), 0x0f + 1))
end
...
unpackers.fixmap = function(offset)
    return unpack_map(offset + 1, byte_mod(string_byte(strbuf, offset + 1, offset + 1), 0x0f + 1))
end
...
local ljp_pack = function(data)
    ...
    local s = table_concat(strary, "")
    --                    print("strary len:", #strary, strary[1], s,  string_sub(s,1) )
    return s
end
