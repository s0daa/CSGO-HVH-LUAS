...
-- sdelano na kolenke bez silent aima, no rabotaet lol
...
local aimbot_reference = ui.new_combobox("VISUALS", "Other ESP", "Aim at locations", {"Off", "Legit"})
local aimbot_fov_reference = ui.new_slider("VISUALS", "Other ESP", "\nHelper Aimbot FOV", 0, 400, 400, true, "°", 0.1)
local aimbot_speed_reference = 0
...
local dt_last_bind_state = false
local function manage_dt()
	local double_tap_ref = ui.reference("RAGE", "Aimbot", "Double tap")
	local hide_shots_ref = ui.reference("AA", "Other", "On shot anti-aim")
	local super_toss_ref = ui.reference("MISC", "Miscellaneous", "Super toss")
	local dt_current_bind_state = ui.get(hotkey_reference)
	if dt_current_bind_state and location_selected ~= nil then
		client.delay_call(0.1, function()
			ui.set(double_tap_ref, false)
			ui.set(hide_shots_ref, false)
			ui.set(super_toss_ref, false)
		end)
	end
	if dt_current_bind_state ~= dt_last_bind_state then
        dt_last_bind_state = dt_current_bind_state
		if dt_current_bind_state and location_selected ~= nil then
			ui.set(double_tap_ref, false)
			ui.set(hide_shots_ref, false)
			ui.set(super_toss_ref, false)
		else
			ui.set(double_tap_ref, true)
			ui.set(hide_shots_ref, true)
		end
    end
end
...
local function update_basic_ui()
	...
	if enabled then
		...
		client.set_event_callback("run_command", on_run_command, manage_dt)
		...
		client.set_event_callback("run_command", manage_dt)
