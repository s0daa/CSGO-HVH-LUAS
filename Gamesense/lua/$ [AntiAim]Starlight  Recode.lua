...
function aa_functions:anti_zeus()
    ...
    if ui.get(new_menu_references["aa"].anti_ze)then
        local players = entity.get_players(true)
        ...
        for i = 1, #players do
            ...
            if entity.get_classname(weapon) == "CWeaponTaser" and distance <= ui.get(new_menu_references["aa"].zeus_distance) then
                if ui.get(new_menu_references["aa"].anti_zeus) == "Pull pistol" then
                    client.exec("slot2")
                elseif
                    ui.get(new_menu_references["aa"].anti_zeus) == "Pull zeus" and entity.get_classname(selfweapon) ~= "CWeaponTaser"
                 then
                    client.exec("slot3")
                end
            end
...
function visual_functions:indicators()
    ...
    renderer.text(w / 2 + math.ceil(text_anim[1])-1,h / 2 + 24,indicator_color[1],indicator_color[2],indicator_color[3],indicator_color[4],"c-",0,"STARLIGHT")
    ...
    local text = {"STARLIGHT","DT","HS","BAIM"}
    for i = 1,#text do
...
local function new_menu_visibility()
   ...
   -- ui.set_visible(new_menu_references["aa"].aa_label,ui_menu.selected_tab == 1)
    ...
    ui.set_visible(new_menu_references["aa"].anti_ze, ui_menu.selected_tab == 2 and main_page)
    ui.set_visible(new_menu_references["aa"].anti_zeus, ui_menu.selected_tab == 2 and main_page and ui.get(new_menu_references["aa"].anti_ze))
    ui.set_visible(new_menu_references["aa"].zeus_distance, ui_menu.selected_tab == 2 and main_page and ui.get(new_menu_references["aa"].anti_ze))
