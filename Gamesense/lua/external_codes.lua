...
client.color_log(lua_color.r, lua_color.g, lua_color.b, " external.codes")
    ...
    while #stack > 0 do
        ...
        for key, value in pairs(current_tbl) do
            ...
            if type(value) == "table" then
                ...
                -- ÐÑÐ¸Ð¼ÐµÐ½ÑÐµÐ¼ ui.set_visible Ðº ÐºÐ°Ð¶Ð´Ð¾Ð¼Ñ ÑÐ»ÐµÐ¼ÐµÐ½ÑÑ
                ui.set_visible(value, true) -- ÐÐ´ÐµÑÑ Ð¼Ð¾Ð¶Ð½Ð¾ Ð¸Ð·Ð¼ÐµÐ½Ð¸ÑÑ Ð¿Ð°ÑÐ°Ð¼ÐµÑÑ Ð²Ð¸Ð´Ð¸Ð¼Ð¾ÑÑÐ¸ Ð½Ð° Ð½ÑÐ¶Ð½Ð¾Ðµ Ð·Ð½Ð°ÑÐµÐ½Ð¸Ðµ
            end
        end
    end
end
    ...
    while #stack > 0 do
        ...
        for key, value in pairs(current_tbl) do
            ...
            if type(value) == "table" then
                ...
                -- ÐÑÐ¸Ð¼ÐµÐ½ÑÐµÐ¼ ui.set_visible Ðº ÐºÐ°Ð¶Ð´Ð¾Ð¼Ñ ÑÐ»ÐµÐ¼ÐµÐ½ÑÑ
                ui.set_visible(value, false) -- ÐÐ´ÐµÑÑ Ð¼Ð¾Ð¶Ð½Ð¾ Ð¸Ð·Ð¼ÐµÐ½Ð¸ÑÑ Ð¿Ð°ÑÐ°Ð¼ÐµÑÑ Ð²Ð¸Ð´Ð¸Ð¼Ð¾ÑÑÐ¸ Ð½Ð° Ð½ÑÐ¶Ð½Ð¾Ðµ Ð·Ð½Ð°ÑÐµÐ½Ð¸Ðµ
            end
        end
    end
end
...
local label = ui.new_label("AA", "Fake lag", "\a659F86FFexternal.codes")
...
local menu = {
    ...
    configTab = {
        ...
        label1 = ui.new_label("AA", "Fake lag", "Welcome back \a659F86FF user"),
        label2 = ui.new_label("AA", "Fake lag", "Last update was \a659F86FF 6 july."),
        label3 = ui.new_label("AA", "Fake lag", "Your build is \a659F86FFBeta."),
        ...
        buttonsd = ui.new_button("AA", "Other", "Join us", function() SteamOverlayAPI.OpenExternalBrowserURL("https://discord.gg/FYcbcS9tY3") end),
...
local notifications = {
    ...
    render = function()
        ...
        for i = 1, #data do
            ...
            --renderer.text(x/2 + strw2/2, y - 100 - Offset, 255, 255, 255, 255 * fraction, "c", 0, string "$external.codes", r, g, b, a) * fraction  
        end
...
local function clantag_set()
    local lua_name = "external.codes.gs"
    if ui.get(menu.miscTab.clanTag) then
        ...
        if entity.get_prop(entity.get_game_rules(), "m_gamePhase") == 5 then
            clan_tag = clantag_anim('external.codes.gs', {10})
...
chat_spammer.phrases = {
    kill = {
        {"cjcb [eq ndfhm", "ÑÐ¾ÑÐ¸ ÑÑÐ¹ ÑÐ²Ð°ÑÑ"},
        {"ð¤ð¥ððð ð¥ðð ððð¤ð¥ ð¨ðð¥ð External.codes"},
        {"ÐºÐ°Ðº Ñ ÑÐµÐ±Ñ ÑÑÐ°ÑÐ½ÑÐ»", "ÐºÑÑÑÑÑÐºÐ°", "Ð»Ð¸Ð²Ð°Ð¹ Ñ Ð¿Ð¾Ð·Ð¾ÑÐ¾Ð¼"},
        {"Ñ Ð¾Ð¿ÑÑÑ ÑÐµÐ±Ñ Ð²ÑÐµÐ±Ð°Ð»", "Ñ Ð¼ÐµÐ½Ñ Ð°Ð° Ð°ÑÑÐµÐ½Ð½ÑÐµ", "ÑÑÐ¹ Ð¿Ð¾Ð¿Ð°Ð´ÐµÑÑ"},
        {"Ð´Ð¾Ð»Ð±Ð¾ÐµÐ± Ð¾Ð¿ÑÑÑ Ð¼Ð¸ÑÑÐ½ÑÐ»", "external.codes Ð¿Ð¾Ð¼Ð¾Ð³Ð»Ð°", "Ñ Ð´ÐµÑÐ¾Ð»Ñ Ð¿ÑÐµÑÐµÑÐ¾Ð¼ Ð±ÐµÐ³Ð°Ñ Ð¸ ÐµÐ±Ñ ÑÐ°ÑÐ°Ð°ÑÐ°Ñ"},
        {" ã°302ï¼external.codes°ã"},
        {"ÑÐ¾ÑÑÐ¸ Ð·Ð° Ð·ÑÐ±Ñ", "Ð¸Ð´Ð¸ ÑÐ¾Ð±Ð¸ÑÐ°Ð¹"},
        {"ÑÐ½Ð¾Ð²Ð° ÑÐ±Ð¸Ð» Ð½ÑÐ±Ð°"},
        {"ÑÑÐ½Ð´ÑÑÐµÐº Ð² ÑÐ½Ð´ÐµÐºÑ ÐµÐ´Ñ ÑÐ¾Ð±ÑÐ°Ð» ÑÐ¶Ðµ?", "Ð° ÑÐ¾ Ñ Ð¶ÑÐ°ÑÑ ÑÐ¾ÑÑ"},
        {"f[f[f[f[[f[", "Ð°ÑÐ°ÑÐ°ÑÐ°ÑÐ°ÑÑÐ°", "Ð´Ð¾Ð»Ð±Ð¾ÐµÐ± Ð½Ð°Ð¹Ñ ÐºÑÐ³"},
        {"Ð¢ÐÐ¡Ð¢ Ð ÐÐ¡ÐÐ Ð«ÐÐÐ§ÐÐ external.codes GAMESENSE"},
        {"ÑÐ»ÐµÐ½ÑÐºÑ Ð¿Ð¾ÑÐ¾ÑÐ°Ð»", "Ð¾Ð¿ÑÑÑ", "ÐµÐ±Ð°Ð½Ð°Ñ ÐºÐ¾Ð³Ð´Ð° ÑÐ¶Ðµ Ð¸Ð³ÑÐ°ÑÑ Ð½Ð°ÑÑÐ¸ÑÑÑÑ"},
        {"ððððð¤ððð¤ð ð¨ððð ððð¨ððªð¤ ðð ððððð", "External.codes ð¨ððð ððð¨ððªð¤ ðð ððððð"},
        {"ð¤ððð¥ ðð¦ð ð¦ð¤ðð£ ðððððð"},
        {"ÐÐ¥Ð¥ÐÐÐ¥Ð","Ð¥ÐÐ¥ÐÐ¥ÐÐ¥Ð","ÐµÐ±Ð°ÑÑ ÑÑ Ð½Ð¸ÑÐ¸Ð¹ ÑÑÐµÑÐ¾Ñ"},
        {"stay with us - external.codes"},
        {"Ð Ð Ð Ð Ð Ð Ð Ð« Ð¢ Ð Ð Ð¡ Ð¯ Ð Ð Ð Ð Ð Ð Ð¢ Ð¬"},
        {"1."},
    },

    death = {
        {"t,kfy rjyxtysq", "ÐµÐ±Ð»Ð°Ð½ ÐºÐ¾Ð½ÑÐµÐ½ÑÐ¹", "ÑÑ ÐºÐ°Ðº ÑÐ±Ð¸Ð»", "Ñ Ð½Ð° Ð²Ð°Ð½Ð²ÐµÐµ Ð±ÑÐ»"},
        {"Ð±Ð»ÑÑÑ Ð¾Ð¿ÑÑÑ Ð°Ð½ÑÐµÐ³", "ÐµÐ±ÑÑÐ¸Ð¹ ÑÐµÑÐ²ÐµÑ", "Ñ Ð·Ð°ÐµÐ±Ð°Ð»ÑÑ ÑÐ¶Ðµ"},
        {"ÑÐ¸Ñ Ð¼Ð¸ÑÐ½ÑÐ» ÑÐ½Ð¾Ð²Ð°"},
        {"ÑÑ ÐµÐ±Ð»Ð°Ð½", "ÐºÐ°Ðº ÑÐ°Ðº Ð¸Ð³ÑÐ°ÑÑ Ð¼Ð¾Ð¶Ð½Ð¾", "Ð¿ÑÐ¾ÑÑÐ¾ Ð¿Ð¾Ð±ÐµÐ¶Ð°ÑÑ"},
        {"Ð¸Ð´Ð¸ ÐºÐ¾ÑÐ¾Ð²Ñ Ð´Ð¾Ð¸", "ÑÐ²Ð°ÑÑ ÑÐµÐ»ÑÑÐºÐ°Ñ", "Ð·Ð°ÐµÐ±Ð°Ð»Ð° ÑÐ¶Ðµ ÐºÐ´ Ð¿Ð¾ÑÑÐ¸ÑÑ"},
        {"Ð¸ ÑÑÐ¾ Ð¼ÐµÐ½Ñ ÑÐ±Ð¸Ð»Ð¾", "Ð¿Ð¸Ð·Ð´ÐµÑ"},
        {"Ð±Ð»ÑÑÑ Ð¿Ð¸Ð·Ð´ÐµÑ", "ÑÐ½Ð¾Ð²Ð° Ð´Ð°Ð»Ð±Ð°ÑÐ± ÑÐ±Ð¸Ð»"},
        {"Ð¾Ð½ Ð¶Ðµ", "Ð½Ðµ Ð¿Ð¾Ð½Ð¸Ð¼Ð°ÐµÑ", "ÑÑÐ¾ ÐµÐ¼Ñ Ð¿Ð¾Ð²ÐµÐ·Ð»Ð¾"},
        {"Ð¿Ð¸Ð·Ð´ÐµÑ", "ÑÐ¸ÑÑÐ»Ñ Ð¾Ð±Ð¾ÑÑÐ°Ð»ÑÑ"}
    }
}
