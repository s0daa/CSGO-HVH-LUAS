...
local ctx ; ctx = new_class()
    ...
    :struct 'ui' {
        ...
        execute = function (self)
			...
			self.menu.antiaim.exploit_key = other:checkbox('Lagswitch exploit', 0x00)
			self.menu.antiaim.trigger_speed = other:slider('Timer', 0, 20, 10):depend(self.menu.antiaim.exploit_key)
			...
			self.menu.settings.animations = group:multiselect('Animations', {'Pitch 0 on land', 'Static legs in air', 'Static direction'})
	...
	:struct 'features' {
		fast_ladder = function (cmd)
			local me = entity.get_local_player()
			if entity.get_prop(me, "m_MoveType") ~= 9 or cmd.forwardmove == 0 then return end
			...
			cmd.pitch, cmd.yaw = 89, ctx.helpers:normalize(cmd.yaw + 90)
		end,
		clan_tag = {
			latest = 0,
			list = {
			...
			work = function ()
				local self = ctx.features.clan_tag

				local time = math.floor( globals.curtime() * 3 + .5 )
				local i = time % #self.list + 1

				if i == self.latest then return end
				self.latest = i

				client.set_clan_tag(self.list[i])
			end,
			restore = function ()
				local self = ctx.features.clan_tag
				client.set_clan_tag()
				client.unset_event_callback("net_update_end", self.work)
				client.unset_event_callback("net_update_end", self.restore)
			end
		},
		kill_say = {
			phrases = {
				...
				"𝕪𝕠𝕦 > 𝕀? 𝕟𝕠 𝕠𝕥𝕙𝕖𝕣 𝕨𝕒𝕪",
				"𝕚𝕞 𝕜𝕚𝕝𝕝 𝕪𝕠𝕦 𝕒𝕟𝕕 𝕪𝕠𝕦'𝕣 𝕥𝕣𝕪𝕚𝕟 𝕓𝕦𝕥 𝕚𝕞 𝕒𝕞 𝕟𝕠 𝕤𝕥𝕠𝕡",
				"𝕪𝕠𝕦𝕣'𝕣𝕖 𝕒𝕣𝕖 𝕘𝕠𝕥 𝕕𝕖𝕒𝕕𝕖𝕕 𝕓𝕦𝕥 𝕪𝕠𝕦𝕣 𝕕𝕠𝕟𝕥 𝕜𝕚𝕝𝕝 𝕀",
				"𝕪𝕠𝕦𝕣'𝕒𝕣𝕖 𝕥𝕒𝕝𝕜𝕚𝕟𝕘 𝕞𝕖? 𝕓𝕠𝕥.",
				"𝕨𝕙𝕠 𝕒𝕞 𝕦? 𝕟𝕟",
				"𝕦'𝕒𝕣𝕖 𝕒𝕣𝕖 𝕓𝕠𝕥? 𝕔𝕠𝕣𝕖𝕔𝕥.",
				"𝕒𝕞 𝕟𝕠𝕥 𝕥𝕣𝕪 𝕒𝕟 𝕤𝕥𝕚𝕝 𝕨𝕚𝕟 𝕙𝕙𝕙 𝕪𝕠𝕣 𝕤𝕦𝕔𝕜",
				"𝕪𝕠𝕦𝕣 𝕕𝕖𝕒𝕕 𝕞𝕖? 𝕟𝕠 𝕒𝕞' 𝕚 𝕜𝕚𝕝𝕝𝕖𝕕 𝕪𝕠𝕦𝕣𝕖",
				"𝕦'𝕣𝕖 𝕒𝕣𝕖 𝕥𝕣𝕪 𝕓𝕖 𝕒𝕞 𝕓𝕦𝕥 𝕕𝕠𝕟𝕥 𝕔𝕒𝕟",
				"𝕨𝕙𝕪 𝕞𝕚𝕤𝕤? 𝕓𝕖𝕔𝕒𝕦𝕤𝕖 𝕒𝕞 𝕦𝕤𝕖 𝕝𝕒𝕧𝕖𝕟𝕕𝕖𝕣",
				"𝕘𝕠𝕥 𝕕𝕖𝕒𝕕𝕖𝕕? 𝕧𝕚𝕤𝕚𝕥 𝕝𝕒𝕧𝕖𝕟𝕕𝕖𝕣.𝕡𝕦𝕓",
				"𝕝𝕒𝕧𝕖𝕟𝕕𝕖𝕣 𝔸ℂ𝕋𝕀𝕍𝔸𝕋𝔼𝔻 (◣_◢)",
			},
			work = function (event)
				local me = entity.get_local_player()
				local attacker, threat = client.userid_to_entindex(event.attacker), client.userid_to_entindex(event.userid)

				if attacker == me and threat ~= me then
					local self = ctx.features.kill_say

					local phrase = self.phrases[ math.random(1, #self.phrases) ]

					client.delay_call(client.random_float(.5, 1), client.exec, "say ", phrase)
				end
			end
		},
		animations = {
			...
			pre_render = function ()
				...
				local animstate = ctx.animation_layers:get_animation_state(me)
				if not animstate then return end

				local self = ctx.features.animations
				local vars = ctx.ui.menu.settings.animations

				if vars:get "Pitch 0 on land" and (not self.in_air and animstate.m_bInHitGroundAnimation) then
					entity.set_prop(me, 'm_flPoseParameter', .5, 12)
				end
				if vars:get "Static legs in air" and self.in_air then
					entity.set_prop(me, 'm_flPoseParameter', 1, 6)
				end
				if vars:get "Static direction" and not self.in_air then
		...
		lag_exploit = {
			...
			local keybind = ui.new_hotkey("lua", "b", "keybind")

			local trigger_speed = ui.new_slider("lua", "b", "Timer", 0, 100, 10)]]
			refs = {
				aim_check = pui.reference("rage", "aimbot", "enabled"),
			...
			work = function (cmd)
				...
				local self = ctx.features.lag_exploit

				local timer = entity.get_prop(me, "m_nTickbase") % ctx.ui.menu.antiaim.trigger_speed.value == 0
				local air = bit.band(entity.get_prop(me, "m_fFlags"), 1) == 0

				if air and ctx.ui.menu.antiaim.exploit_key.hotkey:get() then
					self.refs.duckpeek:override(timer and {"Always on", 0} or nil)
					self.refs.aim_check:set(false)

					cmd.in_duck = not timer
				else
					self.refs.aim_check:override()
					self.refs.duckpeek:set(true)
				end
			end
		},
		setup = function (self)
			self.ui.menu.settings.fast_ladder:set_event("setup_command", self.fast_ladder)

			self.ui.menu.settings.clan_tag:set_event("net_update_end", self.clan_tag.work)
			self.ui.menu.settings.clan_tag:set_event("net_update_end", self.clan_tag.restore, false)
			defer(self.clan_tag.restore)

			self.ui.menu.settings.kill_say:set_event("player_death", self.kill_say.work)

			self.ui.menu.settings.animations:set_event("pre_render", self.animations.pre_render, function (s) return #s.value > 0 end)
			self.ui.menu.settings.animations:set_event("setup_command", self.animations.setup_command, function (s) return #s.value > 0 end)

			self.ui.menu.antiaim.exploit_key:set_event("setup_command", self.lag_exploit.work)
		end
	}
