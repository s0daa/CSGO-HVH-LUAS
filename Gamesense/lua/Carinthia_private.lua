...
local client_current_threat, entity_get_prop, entity_get_local_player, entity_is_alive, entity_get_player_weapon, entity_get_classname, entity_get_origin, globals_frametime, client_screen_size, globals_framecount, is_menu_open, menu_mouse_position, client_key_state, table_insert, entity_get_steam64, render_circle_outline, entity_get_all, globals_tickinterval, client_set_clantag = client.current_threat, entity.get_prop, entity.get_local_player, entity.is_alive, entity.get_player_weapon, entity.get_classname, entity.get_origin, globals.frametime, client.screen_size, globals.framecount, ui.is_menu_open, ui.mouse_position, client.key_state, table.insert, entity.get_steam64, renderer.circle_outline, entity.get_all, globals.tickinterval, client.set_clan_tag
...
---------------------------------------------
---------------
local aa_config = { 'Global', 'Standing', 'Slowmo', 'Walking' , 'InAir', 'Air duck', 'Duck T', 'Duck CT', 'Height advantage', 'Height dissadvantage' }
local rage = {}
---------------
...
local reference = {
	...
	processticks = menu_reference('MISC', 'Settings', 'sv_maxusrcmdprocessticks'),  
...
local mansory = {
    ...
    mansory_title        = menu_label(x.tab, x.section, '                  MANS' .. colours.special .. 'ORY'),
    miscspace            = menu_label('AA', 'Fake Lag', ' '),
    misc_title           = menu_label('AA', 'Fake Lag', '              MANS' .. colours.special .. 'ORY' .. colours.default .. ' MISC'),
    
    luaenable            = menu_combobox(x.tab, x.section, 'POWER', colours.special .. 'ON', colours.theme.r .. 'OFF'),
    mainspace            = menu_label(x.tab, x.section, ' '),

	antiaim = {
        page_select      = menu_combobox(x.tab, x.section, 'BUILDER / KEYBINDS', 'Builder', 'Keybinds'),
    ...
    misc = {
        dtenable         = menu_checkbox('AA', 'Fake Lag', 'doubletap ' .. colours.default .. 'modifier'),   
        dtspeed          = menu_combobox('AA', 'Fake Lag', 'Modify tickbase', {'Default', 'Fast', 'mansory'}), 
        fastswap         = menu_checkbox('AA', 'Fake Lag', colours.special .. 'Fast weapon switch'), 
        defensive_air    = menu_checkbox('AA', 'Fake Lag', colours.special .. 'Force defensive in air'),
	},

    config = {
        configenable     = menu_checkbox('LUA', 'A', 'mansory Config system'),
    }
}
---------------------------------------------
...
--add to list
local db_configs = database.read('mansory_configs') or {
...
configs.list = menu_listbox(y.tab1, y.section1, 'configs list', function(self)
...
configs.config_name = menu_textbox(y.tab1, y.section1, 'Config')

local valueTable = {}

configs.save = menu_button(y.tab1, y.section1, 'Save', function(self)
	...
	if name == txtbox then
		-- get stuff
		configs.saveTable(mansory, valueTable)
	...
	database.write('mansory_configs', db_configs)
...
configs.load = ui.new_button(y.tab1, y.section1, 'Load', function()
    local configID = menu_get(configs.list)
	if configID == nil then return end
    ...
    local protected = function()
        ...
        configs.loadTable(json_config, mansory)
    end
...
configs.delete = ui.new_button(y.tab1, y.section1, 'Delete', function()
    local configID = menu_get(configs.list)
	if configID == nil then return end
	local txtbox = menu_get(configs.config_name)
	local name = configs.getConfigNameForID(configID + 1)
...
configs.import = menu_button(y.tab1, y.section1, 'Import from clipboard', function(self)
    local protected = function()
...
configs.export = menu_button(y.tab1, y.section1, 'Export to clipboard', function()
    local configID = menu_get(configs.list)
...
local function get_mode(e)
    ...
    local neutral = threat and math_abs(height_to_threat) < 25
    ...
    local hadv_on   = menu_get(mansory.builder[state_to_num['Height advantage']].build_enabled)
    local hdadv_on = menu_get(mansory.builder[state_to_num['Height dissadvantage']].build_enabled)

    if not on_ground then
        xxx = ((entity_get_prop(localplayer, 'm_flDuckAmount') > 0.7) and menu_get(mansory.builder[state_to_num['Air duck']].build_enabled)) and 'Air duck' or 'InAir'
    else
        if hadv_on and threat and height_to_threat > 25 then
            
            xxx = 'Height advantage'
        elseif hdadv_on and threat and height_to_threat < -25 then 

            xxx = 'Height dissadvantage'
        elseif menu_get(reference.fakeduck) or (entity_get_prop(localplayer, 'm_flDuckAmount') > 0.7) then
            if ct then 
                xxx = 'Duck CT'
            elseif t then
                xxx = 'Duck T'
...
local gradientanim = function(text_to_draw, speed)
    local base_r, base_g, base_b,base_a = menu_get(mansory.visual.clr_col1)
    local r2, g2, b2, a2 = menu_get(mansory.visual.clr_col2)
...
local menu = {
    ...
    mansory_vis = function(self)
        --Menu checks
        local luaenabled   = menu_get(mansory.luaenable) == colours.special .. 'ON'
        ---------------------------
        local menu_config  = menu_get(mansory.config.configenable) and luaenabled
        ---------------------------
        local keybinds_m   = menu_get(mansory.antiaim.page_select) == 'Keybinds' and menu_get(mansory.antiaim.manual_master)
        local keybinds_o   = menu_get(mansory.antiaim.page_select) == 'Keybinds' and menu_get(mansory.antiaim.other_master)
        local builder      = menu_get(mansory.antiaim.page_select) == 'Builder'
        ---------------------------

        --Anti-aim
        menu_setvisible(mansory.antiaim.page_select, luaenabled)
        menu_setvisible(mansory.antiaim.manual_master, luaenabled and menu_get(mansory.antiaim.page_select) == 'Keybinds')
        menu_setvisible(mansory.antiaim.key_forward, luaenabled and keybinds_m)
        menu_setvisible(mansory.antiaim.key_left, luaenabled and keybinds_m)
        menu_setvisible(mansory.antiaim.key_right, luaenabled and keybinds_m)
        menu_setvisible(mansory.antiaim.other_master, luaenabled and menu_get(mansory.antiaim.page_select) == 'Keybinds')
        menu_setvisible(mansory.antiaim.key_legit, luaenabled and keybinds_o)
        menu_setvisible(mansory.antiaim.key_freestand, luaenabled and keybinds_o)
        menu_setvisible(mansory.antiaim.static_fs, luaenabled and keybinds_o)
        menu_setvisible(mansory.antiaim.aa_cond, luaenabled)

        --Visual

        
        --Misc
        menu_setvisible(mansory.misc_title, luaenabled)
        menu_setvisible(mansory.misc.dtenable, luaenabled)
        menu_setvisible(mansory.misc.dtspeed, luaenabled and menu_get(mansory.misc.dtenable))
        menu_setvisible(mansory.misc.fastswap, luaenabled)
        menu_setvisible(mansory.misc.defensive_air, luaenabled)
    ...
    handle_menu = function(self)
        local enabled = menu_get(mansory.luaenable) == colours.special .. 'ON'  and menu_get(mansory.antiaim.page_select) == 'Builder'
        menu_setvisible(mansory.antiaim.aa_cond, enabled)
    
        for i=1, #aa_config do
            local show = menu_get(mansory.antiaim.aa_cond) == aa_config[i] and enabled 
            menu_setvisible(mansory.builder[i].build_enabled, show and i > 1)

            if i > 1 then
                show = show and menu_get(mansory.builder[i].build_enabled)
            end

            local lryaw_show    = menu_get(mansory.builder[i].build_yaw) ~= 'Off'
            local lrjitter_show = menu_get(mansory.builder[i].build_jitter) ~= 'Off' 
            local lrfake_show   = menu_get(mansory.builder[i].build_body) ~= 'Off' 

            menu_setvisible(mansory.builder[i].build_pitch, show)
            menu_setvisible(mansory.builder[i].build_yawbase, show)
            menu_setvisible(mansory.builder[i].build_yaw, show)
            menu_setvisible(mansory.builder[i].l_yaw_limit, show and lryaw_show)
            menu_setvisible(mansory.builder[i].r_yaw_limit, show and lryaw_show)
            menu_setvisible(mansory.builder[i].build_jitter, show)
            menu_setvisible(mansory.builder[i].build_l_jitter_sli, show and lrjitter_show)
            menu_setvisible(mansory.builder[i].build_r_jitter_sli, show and lrjitter_show)
            menu_setvisible(mansory.builder[i].build_body, show)
            menu_setvisible(mansory.builder[i].build_body_sli, show and lrfake_show and menu_get(mansory.builder[i].build_body) ~= 'Opposite')
            menu_setvisible(mansory.builder[i].l_fake_limit, show and lrfake_show)
            menu_setvisible(mansory.builder[i].r_fake_limit, show and lrfake_show)
            menu_setvisible(mansory.builder[i].build_free_b_yaw, show and lrfake_show)
            menu_setvisible(mansory.builder[i].build_roll, show and lrfake_show)
...
local legit = {
    ...
    aa_on_use = function(self, e)
        if not menu_get(mansory.antiaim.key_legit) or not menu_get(mansory.antiaim.other_master) then 
            return 
        end
...
local run_direction = function()
	menu_set(mansory.antiaim.key_forward, 'On hotkey')
	menu_set(mansory.antiaim.key_left, 'On hotkey')
	menu_set(mansory.antiaim.key_right, 'On hotkey')

    local static_fs       = menu_get(mansory.antiaim.static_fs)
    local master_o_enable = menu_get(mansory.antiaim.other_master)

    if static_fs and master_o_enable and menu_get(mansory.antiaim.key_freestand) then
        menu_set(reference.yawjitter[2], 0)
    end

    menu_set(reference.freestand[1], menu_get(mansory.antiaim.key_freestand) and 'Default' or '-')
    menu_set(reference.freestand[2], menu_get(mansory.antiaim.key_freestand) and 'Always on' or 'On hotkey')

	if menu_get(mansory.antiaim.key_freestand) and menu_get(mansory.antiaim.manual_master) or not menu_get(mansory.antiaim.manual_master) then
		yaw_direction = 0
		last_press_t_dir = globals.curtime()
	else
		if menu_get(mansory.antiaim.key_forward) and last_press_t_dir + 0.2 < globals_curtime() then
            yaw_direction = yaw_direction == 180 and 0 or 180
			last_press_t_dir = globals_curtime()
		elseif menu_get(mansory.antiaim.key_right) and last_press_t_dir + 0.2 < globals_curtime() then
			yaw_direction = yaw_direction == 90 and 0 or 90
			last_press_t_dir = globals_curtime()
		elseif menu_get(mansory.antiaim.key_left) and last_press_t_dir + 0.2 < globals_curtime() then
			yaw_direction = yaw_direction == -90 and 0 or -90
			last_press_t_dir = globals_curtime()
...
local handle_keybinds = function()
    local freestand = menu_get(mansory.antiaim.key_freestand)
...
client_set_event_callback('setup_command', function(e)
    ...
    local localplayer = entity_get_local_player()
    menu_set(reference.enabled, menu_get(mansory.luaenable) == colours.special .. 'ON')
    if localplayer == nil or not entity_is_alive(localplayer) or not menu_get(mansory.luaenable) == colours.special .. 'ON' then
        return
    end
    ...
    if hide_check and not fakeduck_check then
	    ...
	    menu_set(reference.fl_limit, menu_get(mansory.fl_slider))
    end
    ...
    local legit_aa = menu_get(mansory.antiaim.key_legit) and menu_get(mansory.antiaim.other_master)

    state = menu_get(mansory.builder[state_to_num[state]].build_enabled) and state_to_num[state] or state_to_num['Global']
    
    local defensive_air = menu_get(mansory.misc.defensive_air)
    ...
    if desync_side == 1 then
        --left
        menu_set(reference.yaw[2], yaw_direction == 0 and menu_get(mansory.builder[state].l_yaw_limit) or yaw_direction)
        menu_set(reference.fakeyawlimit, legit_aa and 58 or menu_get(mansory.builder[state].l_fake_limit))
        menu_set(reference.yawjitter[2], menu_get(mansory.builder[state].build_l_jitter_sli))
    elseif desync_side == -1 then
        --right
        menu_set(reference.yaw[2], yaw_direction == 0 and menu_get(mansory.builder[state].r_yaw_limit) or yaw_direction)
        menu_set(reference.fakeyawlimit, legit_aa and 58 or menu_get(mansory.builder[state].r_fake_limit))
        menu_set(reference.yawjitter[2], menu_get(mansory.builder[state].build_r_jitter_sli))
    end

    menu_set(reference.pitch, legit_aa and 'Off' or menu_get(mansory.builder[state].build_pitch))
    menu_set(reference.yawbase, legit_aa and 'Local view' or menu_get(mansory.builder[state].build_yawbase))
    menu_set(reference.yaw[1], legit_aa and 'Off' or menu_get(mansory.builder[state].build_yaw))
    menu_set(reference.yawjitter[1], menu_get(mansory.builder[state].build_jitter))
    menu_set(reference.bodyyaw[1], legit_aa and 'Opposite' or menu_get(mansory.builder[state].build_body))
    menu_set(reference.bodyyaw[2], menu_get(mansory.builder[state].build_body_sli))
    menu_set(reference.fsbodyyaw, legit_aa and true or menu_get(mansory.builder[state].build_free_b_yaw))
...
client_set_event_callback('grenade_thrown', function(e)
    if menu_get(mansory.misc.fastswap) then
