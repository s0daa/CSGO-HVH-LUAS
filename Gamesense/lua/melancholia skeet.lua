lib_error = function(library)
    error(string.format("melancholia - failed to retrieve '%s' library. Head over to our discord and subscribe to all the libraries and reload your cheat", library))
end
...
-- Obex
local obex_data = obex_fetch and obex_fetch() or {username = "embrace", build = "crack by embrace team", discord = "embrace"}
local username = obex_data.username:lower()
local build = obex_data.build:lower() == "debug" and "alpha" or obex_data.build:lower()
local version = "xz prosto sobrali cherez dump"
...
lavender.funcs = {
    ...
    misc = {
        ...
        colour_console = LPH_JIT(function(prefix, string)
            client.color_log(prefix[1], prefix[2], prefix[3], "melancholia ~ \0")
...
function startup()
    ...
    "   #  :+  #  :+. .=+=*                             ",
    " -+    *  * -=  =-  +:                             ",
    " #     -=:*++  =-   *.         Welcome to melancholia, " .. username,
...
lavender.handlers.control_animation_main = function()
    ...
    if lavender.ui.current_tab ~= "HOME" then
        ...
        ui.set(lavender.ui.tab_visualize, lavender.funcs.renderer.colour_text_menu("• ") .. "welcome to " .. lavender.funcs.renderer.two_gradient_text("melancholia.ws", colour[1], colour[2], colour[3], 6))
    end
end
...
lavender.handlers.visuals.indicators = function()
	...
	if ui.get(lavender.ui.visuals.crosshair_indicator) == "modern" then
        ...
        local measure_title = vector(renderer.measure_text("-c", "MELANCHOLIA"))
        local keystate_active = os and not dt and "OS" or dt and not fd and "DT" or fd and "FD" or ""

        renderer.text(lavender.pos.modern.x + modern_edit, lavender.pos.modern.y + 25, main_acc[1], main_acc[2], main_acc[3], main_acc[4], "-c", 0, lavender.funcs.renderer.gradient_text(main_acc[1], main_acc[2], main_acc[3], main_acc[4], "MELANCHOLIA", 2.42, trail_accent[1], trail_accent[2], trail_accent[3], trail_accent[4]))
	...
	elseif ui.get(lavender.ui.visuals.crosshair_indicator) == "default" then
        ...
        local w1 = renderer.measure_text("-", " MELANCHOLIA ")

        local w2 = renderer.measure_text("-", string.upper(build))

        renderer.text(x/2 - ((w1 + w2)/2 * fraction), y/2 + 20, r, g, b, a, "-", 0, "MELANCHOLIA ")
...
lavender.handlers.visuals.watermark = function()
    ...
    local string = "melancholia".. lavender.funcs.renderer.colour_text(".ws", colour) .. " [" .. build .. "] | " .. lavender.funcs.renderer.colour_text(username, colour) .. " | " .. lavender.funcs.renderer.colour_text(hr, colour) .. ":" .. lavender.funcs.renderer.colour_text(m, colour) .. ":" .. lavender.funcs.renderer.colour_text(s, colour)
...
client.set_event_callback("aim_hit", function(e)
    ...
    if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "shot log (notify)") then
        notify.new_bottom(2, {ui.get(lavender.ui.visuals.log_notify_hit_accent)}, "shot_log_hit", "", "melancholia", "~ hit", entity.get_player_name(e.target), "for", e.damage, "in", hgroup)
    end
...
client.set_event_callback("aim_miss", function(e)
    ...
    if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "shot log (notify)") then
        notify.new_bottom(2, {ui.get(lavender.ui.visuals.log_notify_miss_accent)}, "shot_log_miss", "", "melancholia.ws", "~ missed due to", e.reason, "(hc: " .. math.floor(e.hit_chance) .. ", aimed: " .. hgroup .. ")")
    end
...
local killsay_hs = {
    ...
    "ⓜⓔⓛⓐⓝⓒⓗⓞⓛⓘⓐ ???最好的lua ｄｏｍｉｎａｔｅｓ ｈｖｈ ｓｃｅｎｅ",
    "𝕪𝕠𝕦 𝕛𝕦𝕤𝕥 𝕘𝕠𝕥 𝟙𝕕 𝕗𝕠𝕣 𝟜$",
    "1"
}

local killsay_baim = {
    "𝚠𝚎𝚊𝚔 𝚍𝚘𝚐 *ＦＬＯＷ ＨＶＨ* sent to hell by » 𝚚𝚞𝚎𝚌𝚢?",
    "𝐦𝐞𝐥𝐚𝐧𝐜𝐡𝐨𝐥𝐢𝐚.𝐰𝐬 𝐨𝐰𝐧𝐢𝐧𝐠 𝐦𝐞 𝐚𝐧𝐝 𝐚𝐥�",
    "𝕚 𝕒𝕞 𝕦𝕤𝕖 𝕓𝕖𝕤𝕥 𝕒𝕟𝕥𝕚𝕒𝕚𝕞 𝕝𝕦𝕒",
    "𝐫𝐮 𝐩𝐚𝐬𝐭𝐞 𝐝𝐞𝐬𝐭𝐫𝐨𝐲𝐬 𝐟𝐫𝐨𝐦 𝐫𝐞𝐥𝐞𝐚𝐬𝐞 𝐦𝐞𝐥𝐚𝐧𝐜𝐡𝐨𝐥𝐢𝐚.𝐭𝐞𝐜�"
}
    ...
    if lavender.funcs.misc.contains(ui.get(lavender.ui.aa.reset_conditions), "death") and ui.get(lavender.ui.aa.antibrute_master) and abstage > 0 then
        if client.userid_to_entindex(e.userid) == entity.get_local_player() then
            ...
            if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "anti brute log (notify)") then
                notify.new_bottom(2, {ui.get(lavender.ui.visuals.log_ab_notify_accent)},"", "", "melancholia.ws", "~ anti brute reset", "death")
            end
...
lavender.handlers.aa.round_start = function()
    ...
    if lavender.funcs.misc.contains(ui.get(lavender.ui.aa.reset_conditions), "round start") and ui.get(lavender.ui.aa.antibrute_master) and abstage > 0 then
        ...
        abstage = 0
        if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "anti brute log (notify)") then
            notify.new_bottom(2, {ui.get(lavender.ui.visuals.log_ab_notify_accent)},"", "", "melancholia.ws", "~ anti brute reset", "round end")
        end
        if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "anti brute log (console)") then
    ...
    if lavender.funcs.misc.contains(ui.get(lavender.ui.aa.reset_conditions), "On headshot") and ui.get(lavender.ui.aa.antibrute_master) and abstage > 0 then
        ...
        if attacker ~= me and victim == me then
            if c.hitgroup == 1 then
                ...
                lastmiss = 0
                if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "anti brute log (notify)") then
                    notify.new_bottom(2, {ui.get(lavender.ui.visuals.log_ab_notify_accent)},"", "", "melancholia.ws", "~ anti brute reset", "headshot")
                end
                if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "anti brute log (console)") then
...
client.set_event_callback("bullet_impact", function(e)
    ...
    if math.abs(delta_2d) <= 45 and globals.curtime() - lastmiss > 0.225 then
        ...
        if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "anti brute log (notify)") then
            notify.new_bottom(1, {ui.get(lavender.ui.visuals.log_ab_notify_accent)},"", "melancholia.ws", "~ anti brute activated due to shot stage:", tostring(abstage))
        end
        if lavender.funcs.misc.contains(ui.get(lavender.ui.visuals.informative_visual), "anti brute log (console)") then
...
local clantag_string = {"melancholia.ws", "melancholia.ws", "melancholia.ws", "melancholia.ws", "melancholia.ws", "melancholia.ws", "melancholia.ws", "melancholia.ws ", "melancholia.ws ", "melancholia.ws ", "melancholia.ws"}
...
client.register_esp_flag("melancholia.ws", 185, 190, 255, function(ent)
...
notify.new_bottom(4, {ui.get(lavender.ui.visuals.notification_accent)}, "ON_LOAD", "welcome,", username, "to", "melancholia.ws")
