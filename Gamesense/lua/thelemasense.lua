        ...
        ["command"] = function(arg)
			...
			if defensivemenu ~= "off" and defensivecheck and not freestand and tbl.antiaim.manual.aa == 0 and tbl.contains(ui.get(menu["anti aimbot"]["features"]["states"]), real_state) then
				...
				elseif defensivemenu == "sideways up" then
                    if not ((arg.in_jump == 1 or bit.band(entity.get_prop(entity.get_local_player(), 'm_fFlags'), 1) == 0) and entity.get_prop(entity.get_local_player(), 'm_flDuckAmount') > 0.8 and (entity.get_classname(entity.get_player_weapon(entity.get_local_player())) == 'CKnife' or entity.get_classname(entity.get_player_weapon(entity.get_local_player())) == 'CWeaponTaser')) then
                        rnd_ptch_yw = math.random(0,1)==0
					    ui.set(tbl.items.pitch[1], "custom")
                        ui.set(tbl.items.body[1], "opposite")
                        ui.set(tbl.items.pitch[2], aa_clamp((globals.tickcount() % 6 < 3 and math.floor( -math.cos( (globals.realtime() * (64 - clamp(62, 1, 62))) ) * -89 ))) )
                        ui.set(tbl.items.yaw[1], "spin")
					    ui.set(tbl.items.yaw[2], aa_clamp((globals.tickcount() % 6 < 3 and math.floor( -math.cos( (globals.realtime() * (64 - clamp(55, 1, 62))) ) * 245 ))))
                    else
                        ui.set(tbl.items.pitch[1], "minimal")
        ...
        ["arrows"] = function()
            ...
            if myself ~= nil and entity.is_alive(myself) then
                ...
                if getbuild() == "beta" then
                    output = output .. ("\a%x%x%x%x"):format(200, 69, 69, 255) .. " [DEV] "
                end
