...
local obex_data = obex_fetch and obex_fetch() or {username = 'Sigma', build = 'beta', discord=''}
local tab,place =  "AA","Anti-aimbot angles"
local ffi = require 'ffi'

local what_noti_image = nil

http.get("0.0.0.0", function(s, r)
...
local new_menu_references = {
    ...
    ["AntiAim"] = {
        ...
        anti_ze = ui.new_checkbox(tab, place, "Anti-zeus"),
        anti_zeus = ui.new_combobox(tab, place, "Anti-zeus options", {"Pull pistol", "Pull zeus"}),
        zeus_distance = ui.new_slider(tab, place, "anti-zeus radius", 0, 500, 250),
    ...
    ["Misc"] = {
        ...
        backtrack_new = ui.new_checkbox(tab, place, "Manipulate backtracking"),
        manipsvbt = ui.new_combobox(tab, place, "Backtracking options", {"Game default", "Over-predict fix", "Extended optimal", "Extended"}),
        fix_osaa = ui.new_checkbox(tab, place, "Fix osaa"),
        fast_ladder = ui.new_checkbox(tab, place, "ladder climber"),
        ladder_yaw_slider = ui.new_slider(tab,place, "Ladder angle", -180, 180, 0),
        enable_kill_say = ui.new_combobox(tab, place, "kill say",{"Disabled","Miracle","Yeat"}),
        enable_sun_rise = ui.new_checkbox(tab, place, "sun rise mode"),
        enable_nigthermode = ui.new_checkbox(tab, place, "nightmode v2"),
        enable_local_animations = ui.new_multiselect(tab, place, "animations [\aaac8fbffclient\affffffff]",{"pitch 0","reversed legs","legbreaker","running","moonwalk","remove lean","static legs","blind","t-pose"}),
...
local misc_functions = {
    kill_say = {["Miracle"] = {"U think u good? luckily im here #Miracle","youre value compared to me  is but a grain of sand","all romanian(you) will die to me(gypys king)",
    "this isnt phasmaphobia: global offensive please dont speak","Miracle trap house","I guarntee youre loss forever and always","𝕡𝕣𝕠𝕓𝕝𝕖𝕞?","cope",
    " 格拉格拉 < you? 无功无过 < me B) #Miracle","in hvh war i will win","below average performance Miracle performance","SPEAK BULGARIAN? WILL TALK CN",
    "you are loss it is decided with my Miracle.lua","you do not perform this hvh against Miracle","qahahaha i am top of this region","this weak snail is spoke of victory but is door unhinged to loss",
    ...
    "your mexican familia never make it out from trailer #Miracle","cant understand u. any noname translator? #Miracle","you waste aka fecal matter/shit(you)","sorry for u loss, me always better like life",
    "better luck next round, oh wait i alr won BAHAHHA","ur lua & u sucks get Miracle >.<","when you spawn tell me why u die to me","how hit chance in deagle? i sit.",
    "shitting on your cheat speedrun any% WR run feat Miracle","smelly lapdog dreams of success in 1x1 but is handed 9 casualities","dude where are my diamonds?","all weak dogs fall to starlight","WOW STARLIGHT .。GYPSY 的科技 (TECHNOLOGY) ? "},
    ...
    clan_tag = {"", "st","sta","star","starl","starli","Miracle","Miracle","Miracle","Miracle","Miracle","Miracle","Miracle","Miracle","sta","st"},
    ct_tick = 0,
    ct_old_tick = 0,
    save_tick = 0,
...
for k,v in ipairs(aa.aa_states) do
    ...
    new_menu_references["AntiAim"].aa_state[v] = {}
    new_menu_references["AntiAim"].aa_state[v].jway_counter = ui.new_slider(tab,place, "jitter slider \n" .. new_streng, 2, 12, 2)
    new_menu_references["AntiAim"].aa_state[v].enabled = ui.new_checkbox(tab,place,"enable '" .. light_blue .. new_streng .. "'" .. "\affffffff" )
    new_menu_references["AntiAim"].aa_state[v].pitch = ui.new_combobox(tab,place,light_blue .. new_streng .. white .."pitch\n" .. v,{"off", "default", "down", "minimal","custom"})
    new_menu_references["AntiAim"].aa_state[v].pitch_slider = ui.new_slider(tab,place,"\n" .. light_blue .. new_streng .. white .. "custom pitch", -89, 89, 0)


    new_menu_references["AntiAim"].aa_state[v].yawbase = ui.new_combobox(tab,place,light_blue .. new_streng .. white .."yaw base\n" .. v,{"local view", "at targets"})
    new_menu_references["AntiAim"].aa_state[v].yaw = ui.new_combobox(tab,place,light_blue .. new_streng .. white .."yaw\n" .. v,{"off", "180", "180 z", "crosshair","jittery","delayed","l & r"})
    
    new_menu_references["AntiAim"].aa_state[v].yawadd = ui.new_slider(tab,place, light_blue .. new_streng .. white .."yaw right\n" .. v, -180, 180, k == 9 and 180 or 0)

    if k ~= "manual" and k ~= "leigt" then
        new_menu_references["AntiAim"].aa_state[v].yawaddleft = ui.new_slider(tab,place, light_blue .. new_streng .. white .."yaw left \n" .. v, -180, 180, 0)
        new_menu_references["AntiAim"].aa_state[v].ticker_slider = ui.new_slider(tab,place, light_blue .. new_streng .. white .."ticks delay \n" .. v, 1, 10, 1)
    end
    new_menu_references["AntiAim"].aa_state[v].yawjitter = ui.new_combobox( tab,place,light_blue .. new_streng .. white .."yaw jitter\n" .. v,{"off", "offset", "center", "random","skitter","jitter ways","sway","l & r"})
    new_menu_references["AntiAim"].aa_state[v].yawjitteradd = ui.new_slider(tab,place,light_blue .. new_streng .. white .. "yaw jitter" , -180, 180, 0)
    new_menu_references["AntiAim"].aa_state[v].yawjitteradd2 = ui.new_slider(tab,place, light_blue .. new_streng .. white .."yaw jitter left", -180, 180, 0)
    new_menu_references["AntiAim"].aa_state[v].swaym = ui.new_slider(tab,place,light_blue .. new_streng .. white .. "sway min\n" .. v, -180, 180, 0)
    new_menu_references["AntiAim"].aa_state[v].swaymx = ui.new_slider(tab,place,light_blue .. new_streng .. white .. "sway max\n" .. v, -180, 180, 0)
    new_menu_references["AntiAim"].aa_state[v].swayspeed = ui.new_slider(tab,place,light_blue .. new_streng .. white .. "sway speed\n" .. v, 1, 10,1)
    new_menu_references["AntiAim"].aa_state[v].sway_value = 0
    new_menu_references["AntiAim"].aa_state[v].to_sway = false
    new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw = ui.new_combobox( tab,place,light_blue .. new_streng .. white .."body yaw\n" .. v,{"off","jitter", "static","anti bruteforce","auto jitter","delayed yaw"})
    new_menu_references["AntiAim"].aa_state[v].delayed_method = ui.new_combobox( tab,place,light_blue .. new_streng .. white .."delayed method\n" .. v,{"simple","advanced"})
    new_menu_references["AntiAim"].aa_state[v].gs_bodyyawadd = ui.new_slider(tab,place,"\n" .. light_blue .. new_streng .. white .. "body yaw\n" .. v, -180, 180, 0)

    new_menu_references["AntiAim"].aa_state[v].pedro_aas2 = { ui.new_slider(tab,place, light_blue .. new_streng .. white .."jitter way 1 \n" .. k, -180, 180, 0),ui.new_slider(tab,place, "jitter way 2\n" .. k, -180, 180, 0)
    ...
    if k ~= "manual" and k ~= "fakelag" and k ~= "Legit" then
        new_menu_references["AntiAim"].aa_state[v].enabled_exploit = ui.new_checkbox(tab,place,light_blue .. new_streng .. white .."enable defensive\n " .. v)
        new_menu_references["AntiAim"].aa_state[v].enabled_defensive_force = ui.new_checkbox(tab,place,light_blue .. new_streng .. white .."force defensive\n " .. v)
        new_menu_references["AntiAim"].aa_state[v].pitch_defensive = ui.new_combobox(tab,place,light_blue .. new_streng .. white .."pitch defensive\n" .. v,{"off", "up", "random","zero"})
        new_menu_references["AntiAim"].aa_state[v].yaw_defensive = ui.new_combobox(tab,place,light_blue .. new_streng .. white .."yaw defensive\n" .. v,{"off", "180", "spin"})
        new_menu_references["AntiAim"].aa_state[v].yawadd_defensive = ui.new_slider(tab,place,light_blue .. new_streng .. white .. "add defensive\n" .. v, -180, 180,0)
        new_menu_references["AntiAim"].aa_state[v].skitter_defensive = ui.new_checkbox(tab,place,light_blue .. new_streng .. white .."skitter defensive\n " .. v)
        new_menu_references["AntiAim"].aa_state[v].skitter_slider_defensive = ui.new_slider(tab,place,light_blue .. new_streng .. white .. "skitter add defensive\n" .. v, -180, 180,0)
    end
...
local jitter_add = ui.new_button(tab,place,"add jitter",function()
    local get_value = new_menu_references["AntiAim"].aa_state[ui.get(new_menu_references["AntiAim"].aa_state_menu)].jway_counter
...
function aa_functions:manual_anti_aim()
    if ui.get(new_menu_references["AntiAim"].aa_state["Manual"].enabled) then
        
        ui.set(new_menu_references["AntiAim"].aa_state["Manual"].yaw,"180")
        if ui.get(new_menu_references["Keybinds"].manual_right) and aa.last_pressed + 0.2 < globals.curtime() then
            aa.manual_state = aa.manual_state == "right" and "reset" or "right"
            aa.last_pressed = globals.curtime()
        elseif ui.get(new_menu_references["Keybinds"].manual_left) and aa.last_pressed + 0.2 < globals.curtime() then
            aa.manual_state = aa.manual_state == "left" and "reset" or "left"
            aa.last_pressed = globals.curtime()
        elseif ui.get(new_menu_references["Keybinds"].manual_forward) and aa.last_pressed + 0.2 < globals.curtime() then
            aa.manual_state = aa.manual_state == "forward" and "reset" or "forward"
            aa.last_pressed = globals.curtime()
        elseif ui.get(new_menu_references["Keybinds"].manual_reset) and aa.last_pressed + 0.2 < globals.curtime() then
...
function aa_functions.anti_brute_bullet_impact(event)
    ...
    if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].gs_bodyyaw ) ~= "anti bruteforce" then return end

    if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw) == "l & r" or ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw) == "delayed" or ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw) == "jittery" then
        return
    end
...
function aa_functions:anti_knife()
    ...
    if ui.get(new_menu_references["AntiAim"].anti_kniv)then
...
function aa_functions:anti_zeus()
    local get_distance = function(x1, y1, z1, x2, y2, z2)
        return math.sqrt((x2 - x1) ^ 2 + (y2 - y1) ^ 2 + (z2 - z1) ^ 2)
    end

    if ui.get(new_menu_references["AntiAim"].anti_ze)then
        local players = entity.get_players(true)
        ...
        for i = 1, #players do
            ...
            if entity.get_classname(weapon) == "CWeaponTaser" and distance <= ui.get(new_menu_references["AntiAim"].zeus_distance) then
                if ui.get(new_menu_references["AntiAim"].anti_zeus) == "Pull pistol" then
                    client.exec("slot2")
                elseif
                    ui.get(new_menu_references["AntiAim"].anti_zeus) == "Pull zeus" and entity.get_classname(selfweapon) ~= "CWeaponTaser"
                 then
                    client.exec("slot3")
                end
            end
...
function misc_functions:manipsvbt()
    if ui.get(new_menu_references["Misc"].backtrack_new) then
        if ui.get(new_menu_references["Misc"].manipsvbt) == "Game default" then
            client.set_cvar("sv_maxunlag", 0.2)
        end
        if ui.get(new_menu_references["Misc"].manipsvbt) == "Over-predict fix" then
            client.set_cvar("sv_maxunlag", 0.19)
        end
        if ui.get(new_menu_references["Misc"].manipsvbt) == "Extended optimal" then
            client.set_cvar("sv_maxunlag", 0.3)
        end
        if ui.get(new_menu_references["Misc"].manipsvbt) == "Extended" then
            client.set_cvar("sv_maxunlag", 1)
        end
    end

    if not ui.get(new_menu_references["Misc"].backtrack_new) then
        client.set_cvar("sv_maxunlag", 0.2)
    end
end

function misc_functions:fix_osaa()
    if ui.get(menu_reference.os[1]) and ui.get(menu_reference.os[2]) and not ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) and not ui.get(menu_reference.fakeduck) then
        if not hsSaved then
...
function aa_functions:fast_ladder(cmd)
    if not ui.get(new_menu_references["Misc"].fast_ladder) then return end
        ...
        if move_type == 9 then
             ...
             if true then
                 if cmd.forwardmove == 0 then
                     cmd.pitch = 89
                     cmd.yaw = cmd.yaw + ui.get(new_menu_references["Misc"].ladder_yaw_slider)
                     if math.abs(ui.get(new_menu_references["Misc"].ladder_yaw_slider)) > 0 and math.abs(ui.get(new_menu_references["Misc"].ladder_yaw_slider)) < 180 and cmd.sidemove ~= 0 then
                         cmd.yaw = cmd.yaw - ui.get(ladder_yaw_slider)
                     end
                     if math.abs(ui.get(new_menu_references["Misc"].ladder_yaw_slider)) == 180 then
...
function aa_functions:legit_aa(cmd)
    ...
    if not ui.get(new_menu_references["AntiAim"].aa_state["Legit"].enabled) then return end
    ...
    ui.set(new_menu_references["AntiAim"].aa_state["Legit"].yaw,"180")
...
function aa_functions:static_in_aerophobic(cmd)
    ...
    if not ui.get(new_menu_references["AntiAim"].safe_dangerous) then return end
...
function aa_functions:defensive_setup(cmd)
    ...
    if not ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].enabled) then return end
    ...
    if aa_functions.can_defensive then
        if ui.get(new_menu_references["AntiAim"].aa_state[old_def_state].enabled_exploit) ~= ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].enabled_exploit) then
    ...
    if aa_functions.aa_number ~= "manual" and aa_functions.aa_number ~= "Legit" and aa_functions.aa_number ~= "fakelag" then
        if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].enabled_exploit) and (ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) or ui.get(menu_reference.os[1]) and ui.get(menu_reference.os[2])) and not ui.get(menu_reference.fakeduck)  then
           
            if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].enabled_defensive_force) then 
           ...
           if aa_functions.def_ticks > 0 or cmd.force_defensive == true then 
            if  ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].pitch_defensive) == "zero" then
                ui.set(menu_reference.pitch[1],"Off")
           elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].pitch_defensive) ~= "off" then
               ui.set(menu_reference.pitch[1],ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].pitch_defensive))
           end
           
           if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw_defensive) ~= "off" then
               ui.set(menu_reference.yaw[1],ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw_defensive))
               ui.set(menu_reference.yaw[2],ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawadd_defensive))
           end
   
           if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].skitter_defensive) then
               ui.set(menu_reference.yawjitter[1],"Skitter")
               ui.set(menu_reference.yawjitter[2],ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].skitter_slider_defensive))
           end
           end
        end
    end
...
function aa_functions:setup(cmd)
    ...
    if ui.get(new_menu_references["AntiAim"].aa_state["Legit"].enabled) and aa_functions:legit_aa(cmd) then
        aa_functions.aa_number = "Legit"
    elseif  ui.get(new_menu_references["AntiAim"].aa_state["Manual"].enabled) and aa.manual_state ~= "reset" then
        aa_functions.aa_number = "Manual"
    elseif  ui.get(new_menu_references["AntiAim"].aa_state["Fakelag"].enabled) and not ui.get(menu_reference.dt[2]) and not ui.get(menu_reference.os[2]) then
    ...
    if cmd.chokedcommands == 0 then
        ...
        if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitter) == "jitter ways" then
       
            aa_functions.jitter_way_number = aa_functions.jitter_way_number + 1 
        
        
            if aa_functions.jitter_way_number > ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].jway_counter) then
                aa_functions.jitter_way_number = 1
            end
        elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitter) == "sway" then
    
           
          
                ui.set(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaym,math.min(ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaym),ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaymx)))
                local number = ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swayspeed) 
                local number_to_add = new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].to_sway and number or -number
                
                new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value = new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value + number_to_add
                if new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value < ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaym) then
                    new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value = ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaym)
                elseif new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value > ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaymx) then
                    new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value = ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaymx)
                end
            
                    if new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value >= ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaymx) and new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].to_sway then
                        new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].to_sway = false
                    elseif new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value <= ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].swaym) and not new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].to_sway  then
                        new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].to_sway = true
                    end
        end
    end
    ...
    if aa_functions.aa_number ~= "Legit" and aa_functions.aa_number ~= "manual" then
        if globals.tickcount() > current_tickcount + ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].ticker_slider)  then
            if cmd.chokedcommands == 0 then
    ...
    ui.set(menu_reference.enabled,ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].enabled))
  
    ui.set(menu_reference.pitch[1], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].pitch))

    ui.set(menu_reference.pitch[2], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].pitch_slider))
        
    
    ui.set(menu_reference.yawbase, ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawbase))

    if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw) == "jittery" then
        ui.set(menu_reference.yaw[1], "180")
        if cmd.chokedcommands == 0 then
            aa_functions.random_extrapolated = client.random_int(0,1) == 1
            ui.set(menu_reference.yaw[2],  aa_functions.random_extrapolated and ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawaddleft) or ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawadd))
        end
        elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw) == "delayed" then

        ui.set(menu_reference.yaw[1], "180")
        if cmd.chokedcommands == 0 then
            ui.set(menu_reference.yaw[2],  aa_functions.to_jitter and ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawaddleft) or ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawadd))
        end
        elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw) == "l & r" then
        ui.set(menu_reference.yaw[1], "180")
        if cmd.chokedcommands == 0 then
            if side ~= nil then 
                ui.set(menu_reference.yaw[2], side and ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawaddleft) or ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawadd))
            end
        end
    
    else
        ui.set(menu_reference.yaw[1], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yaw))
        ui.set(menu_reference.yaw[2], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawadd))
    end

    if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitter) == "jitter ways" then
        
        ui.set(menu_reference.yawjitter[1],"Center")
        ui.set(menu_reference.yawjitter[2], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].pedro_aas2[aa_functions.jitter_way_number]))
        
    elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitter) == "sway" then
        
        ui.set(menu_reference.yawjitter[1],"Center")
        
            ui.set(menu_reference.yawjitter[2], new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].sway_value)
        
    elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitter) == "l & r" then
        --if cmd.chokedcommands == 0 then
            ui.set(menu_reference.yawjitter[1], "Center")
            ui.set(menu_reference.yawjitter[2], side and ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitteradd) or ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitteradd2))
       -- end
    else
        ui.set(menu_reference.yawjitter[1], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitter))
        ui.set(menu_reference.yawjitter[2], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].yawjitteradd))
    end

    if cmd.chokedcommands == 0 then
        if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].gs_bodyyaw) == "anti bruteforce" then
            aa_functions.anti_brute_anti_aim()
        elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].gs_bodyyaw) == "auto jitter" then
            ui.set(menu_reference.bodyyaw[1], "Jitter")
            ui.set(menu_reference.bodyyaw[2], ui.get(menu_reference.yawjitter[2]))
        elseif ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].gs_bodyyaw) == "delayed yaw" then
            if ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].delayed_method) == "simple" then
                ui.set(menu_reference.bodyyaw[1], "Static")
            ...
            ui.set(menu_reference.bodyyaw[1], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].gs_bodyyaw))
            ui.set(menu_reference.bodyyaw[2], ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].gs_bodyyawadd))
        end
    end
    

    if ui.get(new_menu_references["Keybinds"].global_fs) and aa_functions.aa_number ~= "Legit" and aa_functions.aa_number ~= "manual" and not table_contains(ui.get(new_menu_references["AntiAim"].freestanding_disabler),aa_functions.aa_number) then
        ui.set(menu_reference.freestand[1],true)
        ui.set(menu_reference.freestand[2],"Always on")
      
    elseif not ui.get(new_menu_references["Keybinds"].global_fs) or aa_functions.aa_number == "Legit" or aa_functions.aa_number == "manual" or table_contains(ui.get(new_menu_references["AntiAim"].freestanding_disabler),aa_functions.aa_number) then
        ui.set(menu_reference.freestand[1],false)
    end


    if ui.get(menu_reference.freestand[1]) == true then
        if ui.get(new_menu_references["AntiAim"].freestanding_options) == "static" then
            ui.set(menu_reference.bodyyaw[1],"Static")
            ui.set(menu_reference.bodyyaw[2],1)
            ui.set(menu_reference.yawjitter[1],"Off")
            ui.set(menu_reference.yaw[2],0)
        elseif ui.get(new_menu_references["AntiAim"].freestanding_options) == "jitter" then
            ui.set(menu_reference.bodyyaw[1],"Jitter")
            ui.set(menu_reference.yawjitter[1],"Off")
            ui.set(menu_reference.yaw[2],0)
        end
    ...
    aa_functions:anti_zeus()
    misc_functions:manipsvbt()
    aa_functions:fast_ladder(cmd)
end
...
function visual_functions:indicators()
    ...
    renderer.text(w / 2 + math.ceil(text_anim[1])-1,h / 2 + 24,indicator_color[1],indicator_color[2],indicator_color[3],indicator_color[4],"c-",0,"Miracle")
    ...
    for i = 1,#text do
        ...
        if i == 4 then
            ...
            text_anim[i] = lerp(text_anim[i],can2 and measure.x/2 + 9 or 0,globals.frametime() * 15)
        end
...
function visual_functions:indicators2()
    ...
    renderer.text(w / 2 + math.ceil(text_anim[1]),h / 2 + 24,indicator_color[1],indicator_color[2],indicator_color[3],indicator_color[4],"c",0,"Miracle")
    renderer.text(w / 2 + math.ceil(text_anim[2]),h / 2 + 36,255,255,255,255,"c",0,"- " .. text_string .. " -")
   
    if ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) then 
        renderer.text(w / 2 - 1 + math.ceil(text_anim[4]),h / 2 + 36 + math.ceil(add_y[1]),255,255,255,anti_aim_funcs.get_double_tap() and 255 or 130 ,"c-",0,"DT")
    end

    if ui.get(menu_reference.os[1]) and ui.get(menu_reference.os[2]) and not ui.get(menu_reference.dt[2]) then
        renderer.text(w / 2 - 1 + math.ceil(text_anim[4]),h / 2 + 36 + math.ceil(add_y[2]),255,255,255,255 ,"c-",0,"OS")
    end
    ...
    local text = {"Miracle","- " .. text_string .. " -","double tap","on shot","baim"}
    for i = 1,#text do
        local measure = vector(renderer.measure_text("c", text[i]))
        local can2 = scoped_and_box
...
function visual_functions:manual_indicator()
    ...
    elseif ui.get(new_menu_references["Visuals"].enable_manual) == "minimal" then
        ...
        local vel_adap = (ui.get(new_menu_references["Visuals"].enable_velocity_adaptive) and get_velocity(entity.get_local_player()) * 45 / 300)  or 0
        renderer.text(w / 2 + 35 + vel_adap, h / 2 ,aa.color_right[1],aa.color_right[2],aa.color_right[3],aa.color_right[4],"c",0,"▷")
        renderer.text(w / 2 - 35 + -vel_adap, h / 2 ,aa.color_left[1],aa.color_left[2],aa.color_left[3],aa.color_left[4],"c",0,"◁")
    end
end
...
function visual_functions:desync_indicator()
    ...
    if ui.get(new_menu_references["Visuals"].enable_desync_indicator) then
       ...
       visual_functions.offset2 = lerp(visual_functions.offset2,scoped_and_box and 17 or 0,globals.frametime() * 15)
        ...
        renderer.gradient(w / 2 - 13 + indicator_set, h / 2 + 14,desync_bar,2,desync_color[1],desync_color[1],desync_color[1],desync_color[1],  desync_color[1],desync_color[1],desync_color[1],255,true)
    end 
end
...
function visual_functions:watermark()
    ...
    text_fade_animation(w - 50,h / 2,5,{r=255,g=255,b=255,a=255},{r=wm[1],g=wm[2],b=wm[3],a=wm[4]},"Miracle")
end
...
new_menu_references.config.export_file = ui.new_button(tab,place, "export to clipboard", function()
    ...
    for k,v in ipairs(aa.aa_states) do
		...
		str = str
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].enabled)) .. "|"
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].pitch)) .. "|"
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].yawbase)) .. "|"
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].yaw)) .. "|"
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].yawadd)) .. "|"
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].ticker_slider)) .. "|"
        .. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].yawaddleft)) .. "|"
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter)) .. "|"
		.. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitteradd)) .. "|"
        .. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitteradd2)) .. "|"
        .. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw)) .. "|"
        .. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyawadd)) .. "|"    

        for b = 1,12 do
            str = str .. tostring(ui.get(new_menu_references["AntiAim"].aa_state[v].pedro_aas2[b])) .. "|"   
        end
    end
...
new_menu_references.config.import_config = ui.new_button(tab,place,"import from clipboard",function()
    ...
    for k,v in ipairs(aa.aa_states) do
        ...
        ui.set(new_menu_references["AntiAim"].aa_state[v].enabled, to_boolean(tbl[crescente]))
        crescente = crescente + 1
		ui.set(new_menu_references["AntiAim"].aa_state[v].pitch, tostring(tbl[crescente]))
        crescente = crescente + 1
		ui.set(new_menu_references["AntiAim"].aa_state[v].yawbase, tostring(tbl[crescente]))
        crescente = crescente + 1
		ui.set(new_menu_references["AntiAim"].aa_state[v].yaw, tostring(tbl[crescente]))
        crescente = crescente + 1
		ui.set(new_menu_references["AntiAim"].aa_state[v].yawadd, tonumber(tbl[crescente]))
        crescente = crescente + 1
		ui.set(new_menu_references["AntiAim"].aa_state[v].ticker_slider, tonumber(tbl[crescente]))
		crescente = crescente + 1
        ui.set(new_menu_references["AntiAim"].aa_state[v].yawaddleft, tonumber(tbl[crescente]))
        crescente = crescente + 1
		ui.set(new_menu_references["AntiAim"].aa_state[v].yawjitter, tostring(tbl[crescente]))
        crescente = crescente + 1
		ui.set(new_menu_references["AntiAim"].aa_state[v].yawjitteradd, tonumber(tbl[crescente]))
        crescente = crescente + 1
        ui.set(new_menu_references["AntiAim"].aa_state[v].yawjitteradd2, tonumber(tbl[crescente]))
        crescente = crescente + 1
        ui.set(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw, tostring(tbl[crescente]))
        crescente = crescente + 1
        ui.set(new_menu_references["AntiAim"].aa_state[v].gs_bodyyawadd, tonumber(tbl[crescente]))
        crescente = crescente + 1

        for b = 1,12 do
            ui.set(new_menu_references["AntiAim"].aa_state[v].pedro_aas2[b],tonumber(tbl[crescente]))
            crescente = crescente + 1
        end
    end
...
local function new_menu_visibility()
    ...
    local manual_page = ui.get(new_menu_references["AntiAim"].aa_state_menu) == "Manual"
    ...
    --aa
   -- ui.set_visible(new_menu_references["AntiAim"].aa_label,ui_menu.selected_tab == 1)
    ui.set_visible(new_menu_references["AntiAim"].aa_state_menu,ui_menu.selected_tab == 1 or ui_menu.selected_tab == 2 and jitter_page)
    ui.set_visible(new_menu_references.aa_tab,ui_menu.selected_tab == 2)
    
    --local anti_bruteforce = ui.get(new_menu_references["AntiAim"]_tab) == "bruteforce"
    for k,v in ipairs(aa.aa_states) do
        ui.set_visible(new_menu_references["AntiAim"].aa_state[v].enabled,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v)
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].pitch,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v)
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].pitch_slider,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].pitch) == "custom")
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yawbase,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and not manual_page) 
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yaw,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and k ~= 7 and k ~= 9) 
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yawadd,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and not manual_page and ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) ~= "off") 
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yawjitter,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v)  
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yawjitteradd,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter) ~= "off" and ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter) ~= "jitter ways" and ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter) ~= "sway")  
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yawjitteradd2,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter) == "l & r")  
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v) 
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].delayed_method,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw) == "delayed yaw") 
        ui.set_visible( new_menu_references["AntiAim"].aa_state[v].gs_bodyyawadd,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw) ~= "off" and ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw) ~= "anti bruteforce" and ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw) ~= "auto jitter" and  ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw) ~= "delayed yaw") 
        ui.set_visible(new_menu_references["AntiAim"].aa_state[v].jway_counter,false)
        ui.set_visible(new_menu_references["AntiAim"].aa_state[v].swaym, ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter) == "sway")
        ui.set_visible(new_menu_references["AntiAim"].aa_state[v].swaymx, ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter) == "sway")
        ui.set_visible(new_menu_references["AntiAim"].aa_state[v].swayspeed, ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].yawjitter) == "sway")

        if  k ~= "Legit" and k ~= "manual" then
            ui.set_visible(new_menu_references["AntiAim"].aa_state[v].yawaddleft, ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and (ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) == "jittery" or ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) == "delayed" or ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) == "l & r"))
            ui.set_visible(new_menu_references["AntiAim"].aa_state[v].ticker_slider, ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) == "delayed")
        end
        
        if k ~= "fakelag" and k ~= "Legit" and k ~= "manual" then
            ui.set_visible(new_menu_references["AntiAim"].aa_state[v].enabled_exploit,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v)
            ui.set_visible( new_menu_references["AntiAim"].aa_state[v].enabled_defensive_force,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].enabled_exploit)) 
            ui.set_visible( new_menu_references["AntiAim"].aa_state[v].pitch_defensive,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].enabled_exploit)) 
            ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yaw_defensive,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].enabled_exploit)) 
            ui.set_visible( new_menu_references["AntiAim"].aa_state[v].yawadd_defensive,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].yaw_defensive) ~= "off" and ui.get(new_menu_references["AntiAim"].aa_state[v].enabled_exploit)) 
            ui.set_visible( new_menu_references["AntiAim"].aa_state[v].skitter_defensive,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].enabled_exploit)) 
            ui.set_visible(new_menu_references["AntiAim"].aa_state[v].skitter_slider_defensive,ui_menu.selected_tab == 1 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and ui.get(new_menu_references["AntiAim"].aa_state[v].skitter_defensive) and ui.get(new_menu_references["AntiAim"].aa_state[v].enabled_exploit))
        end

        ui.set_visible(new_menu_references["AntiAim"].label_jitter_ways,ui_menu.selected_tab == 2 and jitter_page and ui.get(new_menu_references["AntiAim"].aa_state[ui.get(new_menu_references["AntiAim"].aa_state_menu)].yawjitter) ~= "jitter ways")
        for i = 1, 12 do
            ui.set_visible( new_menu_references["AntiAim"].aa_state[v].pedro_aas2[i]  ,ui_menu.selected_tab == 2 and ui.get(new_menu_references["AntiAim"].aa_state_menu) == v and i <= ui.get(new_menu_references["AntiAim"].aa_state[v].jway_counter) and jitter_page and ui.get(new_menu_references["AntiAim"].aa_state[ui.get(new_menu_references["AntiAim"].aa_state_menu)].yawjitter) == "jitter ways")
            --print(new_menu_references["AntiAim"].aa_state[v].slider[i])
        end
    end

    --aa2
    
    local keys = ui.get(new_menu_references["Keybinds"].enable_key)
    ui.set_visible(new_menu_references["AntiAim"].freestanding_disabler, ui_menu.selected_tab == 2 and main_page)
    ui.set_visible(new_menu_references["AntiAim"].freestanding_options, ui_menu.selected_tab == 2 and main_page)
    ui.set_visible(jitter_add, ui_menu.selected_tab == 2 and jitter_page and ui.get(new_menu_references["AntiAim"].aa_state[ui.get(new_menu_references["AntiAim"].aa_state_menu)].yawjitter) == "jitter ways" )
    ui.set_visible(jitter_remove, ui_menu.selected_tab == 2 and jitter_page and ui.get(new_menu_references["AntiAim"].aa_state[ui.get(new_menu_references["AntiAim"].aa_state_menu)].yawjitter) == "jitter ways")
    ui.set_visible(new_menu_references["Keybinds"].enable_key, ui_menu.selected_tab == 2 and main_page)
    ui.set_visible(new_menu_references["Keybinds"].manual_forward, ui_menu.selected_tab == 2 and main_page and keys)
    ui.set_visible(new_menu_references["Keybinds"].manual_left, ui_menu.selected_tab == 2 and main_page and keys)
    ui.set_visible(new_menu_references["Keybinds"].manual_right, ui_menu.selected_tab == 2 and main_page and keys)
    ui.set_visible(new_menu_references["Keybinds"].manual_reset, ui_menu.selected_tab == 2 and main_page and keys)
    ui.set_visible(new_menu_references["Keybinds"].global_fs, ui_menu.selected_tab == 2 and main_page and keys)
    ui.set_visible(new_menu_references["AntiAim"].anti_kniv, ui_menu.selected_tab == 2 and main_page)
    ui.set_visible(new_menu_references["AntiAim"].anti_ze, ui_menu.selected_tab == 2 and main_page)
    ui.set_visible(new_menu_references["AntiAim"].anti_zeus, ui_menu.selected_tab == 2 and main_page and ui.get(new_menu_references["AntiAim"].anti_ze))
    ui.set_visible(new_menu_references["AntiAim"].zeus_distance, ui_menu.selected_tab == 2 and main_page and ui.get(new_menu_references["AntiAim"].anti_ze))
    ui.set_visible(new_menu_references["AntiAim"].safe_dangerous, ui_menu.selected_tab == 2 and main_page)
    ...
    ui.set_visible(new_menu_references["Misc"].backtrack_new, ui_menu.selected_tab == 4 and main_page)
    ui.set_visible(new_menu_references["Misc"].manipsvbt, ui_menu.selected_tab == 4 and main_page and ui.get(new_menu_references["Misc"].backtrack_new))
    ui.set_visible(new_menu_references["Misc"].fix_osaa,ui_menu.selected_tab == 4)
    ui.set_visible(new_menu_references["Misc"].fast_ladder,ui_menu.selected_tab == 4)
    ui.set_visible(new_menu_references["Misc"].ladder_yaw_slider,ui_menu.selected_tab == 4 and ui.get(new_menu_references["Misc"].fast_ladder))
...
client.set_event_callback("player_death",function(ent)
    ...
    if client.userid_to_entindex(ent.userid) == entity.get_local_player() then
        ...
        for k,v in ipairs(aa.aa_states) do
            ...
            if ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) == "l & r" or ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) == "delayed" or ui.get(new_menu_references["AntiAim"].aa_state[v].yaw) == "jittery" then
                break
            end
          
            if ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw) == "anti bruteforce" and not brute_stage.to_reset then
                brute_stage.brute[v].missed_bullets = 0
...
client.set_event_callback("round_start",function(ent)
    if brute_stage.round_starting == true and brute_stage.to_reset == false then
        ...
        for k,v in ipairs(aa.aa_states) do
            ...
            if ui.get(new_menu_references["AntiAim"].aa_state[v].gs_bodyyaw) == "anti bruteforce" then
...
function misc_functions:local_animations()
    ...
    if table_contains(ui.get(new_menu_references["Misc"].enable_local_animations),"legbreaker") then
        local math_randomized = math.random(3,0)

        ui.set(menu_reference.leg_movement, math_randomized == 1 and "Off" or "Always slide")
        entity.set_prop(entity.get_local_player(), "m_flPoseParameter", 1, globals.tickcount() % 4 > 1 and 5 / 10 or 1)
    end

    if table_contains(ui.get(new_menu_references["Misc"].enable_local_animations),"remove lean") then
    ...
    if table_contains(ui.get(new_menu_references["Misc"].enable_local_animations),"running") then
...
client.set_event_callback("paint",function()
    ...
    --aa_functions.can_defensive = false
    if visual_functions.ticks <= -1 and aa_functions.to_start == false and ui.get(new_menu_references["AntiAim"].aa_state[aa_functions.aa_number].enabled_exploit) then
...
client.set_event_callback("paint_ui", function()
    ...
    if ui.get(new_menu_references["AntiAim"].aa_state_menu) == "Legit" and (ui.get(new_menu_references["AntiAim"].aa_state["Legit"].yaw) == "jittery" or ui.get(new_menu_references["AntiAim"].aa_state["Legit"].yaw) == "delayed")then
        ui.set(new_menu_references["AntiAim"].aa_state[9].yaw,"180")
    end
