...
memorys.visuals = function()
	local local_player = entity_get_local_player()
	if not local_player or not entity.is_alive(local_player) or not ui.get(memorys.data.elements.visuals.master_switch) then
		return
	end
	...
	if memorys.contains(memorys.data.elements.visuals.indication_list, "Debug Window") then               
		...
		local SM4 =  memorys.gradient_text(132, 255, 255, 160, 245, 60, 180, 255, "Technoglogies [Stable]")
...
function lagcomp:should_lag_compensate(player)
    ...
    if entity_is_dormant(player) then
        return false
    end
...
local function ray_predicted(c, player, data)
    ...
    for i = 1, #edges do
        if points[edges[i][1]] ~= nil and points[edges[i][2]] ~= nil then
            ...
            renderer_line(p1[1], p1[2], p2[1], p2[2], 0, 252, 239, 220)
        end
    end
    ...
    if st1[1] ~= nil and st2[1] ~= nil then
        renderer_line(st1[1], st1[2], st2[1], st2[2], 0, 252, 239, 220)
    end
    ...
    if x1 ~= nil and entity_is_alive(player) and a_multiplier > 0 then
        ...
        if x_center ~= nil then
            ...
            client.draw_text(c, x_center, y1 - 15 + y_additional, 0, 255, 0, alpha > 220 and 220 or alpha, "c-", 0, "LC BROKEN")
        end
    end
end
...
local vis_check = ui.new_checkbox("visuals", "Other ESP", "\a12EC0997Enable \aFFFFFFFFLast Seen ESP")
local vis = ui.new_color_picker("visuals", "Other ESP", "Last seen", 255, 255, 255, 255)
...
client_set_event_callback("paint", function()
	...
	for key, vec in pairs(lastPos) do
		...
		if x and ui.get(vis) then
			renderer_text(x+length-49, y-height-10, r, g, b, a, "-", nil, "LAST SEEN")
			renderer_text(x+length-14, y-height-2, r, g, b, a, "-", nil, entity_get_player_name(key):upper())
...
client.set_event_callback("paint_ui", function()
    ...
    if ui.is_menu_open() then
		...
		renderer.text(menux - 189, menuy + 90, 155, 255, 255, 255, '-', 0, 'STABLE')  --VersionReplace
        ...
        renderer.text(menux - 280, menuy + 105, 255, 255, 255, 255, '-', 0, 'LATEST UPDATE: 19.05.2023')
        ...
        renderer.text(menux - 270, menuy + 140, 255, 255, 255, 255, '-', 0, '-[~] FIXED TELL A PROGRAMMER CRASH -> CHANGED INFO PANEL')
        renderer.text(menux - 270, menuy + 150, 255, 255, 255, 255, '-', 0, '-[+] ADDED LAST SEEN ESP WITH CHANGEABLE COLOR ')
		renderer.text(menux - 270, menuy + 160, 255, 255, 255, 255, '-', 0, '-[+] ADDED ANIMATED HITLOGS WITH ALOT OF CUSTOMIZATION')
        --renderer.text(menux - 270, menuy + 170, 255, 255, 255, 255, '-', 0, '-[+] ADDED MEWTWO RESOLVER SAFE POINT LOGIC (EARLY STATE)')
		renderer.text(menux - 280, menuy + 190, 255, 255, 255, 255, '-', 0, '-[LOOKING FORWARD TO ADD]')
        renderer.text(menux - 270, menuy + 200, 0, 255, 0, 255, '-', 0, '-SHARED LOGO (NL USERS WILL SEE MEWTWO GS USERS)')
        renderer.text(menux - 270, menuy + 210, 0, 255, 0, 255, '-', 0, '-RAGEBOT LIVE READER (IF HEAD RESOLVABLE OR PREFERS BODY)')
        renderer.text(menux - 270, menuy + 220, 0, 255, 0, 255, '-', 0, '-3/5 WAY JITTER, CUSTOMIZABLE IN EVERYWAY')
		--renderer.text(menux - 270, menuy + 230, 0, 255, 0, 255, '-', 0, '-FIXING SMALL ERRORS HIDE RAGE AND ERROR IMAGE ON LOAD')
...
local ui_new_combobox, ui_reference, ui_set_visible, ui_get, ui_set_callback = ui.new_combobox, ui.reference, ui.set_visible, ui.get, ui.set_callback
local math_floor = math.floor
...
local function on_enabled_changed()
    ...
    ui.set(default_reference, enabled == "gamesense")
end
...
local label3 = ui.new_label("RAGE", "Aimbot", "                    \abbc4fbffVersion: \ae4aff5ff[Stable]")
local label = ui.new_label("RAGE", "Aimbot", " ")
