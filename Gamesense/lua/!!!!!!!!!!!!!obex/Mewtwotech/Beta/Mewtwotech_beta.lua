...
memorys.visuals = function()
	...
	if memorys.contains(memorys.data.elements.visuals.indication_list, "Debug Window") then               
		...
		local SM4 =  memorys.gradient_text(132, 255, 255, 160, 245, 60, 180, 255, "Technoglogies [Beta]")
...
client.set_event_callback("paint_ui", function()
    ...
    if ui.is_menu_open() then
        if not ui.get(menu_disabler) then
		    ...
		    renderer.text(menux - 183, menuy + 90, 255, 135, 255, 255, '-', 0, 'BETA')  --VersionReplace
            ...
            renderer.text(menux - 280, menuy + 105, 255, 255, 255, 255, '-', 0, 'LATEST UPDATE: 04.08.2023')
            renderer.text(menux - 280, menuy + 120, 255, 255, 255, 255, '-', 0, '[UPDATE 04.08]')
            renderer.text(menux - 270, menuy + 130, 255, 255, 255, 255, '-', 0, '-[+ / *] MADE CHANGES TO LAST SEEN ESP')
            renderer.text(menux - 270, menuy + 140, 255, 255, 255, 255, '-', 0, '-[+] ADDED OPTION TO HIDE THIS SIDE MENU')
            renderer.text(menux - 270, menuy + 150, 255, 255, 255, 255, '-', 0, '-[+] ADDED/OPTIMIZED LC BOX PREDICT -> ABLE TO CHANGE COLORS')
		    renderer.text(menux - 270, menuy + 160, 255, 255, 255, 255, '-', 0, '         IT SHOWS IF ENEMY BREAKS LC IF IT PREDICTS IT ')
            renderer.text(menux - 270, menuy + 170, 255, 255, 255, 255, '-', 0, '         IF ENEMY HAS HIGH JITTER AND SHIFT TICKS IT TELLS YOU ')
            ...
            renderer.text(menux - 270, menuy + 210, 0, 255, 0, 255, '-', 0, '-FINSIHING RESOLVER REWORK/OPTIMIZATION')
            renderer.text(menux - 270, menuy + 220, 0, 255, 0, 255, '-', 0, '-HITLOGS REWORK')
            renderer.text(menux - 270, menuy + 230, 0, 255, 0, 255, '-', 0, '-NEW AA ADJUSTMENTS (NEW MODES JITTER WAYS & DEF STATE)')
