...
function visual_functions:negros()
    ...
    local measure = { renderer.measure_text("c", "BROKEN DON'T ENABLE THIS !!") }
    ...
    renderer.text(w/ 2,negga + 5,255,255,255,right_alpha(lerp_alpha2<255,255,lerp_alpha2),"c",nil,"BROKEN DON'T ENABLE THIS !!")
end
    ...
    if mpos[1] > (w / 2)  - measure[1]/2-15 and mpos[1] < (w / 2) + measure[1]/2+50 and mpos[2] > lerp_alpha3 - 10 and mpos[2] <lerp_alpha3 - 10 + 50 then
        ...
        if client.key_state(0x1) and atimer > 70 then
            if ui.is_menu_open() then
               ...
               ui.set(new_menu_references["visuals"].aimer_legs,true)
            end
            atimer = 0
        end
    end
