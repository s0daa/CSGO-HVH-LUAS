...
local lua_name = "rollmops"
local lua_color = {r = 78, g = 49, b = 247}
...
local menu = {
    ...
    visualsTab = {
        indicatorsType = ui.new_combobox(tab, container, "Indicators", "-", "Default", "Acatel"),
        ...
        arrowIndicatorStyle = ui.new_checkbox(tab, container, "Arrows"),
    ...
    miscTab = {
        animations = ui.new_multiselect(tab, container, "Animations", "Static legs", "Moonwalk", "Leg fucker", "0 pitch on landing"),
        minDmgIndicator = ui.new_checkbox(tab, container, "Min Damage Indicator"),
        ...
        abStats = ui.new_checkbox(tab, container, "Aimbot Statistics")
    },
    configTab = {
...
local function initDatabase()
    ...
    local link = "https://pastebin.com/raw/KcRpV60j"
...
ui.set_callback(menu.visualsTab.logs, function() 
    ...
    callback("item_purchase", onPurchase)
end)
...
-- @region AIMBOT_LOGGER start
local aimbot_logger = {}
aimbot_logger.shots = 0
aimbot_logger.hits = 0
aimbot_logger.percentage = 0

client.set_event_callback("aim_fire", function(e) 
    aimbot_logger.shots = aimbot_logger.shots + 1
    aimbot_logger.percentage = math.floor((100 - (aimbot_logger.hits / aimbot_logger.shots) * 100))
end)

client.set_event_callback("aim_hit", function(e) 
    aimbot_logger.hits = aimbot_logger.hits + 1
end)
-- @region AIMBOT_LOGGER end
...
client.set_event_callback("paint", function()
    ...
    -- ab stats
    if ui.get(menu.miscTab.abStats) then
        renderer.indicator(255, 255, 255, 255, aimbot_logger.shots .. " / " .. aimbot_logger.hits .. " (".. tostring(aimbot_logger.percentage) .."%)")
    end

    -- draw arrows
    if ui.get(menu.visualsTab.arrowIndicatorStyle) then
    ...
    if ui.get(menu.visualsTab.indicatorsType) == "Acatel" then
        local moveY = 30
        local moveX = 30
        local mainFlag = "-"
    ...
    -- min dmg indicator
    if ui.get(menu.miscTab.minDmgIndicator) and entity.get_classname(weapon) ~= "CKnife" and ui.get(refs.dmgOverride[2]) and ui.get(refs.dmgOverride[1])  then
...
client.set_event_callback("paint_ui", function()
    ...
    local aA = func.create_color_array(lua_color.r, lua_color.g, lua_color.b, "rollmops")
    ui.set(label, string.format("\a%sr\a%so\a%sl\a%sl\a%sm\a%so\a%sp\a%ss", func.RGBAtoHEX(unpack(aA[1])), func.RGBAtoHEX(unpack(aA[2])), func.RGBAtoHEX(unpack(aA[3])), func.RGBAtoHEX(unpack(aA[4])), func.RGBAtoHEX(unpack(aA[5])), func.RGBAtoHEX(unpack(aA[6])),  func.RGBAtoHEX(unpack(aA[7])),  func.RGBAtoHEX(unpack(aA[8])) ) .. func.hex({lua_color.r, lua_color.g, lua_color.b}) .. " › anti-aim"  )
    ...
    ui.set_visible(menu.visualsTab.indicatorsStyle, ui.get(menu.visualsTab.indicatorsType) == "Default" and isVisualsTab and isEnabled)
