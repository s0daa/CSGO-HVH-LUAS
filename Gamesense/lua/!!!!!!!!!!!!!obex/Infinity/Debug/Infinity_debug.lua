...
client.color_log(lua_color.r, lua_color.g, lua_color.b, " infinity\0")
...
local menu = {
    ...
    visualsTab = {
        indicatorsType = ui.new_combobox(tab, container, "Indicators", "-", "infinity", "minimal"),
        --indicatorsType = ui.new_combobox(tab, container, "Indicators", "-", "1", "2", "3"),
        indicatorsClr = ui.new_color_picker(tab, container, "Main Color", lua_color.r, lua_color.g, lua_color.b, 211),
        ...
        arrowIndicatorStyle = ui.new_combobox(tab, container, "Arrows", "-", "TeamSkeet", "Infinity"),
        --arrowIndicatorStyle = ui.new_combobox(tab, container, "Arrows", "-", "TeamSkeet", "TeamSkeet Dynamic", "Modern"),
        arrowClr = ui.new_color_picker(tab, container, "Arrow Color", lua_color.r, lua_color.g, lua_color.b, 211),
        logs = ui.new_multiselect(tab, container, "Notifications", "Hit", "Miss", "Purchase"),
        logsClr = ui.new_color_picker(tab, container, "Logs Color", lua_color.r, lua_color.g, lua_color.b, 211),
        screenIndication = ui.new_multiselect(tab, container, "Screen indication", "Defensive Manager", "Slowdown"),
        --screenIndication = ui.new_multiselect(tab, container, "Screen indication", "Defensive Manager", "Slowdown", "Flag"),
        screenClr = ui.new_color_picker(tab, container, "Screen Color", lua_color.r, lua_color.g, lua_color.b, 211),
    },
    miscTab = {
        fixHideshots = ui.new_checkbox(tab, container, "Fix hideshots"),
        avoidBackstab = ui.new_checkbox(tab, container, "Avoid Backstab"),
        fastLadder = ui.new_multiselect(tab, container, "Fast ladder", "Up", "Down"),
        animations = ui.new_multiselect(tab, container, "Anim breakers", "Static legs", "Moonwalk", "Leg fucker", "0 pitch on landing"),
        minDmgIndicator = ui.new_combobox(tab, container, "Minimum Damage Indicator", "-", "skeet", "neverlose"),
    },
    configTab = {
...
local function initDatabase()
    ...
    local link = "https://pastebin.com/raw/ZgQABcWN"
...
local notifications = {
    ...
    render = function()
        ...
        for i = 1, #data do
            ...
            -- renderer.text(x/2 - strw/2, y - 100 - Offset, 255, 255, 255, 255 * fraction, "cb", 0,color_text(" $infinity  ", r, g, b, a * fraction))        
        end
...
client.set_event_callback("paint", function()
    ...
    -- draw arrows
    if ui.get(menu.visualsTab.arrowIndicatorStyle) == "Infinity" then -- arrows modern
    ...
    --local function paint_watermark()
      ...
      --renderer.text(
         -- 40,
          --screen_height / 2-17,
          --211, 183, 211, 255 * math.abs(globals.curtime() * 2 % 2 -1 ),
          --'cd',
         -- 0,
          --'I N F I N I T Y')
    --end
    ...
    if ui.get(menu.visualsTab.indicatorsType) == "minimal" then
        ...
        renderer.text(sizeX/2 + ((namex + 2)/2) * scopedFraction, sizeY/2 + 20 - dpi/10, 255, 255, 255, 255, ui.get(menu.visualsTab.indicatorsType) == "minimal" and "cd-" or "cdb", nil, unpack(logo))
    ...
    if ui.get(menu.visualsTab.indicatorsType) == "infinity" then
    ...
    if func.table_contains(ui.get(menu.visualsTab.screenIndication), "Flag") then
        ...
        if flagimg ~= nil and download ~= nil then
            ...
            renderer.text(33, sizeY/2 - 2 + mainY, 211, 183, 211, 255, "-d", nil, "infinity" .. func.hex({mainClr.r, mainClr.g, mainClr.b}) .. ".RED")
            renderer.text(33, sizeY/2 - 4 + marginY + mainY, 211, 183, 211, 255, "-d", nil, func.hex({mainClr.r, mainClr.g, mainClr.b}) .. "[" .. userdata.build:upper() .. "]")
        else
            downloadFile()
        end
    end
    ...
    if ui.get(menu.miscTab.minDmgIndicator) ~= "-" and entity.get_classname(weapon) ~= "CKnife"  then
        if ui.get(menu.miscTab.minDmgIndicator) == "neverlose" then
        ...
        elseif ui.get(refs.dmgOverride[1]) and ui.get(refs.dmgOverride[2]) and ui.get(menu.miscTab.minDmgIndicator) == "skeet" then
...
client.set_event_callback("paint_ui", function()
    ...
    local aA = func.create_color_array(lua_color.r, lua_color.g, lua_color.b, "infinity")
    ui.set(label, string.format("\a%si\a%sn\a%sf\a%si\a%sn\a%si\a%st\a%sy", func.RGBAtoHEX(unpack(aA[1])), func.RGBAtoHEX(unpack(aA[2])), func.RGBAtoHEX(unpack(aA[3])), func.RGBAtoHEX(unpack(aA[4])), func.RGBAtoHEX(unpack(aA[5])), func.RGBAtoHEX(unpack(aA[6])),  func.RGBAtoHEX(unpack(aA[7])),  func.RGBAtoHEX(unpack(aA[8])),  func.RGBAtoHEX(unpack(aA[9])) ) .. func.hex({lua_color.r, lua_color.g, lua_color.b}) .. " › anti-aim"  )
