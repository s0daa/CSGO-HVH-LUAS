...
if not pcall(client.create_interface) then
	error("[Arctique.polar]: error: please make sure Allow unsafe scripts is enabled and try again")
	return
end
...
local InVincible = {}
InVincible.__index = setmetatable(InVincible, {})
InVincible.data = {
	Flip = false,
	BodyYaw = 0,
	SelfHeight = 0,
	...
	PlayerStateName2 = "",
	WaysAnglesStage = {},
	AntiBruteReset = false,
	UpdateTimerState = {},
	SimulationShifting = 0,
	Token = "Gzq106591.",
	NextTickDesync = false,
	LastAntiBruteTimer = 0,
	ReleaseTickBase = false,
	ScopeLineElement = nil,
	CachedSurfaceFont = {},
	Developer = "SYR1337",
	AntiBruteResetTimer = 5,
	ShouldRecharged = true,
	...
	ElementFirstColor = {185, 181, 241, 255},
	OtherWatermarkColor = {150, 150, 150, 255},
	DefaultPointerColor = {185, 181, 241, 255},
	...
	Configs = database.read("[Arctique.polar]Configs") or {},
	SelfName = panorama.open().MyPersonaAPI.GetName(),
	ConfigsList = database.read("[Arctique.polar]Configs Data") or {},
	BaseConfig = database.read("[Arctique.polar]Base Config") or false,
	IdealGradientColors = {{1, 59, 175, 255}, {202, 70, 205, 255}, {201, 227, 58, 255}},
	LocaltionLastUpdateTimer = database.read("[Arctique.polar]Fired Last Update Timer"),
	PlayerState = {"Global", "Stand", "Run", "Slow", "Air", "Air+", "Crouch", "Crouch+", "Onshot", "FL"},
	Trace = require("gamesense/trace") or error("[Arctique.polar]error: failed load library: gamesense/trace, please go to lua workshop and subscribe this library"),
	Icons = require("gamesense/icons") or error("[Arctique.polar]error: failed load library: gamesense/icons, please go to lua workshop and subscribe this library"),
	Entity = require("gamesense/entity") or error("[Arctique.polar]error: failed load library: gamesense/entity, please go to lua workshop and subscribe this library"),
	Base64 = require("gamesense/base64") or error("[Arctique.polar]error: failed load library: gamesense/base64, please go to lua workshop and subscribe this library"),
	Surface = require("gamesense/surface") or error("[Arctique.polar]error: failed load library: gamesense/surface, please go to lua workshop and subscribe this library"),
	Clipboard = require("gamesense/clipboard") or error("[Arctique.polar]error: failed load library: gamesense/clipboard, please go to lua workshop and subscribe this library"),
	CsgoWeapons = require("gamesense/csgo_weapons") or error("[Arctique.polar]error: failed load library: gamesense/csgo_weapons, please go to lua workshop and subscribe this library"),
...
InVincible.__index.ContainText = function(self, list, match)
...
InVincible.__index.ToInteger = function(self, var)
	return math.floor(var + 0.5)
end

InVincible.__index.GetTimeScale = function(self)
	return client.timestamp() / 1000
end

InVincible.__index.BindArg = function(self, handler, address)
	return function(...)
		return handler(address, ...)
	end
end

InVincible.__index.ToGradientColor = function(self, text)
	return self:RgbaToHexGradientText(self.data.IdealGradientColors[1], self.data.IdealGradientColors[2], text)
end

InVincible.__index.RgbToHexText = function(self, colors, text)
...
InVincible.__index.RgbaToHex = function(self, colors)
...
InVincible.__index.RgbaToHexText = function(self, colors, text)
...
InVincible.__index.Clamp = function(self, var, min, max)
	local math_vars = self:GetMathVars(min, max)
	return math.max(math.min(var, math_vars.max), min)
end

InVincible.__index.GetMathVars = function(self, ...)
...
InVincible.__index.Contains = function(self, tab, this)
...
InVincible.__index.Lerp = function(self, current, target, percentage)
...
InVincible.__index.HexTextToOriginal = function(self, text)
...
InVincible.__index.GetStringBytes = function(self, string, index)
	local count_byte = 1
...
InVincible.__index.UTF8DecodeSupport = function(self, string)
	local string_list = {}
	local last_index = 0
	local last_count = 1
	local start_count = 0
...
InVincible.__index.RgbaToHexGradientText = function(self, color_1, color_2, text)
	local gradient_text = ""
...
InVincible.__index.IsElement = function(self, element)
	return type(element) == "table" and element.metatype and element.metatype == "Element"
end

InVincible.__index.FirstUpper = function(self, text, length)
	return text:sub(1, length or 1):upper()
end

InVincible.__index.FirstUpperColor = function(self, text, color)
	if color then
...
InVincible.__index.NewSliderIdentification = function(self, list)
...
InVincible.__index.ContainsUI = function(self, ui, list)
...
InVincible.__index.MultiColorText = function(self, data)
	local CurrentText = ""
...
InVincible.__index.NewElement = function(self, ref, list)
...
InVincible.__index.WritePackage = function(self, group, key)
...
InVincible.__index.GetPackages = function(self, elements)
	local Configs = {}
	self.data.ConfigPackage = {}
...
InVincible.__index.FiredUpdateLog = function(self)
	if self.data.LocaltionLastUpdateTimer ~= self.data.LastUpdateTimer then
		...
		table.insert(self.data.NotifyCached, {
			...
			Title = ("Arctique%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".polar updated !")),
...
InVincible.__index.CopyCurrentConfigs = function(self, elements)
	...
	table.insert(self.data.NotifyCached, {
		...
		Title = ("Arctique%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".polar")),
...
InVincible.__index.PasteCurrentConfigs = function(self, elements)
	self.data.ConfigPackage = {}
	self:WritePackage(elements)
	local Configs = self.data.Clipboard.get()
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	if not Configs then
		client.error_log(("[%s] Config Failed Paste: The Clipboard Data Is Arctique Data"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
			...
			ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "The Clipboard Data Is Arctique Data")),
			Title = ("Arctique%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".polar -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
			Svg = {
				Size = self.data.Vector(48, 48),
				Texture = self.data.SettingsSvg
			}
	...
	if not Successed or not JsonData then
		client.error_log(("[%s] Config Failed Paste: The Config Is Arctique Config, Please Check Your Clipboard"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
			...
			UserName = ("User: %s"):format(self.data.SelfName),
			AnimationColor = self.data.NotifyWarningAnimationColor,
			ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "This Config Is Arctique Config")),
			Title = ("Arctique%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".polar -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
			Svg = {
				Size = self.data.Vector(48, 48),
				Texture = self.data.SettingsSvg
...
InVincible.__index.CreateElements = function(self)
	self.Elements = {
		MasterSwitch = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", "\nMaster-Switch")),
		MasterSwitchLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "♮ Master-Switch")),
		JoinDiscord = self:NewElement(ui.new_button("AA", "Other", self:RgbaToHexGradientText({255, 255, 255, 220}, {185, 181, 255, 255}, "Join Servers"), function()
			self.data.PanoramaApi.SteamOverlayAPI.OpenExternalBrowserURL("https://discord.gg/ebgQnH9E")
		end)),

		Group = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "£"), self:RgbaToHexText({255, 255, 255, 206}, "Groups")), {"Main", "Visuals", "Misc", "Configs"})),
		OwnerLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "                     Owner: Przunxible")),
		AntiAim = {
			DividingLabel4 = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "                          ")),
			keybinds = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "                          Keybind", {"Basic"}), {"Basic"}),
			LegitOnKey = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", ("%s Legit Key"):format(("%s%s"):format(self:RgbaToHexText(self.data.DefaultPointerColor, "♮"), self:RgbaToHex(self.data.DefaultElementColor))))),
			EdgeYaw = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", ("%s Edge Yaw"):format(("%s%s"):format(self:RgbaToHexText(self.data.DefaultPointerColor, "♮"), self:RgbaToHex(self.data.DefaultElementColor))))),
			Freestanding = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", ("%s Freestanding"):format(("%s%s"):format(self:RgbaToHexText(self.data.DefaultPointerColor, "♮"), self:RgbaToHex(self.data.DefaultElementColor))))),
			DividingLabel5 = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "                          ")),
			DividingLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "                          Anti-Aim")),
			TeamGroup = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "\n player team", self.data.Teammate)),
			PlayerState = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "\n player state", self.data.PlayerState)),
			Custom = (function()
				local Elements = {}
				for i, team in pairs(self.data.Teammate) do
					...
					for index, data in pairs(self.data.PlayerState) do
						...
						Elements[i][index] = {
							Team = team,
							Pointer = data,
							Enabled = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("[%s] Enabled Player-State\n%s%s"):format(self:FirstUpperColor(data, self.data.ElementFirstColor), data, team), data == "Global")),
							Pitch = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Yaw Jitter Offset Left"):format(ThisTeam, data), {"Off", "Default", "Up", "Down", "Minimal", "Random", "Custom", "Jitter", "Tickcount"})),
							PitchAngles = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Pitch Angles"):format(ThisTeam, data), - 90, 90, 0)),
							PitchExtraAngles = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Pitch Extra"):format(ThisTeam, data), - 90, 90, 0)),
							PitchTickcount = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Pitch Tickcount"):format(ThisTeam, data), 0, 3, 0, true, "", 1, self:NewSliderIdentification({"Slow", "Medium", "High", "Fast"}))),
							YawBase = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Yaw Base"):format(ThisTeam, data), {"Local view", "At targets"})),
							Yaw = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - Yaw\n%s%s"):format(ThisTeam, data))), {"Off", "180", "Spin", "Static", "180 Z", "Crosshair", "180 Left/Right"})),
							YawOffset = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Yaw Offset"):format(ThisTeam, data), - 180, 180, 0)),
							YawExtraOffset = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Extra Yaw Offset"):format(ThisTeam, data), - 180, 180, 0)),
							YawJitter = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - Yaw Jitter\n%s%s"):format(ThisTeam, data))), {"Off", "Offset", "Center", "Random", "Skitter", "X-Ways"})),
							YawJitterOffsetLeft = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Yaw Jitter Offset Left Slr"):format(ThisTeam, data), - 180, 180, 0)),
							YawJitterWays = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Yaw Jitter Ways"):format(ThisTeam, data), 2, 10, 2)),
							YawJitterTickcount = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Yaw Jitter Tickcount"):format(ThisTeam, data), 0, 2, 0, true, "", 1, self:NewSliderIdentification({"Default", "Slow", "Auto"}))),
							BodyYaw = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - Body Yaw\n%s%s"):format(ThisTeam, data))), {"Off", "Static", "Jitter", "Jitter #Limitate", "Jitter #Advanced", "Opposite"})),
							BodyYawExtra = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Body Yaw extra"):format(ThisTeam, data), {"Single", "Switch", "Timer", "Tickcount" ,"Random"})),
							BodyYawOffset = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Body Yaw Offset"):format(ThisTeam, data), - 180, 180, 0)),
							BodyYawExtraTickcount = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Body Yaw Tickcount"):format(ThisTeam, data), 0, 3, 0, true, "", 1, self:NewSliderIdentification({"Slow", "Medium", "High", "Fast"}))),
							BodyYawExtraTimer = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Body Yaw Timer"):format(ThisTeam, data), 0, 500, 0, true, "s", 0.01)),
							BodyYawExtraOffset = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Body Yaw Extra Offset"):format(ThisTeam, data), - 180, 180, 0)),
							Freestandingbodyyaw = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - Peek Fake\n%s%s"):format(ThisTeam, data))))),
							FakeYawLimitStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - Fake Limit\n%s%s"):format(ThisTeam, data))), {"Default", "Left/Right", "Jitter", "Tickcount", "Update", "Rotation", "Random"})),
							FakeYawLimitUpdate = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] -  Fake Yaw Update"):format(ThisTeam, data), 1, 10, 2)),
							FakeYawLimitTickcount = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] -  Fake Yaw Tickcount"):format(ThisTeam, data), 0, 3, 0, true, "", 1, self:NewSliderIdentification({"Slow", "Medium", "High", "Fast"}))),
							FakeYawLimit = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Fake Yaw Limit"):format(ThisTeam, data), 0, 60, 60)),
							ExtraFakeYawLimit = self:NewElement(ui.new_slider("AA", "Anti-Aimbot Angles", ("\n[%s - %s] - Extra Fake Yaw Limit"):format(ThisTeam, data), 0, 60, 60)),
							DefensiveTriggers = self:NewElement(ui.new_combobox("AA", "Other", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - trigger defensive\n%s%s"))), {"Always On", "Tickcount", "Simulate", "Shifting", "Random" })),
							DefensiveConditionTickcount = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Condition Tickcount"):format(ThisTeam, data), 2, 64, 6)),
							DefensiveDifferentTickcount = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Different Tickcount"):format(ThisTeam, data), 2, 64, 4)),
							DefensiveOverrideTriggers = self:NewElement(ui.new_combobox("AA", "Other", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - Addon Triggers\n%s%s"):format(ThisTeam, data))), {"Always On", "Tickcount", "Simulate", "Shifting"})),
							DefensiveOverrideConditionTickcount = self:NewElement(ui.new_slider("AA", "Other", ("[%s - %s] - Command Tickcount"):format(ThisTeam, data), 2, 64, 6)),
							DefensiveOverrideDifferentTickcount = self:NewElement(ui.new_slider("AA", "Other", ("[%s - %s] - Step Tickcount"):format(ThisTeam, data), 2, 64, 4)),
							enabled_exploit = self:NewElement(ui.new_checkbox("AA", "Other", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - pitch defensive\n%s%s"):format(ThisTeam, data))))),
							DefensivePitch = self:NewElement(ui.new_combobox("AA", "Other", ("\n[%s - %s] - Defensvie Pitch Ways"):format(ThisTeam, data), {"Sync", "Flick", "Flick Random", "X-Ways", "Flick Mode"})),
							DefensivePitchWays = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Defensvie Pitch Ways"):format(ThisTeam, data), 2, 10, 2)),
							DefensivePitchMode = self:NewElement(ui.new_combobox("AA", "Other", ("\n[%s - %s] - Defensvie Pitch Mode"):format(ThisTeam, data), {"Off", "Default", "Up", "Down", "Minimal", "Random"})),
							DefensivePitchFlickMode = self:NewElement(ui.new_combobox("AA", "Other", ("\n[%s - %s] - Defensvie Pitch Flick Mode"):format(ThisTeam, data), {"Off", "Default", "Up", "Down", "Minimal", "Random"})),
							enabled_exploit1 = self:NewElement(ui.new_checkbox("AA", "Other", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - yaw defensive\n%s%s"):format(ThisTeam, data))))),
							DefensiveYaw = self:NewElement(ui.new_combobox("AA", "Other", ("\n[%s - %s] - brute conditions"):format(ThisTeam, data), {"Sync", "Flick", "Flick Random", "X-Ways"})),
							DefensiveYawWays = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Defensvie Yaw Ways"):format(ThisTeam, data), 2, 10, 2)),
							DefensiveYawAngles = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Defensvie Yaw Angles"):format(ThisTeam, data), - 180, 180, 0)),
							AntiBruteCondition = self:NewElement(ui.new_multiselect("AA", "Other", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" - anti brute\n%s%s"):format(ThisTeam, data))), {"Hurt", "Miss"}), {"Hurt", "Miss"}),
							AntiBruteReset = self:NewElement(ui.new_multiselect("AA", "Other", ("\n[%s - %s] - s"):format(ThisTeam, data), {"Start", "End", "Newmap", "Timer"}), {"Start", "End", "Newmap", "Timer"}),
							AntiBruteTarget = self:NewElement(ui.new_combobox("AA", "Other", ("\n[%s - %s] - target"):format(ThisTeam, data), {"All Target", "Base Threat"})),
							AntiBruteSettings = self:NewElement(ui.new_multiselect("AA", "Other", ("\n[%s - %s] - options"):format(ThisTeam, data), {"Yaw", "Yaw Jitter"}), {"Yaw", "Yaw Jitter"}),
							AntiBruteYawSettings = self:NewElement(ui.new_combobox("AA", "Other", ("\n[%s - %s] - Yaw"):format(ThisTeam, data),  {"Switch", "Randomize", "Phase"})),
							AntiBruteYawSwitchSpeed = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Anti-Brute Yaw Switch"):format(ThisTeam, data), 0, 4, 0, true, "", 1, self:NewSliderIdentification({"Default", "Slow", "Medium", "High", "Fast"}))),
							AntiBruteYawSwitchDefaultAngles = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Anti-Brute Yaw Offset 1"):format(ThisTeam, data), - 180, 180, 0)),
							AntiBruteYawSwitchExtraAngles = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Anti-Brute Yaw Offset 2"):format(ThisTeam, data), - 180, 180, 0)),
							AntiBruteYawSwitchExtendedAngles = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Anti-Brute Yaw Offset 3"):format(ThisTeam, data), - 180, 180, 0)),
							AntiBruteYawJitterSettings = self:NewElement(ui.new_combobox("AA", "Other", ("%s%s"):format(PrefixName, self:RgbaToHexText(self.data.ElementFeaturesColor, (" Yaw Jitter\n%s%s"):format(ThisTeam, data))), {"Offset", "Center", "Random", "Skitter", "X-Ways"})),
							AntiBruteYawJitterOffset = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Anti-Brute Yaw Jitter Offset Right"):format(ThisTeam, data), - 180, 180, 0)),
							AntiBruteYawJitterWays = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Anti-Brute Yaw Jitter Ways"):format(ThisTeam, data), 2, 10, 2)),
							AntiBruteYawJitterTickcount = self:NewElement(ui.new_slider("AA", "Other", ("\n[%s - %s] - Anti-Brute Yaw Jitter Tickcount"):format(ThisTeam, data), 0, 1, 0, true, "", 1, self:NewSliderIdentification({"Default", "Slow"}))),
							CopyCurrentTeamValue = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", (self:RgbaToHexText(self.data.ElementFeaturesColor2, ("copy state"):format(ThisTeam, data))), function()
								self:CopyCurrentConfigs(Elements[i][index])
							end)),

							PasteCurrentTeamValue = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", (self:RgbaToHexText(self.data.ElementFeaturesColor2, ("paste state"):format(ThisTeam, data))), function()
		...
		Fakelag = {
			FakelagEnabled = self:NewElement(ui.new_checkbox("AA", "Fake Lag", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 200}, "Arc Fakelag")))),
			...
			FakelagOptions = self:NewElement(ui.new_multiselect("AA", "Fake Lag", "Options", {"Force Choked", "Break LC In Air", "Reset OS", "Optimize Modifier", "Force Discharge Scan"}), {"Force Choked", "Break LC In Air", "Reset OS", "Optimize Modifier", "Force Discharge Scan"}),
		...
		Visuals = {
			WatermarkIndication = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Watermarker"))), true),
			WatermarkBarColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "Watermark Color", unpack(self.data.AccentWatermarkColor))),
			WatermarkList = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "\nWatermark List", {"Version", "Time", "Latency"}), {"Version", "Time", "Latency"}),
			ManualIndication = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Manual Arrows")))),
			ManualColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "Manual Color", 255, 255, 255, 255)),
			ManualArrowStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "\nManual style", {"Mini", "<>", "⯇⯈"})),
			CrosshairIndication = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Crosshair ind")))),
			CrosshairStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "Pattern", {"Concen", "Manifold"})),
			CrosshairNameStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "Style", {"Bar", "Color Side", "Animation"})),
			CrosshairBarLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "| Bar Color")),
			CrosshairBarColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "| Bar Color", unpack(self.data.LightPinkedColor))),
			CrosshairRealSideLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "| Main Color")),
			CrosshairRealSideColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "| Main Color\n Color", self.data.LightPinkedColor[1], self.data.LightPinkedColor[2], self.data.LightPinkedColor[3], 0)),
			CrosshairFakeSideLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "| Minor Color")),
			CrosshairFakeSideColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "| Minor Color\n Color", unpack(self.data.LightPinkedColor))),
			CrosshairAnimationStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "Animation Style", {"stationary", "opposite", "directory"})),
			CrosshairAnimationSide = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "Animation Side", {"Default", "Reversed", "Repeatedly"})),
			CrosshairHotkeyStyle = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "Hotkeys Style", {"Simplicity", "Complete"}), {"Simplicity", "Complete"}),
			CrosshairSettings = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "\n Settings", {"Text (Glow)", "Hotkey (DT Recharge Circle)", "Hotkey (Animation Text)"}), {"Text (Glow)", "Hotkey (DT Recharge Circle)", "Hotkey (Animation Text)"}),
			CrosshairHotkeysDoubleTapLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "| DT Color")),
			CrosshairHotkeysDoubleTapColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "| DT Color\n Color", unpack(self.data.ElementFeaturesColor))),
			CrosshairHotkeysonshotLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "| Hide Color")),
			CrosshairHotkeysonshotColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "| Hide Color\n Color", unpack(self.data.ElementFeaturesColor))),
			CrosshairHotkeysOtherLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "| Other Color")),
			CrosshairHotkeysOtherColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "| Other Color\n Color", unpack(self.data.LightPinkedColor))),
			CrosshairHotkeysExtraLabel = self:NewElement(ui.new_label("AA", "Anti-Aimbot Angles", "| Complete Color")),
			CrosshairHotkeysExtraColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "| Complete Color\n Color", unpack(self.data.ElementFeaturesColor))),
			CrosshairHotkeySettings = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "Hotkey Settings", {"Fraction", "Thickness", "Animation Speed"}), {"Fraction", "Thickness", "Animation Speed"}),
			DebugPanel = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Debug Panel"))), true),
			HitlogIndication = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Hitlog")))),
			HitlogColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "Hitlog Color", 255, 255, 255, 255)),
			HitlogStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "Style", {"Novel","-"})),
			HitlogBackgroundStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "\nBackground", {"Blur", "Rect"})),
			AntiBruteLogIndication = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Anti-brute Log")))),
			AntiBruteLogColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "Anti-Brute  Log Color", 185, 181, 255, 255)),
			AntiBruteLogBackgroundStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "\nBackground", {"Blur", "Rect"})),
			AutoPeekIndication = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Auto Peek")))),
			AutoPeekColor = self:NewElement(ui.new_color_picker("AA", "Anti-Aimbot Angles", "Peek Circle Color\n Color", unpack(self.data.LightPinkedColor))),
			AutoPeekStyle = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "\nStyle", {"Default", "Overlay"})),
			AutoPeekAnimation = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "\nAnimation", {"Radius", "Modifier"}), {"Radius", "Modifier"}),
			enable_min_dmg_ovrrd = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Min Dmg Override")))),
		},

		Misc = {
			enabled_manual = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Manual AA"))), true),
			ManualLeft = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", "   Left")),
			ManualBack = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", "   Back")),
			ManualRight = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", "   Right")),
			ManualForward = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", "   Forward")),
			FastLadder = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s Fast Ladder"):format(("%s%s"):format(self:RgbaToHexText(self.data.DefaultPointerColor, "♮"), self:RgbaToHex(self.data.DefaultElementColor))))),
			ConsoleFilter = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Console Filter")))),
			AntiBackStab = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s Anti Backstab"):format(("%s%s"):format(self:RgbaToHexText(self.data.DefaultPointerColor, "♮"), self:RgbaToHex(self.data.DefaultElementColor))))),
			ForceDormant = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Dormant Static")))),
			Cmdyaw1 = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s %s"):format(self:RgbaToHexText({185, 181, 255, 255}, "♮"), self:RgbaToHexText({255, 255, 255, 206}, "Fake - \aB9B7F1FFlimit")))),
			UnDistinctionFakelag = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", ("%s Un Distinction DT/Fakelag"):format(("%s%s"):format(self:RgbaToHexText(self.data.DefaultPointerColor, "♮"), self:RgbaToHex(self.data.DefaultElementColor))))),
			DefensiveOverride1 = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "                              \aB9B7F1FF+\aFFFFFFFF/-", {"Force", "Custom","Anti Brute"}), {"Force", "Custom","Anti Brute"}),
			DoubleTapBoost = self:NewElement(ui.new_combobox("AA", "Anti-Aimbot Angles", "DT Boost - \aF65858FFunsafe", {"Off", "Boost", "Fast", "Top Speed"})),
			DoubleTapSettings = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles",("\n[%s - %s] - DT set"):format(ThisTeam, data), {"Dynamic Speed", "Dynamic Hitchance", "Force Discharged", "Disablers Clock Correction", "Anti-Defensive"}), {"Dynamic Speed", "Dynamic Hitchance", "Force Discharged", "Disablers Clock Correction", "Anti-Defensive"}),
			AntiDefensiveKey = self:NewElement(ui.new_hotkey("AA", "Anti-Aimbot Angles", "Anti-Defensive\n Tickbase")),
			DisChargeBackTrackScan = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", "Discharged Backtrack Scan")),
			AnimationBreaker = self:NewElement(ui.new_multiselect("AA", "Anti-Aimbot Angles", "Animation", {"Moon Walk: Move", "Moon Walk: Air", "Static Slow Walk", "Pitch 0 On Land"}), {"Moon Walk: Move", "Moon Walk: Air", "Static Slow Walk", "Pitch 0 On Land"}),
		},

		Configs = {
			ConfigsList = self:NewElement(ui.new_listbox("AA", "Anti-Aimbot Angles", "♮ Configs", self.data.Configs)),
			ConfigName = self:NewElement(ui.new_textbox("AA", "Anti-Aimbot Angles", "\n Config Name")),
			ConfigsControls = function(CallBacks)
				return {
					CreateConfig = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", self:MultiColorText({{"Create", self.data.ElementIdentificationColor}}), CallBacks.Create)),
					LoadConfig = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", self:MultiColorText({{"Load", self.data.ElementIdentificationColor}}), CallBacks.Load)),
					SaveConfig = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", self:MultiColorText({{"Save", self.data.ElementIdentificationColor}}), CallBacks.Save)),
					DeleteConfig = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", self:MultiColorText({{"Delete", self.data.ElementIdentificationColor}}), CallBacks.Delete)),
					ImportConfig = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", self:MultiColorText({{"Import", self.data.ElementIdentificationColor}}), CallBacks.Import)),
					ExportConfig = self:NewElement(ui.new_button("AA", "Anti-Aimbot Angles", self:MultiColorText({{"Export", self.data.ElementIdentificationColor}}), CallBacks.Export))
				}
			end,

			AutomaticSaveConfig = self:NewElement(ui.new_checkbox("AA", "Anti-Aimbot Angles", "Auto Save(\aA8A5F1FFtest\aFFFFFFCE)"))
		}
	}
...
InVincible.__index.HandleReference = function(self, state)
	...
	local FakelagisEnabled = self.Elements.Fakelag.FakelagEnabled:get()
	if FakelagEnabled or (true ~= FakelagEnabled and not FakelagEnabled) then
		ui.set_visible(self.References.fakelag_limit, state)
		ui.set_visible(self.References.fakelag_amount, state)
		ui.set_visible(self.References.fakelag_variance, state)
...
InVincible.__index.HandleMain = function(self)
	self:AutomaticSaveConfig()
	self:HandleReference(not self.Elements.MasterSwitch:get())
	self.Elements.Group:set_visible(self.Elements.MasterSwitch:get())
	self.Elements.JoinDiscord:set_visible(self.Elements.MasterSwitch:get())
	self.Elements.Fakelag.FakelagEnabled:set_visible(self.Elements.MasterSwitch:get())
	self.Elements.OwnerLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.Misc.Cmdyaw1:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Misc.FastLadder:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.AntiAim.keybinds:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.AntiAim.TeamGroup:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.Misc.AntiBackStab:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Misc.ForceDormant:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Misc.ConsoleFilter:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Misc.DoubleTapBoost:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Misc.enabled_manual:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.AntiAim.PlayerState:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.Visuals.DebugPanel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.AntiAim.DividingLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.Misc.AnimationBreaker:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Configs.ConfigName:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
	self.Elements.AntiAim.DividingLabel4:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.AntiAim.DividingLabel5:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.Configs.ConfigsList:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
	self.Elements.Misc.DefensiveOverride1:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main")
	self.Elements.Misc.UnDistinctionFakelag:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Fakelag.FakelagLimit:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Fakelag.FakelagEnabled:get())
	self.Elements.Visuals.HitlogIndication:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.Visuals.ManualIndication:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.Fakelag.FakelagAmount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Fakelag.FakelagEnabled:get())
	self.Elements.Visuals.AntiBruteLogColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.Fakelag.FakelagOptions:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Fakelag.FakelagEnabled:get())
	self.Elements.Configs.AutomaticSaveConfig:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
	self.Elements.Fakelag.FakelagVariance:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Fakelag.FakelagEnabled:get())
	self.Elements.Visuals.CrosshairIndication:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.Visuals.AutoPeekIndication:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.Visuals.enable_min_dmg_ovrrd:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.Visuals.WatermarkIndication:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.Visuals.AntiBruteLogIndication:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals")
	self.Elements.AntiAim.EdgeYaw:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.keybinds:get(1))
	self.Elements.Misc.ManualLeft:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Misc.enabled_manual:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Misc.ManualBack:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Misc.enabled_manual:get() and self.Elements.Group:get() == "Misc")
	self.Elements.AntiAim.LegitOnKey:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.keybinds:get(1) and self.Elements.Group:get() == "Main")
	self.Elements.Misc.ManualRight:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Misc.enabled_manual:get() and self.Elements.Group:get() == "Misc")
	self.Elements.Misc.ManualForward:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Misc.enabled_manual:get() and self.Elements.Group:get() == "Misc")
	self.Elements.AntiAim.Freestanding:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.keybinds:get(1))
	self.Elements.Visuals.HitlogStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.HitlogIndication:get())
	self.Elements.Visuals.HitlogColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.HitlogIndication:get())
	self.Elements.Visuals.ManualColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.ManualIndication:get())
	self.Elements.Visuals.AutoPeekStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.AutoPeekIndication:get())
	self.Elements.Visuals.AutoPeekColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.AutoPeekIndication:get())
	self.Elements.Misc.DoubleTapSettings:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc" and self.Elements.Misc.DoubleTapBoost:get() ~= "Off")
	self.Elements.Visuals.WatermarkList:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.WatermarkIndication:get())
	self.Elements.Visuals.ManualArrowStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.ManualIndication:get())
	self.Elements.Visuals.CrosshairStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get())
	self.Elements.Visuals.AutoPeekAnimation:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.AutoPeekIndication:get())
	self.Elements.Visuals.CrosshairSettings:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get())
	self.Elements.Visuals.CrosshairHotkeyStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get())
	self.Elements.Visuals.WatermarkBarColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.WatermarkIndication:get())
	self.Elements.Visuals.CrosshairRealSideLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get())
	self.Elements.Visuals.CrosshairRealSideColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get())
	self.Elements.Visuals.CrosshairFakeSideLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get())
	self.Elements.Visuals.CrosshairFakeSideColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get())
	self.Elements.Fakelag.FakelagResetonshotStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Fakelag.FakelagEnabled:get() and self.Elements.Fakelag.FakelagOptions:get(3))
	self.Elements.Visuals.AntiBruteLogBackgroundStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.AntiBruteLogIndication:get())
	self.Elements.Misc.AntiDefensiveKey:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc" and self.Elements.Misc.DoubleTapBoost:get() ~= "Off" and self.Elements.Misc.DoubleTapSettings:get(5))
	self.Elements.Misc.DisChargeBackTrackScan:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Misc" and self.Elements.Misc.DoubleTapBoost:get() ~= "Off" and self.Elements.Misc.DoubleTapSettings:get(3))
	self.Elements.Visuals.CrosshairHotkeySettings:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairSettings:get(1))
	self.Elements.Visuals.CrosshairNameStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairStyle:get() == "Concen")
	self.Elements.Visuals.CrosshairHotkeysExtraLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(2))
	self.Elements.Visuals.CrosshairHotkeysExtraColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(2))
	self.Elements.Visuals.CrosshairHotkeysOtherLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(1))
	self.Elements.Visuals.CrosshairHotkeysOtherColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(1))
	self.Elements.Visuals.CrosshairHotkeysonshotLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(1))
	self.Elements.Visuals.CrosshairHotkeysonshotColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(1))
	self.Elements.Visuals.CrosshairHotkeysDoubleTapLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(1))
	self.Elements.Visuals.CrosshairHotkeysDoubleTapColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairHotkeyStyle:get(1))
	self.Elements.Visuals.HitlogBackgroundStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.HitlogIndication:get() and self.Elements.Visuals.HitlogStyle:get() == "Novel")
	self.Elements.Visuals.CrosshairBarLabel:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and (self.Elements.Visuals.CrosshairNameStyle:get() == "Bar" or self.Elements.Visuals.CrosshairStyle:get() == "Style 2"))
	self.Elements.Visuals.CrosshairBarColor:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and (self.Elements.Visuals.CrosshairNameStyle:get() == "Bar" or self.Elements.Visuals.CrosshairStyle:get() == "Style 2"))
	self.Elements.Visuals.CrosshairAnimationStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairNameStyle:get() ~= "Color Side" and self.Elements.Visuals.CrosshairNameStyle:get() ~= "Off")
	self.Elements.Visuals.CrosshairAnimationSide:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Visuals" and self.Elements.Visuals.CrosshairIndication:get() and self.Elements.Visuals.CrosshairNameStyle:get() ~= "Color Side" and self.Elements.Visuals.CrosshairAnimationStyle:get() ~= "Off")



	if self.data.Initialized and type(self.Elements.Configs.ConfigsControls) == "table" then
		self.Elements.Configs.ConfigsControls.SaveConfig:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
		self.Elements.Configs.ConfigsControls.LoadConfig:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
		self.Elements.Configs.ConfigsControls.DeleteConfig:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
		self.Elements.Configs.ConfigsControls.CreateConfig:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
		self.Elements.Configs.ConfigsControls.ExportConfig:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
		self.Elements.Configs.ConfigsControls.ImportConfig:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Configs")
	end

	for i, list in pairs(self.Elements.AntiAim.Custom) do
		for index, data in pairs(list) do
			data.CopyCurrentTeamValue:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer)
			data.PasteCurrentTeamValue:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer)
			data.Yaw:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get())
			data.Pitch:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get())
			data.YawBase:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get())
			data.BodyYaw:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get())
			data.Enabled:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Pointer ~= "Global")
			data.Freestandingbodyyaw:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get())
			data.YawJitter:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.Yaw:get() ~= "Off")
			data.BodyYawExtra:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off")
			data.PitchTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.Pitch:get() == "Tickcount")
			data.YawExtraOffset:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.Yaw:get() == "180 Left/Right")
			data.PitchExtraAngles:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and self:Contains({"Jitter", "Tickcount"}, data.Pitch:get()))
			data.DefensiveOverrideTriggers:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) == "Custom")
			data.YawJitterWays:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.Yaw:get() ~= "Off" and data.YawJitter:get() == "X-Ways")
			data.YawJitterOffsetLeft:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.Yaw:get() ~= "Off" and data.YawJitter:get() ~= "Off")
			data.FakeYawLimit:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.Cmdyaw1:get() and data.FakeYawLimitStyle:get() ~= "Random")
			data.PitchAngles:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and self:Contains({"Custom", "Jitter", "Tickcount"}, data.Pitch:get()))
			data.DefensiveYaw:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.enabled_exploit1:get())
			data.DefensivePitch:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.enabled_exploit:get())
			data.FakeYawLimitStyle:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.Cmdyaw1:get())
			data.BodyYawExtraTimer:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and data.BodyYawExtra:get() == "Timer")
			data.enabled_exploit:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2))
			data.enabled_exploit1:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2))
			data.FakeYawLimitTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.Cmdyaw1:get() and data.FakeYawLimitStyle:get() == "Tickcount")
			data.DefensiveTriggers:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(1))
			data.AntiBruteCondition:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3))
			data.BodyYawExtraTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and not data.BodyYaw:get() ~= "Off" and data.BodyYawExtra:get() == "Tickcount")
			data.AntiBruteReset:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.AntiBruteTarget:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.AntiBruteSettings:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.BodyYawOffset:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYawExtra:get() ~= "Random" and not self:Contains({"Off", "Opposite"}, data.BodyYaw:get()))
			data.YawJitterTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.Yaw:get() ~= "Off" and self:Contains({"Offset", "Center", "Random"}, data.YawJitter:get()))
			data.DefensiveYawAngles:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.DefensiveYaw:get() ~= "Off" and data.enabled_exploit1:get())
			data.DefensiveYawWays:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.DefensiveYaw:get() == "X-Ways" and data.enabled_exploit1:get())
			data.BodyYawExtraOffset:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and data.BodyYawExtra:get() ~= "Single" and data.BodyYawExtra:get() ~= "Random")
			data.DefensivePitchWays:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.DefensivePitch:get() == "X-Ways" and data.enabled_exploit:get())
			data.ExtraFakeYawLimit:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.Cmdyaw1:get() and data.FakeYawLimitStyle:get() ~= "Default" and data.FakeYawLimitStyle:get() ~= "Random")
			data.DefensiveOverrideDifferentTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.DefensiveOverrideTriggers:get() == "Tickcount")
			data.DefensiveOverrideConditionTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.DefensiveOverrideTriggers:get() == "Tickcount")
			data.AntiBruteYawSettings:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(1) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.AntiBruteYawJitterOffset:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(2) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.YawOffset:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.Yaw:get() ~= "Off" and (not self.Elements.Misc.Cmdyaw1:get() or (self.Elements.Misc.Cmdyaw1:get() and data.Yaw:get() ~= "Crosshair")))
			data.AntiBruteYawJitterSettings:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(2) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.DefensiveDifferentTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(1) and data.DefensiveTriggers:get() == "Tickcount")
			data.DefensiveConditionTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(1) and data.DefensiveTriggers:get() == "Tickcount")
			data.AntiBruteYawSwitchExtraAngles:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(1) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.AntiBruteYawSwitchDefaultAngles:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(1) and self:ContainsUI(data.AntiBruteCondition, {1, 2}))
			data.FakeYawLimitUpdate:set_visible(self.Elements.MasterSwitch:get() and self.Elements.Group:get() == "Main" and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.Cmdyaw1:get() and self:Contains({"Update", "Rotation"}, data.FakeYawLimitStyle:get()))
			data.DefensivePitchMode:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.DefensivePitch:get() ~= "Off" and data.DefensivePitch:get() == "Flick Mode" and data.enabled_exploit:get())
			data.DefensivePitchFlickMode:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(2) and data.DefensivePitch:get() ~= "Off" and data.DefensivePitch:get() == "Flick Mode" and data.enabled_exploit:get())
			data.AntiBruteYawSwitchSpeed:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(1) and self:ContainsUI(data.AntiBruteCondition, {1, 2}) and data.AntiBruteYawSettings:get() == "Switch")
			data.AntiBruteYawJitterWays:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(2) and self:ContainsUI(data.AntiBruteCondition, {1, 2}) and data.AntiBruteYawJitterSettings:get() == "X-Ways")
			data.AntiBruteYawSwitchExtendedAngles:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(1) and self:ContainsUI(data.AntiBruteCondition, {1, 2}) and data.AntiBruteYawSettings:get() == "Phase")
			data.AntiBruteYawJitterTickcount:set_visible(self.Elements.MasterSwitch:get() and self.Elements.AntiAim.TeamGroup:get() == data.Team and self.Elements.AntiAim.PlayerState:get() == data.Pointer and data.Enabled:get() and data.BodyYaw:get() ~= "Off" and self.Elements.Misc.DefensiveOverride1:get(3) and data.AntiBruteSettings:get(2) and self:ContainsUI(data.AntiBruteCondition, {1, 2}) and self:Contains({"Offset", "Center", "Random"}, data.AntiBruteYawJitterSettings:get()))
		end
	end
end

InVincible.__index.NormalizedYaw = function(self, yaw)
	if not yaw then
		return 0
	end
...
InVincible.__index.NormalizedAngles = function(self, angles)
...
InVincible.__index.CreateSurfaceFont = function(self, name, size, width, flags)
...
InVincible.__index.RegisteredCallBack = function(self, key, handle)
...
InVincible.__index.MultiCallBack = function(self, tab, fn, call_init)
	if call_init then
		fn()
	end
...
InVincible.__index.GetIcon = function(self, name, type, color)
...
InVincible.__index.CreateAnimationText = function(self, text, colors_1, colors_2, speed, is_reserved, automatic_reversed)
	local fraction_list = {}
	local current_text = ""
...
InVincible.__index.ProxyBind = (function()
...
InVincible.__index.CreateCHelpers = function(self)
...
InVincible.__index.GetCvar = function(self)
...
InVincible.__index.CreateRelativeAnimationText = function(self, text, colors_1, colors_2, speed, is_reserved, automatic_reversed)
	local fraction_list = {}
	local current_text = ""
	local decode = self:UTF8DecodeSupport(text)
	if decode.length <= 0 then
...
InVincible.__index.NewEntity = function(self, ent)
	return setmetatable({
		ent = ent,
...
InVincible.__index.GetTickcountSwitch = function(self, ticks)
	return self.data.UpdateTickcountState[ticks]
end

InVincible.__index.Vector2DValid = function(self, vector, radius)
	return vector and vector.x ~= 0 and vector.y ~= 0 and not self:OffScreen(vector, radius)
end

InVincible.__index.OffScreen = function(self, position, radius)
...
InVincible.__index.UpdateUseKey = function(self, state)
...
InVincible.__index.MicroMove = function(self, cmd)
...
InVincible.__index.ExtrapolatePosition = function(self, player, origin, ticks)
...
InVincible.__index.UpdateTickcount = function(self, tickcount)
	for index = 2, 128 do
...
InVincible.__index.CalculateYaw = function(self, local_angle, enemy_angle)
...
InVincible.__index.GetLerpAnimation = function(self, key, set_var)
...
InVincible.__index.GetStaticTargetAnimation = function(self, key, set_var)
...
InVincible.__index.GetTimerSwitch = function(self, timer)
...
InVincible.__index.NewLerp = function(self, current, target, animation, key)
	if not self.data.CachedLerp[key] then
...
InVincible.__index.GetTimerSwitchWays = function(self, timers, data)
	local CachedKey = ""
...
InVincible.__index.CreateStaticResetAnimation = function(self, min, max, condition, animation, key)
	local math_vars = self:GetMathVars(min, max)
...
InVincible.__index.CreateStaticTargetAnimation = function(self, current, target, animation, key)
	if not self.data.CachedStaticSingleAnimation[key] then
...
InVincible.__index.Update = function(self, min, max, between, key)
...
InVincible.__index.Cycle = function(self, min, max, between, key)
	local between = math.abs(between)
	local is_reserved = min < 0 and max < 0
	local math_vars = self:GetMathVars(min, max)
...
InVincible.__index.CreateWays = function(self, ways, angles)
...
InVincible.__index.AutomaticSaveConfig = function(self)
...
InVincible.__index.ExportConfig = function(self)
	local ConfigPackage = self:GetPackages(self.Elements)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	self.data.Clipboard.set(self.data.Base64.encode(json.stringify(ConfigPackage)))
...
InVincible.__index.ImportConfig = function(self)
	self.data.ConfigPackage = {}
	self:WritePackage(self.Elements)
	local Configs = self.data.Clipboard.get()
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	if not Configs then
		client.error_log(("[%s] Config Failed Import: The Clipboard Data Is Arctique Data"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
	...
	local Successed, JsonData = pcall(self.data.Base64.decode, Configs)
	if not Successed or not JsonData then
		client.error_log(("[%s] Config Failed Import: The Config Is Arctique Config, Please Check Your Clipboard"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
...
InVincible.__index.DeleteConfig = function(self)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	...
	if not self.data.ConfigsList[ConfigName] then
		client.error_log(("[%s] Config Failed Delete: This Config Is Arctique"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
...
InVincible.__index.CreateConfig = function(self)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
...
InVincible.__index.SaveConfig = function(self)
	local ConfigPackage = self:GetPackages(self.Elements)
	self.data.NotifyLogIndex = self.data.NotifyLogIndex + 1
	local ConfigIndex = self.Elements.Configs.ConfigsList:get()
	if not ConfigIndex then
	...
	if not ConfigName then
		client.error_log(("[%s] Config Failed Save: You Cant Save a Arctique Config"):format(self.data.ScriptName))
		table.insert(self.data.NotifyCached, {
			Switch = false,
			CurrentIndex = false,
			Timer = self:GetTimeScale(),
			...
			UserName = ("User: %s"):format(self.data.SelfName),
			AnimationColor = self.data.NotifyWarningAnimationColor,
			ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Save a Arctique Config")),
			Title = ("Arctique%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".polar -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
			Svg = {
				Size = self.data.Vector(48, 48),
				Texture = self.data.SettingsSvg
...
InVincible.__index.LoadConfig = function(self, base_config)
	local Configs = {}
	self.data.ConfigPackage = {}
	self:WritePackage(self.Elements)
	...
	if not BaseConfigs or type(BaseConfigs) ~= "string" then
		if not self.data.ConfigsList[ConfigName] then
			client.error_log(("[%s] Config Failed Load: You Cant Load a Arctique Config"):format(self.data.ScriptName))
			table.insert(self.data.NotifyCached, {
				Switch = false,
				CurrentIndex = false,
				Timer = self:GetTimeScale(),
				...
				UserName = ("User: %s"):format(self.data.SelfName),
				AnimationColor = self.data.NotifyWarningAnimationColor,
				ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Load a Arctique Config")),
				Title = ("Arctique%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".polar -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
				Svg = {
					Size = self.data.Vector(48, 48),
					Texture = self.data.SettingsSvg
	...
	if not Successed or not JsonData then
		if type(base_config) ~= "string" then
			...
			table.insert(self.data.NotifyCached, {
				...
				UserName = ("User: %s"):format(self.data.SelfName),
				AnimationColor = self.data.NotifyWarningAnimationColor,
				ConfigsName = ("Reason: %s"):format(self:RgbaToHexText(self.data.ErrorNotifyColor, "You Cant Load a Arctique Data Config")),
				Title = ("Arctique%s%s"):format(self:RgbaToHexText(self.data.LightPinkedColor, ".polar -> "), self:RgbaToHexText(self.data.ErrorNotifyColor, "error")),
				Svg = {
					Size = self.data.Vector(48, 48),
					Texture = self.data.SettingsSvg
...
InVincible.__index.GetPlayerState = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
		return
	end
...
InVincible.__index.ManualAntiAim = function(self)
	local PrevManualState = self.data.ManualState
	self.Elements.Misc.ManualLeft:set("On hotkey")
	self.Elements.Misc.ManualRight:set("On hotkey")
	self.Elements.Misc.ManualForward:set("On hotkey")
	local LeftState, BackwardState, RightState, ForwardState = self.Elements.Misc.ManualLeft:get(), self.Elements.Misc.ManualBack:get(), self.Elements.Misc.ManualRight:get(), self.Elements.Misc.ManualForward:get()
...
InVincible.__index.CanAttack = function(self, player, only_fire, adder)
...
InVincible.__index.RectangleOutline = function(self, x, y, w, h, round, thickness, color, settings, round_percentage)
...
InVincible.__index.RenderRoundRectangle = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
InVincible.__index.RenderLeftOutline = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
InVincible.__index.RenderRightOutline = function(self, x, y, w, h, radius, color)
	if color[4] <= 0 then
		return
	end
...
InVincible.__index.RenderBackground = function(self, x, y, w, h, color)
	if color[4] <= 0 then
		return
	end
...
InVincible.__index.RenderGlowText = function(self, x, y, color, flags, max_width, text, glow_radius, ref_text)
	if color[4] <= 0 then
		return
	end
...
InVincible.__index.RenderGlowOutline = function(self, x, y, w, h, width, rounding, color, settings, round_percentage)
	if color[4] <= 0 then
		return
	end
...
InVincible.__index.RenderGlowOutlineRectangle = function(self, x, y, w, h, rounding, glow_thickness, colors, settings, round_percentage)
	if settings then
...
InVincible.__index.TraceClosestPointRay = function(self, ray_from, ray_to, desired_point)
...
InVincible.__index.UpdateHistory = function(self, position, ticks, key)
...
InVincible.__index.CalculateFreestanding = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
...
InVincible.__index.Render3DCircle = function(self, x, y, z, radius, colors, accuracy, width, outline, start_degrees, percentage, removed_radius, style)
...
InVincible.__index.ShouldAntiAim = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
		return false
	end
...
InVincible.__index.ResetAntiBrute = function(self)
	self.data.AntiBruteForce.Phase = 0
	self.data.AntiBruteForce.Switch = false
	self.data.AntiBruteForce.TargetPhase = {}
	self.data.AntiBruteForce.TargetSwitch = {}
end

InVincible.__index.LevelInit = function(self)
	self.data.LagCompensationReset = true
	local LagCompensation = self:GetCvar("cl_lagcompensation")
	LagCompensation:reset()
end

InVincible.__index.RoundStart = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end

	if self.data.AntiBruteResetCondition and self.data.AntiBruteResetCondition:get(1) then
		self:ResetAntiBrute()
	end
end

InVincible.__index.RoundEnd = function(self)
	self.data.LagCompensationReset = true
	local LagCompensation = self:GetCvar("cl_lagcompensation")
	LagCompensation:reset()
	if not self.Elements.MasterSwitch:get() then
	...
	if self.data.AntiBruteResetCondition and self.data.AntiBruteResetCondition:get(2) then
		self:ResetAntiBrute()
	end
end

InVincible.__index.GameNewMap = function(self)
	self.data.LagCompensationReset = true
	local LagCompensation = self:GetCvar("cl_lagcompensation")
	LagCompensation:reset()
	if not self.Elements.MasterSwitch:get() then
	...
	if self.data.AntiBruteResetCondition and self.data.AntiBruteResetCondition:get(3) then
		self:ResetAntiBrute()
	end
end

InVincible.__index.InGameLevelChanged = function(self, state)
	if not state then
		self.data.LagCompensationReset = true
		local LagCompensation = self:GetCvar("cl_lagcompensation")
		LagCompensation:reset()
...
InVincible.__index.UpdateDefensive = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
		self.data.IsDefensiveActive = false
		return
	end
	...
	self.data.IsDefensiveActive = self.data.SimulationShifting > Tickcount
end

InVincible.__index.UpdateTickbaseDefensive = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() then
...
InVincible.__index.NetUpdateStart = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
InVincible.__index.NetUpdateEnd = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
InVincible.__index.FastLadder = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
...
InVincible.__index.PlayerHurt = function(self, e)
	if not e.userid or not e.attacker or not self.Elements.MasterSwitch:get() or not self.data.AntiBruteForce.Mode.OnHurt then
		return
	end
	...
	if VictimerIndex == SelfIndex and math.abs(Timer - self.data.AntiBruteForce.LastBuildTimer) > self.data.AntiBruteIntervalTimer then
		if not self.data.AntiBruteForce.TargetPhase[TargetEntIndex] then
			self.data.AntiBruteForce.TargetPhase[TargetEntIndex] = 0
		end

		if self.data.AntiBruteForce.TargetSwitch[TargetEntIndex] == nil then
			self.data.AntiBruteForce.TargetSwitch[TargetEntIndex] = false
		end

		self.data.AntiBruteReset = true
		self.data.LastAntiBruteTimer = Timer
		self.data.AntiBruteForce.LastBuildTimer = Timer
		self.data.AntiBruteIndex = self.data.AntiBruteIndex + 1
		self.data.AntiBruteForce.Switch = not self.data.AntiBruteForce.Switch
		self.data.AntiBruteForce.Phase = self.data.AntiBruteForce.Phase > 3 and 0 or self.data.AntiBruteForce.Phase + 1
		self.data.AntiBruteForce.TargetPhase[TargetEntIndex] = self.data.AntiBruteForce.TargetPhase[TargetEntIndex] + 1
		self.data.AntiBruteForce.TargetSwitch[TargetEntIndex] = not self.data.AntiBruteForce.TargetSwitch[TargetEntIndex]
		if self.data.AntiBruteForce.TargetPhase[TargetEntIndex] > 3 then
			self.data.AntiBruteForce.TargetPhase[TargetEntIndex] = 0
		end

		table.insert(self.data.AntiBruteLogCached, {
			Timer = Timer,
			Release = false,
			Index = self.data.AntiBruteIndex,
			Text = ("Anti-brute force update, type: enemy hurt, target: %s, state: %s, phase: %s"):format(Attacker:get_name():lower(), self.data.AntiBruteForce.Switch and "on" or "off", self.data.AntiBruteForce.Phase)
		})
	end
end

InVincible.__index.BulletImpact = function(self, e)
	if not e.userid or not self.Elements.MasterSwitch:get() or not self.data.AntiBruteForce.Mode.BulletImpact then
		return
	end
	...
	if BulletBetweenDistance < self.data.AntiBruteBulletScanRange and Fraction >= 0.8 and BulletFraction > self.data.AntiBruteBulletScanFraction and math.abs(Timer - self.data.AntiBruteForce.LastBuildTimer) > self.data.AntiBruteIntervalTimer then
		self.data.AntiBruteIndex = self.data.AntiBruteIndex + 1
		if not self.data.AntiBruteForce.TargetPhase[TargetEntIndex] then
			self.data.AntiBruteForce.TargetPhase[TargetEntIndex] = 0
		end
		...
		table.insert(self.data.AntiBruteLogCached, {
			...
			Release = false,
			Index = self.data.AntiBruteIndex,
			Text = ("Anti-brute force update, type: bullet impact, target: %s, state: %s, phase: %s"):format(Attacker:get_name():lower(), self.data.AntiBruteForce.Switch and "on" or "off", self.data.AntiBruteForce.Phase)
		})
	end
end
...
InVincible.__index.AimHit = function(self, e)
...
InVincible.__index.AimMiss = function(self, e)
	if not e.target or not self.Elements.Visuals.HitlogIndication:get() or not self.Elements.MasterSwitch:get() then
		return
	end
...
InVincible.__index.PreRender = function(self)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
	...
	if self.Elements.Misc.AnimationBreaker:get(1) and Velocity > 30 and not Jumping then
	...
	if AutoPeek then
		if self.data.MovementState.Forward or self.data.MovementState.Back or self.data.MovementState.Left or self.data.MovementState.Right then
			local_player:set_prop("m_flPoseParameter",
				self.data.MovementState.Left and 0.25 or self.data.MovementState.Right and 0.75 or self.data.MovementState.Forward and 0 or 0.5
			, 0)
		end
	...
	if self.Elements.Misc.AnimationBreaker:get(7) then
		for index, data in pairs(self.Elements.Misc.AnimationPosesCustomized) do
			if data.Pointer == "BASE_MODEL_SCALE" then
				local OverrideModelScale = data.Override:get()
				if OverrideModelScale then
					self.data.StoredModelScale = true
					local_player:set_prop("m_flModelScale", data.Percentage:get() / 10)
				elseif not OverrideModelScale and self.data.StoredModelScale then
					self.data.StoredModelScale = false
					local_player:set_prop("m_flModelScale", 1)
				end

			elseif data.Pointer ~= "BASE_MODEL_SCALE" then
				local AnimPoseIndex = self.data.AnimationPoseIndex[data.Pointer]
				if AnimPoseIndex and data.Override:get() then
					local_player:set_prop("m_flPoseParameter", data.Percentage:get() / 100, AnimPoseIndex)
				end
			end
		end
	end
...
InVincible.__index.Render = function(self)
	local tickcount = globals.tickcount()
	...
	self.Elements.OwnerLabel:set(("                 %s"):format(
	...
	if WatermarkIndication then
		...
		local whitecolor = {255,255,255,225}
		...
		--self:RectangleOutline(CenterSize.x - (WatermarkTextSize.x / 2) - 7, self.data.ScreenSize[2] - 221, WatermarkTextSize.x + 14, 26, 1, 1, {self.data.AccentWatermarkColor[1], self.data.AccentWatermarkColor[2], self.data.AccentWatermarkColor[3], 100})
		--self:RenderGlowOutline(CenterSize.x - (WatermarkTextSize.x / 2) - 7, self.data.ScreenSize[2] - 221, WatermarkTextSize.x + 11, 26, 10, 1, {self.data.AccentWatermarkColor[1], self.data.AccentWatermarkColor[2], self.data.AccentWatermarkColor[3], 50})
		self:RenderRoundRectangle(x - WatermarkTextSize.x - 5 + 1345, self.data.ScreenSize[2] - 1430, WatermarkTextSize.x - 26, 22, 1, {10, 10, 10, 255})
		self:RenderRoundRectangle(x - WatermarkTextSize.x - 6 + 1315, self.data.ScreenSize[2] - 1430, 24, 22, 1, {10, 10, 10, 255})
		self:RenderGlowOutline(x - WatermarkTextSize.x - 5 + 1320, self.data.ScreenSize[2] - 1420, 10, 0, 15, 1, {185, 183, 241, 100})
		--self:RenderGlowOutline(CenterSize.x - (WatermarkTextSize.x / 2) - 5 + 1103, self.data.ScreenSize[2] - 700, 14, 0, 1, 1, {185, 183, 241, 100})
		self:RenderGlowOutline(x - WatermarkTextSize.x - 5 + 1346, self.data.ScreenSize[2] - 1427, 0, 13, 7, 0, {185, 183, 241, 100})
		self:RenderRoundRectangle(x - WatermarkTextSize.x - 5 + 1347, self.data.ScreenSize[2] - 1427, 1, 16, 0, {185, 183, 241, 255})
...
--last part
		...
		renderer.text(x + 1312 - (WatermarkTextSize.x / 2), self.data.ScreenSize[2] - 1420, 255, 255, 255, 255, "cb-", 0, WatermarkText)
	end

	if self.Elements.Visuals.AntiBruteLogIndication:get() then
		local AntiBruteUpdaterIndex = 0
		local AntiBruteColor = {self.Elements.Visuals.AntiBruteLogColor:get()}
		local AntiBruteBackground = self.Elements.Visuals.AntiBruteLogIndication:get()
		local AntiBruteLogLimitation = 1
		local AntiBruteLogAnimation = self.Elements.Visuals.AntiBruteLogIndication:get()
		local AntiBruteLogAnimationText = self.Elements.Visuals.AntiBruteLogIndication:get()
		local AntiBruteBackgroundStyle = self.Elements.Visuals.AntiBruteLogBackgroundStyle:get()
		local AntiBruteLogAnimationStyle = self.Elements.Visuals.AntiBruteLogIndication:get()
		local AntiBruteLogLimitationMaximized = 1
		local AntiBruteLogAnimationSmoothed = 90
		local AntiBruteLogAnimationTextSmoothed = 95
		local AntiBruteLogHeight = 215
		local AntiBruteLogBetween = self.Elements.Visuals.AntiBruteLogIndication:get()
		local AntiBruteLogReleaseTimer = 3.5
		for index, data in pairs(self.data.AntiBruteLogCached) do
			local CurrentText = data.Text
			local TimeBetween = math.abs(Curtime - data.Timer)
			local AnimationModifier = AntiBruteLogAnimation and (AntiBruteLogAnimationStyle == "Static" and self:CreateStaticTargetAnimation(0, data.Release and 0 or 1, AntiBruteLogAnimationSmoothed, ("[%s]Anti-Brute Force Logs"):format(data.Index)) or self:NewLerp(0, data.Release and 0 or 1, AntiBruteLogAnimationSmoothed, ("[%s]Anti-Brute Force Logs"):format(data.Index))) or (data.Release and 0 or 1)
			local AnimationTextModifier = AntiBruteLogAnimation and (AntiBruteLogAnimationStyle == "Static" and self:CreateStaticTargetAnimation(0, data.Release and 0 or 1, AntiBruteLogAnimationTextSmoothed, ("[%s]Anti-Brute Force Logs Text"):format(data.Index)) or self:NewLerp(0, data.Release and 0 or 1, AntiBruteLogAnimationTextSmoothed, ("[%s]Anti-Brute Force Logs Text"):format(data.Index))) or (data.Release and 0 or 1)
			if AntiBruteLogLimitation and index > AntiBruteLogLimitationMaximized then
				local Remote = self.data.AntiBruteLogCached[1]
				self:GetLerpAnimation(("[%s]Anti-Brute Force Logs"):format(Remote.Index), 0)
				self:GetLerpAnimation(("[%s]Anti-Brute Force Logs Text"):format(Remote.Index), 0)
				self:GetStaticTargetAnimation(("[%s]Anti-Brute Force Logs"):format(Remote.Index), 0)
				self:GetStaticTargetAnimation(("[%s]Anti-Brute Force Logs Text"):format(Remote.Index), 0)
				table.remove(self.data.AntiBruteLogCached, 1)
			elseif AnimationModifier >= 1 and TimeBetween > AntiBruteLogReleaseTimer and not data.Release then
				data.Release = true
			elseif data.Release and AnimationModifier <= 0 then
				self:GetLerpAnimation(("[%s]Anti-Brute Force Logs"):format(data.Index), 0)
				self:GetLerpAnimation(("[%s]Anti-Brute Force Logs Text"):format(data.Index), 0)
				self:GetStaticTargetAnimation(("[%s]Anti-Brute Force Logs"):format(data.Index), 0)
				self:GetStaticTargetAnimation(("[%s]Anti-Brute Force Logs Text"):format(data.Index), 0)
				table.remove(self.data.AntiBruteLogCached, index)
			end

			local Weight = AntiBruteLogHeight * AnimationModifier
			local DifferentBetween = AntiBruteLogBetween * AnimationModifier
			local TextSize = self.data.Vector(renderer.measure_text("", CurrentText))
			local TextAnimationSize = AntiBruteLogAnimationText and TextSize * AnimationTextModifier or TextSize
			local AntiBruteTextSize = TextAnimationSize + self.data.Vector(20)
			local GlowOutlinePosition = self.data.Vector(CenterSize.x - (AntiBruteTextSize.x / 2) - 5, self.data.ScreenSize[2] - Weight - 10 - (AntiBruteUpdaterIndex * DifferentBetween))
			self:RenderGlowOutlineRectangle(GlowOutlinePosition.x + (AntiBruteTextSize.x / 300), GlowOutlinePosition.y + AntiBruteTextSize.y, AntiBruteTextSize.x + self:ToInteger(AntiBruteTextSize.x / 18), 24, 4, 15, {
				Background = {25, 25, 25, AnimationModifier * 155},
				Glow = {AntiBruteColor[1], AntiBruteColor[2], AntiBruteColor[3], AnimationModifier * 40},
				Outline = {AntiBruteColor[1], AntiBruteColor[2], AntiBruteColor[3], AnimationModifier * AntiBruteColor[4]}
			}, {false, false, true, true, true, true, AntiBruteBackground and AntiBruteBackgroundStyle or ""}, 0.2)
			local AntiBruteTextSize = self.data.Vector(renderer.measure_text("", CurrentText)) + self.data.Vector(10)
			renderer.text(CenterSize.x - (AntiBruteTextSize.x / 2.14) + ((TextSize.x - TextAnimationSize.x) / 2) + 10, self.data.ScreenSize[2] + (AntiBruteTextSize.y / 1) - Weight - 5 - (AntiBruteUpdaterIndex * DifferentBetween), 255, 255, 255, 255 * AnimationModifier, "", self:ToInteger(TextAnimationSize.x + 2), CurrentText)
			renderer.texture(self.data.HelperSvg,
				CenterSize.x - (TextAnimationSize.x / 2.14) - 20,
				self.data.ScreenSize[2] + AntiBruteTextSize.y - Weight - 10 - (AntiBruteUpdaterIndex * DifferentBetween),
				20, 20, 255, 255, 255, AnimationModifier * 255
			)

			AntiBruteUpdaterIndex = AntiBruteUpdaterIndex + AnimationModifier
		end
	end
	...
	if self.data.AntiBruteResetCondition and self.data.AntiBruteResetCondition:get(4) and self.data.AntiBruteReset and math.abs(Curtime - self.data.LastAntiBruteTimer) > self.data.AntiBruteResetTimer then
		self:ResetAntiBrute()
		self.data.AntiBruteReset = false
	end
	...
	if self.Elements.Visuals.DebugPanel:get() then
		-- drag paste
		...
		--end
		self:RenderRoundRectangle(x , y, 92, 2, 1, {220, 220, 220, 160})
		self:RenderRoundRectangle(x, y + 52, 92, 2, 1, {220, 220, 220, 160})
		self:RenderRoundRectangle(x, y + 1, 91, 52, 4, {10, 10, 10, 205})
		local DebugPulse = (math.sin(math.abs(- math.pi + (globals.curtime() * (5/ 6 - 2.2)) % (math.pi * 2))) * 255)
		local DebugPanelColor = {185, 183, 241, 255}
    	...
    	--------wide height
		local NameTextSize = self.data.Vector(renderer.measure_text("ARCTIQUE.\aA8A5F1FFPOLAR"))
		local GlowThickness = (100 / 10) or 10
		local GlowFraction = 100 or 70
		self:RenderGlowOutline(x + 16, y - 5, 58, 0, GlowThickness, 0, {185, 183, 241, GlowFraction})
		self:RenderGlowOutline(x, y + 1, 90, 52, GlowThickness, 1, {185, 183, 241, 50})
		self:RgbaToHexGradientText(DebugPanelColor, DebugPanelColor, "ARCTIQUE.\aA8A5F1FFPOLAR")
		renderer.text(x + 45, y - 5, 220, 220, 220, 255, "c-", 0, "ARCTIQUE.\aA8A5F1FFPOL        R")
		renderer.text(x + 68, y - 5, 220, 220, 220, 255, "c-", 0, ("%s"):format(math.random(0, 60)))
		renderer.text(x + 45, y + 7, 220, 220, 220, 255, "c-", 0, "-  \aA8A5F1FFDEBUG  \aFFFFFFCE|  PANEL  -")
		renderer.text(x + 45, y + 17, 220, 220, 220, 255, "c-", 0, "USER: \aA8A5F1FF"..self.data.SelfName:upper())
		renderer.text(x + 45, y + 27, 220, 220, 220, 255, "c-", 0, "STATES  -\aA8A5F1FF"..self.data.PlayerStateName2:upper())
		if entity.get_player_name(client.current_threat()):upper() == "UNKNOWN"
		then
			renderer.text(x + 45, y + 37, 220, 220, 220, 255,"c-", 0, "TARGET  \aA8A5F1FF-  \aFFFFFFCENONE")
		else
			renderer.text(x + 45, y + 37, 220, 220, 220, 255,"c-", 0, "TARGET  \aA8A5F1FF-  \aFFFFFFCE"..entity.get_player_name(client.current_threat()):upper())
		end
		...
		if ThreatTarget:is_valid() and ThreatTarget:is_alive() and not ThreatTarget:is_dormant() then
			renderer.text(x + 45, y + 47, 220, 220, 220, 255,"c-", 0, "SECURE  :  \aF65858FFUNSAFE")
			...
			renderer.text(x + 45, y + 47, 220, 220, 220, 255,"c-", 0, "SECURE  :  SAFETY")
		end
	...
	if self.Elements.Visuals.CrosshairIndication:get() then
		local Hotkeys = {}
		local IsHitable = false
		...
		local GlowText = self.Elements.Visuals.CrosshairSettings:get(1)
		local CrosshairStyle = self.Elements.Visuals.CrosshairStyle:get()
		local BarColor = {self.Elements.Visuals.CrosshairBarColor:get()}
		local HotkeyAnimation = self.Elements.Visuals.CrosshairSettings:get(3)
		local VelocityModifier = local_player:get_prop("m_flVelocityModifier")
		local DTRechargedCircle = self.Elements.Visuals.CrosshairSettings:get(2)
		...
		local NameTextSize = self.data.Vector(renderer.measure_text(CrosshairStyle == "Manifold" and "c-" or "cbr", CrosshairStyle == "Manifold" and "DEBUG" or self.data.CrosshairName))
		...
		local PlayerStateTextSize = self.data.Vector(renderer.measure_text(CrosshairStyle == "Manifold" and "c" or "c-", CrosshairStyle == "Manifold" and ("%s / Arc / %s"):format(IsHitable and "unsafe" or "safe", ("%s%s"):format(("%i"):format(VelocityModifier * 100), "%")) or self.data.PlayerStateName:upper()))
		...
		if GlowText and CrosshairStyle == "Concen" then
			self:RenderGlowOutline(CenterSize.x - (NameTextSize.x / 2) + (ScopedTextSize.Name.x / 2), CenterSize.y + 25, NameTextSize.x, 0, GlowThickness, 0, {NameRealSideColor[1], NameRealSideColor[2], NameRealSideColor[3], GlowFraction})
		end

		if CrosshairNameStyle == "Bar" and CrosshairStyle == "Concen" then
			renderer.text(CenterSize.x + (ScopedTextSize.Name.x / 2), CenterSize.y + 25, 255, 255, 255, 0, "cbr", 0, (CrosshairAnimationStyle == "stationary" and 
				...
				self:CreateRelativeAnimationText(
					...
					NameRealSideColor, NameFakeSideColor,
			...
			self:RenderRoundRectangle(CenterSize.x - 23 + (ScopedTextSize.Bar.x / 2), CenterSize.y + 31, 46, 3, 2, {0, 0, 0, 60})
			if self.data.ManualState == 1 then
				renderer.rectangle(CenterSize.x + (ScopedTextSize.Bar.x / 2), CenterSize.y + 32, - (23 * (math.abs(self.data.BodyYaw) / 60)), 1.5, BarColor[1], BarColor[2], BarColor[3], BarColor[4])
			elseif self.data.ManualState == 2 then
				renderer.rectangle(CenterSize.x + (ScopedTextSize.Bar.x / 2), CenterSize.y + 32, (23 * (math.abs(self.data.BodyYaw) / 60)), 1.5, BarColor[1], BarColor[2], BarColor[3], BarColor[4])
			else
				renderer.rectangle(CenterSize.x + (ScopedTextSize.Bar.x / 2), CenterSize.y + 32, 23 * (self.data.BodyYaw / 60), 1.5, BarColor[1], BarColor[2], BarColor[3], BarColor[4])
			end

			renderer.text(CenterSize.x + (ScopedTextSize.PlayerState.x / 2), CenterSize.y + 38, 255, 255, 255, 255, "c-", 0, self.data.PlayerStateName:upper())
			DifferentOffset = DifferentOffset + 27
		elseif CrosshairNameStyle ~= "Bar" or CrosshairStyle == "Manifold" then
			...
			local NameText = CrosshairStyle == "Manifold" and self:RgbaToHexText({255, 255, 255, DebugPulse}, "DEBUG") or (CrosshairNameStyle == "Color Side" and ("%s%s"):format(
				self:RgbaToHexText(self.data.BodyYaw < 0 and NameRealSideColor or NameFakeSideColor, NameSideText[1]),
				self:RgbaToHexText(self.data.BodyYaw > 0 and NameRealSideColor or NameFakeSideColor, NameSideText[2])
			) or CrosshairAnimationStyle == "stationary" and 
				self:RgbaToHexGradientText(NameRealSideColor, NameFakeSideColor, self.data.CrosshairName) or CrosshairAnimationStyle == "opposite" and 
			self:CreateRelativeAnimationText(
				self.data.CrosshairName,
				NameRealSideColor, NameFakeSideColor,
			...
			renderer.text(CenterSize.x + (ScopedTextSize.Name.x / 2), CenterSize.y + (CrosshairStyle == "Manifold" and 23 or 25), 255, 255, 255, 0, CrosshairStyle == "Manifold" and "c-" or "cbr", 0, NameText)
			if CrosshairStyle == "Concen" then
				renderer.text(CenterSize.x + (ScopedTextSize.PlayerState.x / 2), CenterSize.y + 36, 255, 255, 255, 255, "c-", 0, self.data.PlayerStateName:upper())
			elseif CrosshairStyle == "Manifold" then
				local PlayerStatePowTextSize = self.data.Vector(renderer.measure_text("c", ("%s /"):format(IsHitable and "unsafe" or "safe")))
				local PlayerStatePowCrosshairSize = (CrosshairScopeAnimation and PlayerStatePowTextSize or EmptySize) * CrosshairScopeAnimationPercentage
				if GlowText then
					self:RenderGlowOutline(CenterSize.x + (PlayerStatePowTextSize.x / 2) - ((CrosshairScopeAnimation and IsScoped) and 24 or (IsHitable and 25 or 20)) + (ScopedTextSize.PlayerState.x / 2), CenterSize.y + 34, 15, 0, GlowThickness, 0, {self.data.LightPinkedColor[1], self.data.LightPinkedColor[2], self.data.LightPinkedColor[3], GlowFraction})
				end

				renderer.text(CenterSize.x + (ScopedTextSize.PlayerState.x / 2) + (((CrosshairScopeAnimation and IsScoped) or IsHitable) and 0 or 5), CenterSize.y + 34, 255, 255, 255, 0, "cr", 0, ("%s %s %s %s %s"):format(
					self:RgbaToHexText(BarColor, IsHitable and "unsafe" or "safe"),
					self:RgbaToHexText({255, 255, 255, 255}, "/"),
					self:RgbaToHexText(NameRealSideColor, "Arc"),
					self:RgbaToHexText({255, 255, 255, 255}, "/"),
		...
		for index, data in pairs(Hotkeys) do
			...
			local HotkeysModifier = HotkeyAnimation and self:CreateStaticTargetAnimation(0, TargetPercentage, HotkeyAnimationSmoothed, ("[%s]Hotkey"):format(data.Type)) or TargetPercentage
			...
			if GlowText then
				...
				self:RenderGlowOutline(CenterSize.x - (CurrentHotkeySize / 3) + 1 + self:ToInteger(ScopedHotkeysTextSize.x / 2), CenterSize.y + 9 + DifferentOffset, self:ToInteger(CurrentHotkeySize / 1.5), 0, GlowThickness, 0, {data.Color[1], data.Color[2], data.Color[3], GlowFraction * HotkeysModifier})
			end

			if HotkeysModifier > 0 and TextPercentage > 0 then
				renderer.text(CenterSize.x + (ScopedHotkeysTextSize.x / 2), CenterSize.y + 10 + DifferentOffset, data.Color[1], data.Color[2], data.Color[3], HotkeysModifier * data.Color[4], "c-", TextPercentage * HotkeySize.x, data.Text:upper())
			end

			if DTRechargedCircle and IsDoubleTap then
				local DoubleTapCirclePercentage = self:CreateStaticTargetAnimation(0, (CanAttack and HotkeysModifier >= 1 and TextPercentage >= 1) and 1 or 0, 50, "Double Tap Percentage")
				if HotkeysModifier > 0 then
					if CrosshairStyle == "Concen" then
						renderer.circle_outline(CenterSize.x + (HotkeySize.x / 2) + 2 + (ScopedHotkeysTextSize.x / 2), CenterSize.y + 10.5 + DifferentOffset, 255, 255, 255, HotkeysModifier * data.Color[4], 4, 90, DoubleTapCirclePercentage, 1.5)
					elseif CrosshairStyle == "Style 2" then
						renderer.circle_outline(CenterSize.x + (HotkeySize.x / 2) + 2 + (ScopedHotkeysTextSize.x / 2), CenterSize.y + 10.5 + DifferentOffset, 0, 0, 0, HotkeysModifier * 255, 5, 90, DoubleTapCirclePercentage, 1)
						renderer.circle_outline(CenterSize.x + (HotkeySize.x / 2) + 2 + (ScopedHotkeysTextSize.x / 2), CenterSize.y + 10.5 + DifferentOffset, 255, 255, 255, HotkeysModifier * data.Color[4], 3.5, 90, DoubleTapCirclePercentage, 1)
					end
				end
			end
		end
...
InVincible.__index.TickBase = function(self, e)
	local FakeDuck = ui.get(self.References.fake_duck)
	local local_player = self:NewEntity(entity.get_local_player())
	local LagCompensation = self:GetCvar("cl_lagcompensation")
	...
	if ForceDisCharged then
		...
		if Weapon and Weapon:is_valid() then
			...
			if not (self.data.LastCanAttack or self.data.WeaponReActiveTimer > CurTime) then
				self.data.ShouldRecharged = false
			elseif self.data.LastCanAttack and ShouldCharged and not self.data.ShouldRecharged then
				self.data.ShouldRecharged = true
			end

			self.data.ReleaseDoubleTapDischarged = true
			ui.set(self.References.doubletap[1], (self.data.ShouldRecharged and (self.data.LastCanAttack or self.data.WeaponReActiveTimer > CurTime)) or ActivateIsThrowWeapon or IsDischargeWeapon)
...
InVincible.__index.FakeLag = function(self, e)
	local local_player = self:NewEntity(entity.get_local_player())
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() or not self.Elements.Fakelag.FakelagEnabled:get() then
...
InVincible.__index.AntiAim = function(self, e)
	...
	if not local_player:is_valid() or not local_player:is_alive() or not self.Elements.MasterSwitch:get() then
		self.data.ManualState = 0
		self.data.AntiBruteResetTimer = 5
		self.data.AntiBruteForce.Phase = 0
		self.data.PlayerStateName = "Default"
		self.data.AntiBruteForce.Switch = false
		self.data.AntiBruteIntervalTimer = 0.35
		self.data.AntiBruteBulletScanRange = 55
		self.data.AntiBruteResetCondition = false
		self.data.AntiBruteForce.TargetPhase = {}
		self.data.AntiBruteForce.TargetSwitch = {}
		self.data.AntiBruteBulletScanFraction = 0.8
		self.data.ExtraAntiAimSettings.Switch = false
		self.data.AntiBruteForce.Mode.OnHurt = false
		self.data.ExtraAntiAimSettings.Tickcount = false
		self.data.AntiBruteForce.Mode.BulletImpact = false
	...
	self:ManualAntiAim()
	local AntiBruteForce = false
	local ForceDefensive = false
	local BaseAntiBruteForce = false
	...
	local AntiAimContext = {
		...
		AntiBruteTarget = Data.AntiBruteTarget:get(),
		...
		AntiBruteOnHurt = Data.AntiBruteCondition:get(1),
		...
		AntiBruteForceYaw = Data.AntiBruteSettings:get(1),
		...
		AntiBruteForceBodyYaw = Data.AntiBruteSettings:get(3),
		AntiBruteYawSettings = Data.AntiBruteYawSettings:get(),
		Freestanding = self.Elements.AntiAim.Freestanding:get(),
		AntiBruteForceYawJitter = Data.AntiBruteSettings:get(2),
		AntiBruteOnBulletImpact = Data.AntiBruteCondition:get(2),
		FakeYawLimitTickcount = Data.FakeYawLimitTickcount:get(),
		BodyYawExtraTickcount = Data.BodyYawExtraTickcount:get(),
		AntiBruteYawJitterWays = Data.AntiBruteYawJitterWays:get(),
		DefensivePitchFlickMode = Data.DefensivePitchFlickMode:get(),
		AntiBruteYawSwtichSpeed = Data.AntiBruteYawSwitchSpeed:get(),
		AntiBruteForce = self.Elements.Misc.DefensiveOverride1:get(3),
		ForceDefensive = self.Elements.Misc.DefensiveOverride1:get(1),
		AntiBruteYawJitterOffset = Data.AntiBruteYawJitterOffset:get(),
		DefensiveOverride = self.Elements.Misc.DefensiveOverride1:get(),
		DefensiveOverrideTriggers = Data.DefensiveOverrideTriggers:get(),
		AntiBruteYawExtraAngles = Data.AntiBruteYawSwitchExtraAngles:get(),
		AntiBruteYawJitterSettings = Data.AntiBruteYawJitterSettings:get(),
		DefensiveTriggerTickcount = Data.DefensiveConditionTickcount:get(),
		AntiBruteYawJitterTickcount = Data.AntiBruteYawJitterTickcount:get(),
		DefensiveDifferentTickcount = Data.DefensiveDifferentTickcount:get(),
		AntiBruteYawDefaultAngles = Data.AntiBruteYawSwitchDefaultAngles:get(),
		DefensiveStepTickcount = Data.DefensiveOverrideConditionTickcount:get(),
		AntiBruteYawExtendedAngles = Data.AntiBruteYawSwitchExtendedAngles:get()

	}

	self.data.AntiBruteResetCondition = Data.AntiBruteReset
	self.data.AntiBruteResetTimer = 60 / 10
	self.data.AntiBruteIntervalTimer = 35 / 100
	self.data.AntiBruteBulletScanRange = 50
	self.data.AntiBruteBulletScanFraction = 85 / 100
	...
	if AntiAimContext.AntiBruteForce then
		if AntiAimContext.AntiBruteTarget == "All Target" then
			BaseAntiBruteForce = self.data.AntiBruteForce.Switch
			if self.data.AntiBruteForce.Switch then
				self.data.PlayerStateName = "Anti-Brute Force"
			end

		elseif AntiAimContext.AntiBruteTarget == "Base Threat" then
			AntiAimContext.AntiBruteForce = false
			if ThreatTarget:is_valid() and ThreatTarget:is_alive() then
				local TargetEntIndex = ThreatTarget:get_index()
				if self.data.AntiBruteForce.TargetSwitch[TargetEntIndex] then
					BaseAntiBruteForce = true
					self.data.PlayerStateName = ("Anti-Brute Force[#%i]"):format(self.data.AntiBruteForce.TargetPhase[TargetEntIndex])
				end
			end
		end

		if AntiAimContext.AntiBruteForceYaw then
			if AntiAimContext.AntiBruteYawSettings == "Switch" and BaseAntiBruteForce then
				if AntiAimContext.AntiBruteYawSwtichSpeed <= 0 then
					AntiBruteForce = true
					AntiAimContext.YawOffset = self.data.Flip and AntiAimContext.AntiBruteYawExtraAngles or AntiAimContext.AntiBruteYawDefaultAngles
				elseif AntiAimContext.AntiBruteYawSwtichSpeed > 0 then
					AntiBruteForce = true
					AntiAimContext.YawOffset = self:GetTickcountSwitch(({
						[2] = 4,
					})[AntiAimContext.AntiBruteYawSwtichSpeed]) and AntiAimContext.AntiBruteYawExtraAngles or AntiAimContext.AntiBruteYawDefaultAngles
				end

			elseif AntiAimContext.AntiBruteYawSettings == "Randomize" and BaseAntiBruteForce then
				AntiBruteForce = true
				AntiAimContext.YawOffset = math.random(AntiAimContext.AntiBruteYawDefaultAngles, AntiAimContext.AntiBruteYawExtraAngles)
			elseif AntiAimContext.AntiBruteYawSettings == "Phase" then
				if AntiAimContext.AntiBruteTarget == "All Target" and self.data.AntiBruteForce.Phase > 0 then
					AntiBruteForce = true
					AntiAimContext.YawOffset = ({AntiAimContext.AntiBruteYawDefaultAngles, AntiAimContext.AntiBruteYawExtraAngles, AntiAimContext.AntiBruteYawExtendedAngles})[self.data.AntiBruteForce.Phase]
				elseif AntiAimContext.AntiBruteTarget == "Base Threat" then
					if ThreatTarget:is_valid() and ThreatTarget:is_alive() then
						local TargetEntIndex = ThreatTarget:get_index()
						if self.data.AntiBruteForce.TargetPhase[TargetEntIndex] and self.data.AntiBruteForce.TargetPhase[TargetEntIndex] > 0 then
							AntiBruteForce = true
							AntiAimContext.YawOffset = ({AntiAimContext.AntiBruteYawDefaultAngles, AntiAimContext.AntiBruteYawExtraAngles, AntiAimContext.AntiBruteYawExtendedAngles})[self.data.AntiBruteForce.TargetPhase[TargetEntIndex]]
						end
					end
				end
			end
		...
		if AntiAimContext.AntiBruteForceYawJitter and BaseAntiBruteForce then
			if AntiAimContext.AntiBruteYawJitterSettings == "X-Ways" then
				AntiAimContext.YawJitter = "Off"
				AntiAimContext.YawOffset = AntiAimContext.YawOffset + self:CreateWays(AntiAimContext.AntiBruteYawJitterWays, AntiAimContext.AntiBruteYawJitterOffset)
			elseif AntiAimContext.AntiBruteYawJitterSettings == "Skitter" then
				AntiAimContext.YawJitter = "Off"
				AntiAimContext.YawOffset = AntiAimContext.YawOffset + self:CreateWays(3, AntiAimContext.AntiBruteYawJitterOffset)
			elseif AntiAimContext.AntiBruteYawJitterSettings ~= "Skitter" and AntiAimContext.AntiBruteYawJitterTickcount >= 0 then
				AntiAimContext.YawJitter = "Off"
				local AntiBruteYawJitterTickcountState = self:GetTickcountSwitch(({
					[0] = 2,
					[1] = 6,
				})[AntiAimContext.AntiBruteYawJitterTickcount])
				if AntiAimContext.AntiBruteYawJitterTickcount == 0 then
					AntiBruteYawJitterTickcountState = self.data.Flip
				end

				if AntiAimContext.AntiBruteYawJitterSettings == "Offset" then
					AntiAimContext.YawOffset = AntiAimContext.YawOffset + (AntiBruteYawJitterTickcountState and AntiAimContext.AntiBruteYawJitterOffset or 0)
				elseif AntiAimContext.AntiBruteYawJitterSettings == "Center" then
					local CenterOffset = AntiAimContext.AntiBruteYawJitterOffset / 2
					AntiAimContext.YawOffset = AntiAimContext.YawOffset + (AntiBruteYawJitterTickcountState and CenterOffset or - CenterOffset)
				elseif AntiAimContext.AntiBruteYawJitterSettings == "Random" then
					AntiAimContext.YawOffset = AntiAimContext.YawOffset + (AntiBruteYawJitterTickcountState and math.random(0, AntiAimContext.AntiBruteYawJitterOffset) or 0)
				end
			end
		end

		if AntiAimContext.AntiBruteForceBodyYaw then
			if AntiAimContext.AntiBruteBodyYawSettings ~= "Phase" and BaseAntiBruteForce then
				AntiBruteForce = true
				AntiAimContext.BodyYaw = AntiAimContext.AntiBruteBodyYawSettings
				AntiAimContext.BodyYawExtra = AntiAimContext.AntiBruteBodyYawExtra
				AntiAimContext.BodyYawOffset = AntiAimContext.AntiBruteBodyYawOffset
				AntiAimContext.BodyYawExtraTimer = AntiAimContext.AntiBruteBodyYawExtraTimer
				AntiAimContext.BodyYawExtraOffset = AntiAimContext.AntiBruteBodyYawExtraOffset
				AntiAimContext.BodyYawExtraTickcount = AntiAimContext.AntiBruteBodyYawExtraTickcount
			elseif AntiAimContext.AntiBruteBodyYawSettings == "Phase" then
				if AntiAimContext.AntiBruteTarget == "All Target" and self.data.AntiBruteForce.Phase > 0 then
					AntiBruteForce = true
					AntiAimContext.BodyYaw = "Static"
					AntiAimContext.BodyYawOffset = ({AntiAimContext.AntiBruteBodyYawOffset, AntiAimContext.AntiBruteBodyYawPhaseOffset, AntiAimContext.AntiBruteBodyYawPhaseExtraOffset})[self.data.AntiBruteForce.Phase]
				elseif AntiAimContext.AntiBruteTarget == "Base Threat" then
					if ThreatTarget:is_valid() and ThreatTarget:is_alive() then
						local TargetEntIndex = ThreatTarget:get_index()
						if self.data.AntiBruteForce.TargetPhase[TargetEntIndex] and self.data.AntiBruteForce.TargetPhase[TargetEntIndex] > 0 then
							AntiBruteForce = true
							AntiAimContext.BodyYaw = "Static"
							AntiAimContext.BodyYawOffset = ({AntiAimContext.AntiBruteYawDefaultAngles, AntiAimContext.AntiBruteYawExtraAngles, AntiAimContext.AntiBruteYawExtendedAngles})[self.data.AntiBruteForce.TargetPhase[TargetEntIndex]]
						end
					end
				end
			end
			...
			if AntiAimContext.AntiBruteBodyYawSettings ~= "Phase" and BaseAntiBruteForce then
				AntiAimContext.FakeYawLimit = AntiAimContext.AntiBruteFakeYawLimit
				AntiAimContext.FakeYawLimitStyle = AntiAimContext.AntiBruteFakeYawSettings
				AntiAimContext.ExtraFakeYawLimit = AntiAimContext.AntiBruteFakeYawExtraLimit
				AntiAimContext.FakeYawLimitTickcount = AntiAimContext.AntiBruteFakeYawTickcount
			elseif AntiAimContext.AntiBruteBodyYawSettings == "Phase" then
				if AntiAimContext.AntiBruteTarget == "All Target" and self.data.AntiBruteForce.Phase > 0 then
					AntiBruteForce = true
					AntiAimContext.FakeYawLimitStyle = "Default"
					AntiAimContext.FakeYawLimit = ({AntiAimContext.AntiBruteFakeYawLimit, AntiAimContext.AntiBruteFakeYawExtraLimit, AntiAimContext.AntiBruteFakeYawExtendedLimit})[self.data.AntiBruteForce.Phase]
				elseif AntiAimContext.AntiBruteTarget == "Base Threat" then
					if ThreatTarget:is_valid() and ThreatTarget:is_alive() then
						local TargetEntIndex = ThreatTarget:get_index()
						if self.data.AntiBruteForce.TargetPhase[TargetEntIndex] and self.data.AntiBruteForce.TargetPhase[TargetEntIndex] > 0 then
							AntiBruteForce = true
							AntiAimContext.FakeYawLimitStyle = "Default"
							AntiAimContext.FakeYawLimit = ({AntiAimContext.AntiBruteFakeYawLimit, AntiAimContext.AntiBruteFakeYawExtraLimit, AntiAimContext.AntiBruteFakeYawExtendedLimit})[self.data.AntiBruteForce.TargetPhase[TargetEntIndex]]
						end
					end
				end
			end
	...
	if AntiAimContext.ForceDefensive and AntiAimContext.DefensiveOverride and DefensiveAntiAim then
		...
		if Data.enabled_exploit:get() and (ui.get(self.References.doubletap[1]) and ui.get(self.References.doubletap[2]) or ui.get(self.References.onshot[1]) and ui.get(self.References.onshot[2])) and not ui.get(self.References.fake_duck) then
			...
			elseif AntiAimContext.DefensiveOverrideTriggers == "Simulate" and self.data.IsDefensiveActive then
				ForceDefensiveCondition = true
	...
	if AntiAimContext.YawJitter == "X-Ways" and not AntiAimContext.ForceStatic and not AntiBruteForce then
		AntiAimContext.YawJitter = "Off"
	...
	elseif AntiAimContext.YawJitter ~= "Skitter" and AntiAimContext.YawJitterTickcount > 0 and not AntiAimContext.ForceStatic and not AntiBruteForce then
	...
	elseif ShouldAntiAim and OverrideAntiAim and not AntiAimContext.Freestanding and not AntiAimContext.EdgeYaw then
		...
		if AntiAimContext.ForceDefensive and DefensiveAntiAim then
			...
			if AntiAimContext.DefensiveTriggers == "Always On" then
				e.force_defensive = true
			elseif AntiAimContext.DefensiveTriggers == "Simulate" and self.data.IsDefensiveActive then
				e.force_defensive = true
	...
	self.data.AntiBruteForce.Mode.OnHurt = AntiAimContext.AntiBruteOnHurt
	ui.set(self.References.freestanding_body_yaw, AntiAimContext.FreestandingBodyYaw)
	self.data.AntiBruteForce.Mode.BulletImpact = AntiAimContext.AntiBruteOnBulletImpact
	ui.set(self.References.pitch[2], AntiAimContext.BaseOverride and 0 or AntiAimContext.PitchAngles)
	...
	elseif AntiAimContext.BaseOverride and ShouldAntiAim then
		...
		elseif AntiAimContext.FakeYawLimitStyle == "Random" then
			AntiAimContext.FakeYawLimit =  math.random(1, 60)
		end
...
InVincible.__index.PaintUI = function(self)
	self:Render()
end

InVincible.__index.SetupCommand = function(self, e)
	self:AntiAim(e)
	self:FakeLag(e)
	self:TickBase(e)
	self:FastLadder(e)
...
InVincible.__index.PostConfigLoad = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
...
InVincible.__index.PostConfigSave = function(self)
	if not self.Elements.MasterSwitch:get() then
		return
	end
	...
	if AutomaticSaveConfig then
		local ConfigPackage = self:GetPackages(self.Elements)
		database.write("[Arctique.polar]Configs", self.data.Configs)
		database.write("[Arctique.polar]Configs Data", self.data.ConfigsList)
		self.data.ConfigsList[self.data.AutoSaveConfigName] = self.data.Base64.encode(json.stringify(ConfigPackage))
	end
...
InVincible.__index.ShutDown = function(self)
	self:UpdateUseKey(true)
	...
	local local_player = self:NewEntity(entity.get_local_player())
	database.write("[Arctique.polar]Configs Data", self.data.ConfigsList)
	database.write("[Arctique.polar]Fired Last Update Timer", self.data.LastUpdateTimer)
	database.write("[Arctique.polar]Base Config", self.data.Base64.encode(json.stringify(ConfigPackage)))
	for index, material in pairs(self.data.ZoomMaterials) do
		if material ~= nil then
			material:reload()
		end
...
InVincible.__index.CallBacks = function(self)
	return {
		["aim_hit"] = function(e)
			self:AimHit(e)
		end,
...
InVincible.__index.Work = function(self)
	jit.on()
	self:CreateCHelpers()
	self:CreateElements()
	self:FiredUpdateLog()
	...
	ui.set_visible(self.References.leg_movement, false)
