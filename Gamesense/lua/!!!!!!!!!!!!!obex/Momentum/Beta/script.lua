    ...
    visuals = {
        ...
        render_inds = function()
			...
			if get(vis.indicators) == "simple" then
				...
				renderer.text(x - text_size_x/2 + 44 + math.floor(40 * v['scoped'])-3, y + 25, 255, 255, 255, 255 * v['alpha'], "-c", nil, simple_gradient) 
		...
		watermark = function()
			...
			local animated1 = math.abs(1 * math.cos(2 * math.pi * globals.curtime()/14)) * 20
			...
			text = ("\a"..hex.."%s\aFFFFFFFF [\aFFFFFFFF%s]\a"..hex.."%s \aFFFFFFFFms"):format(name:sub(1, animated1), status.username, latency_text)
        ...
        render_arrows = function()
			...
			m['scope'] = animate.new(m['scope'],1,0, is_scoped)
			...
			if get(vis.manaa_inds) == "classic" then
				if v.right_dir == true then
					renderer.text(x/2 + (25 * m['scope']) + offset - 3 +_inaccuracy*100 or offset, y/2 + 2, m1,m2,m3,255, "c+", 0, "»")
				...
				elseif b_yaw < -10 then
					renderer.text(x/2 + (25 * m['scope']) + offset - 2 +_inaccuracy*80, y/2 + 2, m_1,m_2,m_3,255, "c+", 0, "›")
				end
			elseif get(vis.manaa_inds) == "team skeet" then 
				renderer.triangle(x / 2 + (35 * m['scope']) + offset + 5, y / 2 + 2, x / 2 + (35 * m['scope']) + offset - 8, y / 2 - 7, x / 2 + (35 * m['scope']) + offset - 8, y / 2 + 11, 
				...
				renderer.rectangle(x / 2 + offset + (35 * m['scope']) - 12, y / 2 - 7, 2, 18, 
	...
	_misc = {
		...
		clantag = {
			run = function()
				...
				local final_tag = "invictus.black"
