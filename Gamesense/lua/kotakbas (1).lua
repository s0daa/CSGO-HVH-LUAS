...
local json = require("json")
                ...
                local lua_menu = {
                    main = {
                        enable = lua_group:label("kotakbas"),
                        tab = lua_group:combobox('\vS · \rCurrent Tab', {"Anti-Aim", "Visuals", "Misc"}),
                    },
                    antiaim = {
                        tab = lua_group:combobox("\v#kotak · AntiAim Tab", {"Main", "Builder"}),
                        addons = lua_group:multiselect('\v#kotak · \rHelpers', {'Warmup Anti~Aim', 'Anti~Knife', 'Safe Head'}),
                        safe_head = lua_group:multiselect('\v#kotak · \rSafe Head', {'Air+C Knife', 'Air+C Zeus', 'High Distance'}),
                        yaw_direction = lua_group:multiselect('\v#kotak · \rYaw Override', {'Freestanding', 'Manual'}),
                        key_freestand = lua_group:hotkey('\v#kotak · \rFreestanding'),
                        key_left = lua_group:hotkey('\v#kotak · \rManual Left'),
                        key_right = lua_group:hotkey('\v#kotak · \rManual Right'),
                        key_forward = lua_group:hotkey('\v#kotak · \rManual Forward'),
                        yaw_base = lua_group:combobox("\v#kotak · \rYaw Base", {"Local view", "At targets"}),
                        condition = lua_group:combobox('\v#kotak · \vCurrent Condition', antiaim_cond),
                    },
                    misc = {
                        cross_ind = lua_group:checkbox("\v#kotak · \rCrosshair Indicators", {255, 255, 255}),
                        cross_ind_type = lua_group:combobox("  \v#kotak · \rIndicator Style", {"Newest", "Default", "Modern", "Alternative"}),
                        cross_color = lua_group:checkbox("  \v#kotak · \rIndicator Color", {100, 100, 255}),
                        key_color = lua_group:checkbox("  \v#kotak · \rKeybinds Color", {255, 255, 255}),
                        info_panel = lua_group:checkbox("\v#kotak · \rInfo Panel"),
                        defensive_window = lua_group:checkbox("\v#kotak · \rDefensive Window", {255, 255, 255}),
                        defensive_style = lua_group:combobox("  \v#kotak · \rDefensive Style", {"Default", "Modern"}),
                        velocity_window = lua_group:checkbox("\v#kotak · \rVelocity Window", {255, 255, 255}),
                        velocity_style = lua_group:combobox("  \v#kotak · \rDefensive Style", {"Default", "Modern"}),
                        fast_ladder = lua_group:checkbox("\v#kotak · \rFast Ladder"),
                        log = lua_group:checkbox("\v#kotak · \rRagebot Logs"),
                        log_type = lua_group:multiselect("  \v#kotak · \rLog Types", {"Console", "Screen"}),
                        screen_type = lua_group:combobox("  \v#kotak · \rLog Style", {"Default", "Modern"}),
                        animation = lua_group:checkbox("\v#kotak · \rAnimation Breakers"),
                        animation_ground = lua_group:combobox("  \v#kotak · \rGround", {"Static", "Jitter", "Randomize"}),
                        animation_value = lua_group:slider("  \v#kotak · \rValue", 0, 10, 5),
                        animation_air = lua_group:combobox("  \v#kotak · \rAir", {"Off", "Static", "Randomize"}),
                        third_person = lua_group:checkbox("\v#kotak · \rThird Person Distance"),
                        third_person_value = lua_group:slider("  \v#kotak · \r Third Person Distance Value", 30, 200, 50),
                        aspectratio = lua_group:checkbox("\v#kotak · \rAspect Ratio"),
                        aspectratio_value = lua_group:slider("  \v#kotak · \r Aspect Ratio Value", 00, 200, 133),
                        teleport = lua_group:checkbox("\v#kotak · \rBreak LC Teleport"),
                        teleport_key = lua_group:hotkey("\v#kotak · \rBreak LC Teleport", true),
                        resolver = lua_group:checkbox("\v#kotak · Kotak Resolver"),
                        resolver_type = lua_group:combobox("  \v#kotak · \r Resolver Type", {"Safe", "Experimental", "Defensive"}),
                        spammers = lua_group:multiselect("\v#kotak · \r Spammers", {"Clantag", "TrashTalk"}),
                    },
                }
                ...
                local function screen_indicator()
                    local lp = entity.get_local_player()
                    if lp == nil then return end
                    local ind_size = renderer.measure_text("cb", "kotakbas")
                    ...
                    text_fade_animation(center[1] + scoped_space, center[2] + 30, -1, {r=r1, g=g1, b=b1, a=255}, {r=r2, g=g2, b=b2, a=255}, new_check and string.upper("kotakbas") or "kotakbas", main_font)
                ...
                local function info_panel()
                    ...
                    text_fade_animation(20, center[2], -1, {r=200, g=200, b=200, a=255}, {r=150, g=150, b=150, a=255}, "kotakbas.lua ~ alpha", "d")
                    local textsize = renderer.measure_text("d", "kotakbas.lua ~ alpha")
                ...
                local phrases = {
                    "ты че упал то? гетни kotakbas.lua чмо",
                    "даже твоей старой бабке потребовалась kotakbas.lua ¡",
                    "эмм..нууу.. мать твою ебал by #KOTAKBAS.lua",
                    "кыргыз чо упал то?",
                    "три два раз котакбас",
                    "общий сбор в астане друзья",
                    ">.< бомжичек зачем ты упал?( #kotakbas.lua",
                    "твой ебырь - @voicespam ",
                    "Я играю с лайфхакерской луашкой #kotakbas.lua ^.^",
                    "упал адиничкой ",
                    "ãйфон купил, луа купил, бомжа уронил",
                    "щищина сиким долбоеб #kotakbas",
                ...
                local function update_menu()
                    ...
                    label_text = string.format("­ KOTAKBAS \aFFFFFFFF­", rgba_to_hex(unpack(aA[1])), rgba_to_hex(unpack(aA[2])), rgba_to_hex(unpack(aA[3])), rgba_to_hex(unpack(aA[4])), rgba_to_hex(unpack(aA[5])), rgba_to_hex(unpack(aA[6])), rgba_to_hex(unpack(aA[7])))
                ...
                client.set_event_callback('paint', function()
                    ...
                    text_fade_animation(x_ind/2, y_ind-20, -1, {r=200, g=200, b=200, a=255}, {r=150, g=150, b=150, a=255}, "kotakbas", "cdb")
                end)
