...
-- [ build , nicks ]
    local obex_data = obex_fetch and obex_fetch() or {username = 'user', build = 'debug', discord=''}
    local userdata = {
...
local _ui = {
    lua = {
        enable = group:checkbox("\vdream.yaw"),
        tab = group:combobox("\n ", "General", "Anti-aim", "Visuals", "Miscellaneous")
    },
    ...
    antiaim = {
        ...
        tweaks = group:multiselect("\a869CC3FFdream.yaw > Tweaks", "Modify fakelag on shot", "Anti-backstab", "Safe head (not work with def aa)"),
        freestanding = group:hotkey("\a869CC3FFdream.yaw > Freestanding"),
        edge_yaw = group:hotkey("\a869CC3FFdream.yaw > Edge yaw"),
        manual_right = group:hotkey("\a869CC3FFdream.yaw > Manual Right"),
        manual_left = group:hotkey("\a869CC3FFdream.yaw > Manual Left"),
        cfg_export = group:button("\a869CC3FFdream.yaw > Export anti-aim settings", function() config.export() end),
        cfg_import = group:button("\a869CC3FFdream.yaw > Import anti-aim settings", function() config.import() end),
...
local custom_menu_control = function()
    ...
    animated_text(x + 5, y - 44, -7, {r=clr_r, g=clr_g, b=clr_b, a=get_menu_alpha()}, {r=clr_r, g=clr_g, b=clr_b, a=get_menu_alpha()}, "+", "dream.yaw")
...
local renderer_notification = function()
    ...
    for i, v in ipairs(tables.notifications) do
        ...
        v.timer = v.timer + (0.19*vector(renderer.measure_text(nil, "dream.yaw   » "..v.string)).x*0.0115)*frametime

        if math.max(0,(renderer.measure_text(nil, "dream.yaw   » "..v.string) + 1 - v.timer)) > 0 then
            v.alpha = animations.math_lerp(v.alpha, 1, frametime)
        end
    
        renderer.rectangle((_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "dream.yaw   » "..v.string)/2) - 3, (position_y + add_y) - 3, renderer.measure_text(nil, "dream.yaw   » "..v.string) + 5, 23, 0,0,0,v.alpha*100)
        renderer.rectangle((_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "dream.yaw   » "..v.string)/2) - 1, (position_y + add_y) + 16, renderer.measure_text(nil, "dream.yaw   » "..v.string) + 1, 2, 0,0,0,v.alpha*255)
        renderer.rectangle((_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "dream.yaw   » "..v.string)/2) - 1, (position_y + add_y) + 16, math.max(0,(renderer.measure_text(nil, "dream.yaw   » "..v.string) + 1 - v.timer)), 2, clr_r ,clr_g ,clr_b ,v.alpha*255)

        renderer.text(_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil,  "dream.yaw   » ")/2 - renderer.measure_text(nil, v.string)/2, position_y + add_y, clr_r, clr_g, clr_b, v.alpha*255, nil, 0, "dream.yaw  »")
        renderer.text(_ui.General.notifications_position.value == "Upper-left" and position_x + 58 or position_x - renderer.measure_text(nil, v.string)/2 + renderer.measure_text(nil, "dream.yaw   » ")/2, position_y + add_y, 255, 255, 255, v.alpha*255, nil, 0, v.string)

        if math.max(0,(renderer.measure_text(nil, "dream.yaw   » "..v.string) + 1 - v.timer)) == 0 or #tables.notifications > _ui.General.max_notifications.value then
...
local indicators = function()
    ...
    animated_text(x/2 - renderer.measure_text("b", "dream.yaw")/2 + anim_ind.scope*40, y/2 + 15, 7, {r=clr_r, g=clr_g, b=clr_b, a=255}, {r=50, g=50, b=50, a=255}, "b", "dream.yaw")
...
local watermark = function()
    ...
    animated_text(x/2 - renderer.measure_text("b", "D R E A M . Y A W ")/2, y - 35,5, {r=clr_r, g=clr_g, b=clr_b, a=255}, {r=25, g=25, b=22, a=255}, "b", "D R E A M . Y A W ")
    animated_text(x/2 - renderer.measure_text("b", "[Debug]")/2, y - 22, 5, {r=clr_r, g=clr_g, b=clr_b, a=22}, {r=255, g=22, b=22, a=255}, "b", "[Debug]")
end
...
local phrases = {
    kill = {
        ...
        "D R E A M   Y A W    A N T I   A I M B O T   A N G L E S",
        "you think you are is good but im best 1",
        "fokin dog, get ownet by D R E A M Y A W",
