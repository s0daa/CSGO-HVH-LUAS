...
local infotabflag = gui.add_combo("Info Tab | Flag", "lua>tab b", {"Russia", "Germany", "Estonia", "Hungary", "Reichsflagge", "Ukraine"})

-- others

local clantag = gui.add_checkbox("Clantag", "lua>tab b")
local clantagtype = gui.add_combo("Clantag Type", "lua>tab b", {"Old"})
...
local Checkbox = gui.add_checkbox("Cock Inverter", "lua>tab b")
local keybind = gui.add_keybind("lua>tab b>Cock Inverter")
...
local keybinds_bcolor = gui.add_checkbox("BindList Color", "lua>tab b")
local keybinds_color = gui.add_colorpicker("lua>tab b>BindList Color", true)
local infobar_bcolor = gui.add_checkbox("State Panel Color", "lua>tab b")
local infobar_color = gui.add_colorpicker("lua>tab b>State Panel color", true)
local indicator_bcolor = gui.add_checkbox("Indicators Color", "lua>tab b")
local indicator_color = gui.add_colorpicker("lua>tab b>Indicators color", true)

local WorkAA = gui.add_checkbox("AntiHit", "lua>tab b")

local States = gui.add_combo("Choose AntiHit Condition:", "lua>tab b", {"[None]", "[Standing]", "[Moving]","[Slow Walking]","[Crouching]","[In Air]"})
...
function gui_system() 
    ...
    gui.set_visible("lua>tab b>AntiHit", AntiHit)
    gui.set_visible("lua>tab b>Choose Antihit Condition:", AAON)
...
function guiscc()
    ...
    -- aa addons
    gui.set_visible("lua>tab b>Cock Inverter", tab == 1)

    -- visuals
    gui.set_visible("lua>tab b>Watermark", tab == 2)
    gui.set_visible("lua>tab b>info tab", tab == 2)
    gui.set_visible("lua>tab b>keybinds", tab == 2)
    gui.set_visible("lua>tab b>indicators", tab == 2)
    gui.set_visible("lua>tab b>indicators | type", tab == 2 and indicatorsenb)
    gui.set_visible("lua>tab b>Info Tab | Flag", tab == 2 and infobar)
    ...
    -- colours
    gui.set_visible("lua>tab b>BindList color", tab == 4)
    gui.set_visible("lua>tab b>State Panel color", tab == 4)
    gui.set_visible("lua>tab b>watermark color", tab == 4)
    gui.set_visible("lua>tab b>Indicators color", tab == 4)
end
...
function gui_controller()
    local text =  "amnestia.sys anti-aim"
    ...
    local debug2 = math.floor(math.abs(math.sin(global_vars.realtime) * 2) * 255)
    if infobar:get_bool() then
        render.rect_filled_multicolor(5, y / 2, 150, (y / 2) + 20,infobar_color:get_color(), render.color(0,0,0,0),render.color(0,0,0,0), infobar_color:get_color())
        render.text(pixel, 37,(y / 2) + 2, "amnestia.sys anti-aim", render.color(255,255,255,255))
        render.text(pixel, 37 + textx,(y / 2) + 1, " technologies", infobar_color:get_color())
        render.text(pixel, 37,(y / 2) + 10, "user: " .. username .. "", render.color(255,255,255,255))
        render.text(pixel, 37 + text2x,(y / 2) + 10, " [debug]", render.color(infobar_color:get_color().r, infobar_color:get_color().g, infobar_color:get_color().b, debug2))
        ...
        if infotabflag == 5 then
            render.rect_filled(7, (y / 2) + 2, 32, (y / 2) + 10,render.color(0, 0,255,255))
            render.rect_filled(7, (y / 2) + 10, 32, (y / 2) + 17,render.color(255, 255, 0,255))
        end
    end
end
...
function indicators()
    ...
    if indicatorsmain:get_bool() and indicatorstype:get_int() == 0 then
        ...
        local y = screen_height / 2
        local textx , texty = render.get_text_size(font, "amnestia") 
        ...
        local text =  "amnestia.sys°"
        local text2 = "debug"
        local textx, texty = render.get_text_size(pixel, text)
        local textx2, texty2 = render.get_text_size(pixel, text2)
    
        render.text(font, x+offset_scope + 7 , y + 15 + texty, "amnestia", render.color("#FFFFFF"))
        ...
        if dt then
            ...
            render.text(font, x+ offset_scope - textx + 35  , y + 35 + texty, "DT", render.color("#A1FF97"))
        end
        if not dmg then
...
function ID()
    ...
    if indicatorsmain:get_bool() and indicatorstype:get_int() == 1 then
        ...
        render.text(pixel, x+offset_scope + 11, y + 30, text2, render.color(71, 71, 71, debug2))
...
function on_keybinds()
    ...
    debug = animate(debug or 0, gui.is_menu_open() or override_active or other_binds_active, 1, 0.5, false, true)

    -- glow
    for i = 1, 10 do
        render.rect_filled_rounded(pos[1] - i, pos[2] - i, pos[1] + size[1] + i, pos[2] + size[2] + i, render.color(keybinds_color:get_color().r, keybinds_color:get_color().g, keybinds_color:get_color().b, (25 - (3 * i)) * debug), 10)
    end

    -- top rect
    render.push_clip_rect(pos[1], pos[2], pos[1] + size[1], pos[2] + 5)
    render.rect_filled_rounded(pos[1], pos[2], pos[1] + size[1], pos[2] + size[2], render.color(0, 0, 0, 105 * debug), 5)
    render.pop_clip_rect()

    -- bot rect
    render.push_clip_rect(pos[1], pos[2] + 17, pos[1] + size[1], pos[2] + 220)
    render.rect_filled_rounded(pos[1], pos[2], pos[1] + size[1], pos[2] + 22, render.color(0, 0, 0, 105 * debug), 5)
    render.pop_clip_rect()

    -- other colormain:get_color().r
    render.rect_filled_multicolor(pos[1], pos[2] + 5, pos[1] + size[1], pos[2] + 17, render.color(keybinds_color:get_color().r, keybinds_color:get_color().g, keybinds_color:get_color().b, 255 * debug), render.color(keybinds_color:get_color().r, keybinds_color:get_color().g, keybinds_color:get_color().b, 255 * debug), render.color(keybinds_color:get_color().r, keybinds_color:get_color().g, keybinds_color:get_color().b, 255 * debug), render.color(keybinds_color:get_color().r, keybinds_color:get_color().g, keybinds_color:get_color().b, 255 * debug))
    render.rect_filled_rounded(pos[1] + 2, pos[2] + 2, pos[1] + size[1] - 2, pos[2] + 20, render.color(0, 0, 0, 255 * debug), 5)
    render.text(verdana, pos[1] + size[1] / 2 - render.get_text_size(verdana, "bindlist") / 2 - 1, pos[2] + 7, "bindlist", render.color(255, 255, 255, 255 * debug))


    local bind_offset = 0
    dt_debug = animate(dt_debug or 0, binds[1], 1, 0.5, false, true)
    render.text(verdana, pos[1] + 6, pos[2] + size[2] + 2, binds_name[1], render.color(255, 255, 255, 255 * dt_debug))
    render.text(verdana, pos[1] + size[1] - text_size, pos[2] + size[2] + 2, enabled, render.color(255, 255, 255, 255 * dt_debug))
    if binds[1] then
        bind_offset = bind_offset + 11
    end

    hs_debug = animate(hs_debug or 0, binds[2], 1, 0.5, false, true)
    render.text(verdana, pos[1] + 6, pos[2] + size[2] + 2 + bind_offset, binds_name[2], render.color(255, 255, 255, 255 * hs_debug))
    render.text(verdana, pos[1] + size[1] - text_size, pos[2] + size[2] + 2 + bind_offset, enabled, render.color(255, 255, 255, 255 * hs_debug))
    if binds[2] then
        bind_offset = bind_offset + 11
    end

    dmg_debug = animate(dmg_debug or 0, binds[3], 1, 0.5, false, true)
    render.text(verdana, pos[1] + 6, pos[2] + size[2] + 2 + bind_offset, binds_name[3], render.color(255, 255, 255, 255 * dmg_debug))
    render.text(verdana, pos[1] + size[1] - text_size, pos[2] + size[2] + 2 + bind_offset, enabled, render.color(255, 255, 255, 255 * dmg_debug))
    if binds[3] then
        bind_offset = bind_offset + 11
    end

    fs_debug = animate(fs_debug or 0, binds[4], 1, 0.5, false, true)
    render.text(verdana, pos[1] + 6, pos[2] + size[2] + 2 + bind_offset, binds_name[4], render.color(255, 255, 255, 255 * fs_debug))
    render.text(verdana, pos[1] + size[1] - text_size, pos[2] + size[2] + 2 + bind_offset, enabled, render.color(255, 255, 255, 255 * fs_debug))
    if binds[4] then
        bind_offset = bind_offset + 11
    end

    ho_debug = animate(ho_debug or 0, binds[5], 1, 0.5, false, true)
    render.text(verdana, pos[1] + 6, pos[2] + size[2] + 2 + bind_offset, binds_name[5], render.color(255, 255, 255, 255 * ho_debug))
    render.text(verdana, pos[1] + size[1] - text_size, pos[2] + size[2] + 2 + bind_offset, enabled, render.color(255, 255, 255, 255 * ho_debug))
    if binds[5] then
        bind_offset = bind_offset + 11
    end

    fd_debug = animate(fd_debug or 0, binds[6], 1, 0.5, false, true)
    render.text(verdana, pos[1] + 6, pos[2] + size[2] + 2 + bind_offset, binds_name[6], render.color(255, 255, 255, 255 * fd_debug))
    render.text(verdana, pos[1] + size[1] - text_size, pos[2] + size[2] + 2 + bind_offset, enabled, render.color(255, 255, 255, 255 * fd_debug))
