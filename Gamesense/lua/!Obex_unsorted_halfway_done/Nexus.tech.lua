...
aa_init[0] = {
	...
	fast_weapon_switch = ui.new_checkbox("AA", "Anti-aimbot angles", SM4.." \a4F4F4FFF ~  \a42454fFFFast zeus swap"),
...
client.set_event_callback('aim_fire', function()
	if not ui.get(aa_init[0].fast_weapon_switch) then return end
...
for v=1, #bruteforce_phases do
    ...
    bf[v] = {
        ...
        bf_fake_limit = ui.new_slider('aa', 'anti-aimbot angles', cond_bf..'\a4F4F4FFF ~  \a42454FFFDesync limit', 0, 60, 60, true, '°', 1),
        bf_custom_dsy = ui.new_checkbox('aa', 'anti-aimbot angles', cond_bf..'\a4F4F4FFF ~  \a42454FFFCustom desync'),
		bf_switchticks_amount = ui.new_slider("AA", "Anti-aimbot angles", "\a4F4F4FFF ~  \a42454FFFTicks to switch\n" , 2, 64, 9),
        bf_fake_limit_2 = ui.new_slider('aa', 'anti-aimbot angles', cond_bf..'\a4F4F4FFF ~  \a42454FFFDesync limit #2', 0, 60, 60, true, '°', 1)
	}	
end
local function handle_menu_bf()
    ...
    for v=1, #bruteforce_phases do
		...
		ui.set_visible(bf[v].bf_fake_limit, setvis_bf)
		
		ui.set_visible(bf[v].bf_custom_dsy, setvis_bf)
		
		ui.set_visible(bf[v].bf_switchticks_amount, setvis_bf and ui.get(bf[v].bf_custom_dsy))
		ui.set_visible(bf[v].bf_fake_limit_2, setvis_bf and ui.get(bf[v].bf_custom_dsy))
...
for i=1, #aa_config do
    ...
    rage[i] = {
        ...
        t_fake_limit = ui.new_slider('aa', 'anti-aimbot angles', cond_t..'\a4F4F4FFF ~  \a42454FFFDesync limit', 0, 60, 60, true, '°', 1),
        t_custom_dsy = ui.new_checkbox('aa', 'anti-aimbot angles', cond_t..'\a4F4F4FFF ~  \a42454FFFCustom desync'),
		t_switchticks_amount = ui.new_slider("AA", "Anti-aimbot angles", "\a4F4F4FFF ~  \a42454FFFTicks to switch\n" , 2, 64, 9),
        t_fake_limit_2 = ui.new_slider('aa', 'anti-aimbot angles', cond_t..'\a4F4F4FFF ~  \a42454FFFDesync limit #2', 0, 60, 60, true, '°', 1),
        ...
        ct_fake_limit = ui.new_slider('aa', 'anti-aimbot angles', cond_ct..'\a4F4F4FFF ~  \a42454FFFDesync limit', 0, 60, 60, true, '°', 1),
        ct_custom_dsy = ui.new_checkbox('aa', 'anti-aimbot angles', cond_ct..'\a4F4F4FFF ~  \a42454FFFCustom desync'),
		ct_switchticks_amount = ui.new_slider("AA", "Anti-aimbot angles", "\a4F4F4FFF ~  \a42454FFFTicks to switch\n" , 2, 64, 9),
        ct_fake_limit_2 = ui.new_slider('aa', 'anti-aimbot angles', cond_ct..'\a4F4F4FFF ~  \a42454FFFDesync limit #2', 0, 60, 60, true, '°', 1)
	}	
end
local function handle_menu()
	...
	for i=1, #aa_config do
		...
		ui.set_visible(rage[i].t_custom_dsy, setvis_t or hide_b)
		
		ui.set_visible(rage[i].t_fake_limit, setvis_t or hide_b)
		
		ui.set_visible(rage[i].t_switchticks_amount, setvis_t and ui.get(rage[i].t_custom_dsy) or hide_b)
		ui.set_visible(rage[i].t_fake_limit_2, setvis_t and ui.get(rage[i].t_custom_dsy) or hide_b)
		...
		ui.set_visible(rage[i].ct_custom_dsy, setvis_ct or hide_b)
		
		ui.set_visible(rage[i].ct_fake_limit, setvis_ct or hide_b)
		
		ui.set_visible(rage[i].ct_switchticks_amount, setvis_ct and ui.get(rage[i].ct_custom_dsy) or hide_b)
		ui.set_visible(rage[i].ct_fake_limit_2, setvis_ct and ui.get(rage[i].ct_custom_dsy) or hide_b)
	end
end
...
local function invalid_ticks(cmd)
	...
	if ui.get(aa_init[0].invalid_bt_ticks) then
		if cmd.in_jump == 1 and is_charged then
...
client.set_event_callback("setup_command", function(cmd)
	...
	elseif stage <= 0 or stage > 5 then 
		...
		if ui.get(aa_init[0].brute_enable) then
			...
			local ticks_rate = ui.get(bf[bf_state].bf_switchticks_amount)
			local ticksxd = globals.tickcount() % ticks_rate + 1
			
			if ui.get(bf[bf_state].bf_custom_dsy) then
				if ticksxd >= ticks_rate then
					ui.set(ref.fakeyawlimit, ui.get(bf[bf_state].bf_fake_limit_2))
				else
					ui.set(ref.fakeyawlimit, ui.get(bf[bf_state].bf_fake_limit))
				end
			else
				ui.set(ref.fakeyawlimit, ui.get(bf[bf_state].bf_fake_limit))
			end
...
client.set_event_callback("setup_command", function(e)
	...
	if ui.get(aa_init[0].defensive_inair) then	
		if in_air(lp) and is_dt then
			e.force_defensive = 1
		else
			e.force_defensive = 0
		end
	...
	if ui.get(aa_init[0].legit_aa) and client.key_state(0x45) then
	 ...
	 -- PONER LAS SETTINGS DE LA TABLA legit_setts aki :v
		...
		elseif not ui.get(ui_stuff.exploit_select) == "Defensive anti-aim" or not ui.get(ui_stuff.defensive_aa_key) and not client.key_state(0x45)then
			...
			if ui.get(ui_stuff.exploit_select) == "Pitch exploit" and ui.get(ui_stuff.esploit2) and globals.tickcount() % ticks_amount < 7 and is_dt and not client.key_state(0x45) then
				...
				if is_t then
					...
					local t_ticks_rate = ui.get(rage[state].t_switchticks_amount)
					local t_ticksxd = globals.tickcount() % t_ticks_rate + 1
					
					if ui.get(rage[state].t_custom_dsy) then
						if t_ticksxd >= t_ticks_rate then
							ui.set(ref.fakeyawlimit, ui.get(rage[state].t_fake_limit_2))
						else
							ui.set(ref.fakeyawlimit, ui.get(rage[state].t_fake_limit))
						end
					else
						ui.set(ref.fakeyawlimit, ui.get(rage[state].t_fake_limit))
					end
					...
					local ct_ticks_rate = ui.get(rage[state].ct_switchticks_amount)
					local ct_ticksxd = globals.tickcount() % ct_ticks_rate + 1
					
					if ui.get(rage[state].t_custom_dsy) then
						if ct_ticksxd >= ct_ticks_rate then
							ui.set(ref.fakeyawlimit, ui.get(rage[state].ct_fake_limit_2))
						else
							ui.set(ref.fakeyawlimit, ui.get(rage[state].ct_fake_limit))
						end
					else
						ui.set(ref.fakeyawlimit, ui.get(rage[state].ct_fake_limit))
					end
...
local function draw()
	...
	local mr,mg,mb,ma = ui.get(ui_stuff.main_clr)
	local x, y = client.screen_size()
	...
	local bottom_txt = ctx.helpers:animate_text(globals.curtime() * 1, "nexus[beta] anti-aim technologies", 39, 43, 54, 255)

	renderer.text(x / 2, y / 2 + 532, 16, 16, 18, 255, "cb", 0, "nexus[beta] anti-aim technologies")
	renderer.text(x / 2, y / 2 + 532, 39, 43, 54, 255, "cb", 0, "", unpack(bottom_txt))
	...
	if ui.get(ui_stuff.inds_selct) == "Stylish" and ui.get(ui_stuff.crosshair_inds) then
		...
		wiwi.xpos_dt = animation(scoped_default, wiwi.xpos_dt, 15, 20) or 0
		...
		ctx.m_render:glow_module(x/2 - 25 + wiwi.xpos_gradient, y/2 + 24, 50, -0.2, 9, 0, {rg, gg, bg, alpha}, {rg, gg, bg, alpha})
		ctx.m_render:glow_module(x/2 - 25 + wiwi.xpos_gradient, y/2 + 31, 50, -0.2, 9, 0, {rg, gg, bg, alpha}, {rg, gg, bg, alpha})

		renderer.text(x / 2 - 1 + wiwi.xpos, y / 2 + 24, r, g, b, alpha_scope, "cb", 0, "nexus.tech")
		renderer.text(x / 2 - 1 + wiwi.xpos, y / 2 + 24, r2, g2, b2, alpha_scope, "cb", 0, "",unpack(ret))

		if scoped_default then
			renderer.text(x / 2 - 1 + wiwi.xpos_state , y / 2 + 33, 150, 150, 150, alpha_scope, "-", 0, string.upper(p_state)) -- + n2_x - testting
		else
			renderer.text(x / 2 - 1 + wiwi.xpos_state, y / 2 + 39, 150, 150, 150, alpha_scope, "c-", 0, string.upper(p_state)) -- + n2_x - testting
		end
		
		renderer_rectangle_rounded(x / 2 - 27 + wiwi.xpos_dsy, y / 2 + 30, 53, 4, 0, 0, 0, alpha_scope, 1)
		renderer_rectangle_rounded(x / 2 - 27 + wiwi.xpos_dsy, y / 2 + 31, 0+(math.abs(bodyyaw*58/62)), 2, r, g, b, alpha_scope, 1)

		if is_dt then
			if charge <= 0 then
				renderer.text(x / 2 - 2 + wiwi.xpos_dt, y / 2 + 49 , 110, 110, 110, alpha_scope, "c-", 0, "DT")
			else
				renderer.text(x / 2 - 2 + wiwi.xpos_dt, y / 2 + 49 , 255, 255, 255, alpha_scope, "c-", 0, "DT")
			end
		else
			if is_os then
				renderer.text(x / 2 + wiwi.xpos_os - 1, y / 2 + 49, 255, 255, 255, alpha_scope, "c-", 0, "OS")
			end
		end
		
		if is_sp then
			renderer.text(x / 2 -18 + wiwi.xpos_binds , y / 2  + 49 + wiwi.y_add, 255, 255, 255, alpha_scope, "c-", 0, "SP")
		else 
			renderer.text(x / 2 -18 + wiwi.xpos_binds , y / 2  + 49 + wiwi.y_add, 110, 110, 110, alpha_scope, "c-", 0, "SP")
		end
		
		if is_ba then 
		
			renderer.text(x / 2 + wiwi.xpos_binds - 1, y / 2  + 49 + wiwi.y_add, 255, 255, 255, alpha_scope, "c-", 0, "BAIM")
		else
			renderer.text(x / 2 + wiwi.xpos_binds - 1, y / 2  + 49 + wiwi.y_add, 110, 110, 110, alpha_scope, "c-", 0, "BAIM")
		end

		if is_fs then 
			renderer.text(x / 2+ 15 + wiwi.xpos_binds, y / 2  + 49 + wiwi.y_add, 255, 255, 255, alpha_scope, "c-", 0, "FS")
		else
			renderer.text(x / 2+ 15 + wiwi.xpos_binds, y / 2  + 49 + wiwi.y_add, 110, 110, 110, alpha_scope, "c-", 0, "FS")
		end 
	...
	if ui.get(ui_stuff.inds_selct) == "Modern" and ui.get(ui_stuff.crosshair_inds) then
		...
		renderer.text(x/2 + 27, y/2 + 34, mr, mg, mb, alpha, "-", 0, string.upper(obex_data.build))
...
local cfg_data = {
	...
	data = {
        ...
        rage[1].t_custom_dsy, rage[2].t_custom_dsy, rage[3].t_custom_dsy, rage[4].t_custom_dsy, rage[5].t_custom_dsy, rage[6].t_custom_dsy, rage[7].t_custom_dsy,
		rage[1].t_switchticks_amount, rage[2].t_switchticks_amount, rage[3].t_switchticks_amount, rage[4].t_switchticks_amount, rage[5].t_switchticks_amount, rage[6].t_switchticks_amount, rage[7].t_switchticks_amount, 
        rage[1].t_fake_limit, rage[2].t_fake_limit, rage[3].t_fake_limit, rage[4].t_fake_limit, rage[5].t_fake_limit, rage[6].t_fake_limit, rage[7].t_fake_limit,
        rage[1].t_fake_limit_2, rage[2].t_fake_limit_2, rage[3].t_fake_limit_2, rage[4].t_fake_limit_2, rage[5].t_fake_limit_2, rage[6].t_fake_limit_2, rage[7].t_fake_limit_2,
        ...
        rage[1].ct_custom_dsy, rage[2].ct_custom_dsy, rage[3].ct_custom_dsy, rage[4].ct_custom_dsy, rage[5].ct_custom_dsy, rage[6].ct_custom_dsy, rage[7].ct_custom_dsy,
		rage[1].ct_switchticks_amount, rage[2].ct_switchticks_amount, rage[3].ct_switchticks_amount, rage[4].ct_switchticks_amount, rage[5].ct_switchticks_amount, rage[6].ct_switchticks_amount, rage[7].ct_switchticks_amount,
        rage[1].ct_fake_limit, rage[2].ct_fake_limit, rage[3].ct_fake_limit, rage[4].ct_fake_limit, rage[5].ct_fake_limit, rage[6].ct_fake_limit, rage[7].ct_fake_limit,
        rage[1].ct_fake_limit_2, rage[2].ct_fake_limit_2, rage[3].ct_fake_limit_2, rage[4].ct_fake_limit_2, rage[5].ct_fake_limit_2, rage[6].ct_fake_limit_2, rage[7].ct_fake_limit_2,
        ...
        bf[1].bf_custom_dsy, bf[2].bf_custom_dsy, bf[3].bf_custom_dsy, bf[4].bf_custom_dsy, bf[5].bf_custom_dsy,
		bf[1].bf_switchticks_amount, bf[2].bf_switchticks_amount, bf[3].bf_switchticks_amount, bf[4].bf_switchticks_amount, bf[5].bf_switchticks_amount,
        bf[1].bf_fake_limit, bf[2].bf_fake_limit, bf[3].bf_fake_limit, bf[4].bf_fake_limit, bf[5].bf_fake_limit,
        bf[1].bf_fake_limit_2, bf[2].bf_fake_limit_2, bf[3].bf_fake_limit_2, bf[4].bf_fake_limit_2, bf[5].bf_fake_limit_2,
...
local t_cond_data = {
	t_data_cond = {
        ...
        rage[cond_state].t_custom_dsy,
		rage[cond_state].t_switchticks_amount,
        rage[cond_state].t_fake_limit,
        rage[cond_state].t_fake_limit_2,
...
local ct_cond_data = {
	ct_data_cond = {
        ...
        rage[cond_state].ct_custom_dsy,
		rage[cond_state].ct_switchticks_amount,
        rage[cond_state].ct_fake_limit,
        rage[cond_state].ct_fake_limit_2,
...
local btn_to_tt = ui.new_button("AA", "Anti-aimbot angles", "\a8FBEFFFFSend to T", function()
	...
	ui.set(rage[state].t_custom_dsy, ui.get(rage[state].ct_custom_dsy))
	ui.set(rage[state].t_switchticks_amount, ui.get(rage[state].ct_switchticks_amount))
	ui.set(rage[state].t_fake_limit_2, ui.get(rage[state].ct_fake_limit_2))
	ui.set(rage[state].t_fake_limit, ui.get(rage[state].ct_fake_limit))
...
local btn_to_ct = ui.new_button("AA", "Anti-aimbot angles", "\a8FBEFFFFSend to CT", function()
	...
	ui.set(rage[state].ct_custom_dsy, ui.get(rage[state].t_custom_dsy))
	ui.set(rage[state].ct_switchticks_amount, ui.get(rage[state].t_switchticks_amount))
	ui.set(rage[state].ct_fake_limit_2, ui.get(rage[state].t_fake_limit_2))
	ui.set(rage[state].ct_fake_limit, ui.get(rage[state].t_fake_limit))
...
dt = ui.multiReference("RAGE", "Other", "Double tap")
