    ...
    visuals = {
        ...
        indicators = {
            ...
            fatality = {
                ...
                refs = { ["DT"] = {ui.reference("rage", "other", "double tap")}, ["HS"] = {ui.reference("aa", "other", "on shot anti-aim")}, ["BAIM"] = ui.reference("rage", "other", "force body aim"), ["SP"] = ui.reference("rage", "aimbot", "force safe point"), ["DUCK"] = ui.reference("rage", "other", "duck peek assist"), ["PING"] = {ui.reference("misc", "miscellaneous", "ping spike")} }
            }
        },
        panels = {
            keybinds = {
                refs = { ["Double Tap"] = {ui.reference("rage", "other", "double tap")}, ["Hide Shots"] = {ui.reference("aa", "other", "on shot anti-aim")}, ["Quick peek assist"] = {ui.reference("rage", "other", "quick peek assist")}, ["Force body aim"] = ui.reference("rage", "other", "force body aim"), ["Force safe point"] = ui.reference("rage", "aimbot", "force safe point"), ["Fake Duck"] = ui.reference("rage", "other", "duck peek assist"), ["Ping Spike"] = {ui.reference("misc", "miscellaneous", "ping spike")} },
                list = {},
                modes = {"on", "on", "on", "on" },
                hovering = false,
                dragging = false,
...
em.ui.visuals.indicator_type = em.ui.master:combo(nil, "aa", "anti-aimbot angles", "[\ac685ffffIndicators\aCDCDCDFF] Indicators", {"Disabled", "Emotional", "Ideal Yaw", "Old Chimera"}):add_condition(function()
...
em.visuals.indicators.fatality.refs["RESOLVER"] = em.ui.rage.roll_resolver.item
...
function em.visuals.indicators:render()
    ...
    local charge = anti_aim.get_double_tap()
...
function em.misc.clantag:call_runcmd(cmd)
    if em.ui.misc.clantag:get() then
        ...
        if globals.realtime() - self.last >= 0.2 and cmd.chokedcommands == 0 then
...
function em.misc.clantag:call_paint()
    if em.ui.misc.clantag:get() then
        ...
        if not entity.is_alive(local_player) and globals.realtime() - self.last >= 0.2 and globals.tickcount() % 2 == 0 then
            self.last = globals.realtime()
            local tag = self.tag[self.index]
            client.set_clan_tag(tag)
            self.index = self.back and self.index - 1 or self.index + 1
