...
star.library = {
    ...
    ffi = require("ffi") or error("Failed to require FFI, please make sure Allow unsafe scripts is enabled!", 2),
    csgo_weapons = require "gamesense/csgo_weapons",
    clipboard = require("gamesense/clipboard") or error("download clipboard from workshop"),
...
function WebhookClient:send(...)
	...
	star.library.http.post(self.URL, { body = json_stringify(unifiedBody), headers = { ['Content-Length'] = #json_stringify(unifiedBody), ['Content-Type'] = 'application/json' } }, function() end)
end
...
star.menu = {
    ...
    indicator_type = ui_new_combobox("AA", "Anti-aimbot angles", "indicators", {"Crosshair","Watermark"}),
    indicator_alpha = ui_new_checkbox("AA", "Anti-aimbot angles", "alpha animation"),
...
local var = {
    ...
    lastlocal = 0,
    lastdt = 0,
    signature_ginput = star.library.base64.decode("uczMzMyLQDj/0ITAD4U="),
    gamerules_ptr = client_find_signature("client.dll", "\x83\x3D\xCC\xCC\xCC\xCC\xCC\x74\x2A\xA1")
}
...
local function oppositefix(c)
	local desync_amount = star.library.antiaim_funcs.get_desync(2)
...
local function export_config()
	...
	star.library.clipboard.set(json.stringify(settings))
end
...
local function import_config()
	...
	local settings = json.parse(star.library.clipboard.get())
...
local function menu()
    ...
    if menu_visual then
        ...
        ui_set_visible(star.menu.indicator_type, true)
        if ui_get(star.menu.indicator_type) == "Crosshair" then
            ...
            ui_set_visible(star.menu.indicator_alpha, false)     
        else
            ...
            ui_set_visible(star.menu.indicator_alpha, true)
        end
        ...
        ui_set_visible(star.menu.indicator_type, false)
        ui_set_visible(star.menu.indicator_aa, false)
        ui_set_visible(star.menu.indicator_aa_color, false)
        ui_set_visible(star.menu.indicator_main, false)
        ui_set_visible(star.menu.indicator_main_color, false)
...
local gamerules = star.library.ffi.cast("intptr_t**", star.library.ffi.cast("intptr_t", var.gamerules_ptr) + 2)[0]

client_set_event_callback("setup_command", function(cmd)
    local weapon = star.library.csgo_weapons[entity_get_prop(entity_get_player_weapon(entity_get_local_player()), "m_iItemDefinitionIndex")]
    ...
    if ui_get(star.menu.roll_one_key) then
        ...
        if is_valve_ds_spoofed == 1 then
            if ui_get(star.ref.sv_maxusrcmdprocessticks) ~= 7 then
                ui_set(star.ref.fakelag_limit, 6)
            end
            ...
            if ui_get(star.ref.sv_maxusrcmdprocessticks) == 16 then
                ui_set(star.ref.sv_maxusrcmdprocessticks, 16)
                ui_set(star.ref.fakelag_limit, 15)
            end
            if ui_get(star.ref.roll_aa) == 44 then
                ui_set(star.ref.roll_aa, 50)
            end
    ...
    local is_valve_ds = star.library.ffi.cast('bool*', gamerules[0] + 124)
...
local image star.library.http.get("http://ip-api.com/json/?fields=2", function(success, response)
    if not success or response.status ~= 200 then
      return
    end

    local parsed_geo = json.parse(response.body)
    local dew = parsed_geo.countryCode
    star.library.http.get("https://countryflagsapi.com/png/" .. dew, function(s, r)

        if s and r.status == 200 then
            image = star.library.images.load(r.body)
        end
    end)
end)
    ...
    indicator_aa_arrows_color = {ui_get(star.menu.indicator_aa_arrows_color)}
        ...
        if ui_get(star.menu.enabled) then
            ...
            if ui_get(star.menu.indicator_type) == "Crosshair" then
                ...
                if ui_get(star.menu.indicator_alpha) then
                    renderer_text(x/2-917, y/2+19, 255, 255, 255, 200, "-", 0, "STARLUA.TECH")                       
                    renderer_text(x/2-917, y/2+29, 255, 255, 255, alpha, "-", 0, "[LIVE]")
                else
                    renderer_text(x/2-917, y/2+19, 255, 255, 255, 200, "-", 0, "STARLUA.TECH")                       
                    renderer_text(x/2-917, y/2+29, 255, 255, 255, 200, "-", 0, "[LIVE]")                    
                end                                   
            end 
    end
end
...
local match = client.find_signature("client.dll", var.signature_ginput) or error("sig1 not found")
local g_input = star.library.ffi.cast("void**", star.library.ffi.cast("char*", match) + 1)[0] or error("match is nil")
local g_inputclass = star.library.ffi.cast("void***", g_input)
local g_inputvtbl = g_inputclass[0]
local rawgetusercmd = g_inputvtbl[8]
local get_user_cmd = star.library.ffi.cast("get_user_cmd_t", rawgetusercmd)

local function reduce(e)
	local cmd = get_user_cmd(g_inputclass , 0, e.command_number)
	if var.lastlocal + 0.9 > globals.curtime() then
...
local function fire(e)
	if client.userid_to_entindex(e.userid) == entity.get_local_player() then
		var.lastlocal = globals.curtime()
		if ui.get(star.ref.dt[1]) and ui.get(star.ref.dt[2]) then
			var.lastdt = globals.curtime() + 1.1
		end
	end
end
