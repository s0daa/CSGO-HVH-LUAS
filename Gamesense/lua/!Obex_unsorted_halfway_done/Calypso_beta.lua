...
local visual_tab = {}
local anti_aim_tab = {}
local obex_data = obex_fetch and obex_fetch() or {username = "strexxter", build = "dev"}
--- @endregion
...
references.list = {
    ...
    ["Fake yaw limit"] = {ui.reference("AA", "Anti-aimbot angles", "Fake yaw limit"), true},
    ...
    ["Doubletap"] = {{ui.reference("RAGE", "Other", "Double tap")}, false},
...
function anti_aim_tab:parse_preset_data(cmd)
    ...
    if is_state_applied then
        ...
        data.fake_limit = self.choose_difference(get_item("fake yaw build"), get_item("fake yaw limit"), get_item("fake yaw additional limit"))
    else
...
function anti_aim_tab:override_settings(cmd)
    ...
    ui.set(references.get("Fake yaw limit"), self.data.fake_limit)
