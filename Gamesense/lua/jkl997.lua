...
local corsasshit = "Corsa's Prediction ~ "
...
client.color_log(149, 149, 201, corsasshit.."Welcome back, Hakkai.")
client.delay_call(2, function()
client.color_log(149, 149, 201, corsasshit.."Your subscription expires: never")
end)
...
local levl = {"Adaptive", "Experimental"}
...
b_2.rage = {
    ...
    pingpofass = ui.new_slider("rage", "other", "Correction " .. string.format("\a%02X%02X%02XFFMode", ui.get(menu_color)), 1, 2, 1, true, "", 1, {"Adaptive", "Experimental"}),
    space = ui.new_label("rage", "other", string.format("\a%02X%02X%02XFF‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾‾", ui.get(menu_color))),
    ...
    gay = ui.new_checkbox('rage', 'other', string.format("\v\rAI-\a%02X%02X%02XFFTools\v\r", ui.get(menu_color))),
    shitto = ui.new_checkbox('rage', 'other', string.format("\v\rHitscan \a%02X%02X%02XFFFix\v\r", ui.get(menu_color))),
    aitools = ui.new_checkbox('rage', 'other', string.format("\v\rInterlude \a%02X%02X%02XFFAI\v\r", ui.get(menu_color)))
}
...
client.set_event_callback("paint", function()
    if ui.get(b_2.rage.predict) then
      local r,g,b = ui.get(menu_color)
        renderer.indicator(r,g,b,255, "\a"..rgba_to_hex(r,g,b,255 * math.abs(math.cos(globals.curtime()*1))).."❆ Predict Interpolation")
    end
end)
...
ui.set_enabled(b_2.rage.aitools, false)

ui.set_enabled(b_2.rage.jittercorrectionresolvercorsas, true)

ui.set_enabled(b_2.rage.pingpofass, true)