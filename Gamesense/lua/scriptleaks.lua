...
local tittle = ui.new_label("lua", "b", "\a".. menu_c_hex .."Script Leaks ~\abfbdbdFF Loader")
...
local function list_scripts()
    a.http_req().get("http://cloudpusy.ct8.pl/loader/list_luas.php",
        function(success, response)
            if not success or response.status ~= 200 then
                new_notify("Something went wrong")
                client.color_log(menu_r, menu_g, menu_b, "Script Leaks ~\0")
...
local function load_selected_script(selected_item)
    a.http_req().get("http://cloudpusy.ct8.pl/loader/load_lua.php?lua=" .. selected_item,
        function(success, response)
            if not success or response.status ~= 200 then
                new_notify("Something went wrong")
                client.color_log(menu_r, menu_g, menu_b, "Script Leaks ~\0")
            ...
            if decoded_data.content then
                ...
                a.http_req().get("http://cloudpusy.ct8.pl/loader/list_configs.php?lua=" .. selected_item,
                    function(success, response)
                        if not success or response.status ~= 200 then
                            return
                        end
                        ...
                        if decoded_data.configs then
                            ...
                            local button_export = ui.new_button("lua", "b", "Export to clipboard", function()
                                ...
                                if selected_config then
                                    a.http_req().get("http://cloudpusy.ct8.pl/loader/get_config.php?lua=" .. selected_item .. "&config=" .. selected_config,
                                        function(success, response)
                                            if not success or response.status ~= 200 then
                                                new_notify("Something went wrong")
                                                client.color_log(menu_r, menu_g, menu_b, "Script Leaks ~\0")
