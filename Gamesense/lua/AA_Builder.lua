...
local CONDITIONS = {"Always", "Not moving", "Moving", "Slow motion", "On ground", "In air", "On peek", --[["On shot",]] "Breaking LC", "Vulnerable", "Crouching", "Ducking", "Unducking", "Not crouching",  "Height advantage", "Height neutral", "Height disadvantage", "Knifeable", "Zeusable", "Doubletapping", "Defensive", "Terrorist", "Counter terrorist", "Dormant", "No enemies", "Pre round", "Round end"}
local DESCRIPTIONS = {
    ["Always"] = "Always true.",
    ...
    --["On shot"] = "True while you are shooting.",
    ["Breaking LC"] = "True when you are breaking lagcomp with fakelag.",
    ["Vulnerable"] = "True when enemies can shoot you.",
    ["Crouched"] = "True when you are crouching and not fake ducking.",
    ["Ducking"] = "True while you are mid crouch.",
    ["Unducking"] = "True while you are uncrouching.",
    ...
    ["No enemies"] = "True when there are no living enemies.",
    ["Pre round"] = "True before you can move at the beginning of a round.",
    ["Round end"] = "True when the round is over.",
}
...
local last_duck_amount = 0
local on_ground_ticks = 0
local no_choke_ticks = 0
...
local menu = {
    ...
    choke_modifier = ui_new_combobox("AA", "Anti-aimbot angles", "Choke modifier", {"None", "No choke", "Always choke"}),
    ...
    show_active_block_color = ui_new_color_picker("AA", "Other", "Active block color", 150, 200, 25, 225),
    show_all_blocks = ui_new_checkbox("AA", "Other", "Display all blocks"),
    show_all_blocks_color = ui_new_color_picker("AA", "Other", "Other block color", 200, 200, 200, 100),
    quick_preset_change = ui_new_checkbox("AA", "Other", "Quick preset change"),
...
do
        ...
        for k,v in pairs(base) do
            ...
            if type(tab[k]) == "table" and #tab[k] == 0 then -- Tables with 0 length are either empty or key:pair tables, either way it works
                for set, val in pairs(tab[k]) do
                    if base[k][set] ~= nil and type(base[k][set]) == type(val) then
                        base[k][set] = val
                    end
                end
            elseif base[k] ~= nil and type(base[k]) == type(tab[k]) then
                base[k] = tab[k]
            end
        end

        base:validate_conditions()

        return base
    end

    -- Removes all conditions that do not exist
    function Block:validate_conditions()
        for i = #self.conditions, 1, -1 do
            local cond = self.conditions[i]
            if not includes(CONDITIONS, cond) and not includes(custom_conditions, cond) then
    ...
    function Block:update()
        ...
        self.choke_modifier = ui_get(menu.choke_modifier)
    ...
    function Block:set_antiaim(cmd)
        ...
        if self.choke_modifier == "No choke" then
            cmd.no_choke = 1
        elseif self.choke_modifier == "Always choke" then
    ...
    function Block:is_enabled()
        return self.enabled
    end

    -- Prints a json representation of the block to the console
    function Block:print()
        print(json_stringify(self))
    end
...
local function update_browser()
    ...
    for i,v in ipairs(blocks) do
        local fatal = fatal_block == v

        display[#display+1] = v.enabled and string_format("%s[%i] %s%s", fatal and RED or LIGHTGRAY, num, WHITE, v.name) or string_format("%s[  ] %s", GRAY, v.name)
...
local function update_values()
    ...
    ui_set(menu.choke_modifier, current_block.choke_modifier or "None")
...
local function update_visibility(s)
    ...
    set_table_visibility(screen == 1, menu.name_label, menu.name, menu.pitch, menu.yaw_base, menu.yaw, menu.body,
        menu.edge_yaw, menu.freestanding, menu.freestanding_key, menu.roll, menu.force_defensive, menu.choke_modifier, menu.next)
...
local function get_conditions(cmd, local_player)
    ...
    local duck_diff = duck_amount - last_duck_amount
    last_duck_amount = duck_amount
    ...
    local conds = {
        ...
        ["Dormant"] = #enemies == 0,
        ["No enemies"] = get_total_enemies() == 0,
        ["Pre round"] = (entity_get_prop(game_rules, "m_fRoundStartTime") - curtime) > 0,
        ["Round end"] = entity_get_prop(game_rules, "m_iRoundWinStatus") ~= 0
    }
...
local function run_antiaim(cmd, local_conditions)
    ...
    ui_set(references.freestanding_key, "Always on")

    local quick_change = ui_get(menu.quick_preset_change)
    ...
    if screen == 1 and ui_is_menu_open() then
        ...
        for i,block in ipairs(blocks) do
            if (block:conditions_met(local_conditions) or i == #blocks) and block.enabled then
                if active_block ~= block and quick_change then
                    no_choke_ticks = 2
                end

                if no_choke_ticks > 0 then
                    no_choke_ticks = no_choke_ticks - 1
                    cmd.no_choke = 1
                end
...
-- Calls when a player joins the game
-- There is only here to reset the defensive counter when the user joins a game
--- @param e userdata player_connect_full event data
...
local function on_paint()
    ...
    local show_all = ui_get(menu.show_all_blocks)
    local inactive_color = {ui_get(menu.show_all_blocks_color)}
    local active_color = {ui_get(menu.show_active_block_color)}
    local dpi = tonumber(ui_get(references.dpi_scale):match("(%d+)%%")) * 0.01
    local base_y = sy - 350 * dpi
    local offset = 0

    for i = #blocks, 1, -1 do
        local is_active = blocks[i]:is_active()

        if not blocks[i]:is_enabled() and not is_active then
            goto continue
        end

        if not is_active and not show_all then
            goto continue
        end

        local r, g, b, a = unpack(is_active and active_color or inactive_color)
        local y = base_y - offset
        local tw, th = renderer_measure_text("rd+", blocks[i].name)
        local grad_h = math_ceil(th * 0.1)

        --if tw % 2 == 1 then
        --    tw = tw + 1
        --end

        tw = tw + tw % 2 + 60 -- esoterik moment (part 2)

        local tw2 = renderer_measure_text("rd+", "a")
        local half = math_ceil(tw*0.5)

        renderer_gradient(sx - half, y, -half, th + grad_h, 0, 0, 0, 50, 0, 0, 0, 0, true)
        renderer_gradient(sx - half, y, half, th + grad_h, 0, 0, 0, 50, 0, 0, 0, 0, true)
        renderer_text(sx - 30, y, r, g, b, a, "rd+", 0, blocks[i].name)

        offset = offset + th + 8 + grad_h * dpi + dpi * dpi
...
local function on_init()
    ...
    ui_set_visible(menu.show_all_blocks, ui_get(menu.show_active_block))
    ui_set_visible(menu.show_all_blocks_color, ui_get(menu.show_active_block_color))
    ui_set_callback(menu.show_active_block, function(self)
        ui_set_visible(menu.show_all_blocks, ui_get(self))
        ui_set_visible(menu.show_all_blocks_color, ui_get(self))
    end)
