...
-- local var for API functions. any changes to the line below will be lost on re-generationlocal 
...
bit_band, client_camera_angles, client_color_log, client_create_interface, client_current_threat, client_eye_position, client_key_state, client_latency, client_log, client_random_int, client_register_esp_flag, client_scale_damage, client_screen_size, client_set_clan_tag, client_set_cvar, client_set_event_callback, client_trace_bullet, client_trace_line, client_userid_to_entindex, entity_get_all, entity_get_classname, entity_get_game_rules, entity_get_local_player, entity_get_player_resource, entity_get_player_weapon, entity_get_players, entity_get_prop, entity_hitbox_position, entity_is_alive, entity_is_enemy, entity_set_prop, globals_chokedcommands, globals_curtime, globals_framecount, globals_frametime, globals_maxplayers, globals_tickcount, globals_tickinterval, math_abs, math_atan, math_atan2, math_ceil, math_cos, math_deg, math_floor, math_max, math_min, math_rad, math_sin, math_sqrt, panorama_loadstring, renderer_circle, renderer_circle_outline, renderer_gradient, renderer_measure_text, renderer_rectangle, renderer_text, renderer_triangle, string_char, string_gmatch, string_gsub, table_insert, ui_get, ui_is_menu_open, ui_mouse_position, ui_new_slider, ui_reference, ui_set, ui_set_callback, ui_set_visible, toticks, setmetatable, string_sub, table_remove, ui_new_button, ui_new_checkbox, ui_new_color_picker, ui_new_combobox, ui_new_hotkey, ui_new_label, ui_new_multiselect, ipairs = bit.band, client.camera_angles, client.color_log, client.create_interface, client.current_threat, client.eye_position, client.key_state, client.latency, client.log, client.random_int, client.register_esp_flag, client.scale_damage, client.screen_size, client.set_clan_tag, client.set_cvar, client.set_event_callback, client.trace_bullet, client.trace_line, client.userid_to_entindex, entity.get_all, entity.get_classname, entity.get_game_rules, entity.get_local_player, entity.get_player_resource, entity.get_player_weapon, entity.get_players, entity.get_prop, entity.hitbox_position, entity.is_alive, entity.is_enemy, entity.set_prop, globals.chokedcommands, globals.curtime, globals.framecount, globals.frametime, globals.maxplayers, globals.tickcount, globals.tickinterval, math.abs, math.atan, math.atan2, math.ceil, math.cos, math.deg, math.floor, math.max, math.min, math.rad, math.sin, math.sqrt, panorama.loadstring, renderer.circle, renderer.circle_outline, renderer.gradient, renderer.measure_text, renderer.rectangle, renderer.text, renderer.triangle, string.char, string.gmatch, string.gsub, table.insert, ui.get, ui.is_menu_open, ui.mouse_position, ui.new_slider, ui.reference, ui.set, ui.set_callback, ui.set_visible, toticks, setmetatable, string.sub, table.remove, ui.new_button, ui.new_checkbox, ui.new_color_picker, ui.new_combobox, ui.new_hotkey, ui.new_label, ui.new_multiselect, ipairs
    ...
    gui_handler = {}
    gui_handler.export = {["number"] = {}, ["boolean"] = {}, ["table"] = {}, ["string"] = {}}
    gui_handler.call = {}
    gui_handler.export_config = function()
        local a = function(b)
            b = ui_get(b)
            local c = ""
            for d = 1, #b do
        ...
        local c = ""
        for d, e in pairs(gui_handler.export["number"]) do
            c = c .. tostring(ui_get(e)) .. "|"
        end
        for d, e in pairs(gui_handler.export["string"]) do
            c = c .. ui_get(e) .. "|"
        end
        for d, e in pairs(gui_handler.export["boolean"]) do
            c = c .. tostring(ui_get(e)) .. "|"
        end
        for d, e in pairs(gui_handler.export["table"]) do
            c = c .. a(e) .. "|"
        end
        clipboard.set(base64.encode(c, "base64"))
    end
    gui_handler.load_config = function()
        local f = function(g, h)
            local i = {}
            for c in string.gmatch(g, "([^" .. h .. "]+)") do
                i[#i + 1] = string.gsub(c, "\n", "")
            end
            return i
        end
        ...
        local k = f(base64.decode(clipboard.get(), "base64"), "|")
        local l = 1
        for d, e in pairs(gui_handler.export["number"]) do
            ui_set(e, tonumber(k[l]))
            l = l + 1
        end
        for d, e in pairs(gui_handler.export["string"]) do
            ui_set(e, k[l])
    ...
    gui_handler.load_configdf = function()
        local f = function(g, h)
            local i = {}
            for c in string.gmatch(g, "([^" .. h .. "]+)") do
                i[#i + 1] = string.gsub(c, "\n", "")
        ...
        local k = f(base64.decode('', "base64"), "|")
        local l = 1
        for d, e in pairs(gui_handler.export["number"]) do
            ui_set(e, tonumber(k[l]))
            l = l + 1
    ...
    gui_handler.update_callbacks = function()
        for m, n in pairs(gui_handler.call) do
            if n ~= nil then
                ui_set_visible(n.ref, n.conditon())
            end
        end
    end
    gui_handler.new = function(n, o, ...)
        if type(n) ~= "number" then
            print("unable to create new register,register should be a number")
        end
        if type(n) == "number" then
            table_insert(gui_handler.export[type(ui_get(n))], n)
        end
        if o == nil then
            o = function()
                return true
        ...
        if gui_handler.call[n] == nil then
            gui_handler.call[n] = {}
        end
        gui_handler.call[n] = {ref = n, conditon = function()
                return o
            end}
        gui_handler.update_callbacks()
        ui_set_callback(n, gui_handler.update_callbacks)
        return n
    end
    ...
    local ref = {
        ...
        doubletapfl = ui.reference('RAGE', 'Aimbot', 'Double tap fake lag limit'),
    ...
    local aa_tab = {
        ...
        binds = ui_new_multiselect(menudir[ 1 ], menudir[ 2 ], 'Binds', { 'Legit anti-aimbot', 'Edge yaw','Roll anti-aim', 'Manual anti-aim','Defensive flick' }),
        ...
        force_flick = ui_new_hotkey(menudir[ 1 ], menudir[ 2 ], 'Defensive flick', false),
    ...
    local misc_tab = {
        ...
        anti_backstab_distance = ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Anti backstab Distance', 50, 400, 324, true, '', 1, true),
        force_def_air = ui_new_checkbox(menudir[ 1 ], menudir[ 2 ], 'BreakBT In Air [Defensive]'),
    ...
    for i = 1, #var.override_conditions do
        preset_change[ i ] = {
            enabled = gui_handler.new(ui_new_checkbox(menudir[ 1 ], menudir[ 2 ], 'Enable preset [' .. var.override_conditions[ i ] .. ']')),
            pitch = gui_handler.new(ui_new_combobox(menudir[ 1 ], menudir[ 2 ], 'Pitch\n' .. var.override_conditions[ i ], { 'Off', 'Default', 'Up', 'Down', 'Minimal','Custom','CustomLR','Spin', 'Random', '3way','L/R[Tick]' })),
            custompitch = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'custom pitch [' .. var.override_conditions[ i ] .. ']', -89, 89, 0)),

            pitch_left = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Left Pitch add [' .. var.override_conditions[ i ] .. ']', -89, 89, 0)),
            pitch_right = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Right Pitch add [' .. var.override_conditions[ i ] .. ']', -89, 89, 0)),

            yaw_1 = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Yaw add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),

            yaw_left = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Left Yaw add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),
            yaw_right = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Right Yaw add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),
            yaw_jitter = gui_handler.new(ui_new_combobox(menudir[ 1 ], menudir[ 2 ], 'Yaw jitter [' .. var.override_conditions[ i ] .. ']', { 'Off', 'Offset', 'Center', 'Random', 'Skitter', 'CustomLR', '3way' })),
            yaw_jitter_add = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Yaw jitter add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),

            yaw_jitter_add_1 = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Yaw jitter add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),

            yaw_jitter_left = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Left Yaw jitter add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),
            yaw_jitter_right = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Right Yaw jitter add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),

            freestand_body_yaw = gui_handler.new(ui_new_combobox(menudir[ 1 ], menudir[ 2 ], 'Freestand body yaw [' .. var.override_conditions[ i ] .. ']', { 'Gamesense', 'Freestanding', 'Reversed Freestanding', 'Jitter', 'Delay body', 'Random body' })),--[[, 'Smart']]
            body_yaw = gui_handler.new(ui_new_combobox(menudir[ 1 ], menudir[ 2 ], 'Body yaw [' .. var.override_conditions[ i ] .. ']', { 'Off', 'Opposite', 'Jitter', 'Static' })),
            body_yaw_left = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Right Body yaw add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),
            body_yaw_right = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Left Body yaw add [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),

            defensive_brute = gui_handler.new(ui_new_multiselect(menudir[ 1 ], menudir[ 2 ], 'Defensive break [' .. var.override_conditions[ i ] .. ']', {'Yaw', 'Pitch'})),
            defensive_brute_tick = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Break Tick [' .. var.override_conditions[ i ] .. ']', 2, 12, 12)),
            defensive_brute_yaw = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Yaw break [' .. var.override_conditions[ i ] .. ']', -180, 180, 0)),
            defensive_brute_pitch = gui_handler.new(ui_new_slider(menudir[ 1 ], menudir[ 2 ], 'Pitch break [' .. var.override_conditions[ i ] .. ']', -89, 89, 0)),

            freestanding_body_yaw = gui_handler.new(ui_new_checkbox(menudir[ 1 ], menudir[ 2 ], 'Freestanding body yaw [' .. var.override_conditions[ i ] .. ']')),
        }
    end
    ---
    ...
    local load_def = ui_new_button(menudir[ 1 ], menudir[ 2 ], 'Load default settings', function()
        ...
        ui_set(aa_tab.binds, { 'Legit anti-aimbot','Manual anti-aim'})
    ...
    local function handle_menu()
        ...
        ui_set_visible(aa_tab.force_flick, master_switch and contains(aa_tab.binds, 'Defensive flick') and activetab == 'Anti-Aim')
        ...
        ui_set_visible(misc_tab.force_def_air, master_switch and activetab == 'Misc')
        ...
        for i = 1, #var.override_conditions do
            ...
            ui_set_visible(preset_change[ i ].defensive_brute_tick, show_conditions and (contains(preset_change[ i ].defensive_brute, 'Yaw') or contains(preset_change[ i ].defensive_brute, 'Pitch')))
    ...
    local function player_condition_main(e)
        ...
        local forceflick = ui_get(aa_tab.force_flick) and contains(aa_tab.binds, 'Defensive flick')
    
        if legit_aa then
            var.player_state = 8
            var.antiaim_state = 'Legitaa'
        elseif forceflick and ui_get(ref.doubletap[2]) and (tbl.defensive > 2 and tbl.defensive < 14) then
            var.player_state = 10
    ...
    local function handle_main_antiaim(e)
        ...
        if not preset_changer_enabled then return end
        ...
        elseif globals_chokedcommands() == 0 and var.changer_state ~= 0 and preset_changer_enabled and (ui_get(preset_change[ var.changer_state ].pitch) == '3way') and var.aa_dir == 0 then
            ...
            ui_set(ref.yaw[2], var.player_state == 8 and -180 or var.preset_yaw_value)
        elseif var.changer_state ~= 0 and preset_changer_enabled and (ui_get(preset_change[ var.changer_state ].pitch) == 'L/R[Tick]') and var.aa_dir == 0 then
            if e.chokedcommands ~= 0 then
                if tickbase > var.yaw_tick + 2 then
                    var.yaw_choke = not var.yaw_choke
                    var.yaw_tick = tickbase
                end
            end
            var.preset_yaw_value = (var.yaw_choke and ui_get(preset_change[ var.changer_state ].yaw_left) or ui_get(preset_change[ var.changer_state ].yaw_right))
            jittervalue = lp_bodyyaw > 0 and ui_get(preset_change[ var.changer_state ].yaw_jitter_left) or ui_get(preset_change[ var.changer_state ].yaw_jitter_right)
            if e.chokedcommands == 0 and var.changer_state ~= 0 and preset_changer_enabled and (ui_get(preset_change[ var.changer_state ].pitch) == 'L/R[Tick]') and var.aa_dir == 0 then
                ui_set(ref.yaw[2], var.player_state == 8 and -180 or var.preset_yaw_value)
            end
        elseif globals_chokedcommands() == 0 and var.changer_state ~= 0 and preset_changer_enabled  and var.aa_dir == 0 then
        ...
        if var.changer_state ~= 0 and preset_changer_enabled and var.aa_dir == 0 then
            ...
            elseif ui_get(preset_change[ var.changer_state ].pitch) == 'L/R[Tick]' then
                ui_set(ref.pitch[1], 'Minimal')
                ui_set(ref.yaw[1], '180')
            else
            ...
            if var.changer_state ~= 0 and preset_changer_enabled and (ui_get(preset_change[ var.changer_state ].pitch) == 'L/R[Tick]') and var.aa_dir == 0 then
                var.preset_bodyyaw_value = ui_get(ref.yaw[2]) > 0 and ui_get(preset_change[ var.changer_state ].body_yaw_left) or ui_get(preset_change[ var.changer_state ].body_yaw_right)
            else
    ...
    local function handle_indicators()
        ...
        renderer.text(x/45,y/1.2,255,255,255,255,'-',0,'ETHEREAL[DEBUG] - V3 PANEL')
        renderer.text(x/45,y/1.2 + 8,255,255,255,255,'-',0,'LAST UPDATE 19/3/2023')
    ...
    client_set_event_callback('setup_command', function(cmd)
        local onground = on_ground(entity_get_local_player()) and cmd.in_jump == 0
        if not onground and ui_get(misc_tab.force_def_air) then
            cmd.force_defensive = var.update_dt + 0.1 < globals.tickcount()
            var.update_dt = globals.tickcount()
        end
