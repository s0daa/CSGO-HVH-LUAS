...
local data = {
    lua_name = "luno",
    lua_build = "nightly",
    user_name = string.format("%s. \r%s", panorama.open("CSGOHud").MyPersonaAPI.GetName(), "owned by w.owned & blyazakroyrot"), -- making our shit our
    upd = "18.01.2025"
}
local config
...
local ui_elements = {
    ...
    main = {
        ...
        ds = ui.button("AA", "Other", "discord: w.owned", function() panorama.loadstring(panorama.open("CSGOHud").SteamOverlayAPI.OpenExternalBrowserURL("https://t.me/ftlua")) end), -- "discord: 1ne.moloko -> discord: w.owned" and added button that leads to #rare.star tgc
    ...
    visuals = {
        elements = ui_multiselect("AA", "Anti-aimbot angles", "Elements", {"Watermark", "Hit Logs", "Indicators", "Velocity Warning", "Manual arrows"}),
...
if not ui_elements.main_check.value or not contains(ui_elements.visuals.elements.value, "Indicators") or not entity_is_alive(ent) then return end
     ...
     if not check then
        if v.alpha > 0 then 
            v.alpha = main_funcs:lerp(v.alpha, -0.1, 6 * globals_frametime())
