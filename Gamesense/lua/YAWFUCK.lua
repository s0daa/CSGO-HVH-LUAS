...
-- Data to fetch
local obex_data = obex_fetch and obex_fetch() or {username = '', build = 'PRIV'}
...
_G.YAWFUCK_push=(function()
	_G.YAWFUCK_notify_cache={}
	...
	function a:register_callback()
		...
		client.set_event_callback("paint_ui",function()
			...
			local d={0,0,0}
			local e=1;
			local f=_G.YAWFUCK_notify_cache;
			for g=#f,1,-1 do
				_G.YAWFUCK_notify_cache[g].time=_G.YAWFUCK_notify_cache[g].time-globals.frametime()
				local h,i=255,0;
				local i2 = 0;
				local lerpy = 150;
				local lerp_circ = 0.5;
				local j=f[g]
				if j.time<0 then
					table.remove(_G.YAWFUCK_notify_cache,g)
				else
			...
			local l={ui.get(b)}
			local m={math.floor(renderer.measure_text(nil,"[YAWFUCK]  "..j.draw)*1.03)}
			local n={renderer.measure_text(nil,"[YAWFUCK]  ")}
			...
			renderer.line(p[1]-1,p[2]-21,p[1]-149+m[1]+lerpy,p[2]-21,c1,c2,c3,h>255 and 255 or h)
			renderer.text(p[1]+m[1]/2-o[1]/2,p[2] - 9,c1,c2,c3,h,"c",nil,"[YAWFUCK]  ")
...
function a:paint(q,r)
	...
	for g=self.maximum_count,2,-1 do
		_G.YAWFUCK_notify_cache[g]=_G.YAWFUCK_notify_cache[g-1]
	end;
	_G.YAWFUCK_notify_cache[1]={time=s,def_time=s,draw=r}
self:register_callback()end;return a end)()

YAWFUCK_push:paint(4, "Welcome - " .. global.username ..  " to YAWFUCK.lua 2.0 Recode - build: " .. global.build)
...
local function loadDependencies()
    for i,v in pairs(dependencies) do
        if not pcall(require, v) then
            client.error_log("The " .. v .. " library is needed for the YAWFUCK.LUA to work. Make sure to subscribe it on Gamesense Workshop!")
...
local function print_debug(text)
    if global.developerMode then
        printColor("\a0a99f2[YAWFUCK | debug system] \a00ff00" .. text)
    end
end

local function prefix_print(text)
    printColor("\a0a99f2[YAWFUCK] \affffff" .. text)
end

-- Normalizing globals to YAWFUCK slang
...
local function welcomeUser()
    ...
    printColor("\affffffIts freaking good to see u using the new \a0a99f2YAWFUCK.LUA \acc7e18[" .. global.build .. "]\affffff Recode 2.0 :D")
    ...
    printColor("\a0a99f2YAWFUCK STAFF <3")
end
...
local function run_tag_animation()
    local clan_tag = gamesense_anim("YAWFUCK",
...
local function renderMenu()
    ...
    menuElements.labels["script_name"] = ui.new_label("AA", "Anti-aimbot angles", "\a0A99F2FFYAWFUCK [\acc7e18ff"
    ...
    menuElements.aa["text"] = ui.new_label("AA", "Fake lag", "\a0A99F2FF--- [ YAWFUCK | \acc7e18ff other \a0A99F2FF] ---")
    ...
    menuElements.misc["clantag"] = ui.new_checkbox("AA", "Anti-aimbot angles", "YAWFUCK clantag")
    ...
    menuElements.config["export"] = ui.new_button("AA", "Anti-aimbot angles", "export config to clipboard", function()
        if not pcall(exportConfig) then 
            prefix_print("\aff0000failed to export config. make ticket on YAWFUCK discord to help in fixing it.") 
        else
    ...
    menuElements.config["default"] = ui.new_button("AA", "Anti-aimbot angles", "load default settings (experimental)", function()
            if not pcall(importConfig, true) then
                prefix_print("\aff0000failed to import default config. make ticket on YAWFUCK discord to help in fixing it.") 
            else
...
local function onDraw()
    ...
    local YAWFUCK_w, YAWFUCK_h = renderer.measure_text("-", "YAWFUCK")
    ...
    if watermarkType  == "gradient" then
        ...
        renderer.text(screen[1] - 62 - measurement / 4, 25, 255, 255, 255, 255, "b", nil, "YAWFUCK")
    ...
    elseif watermarkType == "old" then
        ...
        renderer.text(screen[1] - 62 - measurement / 3,25, 255, 255, 255, 255, "", 0,'YAWFUCK')
    ...
    elseif watermarkType == "newest" then
        ...
        renderer.text(screen[1] - 160, 27, 255, 255, 255, 255, "b", nil, "YAWFUCK")
    ...
    if table_contains(custom_visuals, "debug panel") then
        ...
        local buildString = ">/ YAWFUCK anti-aim technologies - "..global.username:lower().." - "..global.build:lower().." \\<"
