...
-- Obex
local obex_data = obex_fetch and obex_fetch() or {username = "beazt", build = "private", discord = "beazt"}
...
--> informative
lavender.ui.visuals.informative_visual = lavender.handlers.ui.new(ui.new_multiselect("AA", "Anti-aimbot angles", "‹\aB9BEFFFFvisuals\aCDCDCDFF› informative \aB9BEFFFFindicators", "min damage indicator", "shot log (notify)", "shot log (console)", "force yaw"), function() return lavender.ui.current_tab == "VISUALS" end)
...
client.set_event_callback("setup_command", function(cmd)
    ...
    if not end_result and (state == "air duck" or state == "air" or not landing6) and check_vel then
    ...
    for id = 1, globals.maxplayers() do
        ...
        if self.old_data.eye_angles[ent] ~= nil then
            ...
            if self.old_data.yaw_delta[ent] ~= nil then
                ...
                if math.abs(self.data.yaw_delta[ent]) > 20 or math.abs(self.old_data.yaw_delta[ent]) > 20 then

                    body_yaw = max_body_yaw
                    side =  (self.data.yaw_delta[ent] > 0) and 0 or 1
                    self.mode[ent] = "JITTER"
...
client.register_esp_flag("LAVENDER", 185, 190, 255, function(ent)
    ...
    if resolver.data.body_yaw[ent] ~= nil then
        ...
        if resolver.mode[ent] == "BRUTE" then
            ...
            if resolver.mode[ent] == "JITTER" then

                return true, "JITTER"
            
            else

                return true, tostring(math.ceil(resolver.data.body_yaw[ent])) .. " : " .. resolver.mode[ent]
