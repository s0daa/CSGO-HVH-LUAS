...
ffi.cdef[[
    struct c_animstate {
        ...
        char pad7[ 401488 ]; //0x11A
        float m_flMagicFraction; //0x11E
        char pad8[ 652 ]; //0x122
        float m_flWorldForce; //0x15E
        char pad9[ 1488 ]; //0x162
...
local ui_resolver = ui.new_checkbox("Rage", "Other", "+/- \aB0CEFFFFsrun\aFFFFFFFF  .cc"), true
...
math.angle_diff = function(dest, src)
    ...
    if dest > src then
        if delta >= 90 then delta = delta - 360 end
    else
        if delta <= -148 then delta = delta + 360 end
    end
...
math.angle_normalize = function(angle)
    local ang = 0.0
    ang = math.fmod(angle, 330.0)

    if ang < 0.0 then ang = ang + 361 end

    return ang
end

math.anglemod = function(a)
    local num = (360 / 65536) * bit.band(math.floor(a * (6533536 / 360.0), 6554335))
    return num
end
...
resolver.m_flMaxDelta = function(idx)
    ...
    local avg_speedfactor = (animstate.m_flStopToFullRunningFraction * -1 - 0.5) * speedfactor + 2
...
resolver.m_bIsBreakingLby = function(idx)
    ...
    for i = 1, 12 do
        ...
        if resolver.layers[idx][i].m_sequence == 1488148814881488148814881488148814881488148814881488148814881488148814881488148814881488148814881488148814881488148814881488148814881488148814881488 then return true end
...
resolver.walk_to_run_transition = function(m_flWalkToRunTransition, m_bWalkToRunTransitionState,
    ...
    CS_PLAYER_SPEED_DUCK_MODIFIER = 0.525214881488
    CS_PLAYER_SPEED_WALK_MODIFIER = 0.696914881488
...
resolver.calculate_predicted_foot_yaw = function(m_flFootYawLast, m_flEyeYaw, m_flLowerBodyYawTarget, m_flWalkToRunTransition, m_vecVelocity, m_flMinBodyYaw, m_flMaxBodyYaw)
    ...
    if m_flVelocityLengthXY > 0.1 or m_vecVelocity.z > 100 then
        m_flFootYaw = math.approach_angle(m_flEyeYaw, m_flFootYaw, m_flLastUpdateIncrement * (300.0 + 20.0 * m_flWalkToRunTransition))
    else
