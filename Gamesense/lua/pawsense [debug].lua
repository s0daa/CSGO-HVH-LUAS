...
local json = require("json")
                ...
                local other_group = pui.group("aa", "Fake lag")
                local config_group = pui.group("aa", "other")
                pui.accent = "9FCA2BFF"

                local antiaim_cond = { '\vGlobal\r', '\vStand\r', '\vWalking\r', '\vRunning\r' , '\vAir\r', '\vAir+\r', '\vDuck\r', '\vDuck Move\r' }
                local short_cond = { '\vS ·\r', '\vS ·\r', '\vSW ·\r', '\vR ·\r' ,'\vA ·\r', '\vAC ·\r', '\vD ·\r', '\vDM ·\r' }
                ...
                local lua_menu = {
                    main = {
                        enable = lua_group:label("pawsense"),
                        tab = lua_group:combobox('\v> · \rSelect Tab', {"AntiPopadayki", "Krasivaya~Xyuna", "Drugaya~Xyuna"}),
                    },
                    antiaim = {
                        tab = lua_group:combobox(" AntiPopadayki Tab", {"Main", "Builder"}),
                        addons = lua_group:multiselect(' \rTweaks', {'Warmup AA', 'AntiBackstab', 'Safe Head'}),
                        safe_head = lua_group:multiselect(' \rSafe Head', {'Air+C Knife', 'Air+C Zeus', 'High Distance'}),
                        yaw_direction = lua_group:multiselect(' \rAdditional Yaw', {'Freestanding', 'Manual'}),
                        key_freestand = lua_group:hotkey(' \rFreestanding'),
                        key_left = lua_group:hotkey(' \rManual Left'),
                        key_right = lua_group:hotkey(' \rManual Right'),
                        key_forward = lua_group:hotkey(' \rManual Forward'),
                        yaw_base = lua_group:combobox(" \rYaw Base", {"Local view", "At targets"}),
                        condition = lua_group:combobox(' \rCurrent Condition', antiaim_cond),
                    },
                    misc = {
                        cross_ind = lua_group:checkbox(" \rIndicators", {255, 255, 255}),
                        cross_ind_type = lua_group:combobox("   \rIndicator Style", {"Pixel", "Standart", "Sigma", "Krutie"}),
                        cross_color = lua_group:checkbox("   \rIndicator Color", {100, 100, 255}),
                        key_color = lua_group:checkbox("   \rKeybinds Color", {255, 255, 255}),
                        info_panel = lua_group:checkbox(" \rDebug Panel"),
                        defensive_window = lua_group:checkbox(" \rDefensive Warning", {255, 255, 255}),
                        defensive_style = lua_group:combobox("   \rDefensive Style", {"Standart", "Sigma"}),
                        velocity_window = lua_group:checkbox(" \rVelocity Warning", {255, 255, 255}),
                        velocity_style = lua_group:combobox("   \rVelocity", {"Standart", "Sigma"}),
                        fast_ladder = lua_group:checkbox(" \rFast Ladder"),
                        log = lua_group:checkbox(" \rHit/Miss Logs"),
                        log_type = lua_group:multiselect("   \rLog Output", {"Console", "Screen"}),
                        screen_type = lua_group:combobox("   \rLog Style", {"500$", "Solus"}),
                        animation = lua_group:checkbox(" \rAnimbreaker"),
                        animation_ground = lua_group:combobox("   \rGround", {"Static", "Jitter", "Randomize"}),
                        animation_value = lua_group:slider("   \rValue", 0, 10, 5),
                        animation_air = lua_group:combobox("   \rAir", {"Off", "Static", "Randomize"}),
                        third_person = lua_group:checkbox(" \rThird Person Distance"),
                        third_person_value = lua_group:slider("   \r Third Person Distance Value", 30, 200, 50),
                        aspectratio = lua_group:checkbox(" \rAspect Ratio"),
                        aspectratio_value = lua_group:slider("   \r Aspect Ratio Value", 00, 200, 133),
                        teleport = lua_group:checkbox(" \rAuto LC Break"),
                        teleport_key = lua_group:hotkey(" \rAuto LC Break", true),
                        resolver = lua_group:checkbox(" Pawsense Resolver"),
                        resolver_type = lua_group:combobox("   \r Resolver Type", {"Paw Logic", "Experimental", "Defensive"}),
                        spammers = lua_group:multiselect(" \r Lua Branded", {"Clantag", "TrashTalk"}),
                    },
                }
                ...
                for i=1, #antiaim_cond do
                    antiaim_system[i] = {
                        ...
                        yaw_type = lua_group:combobox(short_cond[i]..' Yaw Type', {"Standart", "Delay"}),
                        ...
                        defensive_type = lua_group:combobox(short_cond[i]..' Defensive Type', {'Standart', 'Builder'}),
                ...
                local aa_tab = {lua_menu.main.tab, "AntiPopadayki"}
                local misc_tab = {lua_menu.main.tab, "Drugaya~Xyuna"}
                local visual_tab = {lua_menu.main.tab, "Krasivaya~Xyuna"}
                ...
                for i=1, #antiaim_cond do
                    ...
                    local def_def = {antiaim_system[i].defensive_type, "Standart"}
                ...
                local function aa_setup(cmd)
                    ...
                    if is_defensive_active(lp) and antiaim_system[id].defensive:get() and antiaim_system[id].defensive_type:get() == "Standart" and antiaim_system[id].defensive_yaw:get() == "Spin" then
                    ...
                    if is_defensive_active(lp) and antiaim_system[id].defensive:get() and antiaim_system[id].defensive_type:get() == "Standart" then
                    ...
                    if lua_menu.antiaim.addons:get("Warmup AA") then
                        if entity.get_prop(entity.get_game_rules(), "m_bWarmupPeriod") == 1 then
                            ui.set(ref.yaw[2], math.random(-57, 90))
                            ui.set(ref.yawjitter[2], math.random(-98, 27))
                            ui.set(ref.bodyyaw[2], math.random(-85, 94))
                            ui.set(ref.pitch[1], "Custom")
                            ui.set(ref.pitch[2], math.random(-89, 89)) 
                        end
                    end
                    ...
                    if lua_menu.antiaim.addons:get("AntiBackstab") then
                        for i=1, #players do
                ...
                client.set_event_callback("bullet_impact", function(e)                  
                    ...
                    if math.abs(delta_2d) <= 60 and globals.curtime() - lastmiss > 0.015 then
                        ...
                        if lua_menu.misc.log_type:get("Screen") then
                            renderer.log(entity.get_player_name(ent).." Registered shot")
                        end
                    end
                end)
                ...
                local function ragebot_logs()
                    ...
                    for idx, data in ipairs(logs) do
                        ...
                        if lua_menu.misc.screen_type:get() == "500$" then
                ...
                local function screen_indicator()
                    local lp = entity.get_local_player()
                    if lp == nil then return end
                    local ind_size = renderer.measure_text("cb", "pawsense")
                    ...
                    if lua_menu.misc.cross_ind_type:get() == "Standart" then
                        main_font = "c-b"
                        key_font = "c"
                    elseif lua_menu.misc.cross_ind_type:get() == "Sigma" then
                        main_font = "c-b"
                        key_font = "c-b"
                    elseif lua_menu.misc.cross_ind_type:get() == "Krutie" then
                        main_font = "c"
                        key_font = "c"
                    else
                        main_font = "c-d"
                    ...
                    local new_check = lua_menu.misc.cross_ind_type:get() == "Pixel"
                    ...
                    text_fade_animation(center[1] + scoped_space, center[2] + 30, -1, {r=r1, g=g1, b=b1, a=255}, {r=r2, g=g2, b=b2, a=255}, new_check and string.upper("pawsense") or "pawsense", main_font)
                ...
                local function velocity_ind()
                    ...
                    if lua_menu.misc.velocity_style:get() == "Standart" then
                ...
                local function defensive_ind()
                    ...
                    if lua_menu.misc.defensive_style:get() == "Standart" then
                ...
                local function info_panel()
                    ...
                    text_fade_animation(20, center[2], -1, {r=200, g=200, b=200, a=255}, {r=150, g=150, b=150, a=255}, "pawsense.lua [debug]", "d")
                    local textsize = renderer.measure_text("d", "pawsense.lua [debug]")
                    renderer.text(20, center[2] + 20, 255, 255, 255, 255, "d", 0, "player: "..string.lower(showed_name))
                    renderer.text(20, center[2] + 30, 255, 255, 255, 255, "d", 0, "kondicii: "..condition.." "..math.abs(desync_amount).."°")
                    renderer.text(20, center[2] + 40, 255, 255, 255, 255, "d", 0, "piu piu: "..string.lower(name).." "..math.abs(threat_desync).."°")
                ...
                expres.handle = function(current_threat)
                    ...
                    if expres.body_yaw[current_threat][prev_simtime] ~= nil then
                        ...
                        local pitch_e = Pitch > -25 and Pitch < 59
                        local curr_side = globals.tickcount() % 4 > 1 and 1 or - 1

                        if lua_menu.misc.resolver_type:get() == "Paw Logic" then
                            local should_correct = (simtime - prev_simtime >= 4) and math.abs(max_desync) < 75 and expres.body_yaw[current_threat][prev_simtime] ~= 0
                            if should_correct then
                                local value = math.random(0, expres.body_yaw[current_threat][prev_simtime] * math.random(-1, 1)) * .45
                        ...
                        elseif lua_menu.misc.resolver_type:get() == "Experimental" then
                            if pitch_e then
                                ...
                                value_body = curr_side * (max_desync * math.random(0, 62))
                            end
                            plist.set(current_threat, 'Force body yaw', true)  
                            ...
                            if pitch_e then
                                ...
                                value_body = math.random(0, expres.body_yaw[current_threat][prev_simtime] * math.random(-3, 3)) * .24
                            end
                            plist.set(current_threat, 'Force body yaw', true)  
                            plist.set(current_threat, 'Force body yaw value', value_body) 
                        end
                ...
                local phrases = {
                    "missed shot due to ?",
                    "1. Pawsense resolver <3",
                    "U want good aa ? Write to meidexz",
                    "10 euro lua, lol",
                    "Free kill",
                    "+1 in stats",
                    "where my souls ??",
                    "XD BUY PAWSENSE",
                    "pau pau pau pau mi strilaem po hohlam",
                    "Goodnight!",
                    "９ｍｉｃｅ - ｉｍｙａ",
                    "ｙｏｕ ｈａｓ ｂｅｅｎ ｏｗｎｅｄ ｂｙ ｐａｗｓｅｎｓｅ ? Ｓａｙ ｖｉｐｅｒｒ",
                ...
                buttom_default = config_group:button("\vStandart Config", function() 
                    config.import("")
                end)

                hinigga = other_group:label("WELCOME TO \vPAWSENSE")
                newadds = other_group:label("UPDATE \vLIST \r:")
                add1 = other_group:label("+ Improved resolver [ \vbetter \r]")
                add2 = other_group:label("+ New crosshair indicators [ \vnew styles \r]")
                add3 = other_group:label("+ Added new logs [ \vnew styles \r]")
                add4 = other_group:label("+ Added better animbreaker [ \vanimations \r]")
                add5 = other_group:label("+ New warmup aa [ \vother sets \r]")
                add6 = other_group:label("+ New standart config [ \vbetter sets \r]")
                ...
                local function update_menu()
                    ...
                    label_text = string.format("\vP A W S E N S E \aFFFFFFFF| \vGS", rgba_to_hex(unpack(aA[1])), rgba_to_hex(unpack(aA[2])), rgba_to_hex(unpack(aA[3])), rgba_to_hex(unpack(aA[4])), rgba_to_hex(unpack(aA[5])), rgba_to_hex(unpack(aA[6])), rgba_to_hex(unpack(aA[7])))
                ...
                client.set_event_callback('paint', function()
                    ...
                    text_fade_animation(x_ind/2, y_ind-20, -1, {r=200, g=200, b=200, a=255}, {r=150, g=150, b=150, a=255}, "P A W S E N S E")
                end)
