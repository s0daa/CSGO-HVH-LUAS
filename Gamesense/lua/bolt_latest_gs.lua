...
-- [ build , nicks ]
    local obex_data = obex_fetch and obex_fetch() or {username = 'user', build = 'crack', discord=''}
    local userdata = {
    username = obex_data.username == nil or obex_data.username,
    build = obex_data.build ~= nil and obex_data.build:gsub("Private", "debug"):gsub("Beta", "beta"):gsub("User", "Debug")
}
...
local _ui = {
    lua = {
        enable = group:checkbox("\vBOLT"),
        tab = group:combobox(" ", "General", "Anti-aim", "Visuals", "Miscellaneous")
    },
    ...
    antiaim = {
        ...
        tweaks = group:multiselect("\a869CC3FFTweaks", "Modify fakelag on shot", "Anti-backstab", "Safe head (not work with def aa)"),
        freestanding = group:hotkey("\a869CC3FF>Freestanding"),
        edge_yaw = group:hotkey("\a869CC3FFEdge yaw"),
        manual_right = group:hotkey("\a869CC3FFManual Right"),
        manual_left = group:hotkey("\a869CC3FFManual Left"),
        cfg_export = group:button("\a869CC3FFExport anti-aim settings", function() config.export() end),
        cfg_import = group:button("\a869CC3FFImport anti-aim settings", function() config.import() end),
...
-- General
_ui.lua.enable:depend({_ui.lua.tab, "General"})
for k,v in pairs(_ui.General) do v:depend({_ui.lua.enable, true}, {_ui.lua.tab, "General"}) end
...
-- [ custom menu control ]
local General_tab_active, antiaim_tab_active, visuals_tab_active, misc_tab_active = true, false, false, false
local custom_menu_control = function()
    ...
    if client.key_state("0x01") then
        if in_bounds(x + 146 + w/2 - 330, y - 44, x + 210 + w/2 - 330, y - 20) then
            General_tab_active, antiaim_tab_active, visuals_tab_active, misc_tab_active = true, false, false, false
            ui.set(_ui.lua.tab.ref, "General")
        elseif in_bounds(x + 270 + w/2 - 330, y - 44, x + 360 + w/2 - 330, y - 20) then
            General_tab_active, antiaim_tab_active, visuals_tab_active, misc_tab_active = false, true, false, false
            ui.set(_ui.lua.tab.ref, "Anti-aim")
        elseif in_bounds(x + 421 + w/2 - 330, y - 44, x + 505 + w/2 - 330, y - 20) then
            General_tab_active, antiaim_tab_active, visuals_tab_active, misc_tab_active = false, false, true, false
            ui.set(_ui.lua.tab.ref, "Visuals")
        elseif in_bounds(x + 564 + w/2 - 330, y - 44, x + 615 + w/2 - 330, y - 20) then
            General_tab_active, antiaim_tab_active, visuals_tab_active, misc_tab_active = false, false, false, true
    ...
    animated_text(x + 5, y - 44, -7, {r=clr_r, g=clr_g, b=clr_b, a=get_menu_alpha()}, {r=clr_r, g=clr_g, b=clr_b, a=get_menu_alpha()}, "+", "a")

    renderer.text(x + 146 + w/2 - 330, y - 44, General_tab_active and clr_r or 100, General_tab_active and clr_g or 100, General_tab_active and clr_b or 100, get_menu_alpha(), "+", nil, "General")
    renderer.text(x + 270 + w/2 - 330, y - 44, antiaim_tab_active and clr_r or 100, antiaim_tab_active and clr_g or 100, antiaim_tab_active and clr_b or 100, get_menu_alpha(), "+", nil, "AA")
    renderer.text(x + 421 + w/2 - 330, y - 44, visuals_tab_active and clr_r or 100, visuals_tab_active and clr_g or 100, visuals_tab_active and clr_b or 100, get_menu_alpha(), "+", nil, "VS")
...
local renderer_notification = function()
    ...
    local position_x = _ui.General.notifications_position.value == "Upper-left" and 5 or x/2
    local position_y = _ui.General.notifications_position.value == "Upper-left" and 5 or y/2 + 150
    ...
    for i, v in ipairs(tables.notifications) do
        ...
        v.timer = v.timer + (0.19*vector(renderer.measure_text(nil, "BOLT   » "..v.string)).x*0.0115)*frametime

        if math.max(0,(renderer.measure_text(nil, "BOLT   » "..v.string) + 1 - v.timer)) > 0 then
            v.alpha = animations.math_lerp(v.alpha, 1, frametime)
        end
    
        renderer.rectangle((_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "BOLT   » "..v.string)/2) - 3, (position_y + add_y) - 3, renderer.measure_text(nil, "BOLT   » "..v.string) + 5, 23, 0,0,0,v.alpha*100)
        renderer.rectangle((_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "BOLT   » "..v.string)/2) - 1, (position_y + add_y) + 16, renderer.measure_text(nil, "BOLT   » "..v.string) + 1, 2, 0,0,0,v.alpha*255)
        renderer.rectangle((_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "BOLT   » "..v.string)/2) - 1, (position_y + add_y) + 16, math.max(0,(renderer.measure_text(nil, "BOLT   » "..v.string) + 1 - v.timer)), 2, clr_r ,clr_g ,clr_b ,v.alpha*255)

        renderer.text(_ui.General.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil,  "BOLT   » ")/2 - renderer.measure_text(nil, v.string)/2, position_y + add_y, clr_r, clr_g, clr_b, v.alpha*255, nil, 0, "BOLT  »")
        renderer.text(_ui.General.notifications_position.value == "Upper-left" and position_x + 58 or position_x - renderer.measure_text(nil, v.string)/2 + renderer.measure_text(nil, "BOLT   » ")/2, position_y + add_y, 255, 255, 255, v.alpha*255, nil, 0, v.string)

        if math.max(0,(renderer.measure_text(nil, "BOLT   » "..v.string) + 1 - v.timer)) == 0 or #tables.notifications > _ui.General.max_notifications.value then
            v.alpha = animations.math_lerp(v.alpha, 0, frametime)
        end

        if v.alpha < 0.01 or #tables.notifications > _ui.General.max_notifications.value then
            table.remove(tables.notifications, i)
        end

        add_y = math.floor(add_y + _ui.General.notifications_spacing.value * v.alpha)
    end
end
...
local antiaim_features = function(cmd)
    ...
    -- safe head
    if contains(_ui.antiaim.tweaks.value, "Safe head (not work with def aa)") then
...
local indicators = function()
    ...
    animated_text(x/2 - renderer.measure_text("b", "BOLT")/2 + anim_ind.scope*40, y/2 + 15, 7, {r=clr_r, g=clr_g, b=clr_b, a=255}, {r=50, g=50, b=50, a=255}, "b", "BOLT")
...
local watermark = function()
    ...
    animated_text(x/2 - renderer.measure_text("b", "BOLT")/2, y - 35,5, {r=clr_r, g=clr_g, b=clr_b, a=255}, {r=25, g=25, b=22, a=255}, "b", "B O L T . Y A W ")
    animated_text(x/2 - renderer.measure_text("b", "[crack]")/2, y - 22, 5, {r=clr_r, g=clr_g, b=clr_b, a=22}, {r=255, g=22, b=22, a=255}, "b", "[crack]")
end
...
local phrases = {
    kill = {
        ...
        "B O L T   Y A W    A N T I   A I M B O T   A N G L E S",
        "you think you are is good but im best 1",
        "fokin dog, get ownet by B O L T  Y A W",
...
client.set_event_callback("paint_ui", function()
    if ui.is_menu_open() then
        ...
        ui.set(_ui.General.session_text.ref, "Session time: \aC3C6FFFF"..get_elapsed_time())
    end
