...
lua.pui = {} do
    ...
    pui.macros.name_lua = '\bFFFFFF90\bA1A1A190[cstm v3 ~ Debug]'
    ...
    lua.pui.ui.search = {
        group = lua.pui.ui.group.main:combobox(' ' .. '\f<color_start> cstm v3 \f<color_tabs> / \f<color_sad>  \n ~ Group', {'Home', 'Main', 'Other'}),
    ...
    lua.pui.ui.home = {
        server = lua.pui.ui.group.main:button('\f<color_tabs> cstm server', function ()
            panorama.open().SteamOverlayAPI.OpenExternalBrowserURL('https://discord.gg/')
        end),
        antxjav = lua.pui.ui.group.main:label('\f<color_tabs> cstm v3' .. '\f<color_start> ~ ' .. '\f<color_tabs> @9essey')
    }
...
lua.animations = {} lua.render = {} do
    ...
    function draggable:indicator ()
        ...
        local antarctica = vector(render.measure_text('b', 'cstm v3'))
        local antarctica_gr = mathematic.animate_text(globals.curtime(), 'cstm v3', r2, g2, b2, 255 * lua.render.lni.center.a, r, g, b, 255 * lua.render.lni.center.a)
    ...
    function draggable:watermark()
        ...
        render.text(x + 10, y + self.height / 4, 255, 255, 255, 255 * lua.render.anims.a, nil, 0, '✨ cstm v3   9essey   Developer')
    end
    ...
    local watermark = function()
        ...
        render.text(screen.x / 2 - text_xy.x / 2, screen.y - 15, 255, 255, 255, 255 * (1 - lua.render.anims.a), '-', 0, 'cstm v3')
    end
...
lua.createmove.builder = function (cmd)
    ...
    if lua.pui.ui.antiaim.manuals:get() and lua.createmove.selected_manual == 2 and not lua.pui.ui.conditions[''].override:get() then
    ...
    elseif lua.pui.ui.antiaim.manuals:get() and lua.createmove.selected_manual == 1 and not lua.pui.ui.conditions[''].override:get() then
        if lua.helps.exploits.defensive() > 3
        and lua.helps.exploits.defensive() < 11
        then
            yaw_da = lua.helps.additions.normalize_yaw(direction_freestand * yawd_defensive_s + d_yaw)
    ...
    elseif lua.pui.ui.antiaim.manuals:get() and lua.createmove.selected_manual == 4 and not lua.pui.ui.conditions[''].override:get() then
        if lua.helps.exploits.defensive() > 3
        and lua.helps.exploits.defensive() < 11
        then
            yaw_da = lua.helps.additions.normalize_yaw(direction_freestand * yawd_defensive_s + d_yaw)
    ...
    elseif lua.pui.ui.antiaim.manuals:get() and lua.createmove.selected_manual == 3 and not lua.pui.ui.conditions[''].override:get() then
        if lua.helps.exploits.defensive() > 3
        and lua.helps.exploits.defensive() < 11
        then
            yaw_da = lua.helps.additions.normalize_yaw(direction_freestand * yawd_defensive_s + d_yaw)
