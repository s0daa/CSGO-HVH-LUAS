...
-- Data to fetch
local obex_data = obex_fetch and obex_fetch() or {username = 'Venom', build = 'Release'}
...
local configuration = {
    ...
    trashtalkTypes = {"disabled", "enabled"; },
    freestandingOptions = {"static", "jitter"},
    bfPhases = {1,2,3,4,5},
    trashtalkAll = {"Venom", "Buy VenomLua", "1", "хочешь ебать всех? t.me/venomluabest <- покупай VenomLua", "наколенинаколени by (t.me/venomluabest)", "SOSI", "упал от VenomLua"},
    trashtalkEnabled = {"Venom", "Buy VenomLua", "1", "хочешь ебать всех? t.me/venomluabest <- покупай VenomLua", "наколенинаколени by (t.me/venomluabest)", "SOSI", "упал от VenomLua"},
}
...
_G.VenomLua_push=(function()
	_G.VenomLua_notify_cache={}
	...
	function a:register_callback()
		...
		client.set_event_callback("paint_ui",function()
			...
			local d={0,0,0}
			local e=1;
			local f=_G.VenomLua_notify_cache;
			for g=#f,1,-1 do
				_G.VenomLua_notify_cache[g].time=_G.VenomLua_notify_cache[g].time-globals.frametime()
				local h,i=255,0;
				local i2 = 0;
				local lerpy = 150;
				local lerp_circ = 0.5;
				local j=f[g]
				if j.time<0 then
					table.remove(_G.VenomLua_notify_cache,g)
				else
			...
			local l={ui.get(b)}
			local m={math.floor(renderer.measure_text(nil,"[VenomLua]  "..j.draw)*1.03)}
			local n={renderer.measure_text(nil,"[VenomLua]  ")}
			...
			renderer.line(p[1]-1,p[2]-21,p[1]-149+m[1]+lerpy,p[2]-21,c1,c2,c3,h>255 and 255 or h)
			renderer.text(p[1]+m[1]/2-o[1]/2,p[2] - 9,c1,c2,c3,h,"c",nil,"[VenomLua]  ")
...
function a:paint(q,r)
	...
	for g=self.maximum_count,2,-1 do
		_G.VenomLua_notify_cache[g]=_G.VenomLua_notify_cache[g-1]
	end;
	_G.VenomLua_notify_cache[1]={time=s,def_time=s,draw=r}
self:register_callback()end;return a end)()

VenomLua_push:paint(4, "Welcome - " .. global.username ..  " to VenomLua - build: " .. global.build)
...
local function loadDependencies()
    for i,v in pairs(dependencies) do
        if not pcall(require, v) then
            client.error_log("The " .. v .. " library is needed for the VenomLua.LUA to work. Make sure to subscribe it on Gamesense Workshop!")
...
local function print_debug(text)
    if global.developerMode then
        printColor("\a0a99f2[VenomLua | Release system] \a00ff00" .. text)
    end
end

local function prefix_print(text)
    printColor("\a0a99f2[VenomLua] \affffff" .. text)
end

-- Normalizing globals to VenomLua slang
local function normalizeGlobals()
    local build = global.build
    if build == "Debug" then
        global.build = "DEV"
    elseif build == "Release" then
        global.build = "Release"
...
local function welcomeUser()
    ...
    printColor("\a0a99f2  VenomLua                                 _     _    _               ")
    printColor("\a0a99f2 VenomLua     ")
    printColor("\a0a99f2 VenomLua     ")
    printColor("\a0a99f2 VenomLua    ")
    printColor("\a0a99f2 VenomLua  ")
    printColor("\a0a99f2 VenomLua  ")
    ...
    printColor("\affffffIts venom.. VenomLua \a0a99f2VenomLua.LUA \acc7e18[" .. global.build .. "]\affffff :D")
    printColor(" ")
    printColor(" ")
    printColor("\a0a99f2VenomLua SeKS <3")
end
...
local function run_tag_animation()
    local clan_tag = gamesense_anim("VenomLua",
...
local function renderMenu()
    ...
    menuElements.labels["script_name"] = ui.new_label("AA", "Anti-aimbot angles", "\a0A99F2FFVenomLua [\acc7e18ff"
    ...
    menuElements.aa["text"] = ui.new_label("AA", "Fake lag", "\a0A99F2FF--- [ VenomLua | \acc7e18ff other \a0A99F2FF] ---")
    ...
    menuElements.misc["clantag"] = ui.new_checkbox("AA", "Anti-aimbot angles", "VenomLua clantag")
    ...
    menuElements.config["export"] = ui.new_button("AA", "Anti-aimbot angles", "export config to clipboard", function()
        if not pcall(exportConfig) then 
            prefix_print("\aff0000failed to export config. send message tg @slxzp.") 
        else
    ...
    menuElements.config["default"] = ui.new_button("AA", "Anti-aimbot angles", "load default settings (experimental)", function()
            if not pcall(importConfig, true) then
                prefix_print("\aff0000failed to import default config. send message tg @slxzp.") 
            else
...
local function onDraw()
    ...
    local VenomLua_w, VenomLua_h = renderer.measure_text("-", "VenomLua")
    ...
    if watermarkType  == "gradient" then
        ...
        renderer.text(screen[1] - 62 - measurement / 4, 25, 255, 255, 255, 255, "b", nil, "VenomLua")
    ...
    elseif watermarkType == "old" then
        ...
        renderer.text(screen[1] - 62 - measurement / 3,25, 255, 255, 255, 255, "", 0,'VenomLua')
    ...
    elseif watermarkType == "newest" then
        ...
        renderer.text(screen[1] - 160, 27, 255, 255, 255, 255, "b", nil, "VenomLua")
    ...
    if table_contains(custom_visuals, "debug panel") then
        ...
        local buildString = ">/ VenomLua anti-aim technologies - "..global.username:lower().." - "..global.build:lower().." \\<"
    ...
    if indicatorType == "old" then
        ...
        local measurement2 = calcMeas("VenomLua")
        ...
        renderer.text((screen[1] / 2) - measurement2 / 2 - buildMeas + 5, (screen[2] / 2) + 27, mr,mg,mb, 255, "-", 0,'VenomLua')
    ...
    elseif indicatorType == "modern" then
        ...
        -- Glow behind VenomLua and VenomLua text
        glow_module(x / 2 + ((VenomLua_w + 2) / 2) * scoped_fraction - VenomLua_w / 2 + 4, y / 2 + 30,
            VenomLua_w - 3, 0, 10, 0, { r3, g3, b3, 100 * math.abs(math.cos(globals.curtime() * glowspeed)) },
            { r3, g3, b3, 100 * math.abs(math.cos(globals.curtime() * glowspeed)) })
        renderer.text(x / 2 + ((VenomLua_w + 2) / 2) * scoped_fraction, y / 2 + 30, 255, 255, 255, 255,
            "-c", 0, "\a" .. rgba_to_hex(r1, g1, b1, 255 * math.abs(math.cos(globals.curtime() * merspeed))) .. "Venom",
            "\a" .. rgba_to_hex(r2, g2, b2, 255 * math.abs(math.cos(globals.curtime() * curyspeed))) .. "Lua")
...
local function VenomLuaResolverIndicator(player)
...
local function VenomLuaResolverEvent()
    local localPlayer = entity.get_local_player()
...
local function brute_impact(e)
	...
	if math.abs(dist) <= 35 and globals.realtime() - antibruteforce.lastBruteChange > 0.015 then
		if ui.get(menuElements.aa["conditional"][playerStates.state]["antibftoggle"]) then
			VenomLua_push:paint(5,"Switched anti-brute due to enemy shot at you")
            incrementBfPhase()
		end
	end
...
client.set_event_callback("net_update_end", VenomLuaResolverEvent)
client.register_esp_flag('VenomLua', 224, 29, 29, VenomLuaResolverIndicator)
...
client.set_event_callback("player_death", function(e)
    if client.userid_to_entindex(e.userid) == entity.get_local_player() then
        VenomLua_push:paint(5, "Anti-bruteforce data got reset due to player death.")
        antibruteforce.brutestate = 0
    end
...
client.set_event_callback("round_start", function()
    ...
    local me = entity.get_local_player()
    if not entity.is_alive(me) then return end
    VenomLua_push:paint(5, "Anti-bruteforce data got reset due to new round.")
end)