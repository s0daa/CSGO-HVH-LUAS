...
local info_t = obex_fetch and obex_fetch() or { username = "Hitura", build = "Beta", discord = 0 }

local _VERSION = "2.0"
local _LOCALLIB = info_t.username == "Hitura"
...
local ui_get = ui.get
local trashtalk = ui.new_checkbox("Lua", "A", "Killsay")

local phrases = {
	"Ваши причины, ваши отговорки",
	"Вас ждет та же участь, что и вас",
	"Я был таким же в начале",
	"Пришло время для конца",
	"В то время, когда слово «пирог» еще не появилось, оно называлось обязательством.",
	"Проводите больше времени с родителями и играйте меньше в игры",
	"Вы проигрываете беспорядок в игре, но это не так",
	"На экране может повлиять мое настроение Вы выиграете.",
	"Я также думал о том, чтобы иметь много денег.",
	"Сны – это единственное место, где я могу встретиться с вами.",
	"Посмотрите нашу историю чатов и узнайте, как вы меня любите.",
	"Упоминайте о вас, полные сожалений, думайте о вас, полные радости.",
	"Какой сейчас год и настоящее?",
	"Я не могу отпустить тебя в течение долгого времени после того, как меня разлучили.",
	"Вам не нужна причина, чтобы любить вас",
	"Я любил тебя, и я не жалею об этом.",
	"Начало чуда",
	"Если конец – разлука, то в чем смысл знакомства?",
	"Это не мир неправильный, а неспособное «я».",
	"Ответ на вопрос",
	"Если вы не дорожите моими словами, завтра обо мне не будет никаких известий.",
	"Я больше не могу прикрывать его сердце.",
	"У вас может не быть ничего, но вам не может не хватать смелости, чтобы начать все сначала.",
	"Подумайте почему?",
	"В чём проблема?",
	"?",
	"!",
...
local num_quotes_hs = get_table_length(phrases)
...
local function on_player_death(e)
	...
	if attacker_entindex == get_local_player() and is_enemy(victim_entindex) then
		if e.headshot then
			    local commandhs = 'say ' .. phrases[math.random(num_quotes_hs)]
