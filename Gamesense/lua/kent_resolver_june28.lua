...
function resolver_c:render()
    ...
    for i=1, #entities do
		...
		local p_hit = client.random_int(-60, 60)
		local tactu_mare = client.random_int(37, 48)
		...
		-- slowwalk
		local slowwalk = in_air(target) == false and velocity > 1.1 and self:GetPlayerMaxFeetYaw(target) >= -90
		--moving
		local moving = in_air(target) == false and velocity > 1.1 and self:GetPlayerMaxFeetYaw(target) <= 180
		-- air
