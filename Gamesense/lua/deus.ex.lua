...
local H = 'lua>tab a>'
local v = 'lua>tab b>'
local l = gui.add_listbox('deus.ex', H, 7, true, { '                  ~~  Home  ~~         ', '                ~~  Rage Bot  ~~         ',  '                 ~~ Anti Aim ~~          ', '                     ~~  UI  ~~         ', '                    ~~  Misc  ~~         ', '                ~~ Lua Config ~~          ', '                                          ','             ~~ Made by xLeo. ~~          '})
...
local F = gui.add_button('nHub Discord', v, function()
    utils.print_console('\n[deus.ex] ', M:get_color())
    utils.print_console('https://discord.gg/38Ww3kqWeM. Copied link to clipboard.', render.color('#ffffff'))
    k.set('https://discord.gg/38Ww3kqWeM')
end)
--local O = gui.add_button('deus.ex Coder', v, function()
--    utils.print_console('\n[deus.ex] ', M:get_color())
...
function checkinghome()
    ...
    gui.set_visible(v .. 'nHub Discord', k == 0)
--    gui.set_visible(v .. 'deus.ex Coder', k == 0)
end
...
local WU = gui.add_combo('Indicators Style', v, { 'deus.ex (Old)', 'deus.ex (New)' })
...
function watermark()
    if LU:get_bool() then
        local k, j = render.get_text_size(AU, 'deus.ex')
        local s, c = render.get_text_size(AU, ' | ' .. (U:get_string() .. ' | debug build'))
        local N, m = render.get_text_size(AU, 'deuse.ex | ' .. (U:get_string() .. ' | made for destroy'))
        local D = 255
        ...
        render.text(AU, PU / 2 - N / 2, dU - 14, 'deus.ex', render.color((M:get_color()).r, (M:get_color()).g, (M:get_color()).b, 255))
        render.text(AU, (PU / 2 - N / 2) + k, dU - 14, ' | ' .. (U:get_string() .. ' | made for destroy'), render.color(98, 98, 98, 255))
    end
end
...
{ text = 'Roll', path = 'rage>anti-aim>desync>ensure lean' }, { text = 'DA', path = 'rage>aimbot>aimbot>Target dormant' }, { text = 'AX', path = 'rage>aimbot>aimbot>Anti-exploit' }, { text = 'DEUS', path = 'lua>tab b>enabled aa' } }
local VU = function()
    local k = entities.get_entity(engine.get_local_player())
...
function gui_controller()
    ...
    local v, l = render.get_text_size(j, ' [deus.ex]')
    local Y = BU:get_int()
    ...
    if XU:get_bool() then
        ...
        render.text(j, 52, T / 2 + 43, 'deus.ex', render.color(255, 255, 255, 255))
        render.text(j, 52, T / 2 + 56, 'user: ' .. (s .. ''), render.color(255, 255, 255, 255))
        render.text(j, 52 + D, T / 2 + 56, ' [deus.ex]', render.color((M:get_color()).r, (M:get_color()).g, (M:get_color()).b, F))
        if Y == 0 then
...
local DC = gui.add_combo('Clantag Type', v, { 'deus.ex', 'botlucky.com', 'primordial.dev', 'rawetrip', 'onetap.cc', 'gamesense.pub', 'EZfrags.co', 'neverlose.cc', 'ev0lve.xyz', 'pandora.gg' })
...
function indicators2func()
    ...
    if QU:get_bool() then
        ...
        local U, Y = render.get_text_size(s, 'deus.ex')
        for k = 1, 10, 1 do
        ...
        render.text(s, (m - U / 2) + v, D + 15, 'deus.ex', render.color(255, 255, 255, 255))
        if info.fatality.can_fastfire then
...
local bC = { 'Need to buy deus.ex ', 'buy deus.ex for destroy', 'Got raped dog', '˜”°• sit poor nc by deus.ex best AA World. •°”', 'nice iq', '1', 'ez owned by deus.ex', 'refund spirtware',
'get fatal with deus.ex', 'deus.ex ♛ Never Lose Again', 'I understand you have neverlose but resolver when?' , 'deus.ex after update is god mode',  'You have fatality and you did not take why you do not buy deus.ex?', 
'ez, resolver issue',}
...
local uC = 0
local EC = { 'd➚➙➙➙ ', 'de➘➙➙ ', 'deu➚➙', 'deus➘', 'deus ', 'deus.➙', 'deus.E➙➙', 'deus.Ex➙➙➙', 'deus.E➙➙', 'deus.➙', 'deus.ex', 'deus.ex', 'deus.ex|', 'deus.e|',
'deus.|', 'deus|', 'deus|', 'deu|', 'de|', 'd ', '➚', '➙➙➙ ', '➙➙➙➙' }
...
-- Made by xLéo. Thx you for buying my lua and if you crack it your gay as fuck! --
...
local path = {
    ...
    "models/player/custom_player/voikanaa/misc/bikini_girl.mdl",
...
menu.add = {
    ...
    path = gui.add_combo("Player Model Changer (deus.ex)", "lua>tab a", path),
}
