...
return (function(tbl)
    ...
    local prefix = function(x, z) 
        return (z and ("\a32a852FFORESHNIK \a698a6dFF~ \a414141FF(\ab5b5b5FF%s\a414141FF) \a89f596FF%s"):format(x, z) or ("\a32a852FFORESHNIK \a698a6dFF~ \a89f596FF%s"):format(x)) 
    end
    ...
    local category = ui.new_combobox("aa", "anti-aimbot angles", prefix("category" .. rgba(69,169,155,255," " .. (getbuild() == "OREX" and " (OREX)" or ""))), {"anti aimbot", "visuals & misc", "config"})
    local aa = {}
    local menu = {}
	local notifications = {}
    ...
    menu = {
        ["anti aimbot"] = {
            ...
            ["features"] = {
                legit = ui.new_combobox("aa", "anti-aimbot angles", prefix("legit"), {"off", "default", "ORESHNIK"}),
        ...
        ["visuals & misc"] = {
            ...
            ["visuals"] = {
                ...
                arrows = ui.new_combobox("aa", "anti-aimbot angles", prefix("arrows"), {"-", "simple", "body", "ORESHNIK"}),
        ...
        ["config"] = {
            export = ui.new_button("aa", "anti-aimbot angles", "\a89f596FF export", function()
                ...
                clipboard.export(json.stringify({["ORESHNIK"] = tbl}))
            end),
            import = ui.new_button("aa", "anti-aimbot angles", "\a32a852FF import", function()
                local check, message = pcall(function()
                    ...
                    for i, v in next, tbl["ORESHNIK"]["extra"] do
                        if i == "submenu" then
                    ...
                    tbl["ORESHNIK"]["extra"] = nil
                    for i, v in next, tbl["ORESHNIK"] do
                        for index, value in next, v do
                            for ii, vv in next, value do
                                if ii == "type" then
    ...
    for i, v in next, tbl.states do
        ...
        for index, value in next, {"ct", "t"} do
            aa[v][value] = {
                ["type"] = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "type"), (v == "global" and {"normal", "ORESHNIK", "advanced", "auto"} or {"disabled", "normal", "ORESHNIK", "advanced", "auto"})),
                ["normal"] = {
                    ...
                    method = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "method"), {"default", "ORESHNIK"}),
                    ...
                    body = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "body"), {"off", "ORESHNIK", "opposite", "static", "jitter"}),
                    ...
                    defensive = ui.new_combobox("aa", "anti-aimbot angles", prefix(v .. " " .. value, "defensive"), {"off", "always on", "ORESHNIK"})
                },
                ["ORESHNIK"] = {
                    ORESHNIK = ui.new_slider("aa", "anti-aimbot angles", prefix(v .. " " .. value, "ORESHNIK"), 1, 10, 1),
	...
	tbl.callbacks = {
        ...
        ["command"] = function(arg)
            ...
            if ui.get(menutbl["type"]) == "normal" then
                ...
                if tbl.contains(ui.get(menutbl["mode"]), "left right") then
                    ...
                    if ui.get(menutbl["method"]) == "ORESHNIK" then
                        method = arg.chokedcommands ~= 0
                    end
                    if method and ui.get(menutbl["body"]) ~= "ORESHNIK" then
                ...
                if ui.get(menutbl["body"]) ~= "ORESHNIK" then
					...
					if tbl.antiaim.ready and arg.chokedcommands == 0 then
						...
						tbl.antiaim.ORESHNIKfake = not tbl.antiaim.ORESHNIKfake
                        ui.set(tbl.items.body[2], tbl.antiaim.ORESHNIKfake and -fake or fake)
                        ORESHNIKfake = true
					end
                    if tbl.contains(ui.get(menutbl["mode"]), "left right") then
                        if ORESHNIKfake then
                            if tbl.antiaim.ORESHNIKfake then
                                ui.set(tbl.items.yaw[2], tbl.clamp(yaw + ui.get(menutbl["right"])))
                            else
                                ui.set(tbl.items.yaw[2], tbl.clamp(yaw + ui.get(menutbl["left"])))
                            end
                ...
                if ui.get(menutbl["defensive"]) == "ORESHNIK" then
            ...
            elseif ui.get(menutbl["type"]) == "ORESHNIK" then
                menutbl = menutbl[ui.get(menutbl["type"])]
                ui.set(tbl.items.jitter[1], "off")
                ui.set(tbl.items.body[1], "static")
                if arg.command_number % (ui.get(menutbl["ORESHNIK"])+1+1) == 1 then
					tbl.antiaim.ready = true
                end
				if tbl.antiaim.ready and arg.chokedcommands == 0 then
            ...
            elseif ui.get(menutbl["type"]) == "auto" then
                ...
                if ui.get(menutbl["method"]) == "ORESHNIK" then
					...
					if tbl.antiaim.fs == 0 then
						...
						local yaw = tbl.antiaim.manual.aa
						check = tbl.antiaim.ORESHNIKfake
						if tbl.antiaim.ORESHNIKfake then
							ui.set(tbl.items.yaw[2], tbl.clamp(yaw + ui.get(menutbl["right"])))
						else
							ui.set(tbl.items.yaw[2], tbl.clamp(yaw + ui.get(menutbl["left"])))
			...
			if tbl.normal_aa and tbl.reset_aa and ui.get(menu["anti aimbot"]["features"]["fixer"]) == "ORESHNIK" then
				tbl.reset_aa = false
				for i = 1, 69 do
                            ...
                            for iii, vvv in next, vv do
                                ...
                                if ii == "normal" then
                                    ...
                                    if iii == "body_slider" then
                                        fix = ui.get(vv["body"]) ~= "off" and ui.get(vv["body"]) ~= "opposite" and ui.get(vv["body"]) ~= "ORESHNIK"
                                    end
                                    if iii == "custom_slider" then
                                        fix = ui.get(vv["body"]) == "ORESHNIK"
                                    end
                                    if iii == "yaw" then
        ...
        ["arrows"] = function()
            ...
            if myself ~= nil and entity.is_alive(myself) then
                ...
                if getbuild() == "OREX" then
                    output = output .. ("\a%x%x%x%x"):format(200, 69, 69, 255) .. " [OREX] "
                end
                ...
                elseif ui.get(menu["visuals & misc"]["visuals"]["arrows"]) == "ORESHNIK" then
                    local xv, yv, zv = entity.get_prop(myself, "m_vecVelocity")
		...
		["indicator"] = function()
			...
			if ui.get(menu["visuals & misc"]["visuals"]["indicators"]) == "default" then
				...
				if yaw_body > 0 then
					renderer.text( w, h + ind_height, 255, 255, 255, 255, "cb", nil, gradient(r2, g2, b2, a2, r1, g1, b1, a1, "ORESHNIK") )
				else
					renderer.text( w, h + ind_height, 255, 255, 255, 255, "cb", nil, gradient(r1, g1, b1, a1, r2, g2, b2, a2, "ORESHNIK") )
				end
