...
angelwings.refs.aa.fake_yaw_limit = ui.reference("AA", "Anti-aimbot angles", "Fake Yaw limit")
...
local ref = {
    ...
    FBODY = ui.reference("Rage", "Other", "Force body aim"),
    SafePoint = ui.reference("Rage", "Aimbot", "Force safe point"),
    FD = ui.reference("Rage", "Other", "Duck peek assist"),
    DT = {ui.reference("Rage", "Other", "Double Tap")},
    DT_Mode = ui.reference("Rage", "Other", "Double tap mode"),
    DT_LIM = ui.reference("Rage", "Other", "Double tap fake lag limit"),
    ...
    MaxUnlag = ui.reference("MISC", "Settings", "sv_maxunlag"),
...
for k, x in pairs(angelwings.ui.builder_states) do
    ...
    angelwings.ui.builder.states[x].fake_yaw_limit = angelwings.handlers.ui.new(ui.new_slider("AA", "Anti-aimbot angles", "\aFADADDFFFake Yaw Limit \n"..tostring(x), 0, 60, 60, true, "°"), function() return ui.get(angelwings.ui.aa.master) and ui.get(angelwings.ui.tab) == "Builder" and ui.get(angelwings.ui.builder.state) == x; end, true)
...
local function sidef(l,r)
    local body_yaw = entity.get_prop(entity.get_local_player(), "m_flPoseParameter", 11) * 120 - 60
    if body_yaw < 0 then
        ui.set(angelwings.refs.aa.fake_yaw_limit, r)
    elseif body_yaw > 0 then
        ui.set(angelwings.refs.aa.fake_yaw_limit, l)
    end
...
local aa_help = {
    use_aa = function(self)
        ...
        ui.set(angelwings.refs.aa.fake_yaw_limit, 60)
    end,
...
local function custom(cmd)
    ...
    elseif state == "Roll" then
        ...
        local fakeslider = ui.get(angelwings.ui.builder.states[state].fake_yaw_limit) == 60 and 59 or ui.get(angelwings.ui.builder.states[state].fake_yaw_limit)
        if ui.get(angelwings.ui.builder.states[state].fake_custom) == "Jitter" then
            ui.set(angelwings.refs.aa.fake_yaw_limit, client_random_int(0, 1) == 1 and fakeslidercustom or fakeslider)
        elseif ui.get(angelwings.ui.builder.states[state].fake_custom) == "Directional" then
            if cmd.chokedcommands ~= 0 then
            else
                sidef(ui.get(angelwings.ui.builder.states[state].fake_slider_left),ui.get(angelwings.ui.builder.states[state].fake_slider_right))
            end
        else
            ui.set(angelwings.refs.aa.fake_yaw_limit, fakeslider)
        end
...
local function indicators()
    ...
    if ui.get(angelwings.ui.visuals.indicators) then
        ...
        local baim_alpha = ui.get(ref.FBODY) and includes(ui.get(angelwings.ui.visuals.ind_multi), "BAIM") and 255 or 0
