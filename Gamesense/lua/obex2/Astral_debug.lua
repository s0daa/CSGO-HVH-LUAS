...
for l, m in next, menu.categories do
	...
	if m == "Misc" then
		...
		gui[m] = {
			...
			animations = menu_multiselect("aa", "anti-aimbot angles", c1 .. "Animation breakers", {"Static legs", "Static legs in air", "0 Pitch on land"}),
						...
						if k == "Default" then
							if j == "Global" then
								gui[m][d][k][i] = {
									...
									jitter_yaw = menu_combobox("aa", "anti-aimbot angles", mode .. "Jitter Yaw", {"Off", "Center", "Offset", "Random"}),
									...
									limit = menu_slider("aa", "anti-aimbot angles", mode .. "Limit", 0, 60, 60),
									left_right = menu_checkbox("aa", "anti-aimbot angles", mode .. "Left And Right"),
									left = menu_slider("aa", "anti-aimbot angles", mode .. "Left", -50, 50, 0),
									right = menu_slider("aa", "anti-aimbot angles", mode .. "Right", -50, 50, 0),
								}
							else
								gui[m][d][k][i] = {
									yaw = menu_slider("aa", "anti-aimbot angles", mode ..  "Yaw", -180, 180, 0),
...
menu.aa_skeet = {
	...
	limit = menu.f.call("fake yaw limit"),
						...
						for o, k in next, menu.aa.mode do
							for x, z in next, gui[m][d][k][i] do
								...
								if str then
									...
									if gui[m][d][k][i].left_right ~= nil then
										menu_setvisible(gui[m][d][k][i].right, mode == k and active_state and menu_get(gui[m][d][k][i].left_right))
										menu_setvisible(gui[m][d][k][i].left, mode == k and active_state and menu_get(gui[m][d][k][i].left_right))
									end
...
local ticks_dt = menu_reference("MISC", "SETTINGS", "sv_maxusrcmdprocessticks")
...
aa.skeet_builder = function(self, path)
	...
	local limit				= menu_get(path.limit)
	local left_right		= menu_get(path.left_right)
	...
	if left_right then
		if desync_side then
			menu_set(menu.refs.yaw[2], left)
		elseif not desync_side then
			menu_set(menu.refs.yaw[2], right)
		end
	else
	...
	menu_set(menu.refs.pitch[1], "Down")
...
aa.handle_keybinds = function(self, cmd)
    ...
    menu_set(menu.refs.freestand[1], freestand and 'Default' or '')
...
menu.refs.fb = ui.reference("rage", "other", "force body aim")
