...
local pui = requireb(_DEBUG and "lib.pui" or "gamesense/pui", "41761")
local http = require "gamesense/http"
local adata = require "gamesense/antiaim_funcs"
local vector = require "vector"
local msgpack = requireb("gamesense/msgpack", "42280")
local inspect = _DEBUG and require "lib.inspect" or function () end
...
local color do
	...
	--
	local mt = {
		__eq = LPH_NO_VIRTUALIZE(function (a, b)
			return a.r == b.r and a.g == b.g and a.b == b.b and a.a == b.a
		end),
		lerp = LPH_NO_VIRTUALIZE(function (f, t, w)
			return create(f.r + (t.r - f.r) * w, f.g + (t.g - f.g) * w, f.b + (t.b - f.b) * w, f.a + (t.a - f.a) * w)
		end),
		to_hex = helpers.RGBtoHEX,
		alphen = LPH_NO_VIRTUALIZE(function (self, a, r)
...
local render do
	...
	local dpi_check = LPH_NO_VIRTUALIZE(function ()
		local old = DPI
		DPI = tonumber(ui.get(dpi_ref):sub(1, -2)) * 0.01
	...
	render = setmetatable({
		...
		rect_outline = LPH_NO_VIRTUALIZE(function (x, y, w, h, c, n, t)
		...
		triangle = LPH_NO_VIRTUALIZE(function (x1, y1, x2, y2, x3, y3, c)
			x1, y1, x2, y2, x3, y3 = x1 * DPI, y1 * DPI, x2 * DPI, y2 * DPI, x3 * DPI, y3 * DPI
			renderer.triangle(x1, y1, x2, y2, x3, y3, c.r, c.g, c.b, c.a * alpha)
		end),

		circle = LPH_NO_VIRTUALIZE(function (x, y, c, radius, start, percentage)
			renderer.circle(x * DPI, y * DPI, c.r, c.g, c.b, c.a * alpha, radius * DPI, start or 0, percentage or 1)
		end),
		circle_outline = LPH_NO_VIRTUALIZE(function (x, y, c, radius, start, percentage, thickness)
			renderer.circle(x * DPI, y * DPI, c.r, c.g, c.b, c.a * alpha, radius * DPI, start or 0, percentage or 1, thickness * DPI)
		end),

		screen_size = LPH_NO_VIRTUALIZE(function (raw)
			local w, h = client.screen_size()
			if raw then return w, h else return w / DPI, h / DPI end
		end),

		load_rgba = LPH_NO_VIRTUALIZE(function (c, w, h) return renderer.load_rgba(c, w, h) end),
		load_jpg = LPH_NO_VIRTUALIZE(function (c, w, h) return renderer.load_jpg(c, w, h) end),
		load_png = LPH_NO_VIRTUALIZE(function (c, w, h) return renderer.load_png(c, w, h) end),
		load_svg = LPH_NO_VIRTUALIZE(function (c, w, h) return renderer.load_svg(c, w, h) end),
		texture = LPH_NO_VIRTUALIZE(function (id, x, y, w, h, c, mode)
			if not id then return end
			renderer.texture(id, F(x * DPI), F(y * DPI), F(w * DPI), F(h * DPI), c.r, c.g, c.b, c.a * alpha, mode or "f")
		end),

		text = LPH_NO_VIRTUALIZE(function (x, y, c, flags, width, ...)
			renderer.text(x * DPI, y * DPI, c.r, c.g, c.b, c.a * alpha, (flags or "") .. "d", width or 0, ...)
		end),
		measure_text = LPH_NO_VIRTUALIZE(function (flags, text)
...
local anima do
	...
	anima = {
		...
		lerp = LPH_NO_VIRTUALIZE(function (a, b, s, t)
			local c = a + (b - a) * frametime * (s or 8) * g_speed
			return math.abs(b - c) < (t or .005) and b or c
		end),

		condition = LPH_NO_VIRTUALIZE(function (id, c, s, e)
...
my = {
	...
	my.get_simtime = LPH_NO_VIRTUALIZE(function ()
	...
	local highest_tick_base = 0

	local is_defensive_active = LPH_NO_VIRTUALIZE(function ()
		local tick_base = entity.get_prop(my.entity, "m_nTickBase") or highest_tick_base
		local diff = tick_base - highest_tick_base

		highest_tick_base = math.max(tick_base, highest_tick_base)
		my.exploit.diff = diff

		local offset = toticks(client.latency())

		return (diff < -(offset + 1) and diff > -(16 - offset))
	end)
	...
	my.get_defensive = LPH_NO_VIRTUALIZE(function ()
		if not my.valid or not my.exploit.active then return false, false, 0 end
		...
		if difference < 0 then
			latest_act = tickcount + math.abs(difference)
		end

		return (latest_act - toticks(client.latency())) > tickcount, difference < 0, difference
	end)

	local get_state = LPH_NO_VIRTUALIZE(function (cmd)
	...
	callbacks.net_update_end:set(function ()
		...
		-- my.exploit.defensive, my.exploit.lagpeek, my.exploit.diff = my.get_defensive()
		my.exploit.shifted = my.exploit.diff <= 0 or adata.get_double_tap()
		my.exploit.defensive = is_defensive_active()
	end)
	...
	callbacks.setup_command:set(function (cmd)
		...
		if cmd.chokedcommands > 1 and my.exploit.defensive then
			dechoke = true
		end
		if cmd.chokedcommands == 0 and dechoke then
			cmd.no_choke, dechoke = true, false
			debug "dechoke"
		end
...
menu.main = {
	...
	global = groups.fakelag:checkbox("\f<silent>-------------  \vhyst\reria  \f<silent>-------------"),
...
vars.misc = {
	...
	breaker = menu.feature(groups.angles:checkbox("Animation breaker"), function (parent)
		return {
			legs = groups.angles:combobox("\f<p>Legs", {"None", "Static", "No step back"}),
		}, true
	end)
}
...
vars.antiaim = {
	...
	exploits = {
		...
		vulnlc = groups.angles:multiselect("LC breaker", {"Can't shoot", "Jumping", "Crouching"}),
		snap = menu.feature(groups.angles:checkbox("Defensive snap"), function ()
			return {
				lp = groups.angles:checkbox("\f<p>Disable against low-pingers"),
...
do -- states
	...
	for i, v in ipairs(antiaim.states) do
		...
		ctx.r_rs	= new({id, "adv", "restrict"}, groups.other:combobox("Delay tick\f<_p>", {"Off", "Constant", "Triple", "Long", "Irregular"}))
...
local configs = {
	system = nil,
	default = "hysteria::GS::_",
...
antiaim.features = {
	...
	stab = {
		work = function (self)
			...
			if vars.antiaim.general.stab.value and my.threat and my.entity then
				...
				if distance < 200 and weapon_t and weapon_t.type == "knife" then antiaim.data.backstab = true return {180, -89} end
			end
		end
	},
	on_use = {
	...
	snap = {
		check = function (cmd, settings, props)
			if not (settings.on.value) or props.on == "Off" or antiaim.data.useaa or antiaim.data.backstab or adata.get_double_tap() then return false end
			if not (my.exploit.defensive or my.exploit.lagpeek) then return false end

			if settings.lp.value then
				local threat = client.current_threat()
				local resource = entity.get_player_resource(threat)
				if not resource or not threat then return false end

				local ping = entity.get_prop(resource, "m_iPing", threat)
				if not ping or ping < 15 then return false end
			end
		...
		yaw = {
			...
			["Spin"] = function (props)
				return .5 * math.lerp(-props.yaw_min, props.yaw_min, globals.curtime() * 5 % 2 - 1)
			end,
		},
		pitch = {
		...
		work = function (self, cmd, ctx)
			...
			local props = antiaim.data.preset.snap
			local allow = self.check(cmd, settings, props)
			...
			ctx.offset = math.normalize_yaw(yaw)
	...
	restrict = {
		modes = {
			["Constant"] = function ()
				return antiaim.data.lifetime % 2 == 0
			end,
			["Triple"] = function ()
				return antiaim.data.lifetime % 3 == 0
			end,
			["Long"] = function ()
				return antiaim.data.lifetime % 4 == 0
			end,
			["Irregular"] = function ()
		...
		work = function (self, cmd, scene)
			if scene.adv.restrict == "Off" or not my.exploit.active then return true end

			return self.modes[scene.adv.restrict]()
		end
	},
	fakelag = {
		overridden = false,
	...
	head = {
		work = function (self, ctx)
			local triggers = vars.antiaim.general.head
			if not my.threat then return end
			...
			-- 
			if (triggers:get "Air melee" and is_visible and my.jumping and is_melee and height_diff > -16)
			or (triggers:get "Height difference" and is_visible and height_diff > 64 and distance < 256) then
				ctx.offset, ctx.body = 20, 1
			end
		end
	},
	vulnlc = {
		check = function (self, cmd)
			...
			if settings:get("Can't shoot") then
				...
				local attack_diff, shot_diff = toticks(next_attack - globals.curtime()), toticks(next_shot - globals.curtime())
	
				local weapon_t = weapondata(my.weapon)
				local condition = (weapon_t and weapon_t.weapon_type_int ~= 9) and (cmd.quick_stop or attack_diff > 2 or shot_diff > 1)
...
antiaim.venture = {
	...
	trigger = function (event)
		...
		-- debug(math.round(closest), " / ", math.round(dists.mine))

		-- 
		if dists.mine < 40 or (closest == dists.mine and dists.mine < 128) then
...
antiaim.arrange = LPH_NO_VIRTUALIZE(function ()
...
antiaim.dispatch = LPH_NO_VIRTUALIZE(function (data)
	for k, v in next, antiaim.refs do
		v:override(data[k])
	end
end)

antiaim.manage = LPH_NO_VIRTUALIZE(function (cmd)
	antiaim.latest = {}
	...
	if not antiaim.data.manual then
...
antiaim.run = LPH_NO_VIRTUALIZE(function ()
...
misc.clantag = {
	...
	work = LPH_NO_VIRTUALIZE(function ()
	...
	run = LPH_NO_VIRTUALIZE(function (self)
...
misc.ladder = {
	work = LPH_NO_VIRTUALIZE(function (cmd)
...
misc.marker = {
	...
	marker = LPH_NO_VIRTUALIZE(function (shot, progress, ascend)
	...
	append = {
		temp = {},
		LPH_NO_VIRTUALIZE(function (shot)
			local self = misc.marker
...
misc.breaker = {
	work = LPH_NO_VIRTUALIZE(function ()
		...
		if vars.misc.breaker.legs.value == "Static" then
			refs.aa.other.legs:override("Always slide")
			entity.set_prop(my.entity, "m_flPoseParameter", 0, 0)
		elseif vars.misc.breaker.legs.value == "No step back" then
	...
	run = LPH_NO_VIRTUALIZE(function (self)
		vars.misc.breaker.on:set_callback(function (this)
			callbacks.net_update_end(this.value, self.work)
...
logger.events = {
	...
	receive = function (event, target, attacker)
		...
		local weapon, damage, hitbox = event.weapon, event.dmg_health, enums.hitgroups[event.hitgroup + 1] or "generic"
...
logger.invent = LPH_NO_VIRTUALIZE(function (event, main, add)
...
logger.push = LPH_NO_VIRTUALIZE(function (event, console, screen, chat)
...
logger.run = LPH_NO_VIRTUALIZE(function (self)
...
callbacks.player_death:set(function (event)
	if event.userid == event.attacker or client.userid_to_entindex(event.attacker) ~= my.entity then return end
...
do
	local user, cheat = hysteria.user.name, "gamesense"
	...
	ui.set_callback(menu.general.verify.auth.ref, function ()
		...
		http.get("https://backend.hysteria.one/keygen", {
			...
			local r = json.parse(response.body)
			if r.is_connected then
...
rage.teleport = {
	active = false,
	latest = 0,
	work = LPH_NO_VIRTUALIZE(function (cmd, ctx)
...
local drag do
	...
	local in_bounds = LPH_NO_VIRTUALIZE(function (x, y, xa, ya, xb, yb)
	...
	--
	local process = LPH_NO_VIRTUALIZE(function (self)
	...
	--
	drag = {
		new = LPH_NO_VIRTUALIZE(function (widget, props)
			vars.drag[widget.id] = {
...
local widget do
	local mt; mt = {
		...
		__call = LPH_NO_VIRTUALIZE(function (self)
	...
	{	 -- logo classic
		0, x = 0, function (self, x, y)
			if self[1] > 0 then
				...
				render.gradient(x + 1, y + 12, self.desync * 47, 2, colors.accent:alphen(64), colors.accent, true)
				render.logo(x, y)
			end
		...
		0, x = 0, function (self, x, y)
			...
			if self[1] > 0 then
				...
				local progress = colors.hex:sub(1, -3) .. ("%02x"):format(render.get_alpha() * 255) .. string.insert("llllll", ("\aFFFFFF%02x"):format((dt and 96 or 64) * render.get_alpha()), math.min(charge * 0.5, 6))
...
hud.watermark.items = {
	{
		0, function (self, x, y)
			...
			local t = (#cname > 0 and cname or hysteria.user.name) .. (hysteria.build ~= "stable" and (colors.hex .. " — "..hysteria.build) or "")
			local tw, th = render.measure_text("", t)
...
hud.arrows.paint = function (self, x, y, w, h)
	local neutral = colors.black:alphen(128)
	...
	for i = 1, #hud.keylist.binds do
		...
		elseif v.ref ~= nil then
			...
			if v.ref.hotkey then
				active = active and v.ref.hotkey:get()
			end
		end
...
do
	local fn = LPH_NO_VIRTUALIZE(function ()
