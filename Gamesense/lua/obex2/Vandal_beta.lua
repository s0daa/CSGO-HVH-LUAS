...
local function resolver_handler()
    ...
    for i=1, #enemies do 
        ...
        if ui.get(vandal.menu.misc.resolver_enabled) then 
            ...
            local delta = math.abs(math.floor(yaw - vandal.resolver.data[enemy].yaw.last_yaw))
            ...
            if delta ~= 0 then 
                if vandal.resolver.data[enemy].yaw.delta ~= delta then 
                    vandal.resolver.data[enemy].yaw.last_change = globals.curtime() 
                    vandal.resolver.data[enemy].yaw.delta = delta 
                end 
            end  

            local delta = globals.curtime() - vandal.resolver.data[enemy].yaw.last_change
            local tick_in_seconds = 1.0 / globals.tickinterval()

            local modifier = vandal.utils.clamp(delta / tick_in_seconds, 0, 1)

            desync = (desync * 58) -- + (vandal.resolver.data[enemy].yaw.delta * modifier)
            ...
            plist.set(enemy, "High priority", math.floor(globals.curtime() - entity.get_prop(enemy, "m_flSimulationTime")) ~= 0) -- prevent missing LC
            ...
            if vandal.resolver.data[enemy].server.last_tickcount ~= globals.tickcount() then
                vandal.resolver.data[enemy].yaw.last_yaw = yaw 
                vandal.resolver.data[enemy].server.last_tickcount = globals.tickcount()
            end    
