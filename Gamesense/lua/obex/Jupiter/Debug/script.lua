...
aa_init[0] = {
	...
	presets = ui.new_combobox("AA", "Anti-aimbot angles", "Anti-aim Presets", "Custom Jitter", "Aggressive", "#3","#4" ),
...
local manaa_inds = ui.new_checkbox("AA", "Anti-aimbot angles", "enable manual anti-aim arrows")
...
client.set_event_callback("aim_hit", function(e)
    ...
    if ui.get(logs) then
    	android_notify:paint(3, string.format('[jupiter] Hit '..name.."'s "..hgroup..' for '..e.damage..'hp, remaining: '..health..'hp [dsy: '..angle..']'))
    end
...
client.set_event_callback("aim_miss", function(e)
	...
	if ui.get(logs) then
        android_notify:paint(3, string.format('[jupiter] Missed '..name.."'s "..hgroup..' due to '..e.reason..' [dsy: '..angle..']'))
    end
...
local function set_lua_menu()
	...
	if ui.get(aa_init[0].presets) == "#4" or ui.get(aa_init[0].presets) == "#3" or ui.get(aa_init[0].presets) == "Aggressive" or ui.get(aa_init[0].presets) == "Custom Jitter" or ui.get(aa_init[0].aa_builder) and is_aa and is_enabled then
...
client.set_event_callback("setup_command", function(c)
	...
	if aa.ignore == false then
		...
		if ui.get(aa_init[0].aa_builder) then
			...
			ui.set(ref.pitch, "Minimal")
			if ui.get(aa_init[0].presets) == "#4" then
				if var.p_state == 1 then -- standing
					...
					ui.set(ref.fakeyawlimit, (math.random (20,60)))
				elseif var.p_state == 2 then --run
					...
					if c.chokedcommands ~= 0 then
						...
						ui.set(ref.yaw[2],(side == 1 and -5 or 10))
					end
					ui.set(ref.fakeyawlimit, (math.random (20,60)))
				elseif var.p_state == 3 then --slow
					ui.set(ref.yawjitter[1], "Center")
					ui.set(ref.yawjitter[2], 41)
					ui.set(ref.bodyyaw[1], "Static")
					ui.set(ref.bodyyaw[2], (math.random (-180, 180)))
					if c.chokedcommands ~= 0 then
					else
						ui.set(ref.yaw[2],(side == 1 and 10 or 10))
					end
					ui.set(ref.fakeyawlimit, (math.random (20,60)))
				elseif var.p_state == 4 then --air
					...
					if c.chokedcommands ~= 0 then
						...
						ui.set(ref.yaw[2],(side == 1 and 6 or 20))
					end
					ui.set(ref.fakeyawlimit, (math.random (20,60)))
				...
				elseif var.p_state == 6 then --duck
					...
					if c.chokedcommands ~= 0 then
						...
						ui.set(ref.yaw[2],(side == 1 and -10 or 35))
					end
					ui.set(ref.fakeyawlimit, 60)
				end
			elseif ui.get(aa_init[0].presets) == "#3" then
				if var.p_state == 1 then -- standing
					ui.set(ref.yawjitter[1], "Center")
			...
			elseif ui.get(aa_init[0].presets) == "Aggressive" then
					if var.p_state == 1 then -- standing
...
local function watermark()
		...
		-- create text
		local text = " jupiter.lua ".. string_format("[%s]", build) .. " |".. string_format(" %s", name2) .. " | " ..  string_format("%dms", latency) .. " | " .. string_format("%02d:%02d:%02d", hours, minutes, seconds)
	...
	if ui.get(watermark_tgl) then
		renderer.gradient(screen_width-text_width-margin-padding + 6, margin-padding - 6, 1     , text_height+padding*2 , mr,mg,mb, 255, mr,mg,mb, 0, false)
		renderer.gradient(screen_width-text_width-margin-padding + text_width+padding*2 + 7, margin-padding - 6, 1      , text_height+padding*2  , mr,mg,mb, 255, mr,mg,mb, 0, false)
		renderer_rectangle(screen_width-text_width-margin-padding + 7, margin-padding - 5, text_width+padding*2, text_height+padding*2, 33, 33, 33, 155)
		renderer_text(screen_width-text_width-margin + 7, margin - 5, 235, 235, 235, 255, flags, 0, text)
		renderer.gradient(screen_width-text_width-margin-padding + 7, margin-padding - 6, text_width+padding*2     , text_height+padding*2 - 19, mr,mg,mb, 255, mr,mg,mb, 255, true)
...
local function draw()
	...
	if ui.get(inds_selct) =="#1" and ui.get(crosshair_inds) then
		...
		renderer.text(x / 2-29 + testx - funny, y / 2 + 30, mr,mg,mb, 255, "-", 0, 'JUPITER')
		renderer.text(x / 2+2 + testx - funny, y / 2 + 30, 255,255,255, alpha, "-", 0, 'DEBUG')
		renderer.text(x / 2 + n2_x - testting, y / 2 + ting + 15 , 223, 223, 223, 180, flag, 0,  state)
	...
	if ui.get(inds_selct) == "#3" and ui.get(crosshair_inds) then
		...
		local desync_type2 = antiaim_funcs.get_desync(2)
		local desync_strength = math.floor(math.min(58, math.abs(entity_get_prop(entity_get_local_player(), "m_flPoseParameter", 11)*120-60)))
		renderer.text(x / 2 ,  y / 2 + 32, mr2,mg2,mb2, 255, "c-", 0, 'JUPITER   TECH')
		renderer.text(x / 2  , y2 / 2 + 41, dr2,dg2,db2, dt_a, "-c", 0, "RAPID")
		if is_os and not is_dt then
			renderer.text(x / 2  , y2 / 2 + 41, dr, dg, db, os_a, "c-", 0, "ONSHOT")
		end
		if is_dt or is_os then
			renderer.text(x / 2  , y2 / 2 + 50, 255, 255, 255, fs_a, "c-", 0, "FREESTAND")
		else 
			renderer.text(x / 2   , y2 / 2 + 41, 255, 255, 255, fs_a, "c-", 0, "FREESTAND")
			
		end
		--renderer.gradient(x / 2  - 35 + testx + testting, y / 2 + 39, desync_strength* 1.28, 3, mr2,mg2,mb2, 255, mr2,mg2,mb2, 0, true)
