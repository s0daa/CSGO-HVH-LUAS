...
local grobold = surface.create_font("Avenir  LT Std 55 Roman", 11, 500, {0x200})
...
-- menu controls created by the user
gui.menu = 
{
    ...
    enable_indicators = main:ui(ui.new_combobox("AA","Anti-aimbot angles","Enable \aB4711BFFindicators\aFFFFFFB6",{"Off","Default","Cartoony"}),true),
    ...
    LC =  main:ui(ui.new_checkbox("AA","Anti-aimbot angles","\a96C83CFFLC\aFFFFFFB6 indicator")),
    ping_spike = main:ui(ui.new_checkbox('AA', 'Anti-aimbot angles', 'Ping spike indicator'),true),
    ...
    manua_color = main:ui(ui.new_color_picker("AA","Anti-aimbot angles", 'Manual color', 255, 255, 255, 255),false),
    ...
    filled_color = main:ui(ui.new_color_picker("AA","Anti-aimbot angles", 'Filled color', 50, 50, 50, 170,false),false),
    out = ui.new_label("AA","Anti-aimbot angles","Outline color"),
    clan_tag = main:ui(ui.new_checkbox("AA","Anti-aimbot angles","Clan tag"),true),
    outline = main:ui(ui.new_color_picker("AA","Anti-aimbot angles", 'Outline color', 255, 255, 255, 255,false),false),
}

--anti-aim menu control state
gui.menu.custom = {}
gui.menu.custom[0] = {
    player_state = main:ui(ui.new_combobox("AA", "Anti-aimbot angles", "\aB4711BFFAnti-aim\aFFFFFFB6 state", var.p_states),true),
}

--anti-aim states controls
for i = 1, 7 do
    gui.menu.custom[i] = {
...
-- default menu control references
    ...
    if ui.get(gui.menu.anti_knife) ~= "Off" and ui.get(lua_enable) then
        ...
        for i = 1, #players do
            ...
            if entity.get_classname(weapon) == "CKnife" and distance <= 250 then
               ...
               -- local extrapolated_x,extrapolated_y,extrapolated_z = hitbox_x + get_velocity(players[i]) * (globals.tickinterval() * globals.tickcount()), hitbox_y + get_velocity(players[i]) * (globals.tickinterval() * globals.tickcount()), hitbox_z + get_velocity(players[i]) * (globals.tickinterval() * globals.tickcount())
...
-- manual anti-aim indicators colors
...
function indicators()
    ...
    if ui.get(gui.menu.min_dmg_indicator) then
        ...
        if ui.get(gui.menu.enable_indicators) == "Default" then
            renderer.text(X / 2 + ui.get(gui.menu.min_dmg_slider), Y / 2 - 12, 255, 255, 255, 255,"c",0,ui.get(damage))
        elseif ui.get(gui.menu.enable_indicators) == "Cartoony" then
            surface.draw_text(X / 2 + ui.get(gui.menu.min_dmg_slider), Y / 2 - 12,255,255,255,255,grobold,string.format("%s",ui.get(damage)))
             --surface.draw_text(X / 2 - 10, Y / 2 + 20 + indicators_spacing, 100, 100, 100, 255,grobold,"DT")
        else
            renderer.text(X / 2 + ui.get(gui.menu.min_dmg_slider), Y / 2 - 12, 255, 255, 255, 255,"c",0,ui.get(damage))
       end
    end
    ...
    if ui.get(gui.menu.enable_indicators) == "Default" then
        ...
        if pos_changer >= 50 then 
            update_move = true
        elseif pos_changer <= 0 then
            update_move = false
        end

        if update_move == true then
            pos_changer = pos_changer - 30 * globals.absoluteframetime()
        else
            pos_changer = pos_changer + 30 * globals.absoluteframetime()
        end

        renderer.line(X / 2 - pos_changer + 30, Y / 2 + 6, X / 2 - pos_changer + 20, Y / 2 + 6, 255,255,255,255)
        ...
        elseif ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) and not anti_aim.get_double_tap() then
            renderer.text(X / 2, Y / 2 + 20 + indicators_spacing, 100, 100, 100, 255,"-c",0,"DT")
            indicators_spacing = indicators_spacing + 8
        end
        ...
        if ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) and ui.get(menu_reference.os[2]) and ui.get(menu_reference.os[1]) then
            renderer.text(X / 2, Y / 2 + 20 + indicators_spacing, 100, 100, 100, 255,"-c",0,"HS")
        ...
        renderer.line(X / 2 + pos_changer - 28, Y / 2 + indicators_spacing + 27, X / 2 + pos_changer - 18, Y / 2 + indicators_spacing + 27, 255,255,255,255)
    elseif ui.get(gui.menu.enable_indicators) == "Cartoony" then
        if not entity.is_alive(entity.get_local_player()) then return end

        surface.draw_text(X / 2, Y / 2 + 15, 225, 198, 153, 255,grobold,lua_name)
        indicators_spacing = indicators_spacing + 12

        state_in_upper = string.upper(var.p_states[var.p_state])
        surface.draw_text(X / 2, Y / 2 + 15 + indicators_spacing, 255, 255, 255, 255,grobold,state_in_upper )
        indicators_spacing = indicators_spacing + 12

         if ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) and anti_aim.get_double_tap() then
         
             surface.draw_text(X / 2, Y / 2 + 15 + indicators_spacing, 255, 255, 255, 255,grobold,"DT")
            indicators_spacing = indicators_spacing + 12
        elseif ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) and not anti_aim.get_double_tap() then
           -- renderer.text(X / 2, Y / 2 + 20 + indicators_spacing, 100, 100, 100, 255,"-c",0,"DT")
            surface.draw_text(X / 2, Y / 2 + 15 + indicators_spacing, 100, 100, 100, 255,grobold,"DT")
            indicators_spacing = indicators_spacing + 12
        end
    
        if ui.get(menu_reference.dt[1]) and ui.get(menu_reference.dt[2]) and ui.get(menu_reference.os[2]) and ui.get(menu_reference.os[1]) then
            
             surface.draw_text(X / 2, Y / 2 + 15 + indicators_spacing, 100, 100, 100, 255,grobold,"HS")
            
            indicators_spacing = indicators_spacing + 12
        elseif ui.get(menu_reference.dt[1]) and not ui.get(menu_reference.dt[2]) and ui.get(menu_reference.os[1]) and ui.get(menu_reference.os[2]) and ui.get(menu_reference.os[1]) then
           
             surface.draw_text(X / 2, Y / 2 + 15 + indicators_spacing, 255, 255, 255, 255,grobold,"HS")
            indicators_spacing = indicators_spacing + 12
        end
    end
    ...
    if ui.get(gui.menu.avatar_esp) then  
        ...
        for i = 1, #player do
            ...
            if bounding_box[1] == nil and bounding_box[2] == nil and bounding_box[3] == nil and bounding_box[4] == nil then return end
...
-- get the nearest enemy to the crosshair
bestenemy = 0
...
function info_panel()
    ...
    if table_contains(ui.get(gui.menu.info_panel),"Local info panel") then
        ...
        if get_nearest_enemy(entity.get_local_player()) ~= nil then
            by = math.floor(math.min(60, (entity.get_prop(get_nearest_enemy(entity.get_local_player()), "m_flPoseParameter", 11) * 120 - 60)))
        else
            by = 0
        end
...
-- kill say
local cool_say = {'𝕡𝕞 𝕣𝕒𝕫#𝟝𝟠𝟡𝟚 𝕗𝕠𝕣 𝟙𝕧𝟙, 𝟚𝕧𝟚, 𝟝𝕧𝟝','namer.sellix.io solutionz','0.001839490  𝔟𝔱𝔠 𝔯𝔦𝔠𝔥𝖍','Tonka Productionz $ Luh Crank','You are the best at being dead','Lua coded by lord ｋａｙｒｏｎ','Я использую gamesense и Tonka',
...
-- resetting bruteforce values and kill say
...
function unhide_settings()
    ...
    ui.set_visible(gui.menu.clan_tag,ui.get(lua_enable) and ui.get(lua_tab) == "Misc")
    ...
    ui.set_visible(gui.menu.LC,ui.get(lua_enable) and ui.get(lua_tab) == "Indicators")
    ...
    ui.set_visible(gui.menu.ping_spike,ui.get(lua_enable) and ui.get(lua_tab) == "Indicators")
   ...
   -- print(should_animate)
    ...
    if should_animate == "should_intro" then
       -- print(should_animate .. "phase 3")
        ...
        renderer.text(X / 2, real_pos_y - 48, 255, 255, 255, real_alpha,"c",0,"Welcome to Tonka.lua")
        renderer.text(X / 2, real_pos_y - 36, 255, 255, 255, real_alpha,"c",0,string.format("User - %s",obex_data.username))
        --print(real_pos_y)

    
        if real_alpha <= 0 or text_alpha <= 0 then
            should_animate = "stop" -- stopping the intro animation
        end
    end
end

-- get if the lua has updated
...
function if_updated()
    ...
    if UPDATED_LUA then
        print("\n < TONKA.LUA UPDATE LOG > \n - Added Ping Indicator (Enemies) \n - New Indicator (Need to have a pre-installed font \n")
        UPDATED_LUA = false
    end
...
function anti_aim_set()
   ...
   -- print(desync)
    ...
    if desync > 1667266324 and round_start then
        client.exec("quit Crashed at 0x000000 @ client.dll")
    else
    end
end

-- defensive doubletap indicator
local update = 0
local sim_time = 0
to_draw = "no"
ticker = 0
...
-- breaking lag compensation indicator for the lp
breaking_lc = false
breaking_lc_all = false
prev_orig = vector(entity.get_origin(entity.get_local_player()))
new_prev_orig = vector(0,0,0)
new_sim_time = 0
table_origin = {}
table_origin[1] = vector(0,0,0)
table_origin[2] = vector(0,0,0)
size_ticks = 0
function break_lc()
    if entity.is_alive(entity.get_local_player()) then
        if ui.get(gui.menu.LC) then
        if jumping then
            -- prev_orig = origin
            --print(table_origin[2])
            table_origin[size_ticks] = prev_orig
            size_ticks = size_ticks + 1

            if size_ticks > 2 then
                size_ticks = 1
            end

             origin = vector(entity.get_origin(entity.get_local_player()))
             old_delta = (origin - table_origin[1])
             delta = (origin - table_origin[2])
     
             not_new_old_tick = new_sim_time 
             new_sim_time = entity.get_prop(entity.get_local_player(),"m_flSimulationTime")
     
             if (new_sim_time - not_new_old_tick) <= 0 then
                return
             end
              prev_orig = origin
         
     
             if delta:length2dsqr() > 4096 or old_delta:length2dsqr() > 4096 then
                 breaking_lc = true
             else
                 breaking_lc = false
             end   
...
--breaking lag compensation indicator for the enemies
function break_lc_enemies()
    enemies = entity.get_players(true)

    for i = 1, #enemies do
        players = enemies[i]

        if entity.is_alive(players) then
            if in_air(players) then
                origin = vector(entity.get_origin(players))
                delta = (origin - new_prev_orig)
        
                new_old_tick = new_sim_time 
                new_sim_time = entity.get_prop(players,"m_flSimulationTime")
        
               -- if (new_sim_time - new_old_tick) <= 0 then
                 --   return
                 --end
                 new_prev_orig = origin
            
        
                if delta:length2dsqr() > 4096 then
                    breaking_lc_all = true
                elseif delta:length2dsqr() < 1000 then
                    breaking_lc_all = false
                end   
            else 
                breaking_lc_all = false
...
-- draw them
function breaking_lc_indicator()

   -- if not ui.get(gui.menu.LC) then return end
 ...
 -- if not entity.is_alive(entity.get_local_player()) then return end
    if ui.get(gui.menu.LC) then 
        if entity.is_alive(entity.get_local_player()) then
            if breaking_lc then
                if jumping then
                        renderer.indicator(132,195,16,255,"LC")
                end
            else
                vel = vector(entity.get_prop(entity.get_local_player(),"m_vecVelocity"))
                if jumping and vel:length() > 290 then 
                    renderer.indicator(255,0,0,255,"LC")
                end
            end
        end
...
client.register_esp_flag("LC",0,255,0,function(c)
    if breaking_lc_all == true then 
        return true 
    else 
        return false 
    end
...
local function clan_tag()
    ...
    if ui.get(gui.menu.clan_tag) and to_switch == false then
        if go <= 1 then
            switch = false
        elseif go >= #clantag_array then
            switch = true
        end
    
        if switch and hello + 0.7 < globals.curtime() then
            go = go - 1
            hello = globals.curtime()
        elseif switch == false and hello + 0.7 < globals.curtime() then
            go = go + 1
            hello = globals.curtime()
        end

        client.set_clan_tag(clantag_array[go])
        to_switch = true
...
local colorful_text = {lerp=function(self,b,c,d)if type(b)=='table'and type(c)=='table'then return{self:lerp(b[1],c[1],d),self:lerp(b[2],c[2],d),self:lerp(b[3],c[3],d)}end;return b+(c-b)*d end,text=function(self,...)local e=false;local f=255;local g=''for h,i in ipairs({...})do if type(i)=='boolean'then e=i elseif type(i)=='number'then f=i elseif type(i)=='string'then g=g..i elseif type(i)=='table'then if type(i[1])=='table'and type(i[2])=='string'then g=g..('\a%02x%02x%02x%02x'):format(i[1][1],i[1][2],i[1][3],f)..i[2]elseif type(i[1])=='table'and type(i[2])=='table'and type(i[3])=='string'then for j=1,#i[3]do local k=self:lerp(i[1],i[2],j/#i[3])g=g..('\a%02x%02x%02x%02x'):format(k[1],k[2],k[3],f)..i[3]:sub(j,j)end end end end;return('%s\a%s%02x'):format(g,e and'cdcdcd'or'ffffff',f)end}
local set_math = function(b,c,d)local b=b>c and c or b;local e=c/b;local f=d/e;f=f>=0 and math.floor(f+0.5)or math.ceil(f-0.5)return f end
local get_color = function(b,c)local d,e,f;i=set_math(b,c,9)if i<=1 then d,e,f=255,0,0 elseif i==2 then d,e,f=237,27,3 elseif i==3 then d,e,f=235,63,6 elseif i==4 then d,e,f=229,104,8 elseif i==5 then d,e,f=228,126,10 elseif i==6 then d,e,f=220,169,16 elseif i==7 then d,e,f=213,201,19 elseif i==8 then d,e,f=176,205,10 elseif i==9 then d,e,f=124,195,13 end;return d,e,f end
local get_ping = function(player_resource, ent) return entity.get_prop(player_resource, string.format('%03d', ent)) end

client.register_esp_flag('Ping Pong', 255, 255, 255, function(ent)
    local ping = get_ping(entity.get_all('CCSPlayerResource')[1], ent)
    ...
    if ui.get(gui.menu.ping_spike) and ping >= 144 then
        return true, colorful_text:text({{r, g, b}, ('%s MS'):format(ping)})
    end
...
--callbacks
client.set_event_callback("net_update_end",break_lc_enemies)
client.set_event_callback("net_update_end",break_lc)
client.set_event_callback("paint",breaking_lc_indicator)
...
client.set_event_callback("paint_ui",intro_animation)
