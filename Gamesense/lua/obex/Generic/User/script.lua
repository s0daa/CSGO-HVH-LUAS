...
local function presets(player)
    ...
    if generic_body_yaw_g == "Shuffle" then
        ...
        if air then
            ...
            aa.fake_yaw = 60
            --aa.yaw = num2
			client.delay_call(1, ui.set, ref.yaw[2], 35)
			client.delay_call(2, ui.set, ref.yaw[2], -25)
            --print(ref.yaw[2])
            ...
            if speed < 4 then
                ...
                --print('pussy')
                --aa.yaw =  36
                client.delay_call(1, ui.set, ref.yaw[2], 36)
                client.delay_call(2, ui.set, ref.yaw[2], -22)
                aa.yaw_jitter = {"Center", 0}
            end       
        end   
    elseif generic_body_yaw_g == "Defensive" then --to add more modes
        if antibrute == false then --first shot antiaim
            ...
            aa.body_yaw = {"Jitter", shot_at_angle and -33 or 33}
        end      
...
local function smart_peek()
    if(get(generic_smart_peek)) then
        ...
        set(ref.freestanding[1], "Default")
        set(ref.freestanding[2], "On hotkey")
