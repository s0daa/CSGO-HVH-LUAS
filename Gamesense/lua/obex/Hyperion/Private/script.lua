...
download = function()
    if filecheck == true then
        local file = require("csgo/filesystem") 
         ...
         file.create_dir('hyp_test')

        
        if readfile('csgo\\hyp_test\\animate.lua') == nil then

            http.get("https://pastebin.com/raw/TvBHzfku",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
                writefile("csgo\\hyp_test\\animate.lua",r.body)
            end)
            client.color_log(100,100,255,"[Hyperion] Downloading (animate.lua)")
        elseif readfile('csgo\\hyp_test\\animate.lua') ~= nil then
            http.get("https://pastebin.com/raw/TvBHzfku",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
        ...
        if readfile('csgo\\hyp_test\\antiaim_funcs.lua') == nil then
            http.get("https://pastebin.com/raw/3g7jhujz",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end

                writefile("csgo\\hyp_test\\antiaim_funcs.lua",r.body)
            end)
            client.color_log(100,100,255,"[Hyperion] Downloading (antiaim_funcs.lua)")
        elseif readfile('csgo\\hyp_test\\antiaim_funcs.lua') ~= nil then
            http.get("https://pastebin.com/raw/3g7jhujz",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
        ...
        if readfile('csgo\\hyp_test\\color_utils.lua') == nil then
            http.get("https://pastebin.com/raw/e0EMeqw5",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
                writefile("csgo\\hyp_test\\color_utils.lua",r.body)
            end)
            client.color_log(100,100,255,"[Hyperion] Downloading (color_utils.lua)")
        elseif readfile('csgo\\hyp_test\\color_utils.lua') ~= nil then
            http.get("https://pastebin.com/raw/e0EMeqw5",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
        ...
        if readfile('csgo\\hyp_test\\entity.lua') == nil then
            http.get("https://pastebin.com/raw/TZP93L80",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
                writefile("csgo\\hyp_test\\entity.lua",r.body)
            end)
            client.color_log(100,100,255,"[Hyperion] Downloading (entity.lua)")
        elseif readfile('csgo\\hyp_test\\entity.lua') ~= nil then
            http.get("https://pastebin.com/raw/TZP93L80",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
        ...
        if readfile('csgo\\hyp_test\\gui.lua') == nil then
            http.get("https://pastebin.com/raw/Y7CvPkSh",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
                writefile("csgo\\hyp_test\\gui.lua",r.body)
            end)
            client.color_log(100,100,255,"[Hyperion] Downloading (gui.lua)")
        elseif readfile('csgo\\hyp_test\\gui.lua') ~= nil then
            http.get("https://pastebin.com/raw/Y7CvPkSh",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
        ...
        if readfile('csgo\\hyp_test\\reference.lua') == nil then
            http.get("https://pastebin.com/raw/4mKvXenF",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
                writefile("csgo\\hyp_test\\reference.lua",r.body)
            end)
            client.color_log(100,100,255,"[Hyperion] Downloading (reference.lua)")
        elseif readfile('csgo\\hyp_test\\reference.lua') ~= nil then
            http.get("https://pastebin.com/raw/4mKvXenF",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
        ...
        if readfile('csgo\\hyp_test\\render_ui.lua') == nil then
            http.get("https://pastebin.com/raw/TqLvxYp7",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
                writefile("csgo\\hyp_test\\render_ui.lua",r.body)
            end)
            client.color_log(100,100,255,"[Hyperion] Downloading (render_ui.lua)")
        elseif readfile('csgo\\hyp_test\\render_ui.lua') ~= nil then
            http.get("https://pastebin.com/raw/TqLvxYp7",function(s,r)
                if not s or r.status ~= 200 then
                    return
                end
...
local __init = function ()
    ...
    local reference = require("csgo/hyp_test/reference")
    local gui = require "csgo/hyp_test/gui"
    local color_utils = require "csgo/hyp_test/color_utils"
    local ety = require("csgo/hyp_test/entity")
    local anim = require ("csgo/hyp_test/animate")
    local rui = require ("csgo/hyp_test/render_ui")
    local afunc = require('csgo/hyp_test/antiaim_funcs')
    ...
    logs('Hyperion.lua v1.0.0 edition: dev')
    ...
    m.luop_list = gui.new(ui_new_multiselect(lua_dirA,lua_dirB,c_tp.." @options",{'key','vis.','uix'}),reg_call,exp_call)
    ...
    m.arrow_distance = gui.new(ui_new_slider(lua_dirA,lua_dirC,c_tp..' distance',10,100,15,true,'px'),reg_call,exp_call)
    ...
    m.luop_vis = gui.new(ui_new_multiselect(lua_dirA,lua_dirB,c_tp..' vis. elements',{"crosshair",'notification'}),reg_call,exp_call)
    ...
    uix.var =  {
        ...
        ol = {
            ...
            pos = {
                x = sx/2 ,y = sy/2, w = 100,glow = 0,a = 0
            }
        },
    ...
    uix.on_load = function(  )
        ...
        local vx, vy = renderer_measure_text('+',nil,"<-DEVELOPMENT  BUILD->")
        ...
        v.pos.a = anim.new(v.pos.a,100,a,v.alpha <= 0.01,3)
        ...
        renderer_text(sx/2  - math_floor( vx/2 * v.alpha) + mx/2 + 10,sy/2 - vy/2 + 30 ,255,105,105,255 * v.alpha,'-',0,"<-DEVELOPMENT  BUILD->")
    ...
    uix.top_bar = function ()
        ...
        if ui_get(m.luop_uix) == 'menu' then
            ...
            renderer_text(x  + 110 , y  + 5-  math_floor(30 * uix.var.tb.is_open),255,105,105,255 * uix.var.tb.is_open,nil,0,"Dev. edition")
        ...
        elseif ui_get(m.luop_uix) == 'screen' then
            ...
            renderer_text(rex  + 110 , rey  + 5-  30 ,255,105,105,255,nil,0,"Dev. edition")
            --ext
    ...
    uix.crosshair = function( )
        ...
        renderer_text(mx - hy_x/2 + 15+ math_floor(35 * uix.var.cr.add_x) ,my + 8 + hy_y,255,105,105, alpha *  uix.var.cr.alpha ,'-',nil,'DEV')


        local v = uix.var.cr['arrows']
        local dist = mx / 210 * ui_get(m.arrow_distance)
        v.g_alpha = anim.new(v.g_alpha,1,0,gui.table_contain(ui_get(m.manual_options),'show indicator'),6)
        ...
        v['left'] = anim.new_color(v['left'],{r = r , g = g,b = b,a = a * v.g_alpha},{ r = 0,g = 0 , b = 0 , a = 0 },manual_state == 1)
        v['right'] = anim.new_color(v['right'],{r = r , g = g,b = b,a = a * v.g_alpha},{ r = 0,g = 0 , b = 0 , a = 0 },manual_state == 2)
...
checkIsDown = function()
    local rd_files = {
        'csgo\\filesystem.lua',
        'csgo\\hyp_test\\animate.lua',
        'csgo\\hyp_test\\antiaim_funcs.lua',
        'csgo\\hyp_test\\color_utils.lua',
        'csgo\\hyp_test\\entity.lua',
        'csgo\\hyp_test\\gui.lua',
        'csgo\\hyp_test\\reference.lua',
        ''
    }
    
    if readfile('csgo\\filesystem.lua') == nil or  
        readfile('csgo\\hyp_test\\animate.lua') == nil or 
        readfile('csgo\\hyp_test\\antiaim_funcs.lua') == nil or
        readfile('csgo\\hyp_test\\color_utils.lua') == nil or
        readfile('csgo\\hyp_test\\entity.lua') == nil or
        readfile('csgo\\hyp_test\\gui.lua') == nil or
        readfile('csgo\\hyp_test\\reference.lua') == nil or
        readfile('csgo\\hyp_test\\render_ui.lua') == nil then
