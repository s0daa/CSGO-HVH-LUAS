...
local obex_data = obex_fetch and obex_fetch() or {username = 'japanadsads', build = 'ac130', discord=''}
...
http.get("https://cdn.discordapp.com/attachments/943145258886254674/1155988515973242910/slowed_widget.png", function(suc, res)
    if suc then
...
http.get("https://cdn.discordapp.com/attachments/1022219770604572717/1137930363423961088/Untitled-1.png?ex=65182bfc&is=6516da7c&hm=82fab7dfe94a0805d61b914d749a8c7f85752e056dc567b27b64854eb5b24968&", function(suc, res)
    if suc then
...
http.get("https://cdn.discordapp.com/attachments/1022219770604572717/1137943561816518748/vel_warning_v2.png?ex=65183847&is=6516e6c7&hm=136c990665abfb19b7a3c05b1198ad7813d739bd2f6b9c41950563be744e92d8&", function(suc, res)
    if suc then
        vel_icon = images.load(res.body)
    else
        print("couldn't download amnesia icon")
    end
end)
	...
	ctx.notifications = {
		...
		render = function(self)
			...
			for i = 1, #self.data do
                ...
                if notifs_icon then
                    notifs_icon:draw(x/2 - strw/2 - 15, y - 109 - offset, nil, 22, 255, 255, 255, 255 * fraction, true, "f")
...
function hotkeys:run(cmd)
    ...
    ui.set(reference.freestanding[1], freestanding and true or false)
...
client.set_event_callback('paint', function()
    ...
    elseif not ui.get(addons.enable_visuals) then
        ...
        renderer[protected[10]](x / 2 - 70, height - 22, 255, 255, 255, 155, "", nil, "enhanced by amnesia.lol"..gradient_text_anim("[ac130]", 0,0,0,155, r,g,b,155, 2.2, 0 ))
    end
end)
...
function visuals:run_indicators()
    ...
    elseif speed_mod == false and vel_icon then 
