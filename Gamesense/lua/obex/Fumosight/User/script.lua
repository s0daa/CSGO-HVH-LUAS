...
local menu = {
    ...
    anti_aim = {
        ...
        mode = ui.new_combobox(tab, container, "[aa] preset selection", "smart", "euclid", "keter", "thaumiel", "apollyon"),
...
local states, presets = { "air", "air fakelag", "accelerate", "move", "move fakelag", "slow walk", "stand", "duck" }, { "smart", "euclid", "keter", "thaumiel", "apollyon" }
for i = 1, #states do
...
function vis:crosshair()
    ...
    if style == "standard" then
        ...
        util:clr_text(x / 2, y / 2 + 30, "-", { { "FUMOSIGHT", c[1], c[2], c[3], 255 }, { "[LIVE]", s[1], s[2], s[3], pulse } }, 1)
        util:clr_text(x / 2, y / 2 + 30, "-", { { test, 255, 255, 255, 255 }, { "[LIVE]", s[1], s[2], s[3], pulse } }, 1)
    elseif style == "modern" then
        ...
        renderer.text(x / 2 - (renderer.measure_text("-", "[LIVE]") / 2 * self.shift) - (1.00 - self.shift), y / 2 + 30, s[1], s[2], s[3], pulse, "-", nil, "[LIVE]")
