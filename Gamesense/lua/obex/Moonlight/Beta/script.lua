...
local menu = {}
menu.switch = new(ui.new_checkbox(tab1,tab2,">> \aD0B0FFFFHyacinth Anti-Aim System  \aFFFFFFC8<<"))
menu.master_combo = new(ui.new_combobox(tab1,tab2,"Tab Selection",{"Global",'Anti-Aim',"Animations & Visual","Miscellaneous"}))
menu.info = {
    info_label = ui.new_label(tab1,tab2,'Welcome! \a96C83BFF All Hyacinth users'),
    ...
    info_label4 = ui.new_label(tab1,tab2,'\a96C83BFF Users with Hyacinth can find 1sTar to bind the neverlose beta version for free'),
    info_label3 = ui.new_label(tab1,tab2,'Last update time: \a96C83BFF'..Hyacinth.status.last_updatetime),
...
menu.anti_aim = {
    ...
    state = new(ui.new_combobox(tab1,tab2,"\aD0B0FFFFPlayer State",Hyacinth.pstate)),
    keybind = {
...
local visible = function()
    ...
    for i = 1, #Hyacinth.pstate do
        local condition = Hyacinth.pstate[i]
        ...
        ui.set_visible(menu.anti_aim.builder[i].yawr ,buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R" )
        ui.set_visible(menu.anti_aim.builder[i].yawl ,buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R" )
        ...
        ui.set_visible( menu.anti_aim.builder[i].jitterl , buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R")
        ui.set_visible( menu.anti_aim.builder[i].jitterr , buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R")
        ...
        ui.set_visible( menu.anti_aim.builder[i].byawl , buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R")
        ui.set_visible( menu.anti_aim.builder[i].byawr , buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R")
        ...
        ui.set_visible( menu.anti_aim.builder[i].fakelimitl , buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R")
        ui.set_visible( menu.anti_aim.builder[i].fakelimitr , buld and lp_state and else_ and ui.get( menu.anti_aim.builder[i].yawlr_sett)=="L&R")
    ...
    ui.set(Hyacinth.aa.roll,0)
    ui.set_visible(Hyacinth.aa.fs[1],debug)
    ui.set_visible(Hyacinth.aa.fs[2],debug)
	ui.set_visible(Hyacinth.aa.pitch[1],debug)
    ui.set_visible(Hyacinth.aa.pitch[2],debug)
	ui.set_visible(Hyacinth.aa.yawbase,debug)
	ui.set_visible(Hyacinth.aa.yaw[1],debug)
	ui.set_visible(Hyacinth.aa.yaw[2],debug)
	--ui.set_visible(Hyacinth.aa.fakeyawlimit,debug)
	ui.set_visible(Hyacinth.aa.fsbodyyaw,debug)
	ui.set_visible(Hyacinth.aa.edgeyaw,debug)
	ui.set_visible(Hyacinth.aa.yawjitter[1],debug)
	ui.set_visible(Hyacinth.aa.yawjitter[2],debug)
	ui.set_visible(Hyacinth.aa.bodyyaw[1],debug)
	ui.set_visible(Hyacinth.aa.bodyyaw[2],debug)
	ui.set_visible(Hyacinth.aa.roll,debug)
...
handle_aa.desync = function(a, b, name, cmd)
    ...
    if ui.get(Hyacinth.aa.bodyyaw[1]) == "Jitter" then
        return inverted and a or b
    elseif ui.get(Hyacinth.aa.bodyyaw[1]) == "Static" then
...
handle_aa.direction = function()
    ...
    ui.set(Hyacinth.aa.fs[2],"Always on")
    local fs,edge = ui.get(k.freestanding),ui.get(k.edge_yaw)
    ui.set(Hyacinth.aa.fs[1], ui.get(k.freestanding))
    ui.set(Hyacinth.aa.edgeyaw,edge)
    if ui.get(k.legit_aa) then
        ui.set(Hyacinth.aa.fs[1], "-")
        ui.set(Hyacinth.aa.edgeyaw, false)
    end


    if ui.get(menu.misc.quickfall) then
        if v.override_dt~= nil then
            ui.set(Hyacinth.aa.dt[2],v.override_dt and "Always on" or "On hotkey")
        else
            ui.set(Hyacinth.aa.dt[2], ui.get(menu.misc.doubletap_key))
        end
    end
...
handle_aa.set_antiaim =function(cmd)
    ...
    local onexploit = ui.get(Hyacinth.aa.dt[2]) or ui.get(Hyacinth.aa.os[2])
    ...
    elseif ongroud and infiniteduck or ongroud and ui.get(Hyacinth.aa.fakeduck) then
        handle_aa.vars.snum = 8
    elseif  ui.get(Hyacinth.aa.sw[1]) and ui.get(Hyacinth.aa.sw[2])then
    ...
    if mwork then
    	ui.set(Hyacinth.aa.yawbase,"Local view")
	else
		ui.set(Hyacinth.aa.yawbase,"At targets")
	end

    if type(settings.pitch) == "number" then
        ui.set(Hyacinth.aa.pitch[1], "Custom")
        ui.set(Hyacinth.aa.pitch[2], settings.pitch)
    else
        ui.set(Hyacinth.aa.pitch[1], settings.pitch)
    end

    ui.set(Hyacinth.aa.yaw[1],setting.yawmod)
    ui.set(Hyacinth.aa.yaw[2], settings.yaw or setting.yaw)
    ui.set(Hyacinth.aa.yawjitter[1],ui.get(menu.anti_aim.builder[i].jittermode))
    ui.set(Hyacinth.aa.yawjitter[2],setting.jitter)
    ui.set(Hyacinth.aa.bodyyaw[1],ui.get(menu.anti_aim.builder[i].byawmode))
    ui.set(Hyacinth.aa.bodyyaw[2],setting.byaw)
   -- ui.set(Hyacinth.aa.fakeyawlimit,setting.fakelimit)
end
...
handle_aa.quickfall = function (cmd)
    ...
    if air == false or cmd.in_jump == 1 or ui.get(Hyacinth.aa.dt[1]) == false or ui.get(Hyacinth.aa.dt[2]) == false then
        return
    end
...
handle_visual.indicators_render = function()
    ...
    if ui.get(menu.visual.center_mode) == "Text" then
        ...
        local strike_w, strike_h = renderer.measure_text("-", "1STARS")
        ...
        handle_visual.var.inactive_fraction = handle_visual.lerp(handle_visual.var.inactive_fraction,ui.get(Hyacinth.aa.dt[2]) and 255 or 0, globals.frametime() * 18.214)
        handle_visual.var.active_fraction =handle_visual.lerp(handle_visual.var.active_fraction,ui.get(Hyacinth.aa.dt[2]) and strike_h or 0, globals.frametime() * 12)
        handle_visual.text_fade_animation(x/2 + handle_visual.var.scoped_fraction, y/2 + 20, ui.get(menu.visual.speed), {r=r, g=g, b=b, a=255}, {r=r1, g=g2, b=b3, a=255}, "1STARS")
        ...
        create("", ui.get(Hyacinth.aa.os[2]) and white or disabled_color)
        create("",  ui.get(Hyacinth.aa.fakeduck) and white or disabled_color)
        create("", ui.get(menu.anti_aim.keybind.freestanding) and white or disabled_color)
        create("", ui.get(Hyacinth.aa.baim) and white or disabled_color)
...
handle_visual.min_dmg = function ()
    ...
    local current_min
    if ui.get(Hyacinth.aa.min_ovr[1]) and ui.get(Hyacinth.aa.min_ovr[2]) then
        current_min = ui.get(Hyacinth.aa.min_ovr[3])
    else
        current_min = ui.get(Hyacinth.aa.dmg)
    end
