...
local _ui = {
    lua = {
        enable = group:checkbox("\vEthereal"),
        tab = group:combobox("\n ", "Start", "Anti-aim", "Visuals", "Miscellaneous")
    },

    start = {
        ultra_text = group:label("Build: \v"..script_build),
        ...
        group:label("\a                                                        "),
        discord_server = group:button("\aadb3ffffDiscord server", function() end),
        get_code = group:button("Get discord verification code", function() end)
    },
    ...
    antiaim = {
        ...
        tweaks = group:multiselect("Tweaks", "Modify fakelag on shot", "Anti-backstab", "Safe head"),
...
local custom_menu_control = function()
    ...
    animated_text(x + 16, y - 44, -7, {r=clr_r, g=clr_g, b=clr_b, a=get_menu_alpha()}, {r=clr_r, g=clr_g, b=clr_b, a=get_menu_alpha()}, "+", "ethereal")
...
local renderer_notification = function()
    ...
    for i, v in ipairs(tables.notifications) do
        ...
        v.timer = v.timer + (0.19*vector(renderer.measure_text(nil, "ethereal   » "..v.string)).x*0.0115)*frametime

        if math.max(0,(renderer.measure_text(nil, "ethereal   » "..v.string) + 1 - v.timer)) > 0 then
            v.alpha = animations.math_lerp(v.alpha, 1, frametime)
        end
    
        renderer.rectangle((_ui.start.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "ethereal   » "..v.string)/2) - 3, (position_y + add_y) - 3, renderer.measure_text(nil, "ethereal   » "..v.string) + 5, 23, 0,0,0,v.alpha*100)
        renderer.rectangle((_ui.start.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "ethereal   » "..v.string)/2) - 1, (position_y + add_y) + 16, renderer.measure_text(nil, "ethereal   » "..v.string) + 1, 2, 0,0,0,v.alpha*255)
        renderer.rectangle((_ui.start.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil, "ethereal   » "..v.string)/2) - 1, (position_y + add_y) + 16, math.max(0,(renderer.measure_text(nil, "ethereal   » "..v.string) + 1 - v.timer)), 2, clr_r ,clr_g ,clr_b ,v.alpha*255)

        renderer.text(_ui.start.notifications_position.value == "Upper-left" and position_x or position_x - renderer.measure_text(nil,  "ethereal   » ")/2 - renderer.measure_text(nil, v.string)/2, position_y + add_y, clr_r, clr_g, clr_b, v.alpha*255, nil, 0, "ethereal  »")
        renderer.text(_ui.start.notifications_position.value == "Upper-left" and position_x + 58 or position_x - renderer.measure_text(nil, v.string)/2 + renderer.measure_text(nil, "ethereal   » ")/2, position_y + add_y, 255, 255, 255, v.alpha*255, nil, 0, v.string)

        if math.max(0,(renderer.measure_text(nil, "ethereal   » "..v.string) + 1 - v.timer)) == 0 or #tables.notifications > _ui.start.max_notifications.value then
...
local indicators = function()
    ...
    animated_text(x/2 - renderer.measure_text("b", "ethereal")/2 + anim_ind.scope*40, y/2 + 15, 7, {r=clr_r, g=clr_g, b=clr_b, a=255}, {r=50, g=50, b=50, a=255}, "b", "ethereal")
...
local watermark = function()
    ...
    animated_text(x/2 - renderer.measure_text("b", "E T H E R E A L")/2, y - 20, 7, {r=clr_r, g=clr_g, b=clr_b, a=255}, {r=50, g=50, b=50, a=255}, "b", "E T H E R E A L")
end
...
local defensive_indicator = function()
    ...
    renderer.text(x/2 - renderer.measure_text("", "defensive")/2 + anim_di.scope*30, y/2 - 260, 255,255,255, math.min(255, defensive_charge*3), "", 0, "defensive")
...
client.set_event_callback("player_death", function(e)
    ...
    if contains(_ui.misc.st_when.value, "Kill") and attacker == entity.get_local_player() then
        client.delay_call(client.random_int(1, 2.5), function() 
    ...
    if contains(_ui.misc.st_when.value, "Revenge") then
        ...
        elseif who == revenge_target then
            client.delay_call(client.random_int(1, 1.5), function() 
