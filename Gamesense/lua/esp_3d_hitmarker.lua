...
local hit_marker_size   = ui_new_slider("VISUALS", "Player ESP", "\n hitmarker_size", 1, 10, 4, true, "px")
local hit_marker_time   = ui_new_slider("VISUALS", "Player ESP", "\n hitmarker_time", 1, 10, 3, true)
local hit_marker_color  = ui_new_color_picker("VISUALS", "Player ESP", "Hit marker color", 255, 255, 255, 255)

local function handle_menu()
    local state = ui_get(hit_marker)
    ui_set_visible(hit_marker_size, state)
    ui_set_visible(hit_marker_time, state)
end
...
local function player_hurt(e)
    ...
    shot_data[tick] = {
        ...
        y = hit.y,
        z = hit.z,
        time = globals_curtime() + ui_get(hit_marker_time),
        alpha = 255,
        draw = true,
    }
end
