...
local menu = {
    global = {
        information = {
            ...
            current_build = tabs.global.tab_selection:label(" \f<tag>   Build: \vLive"),
            version = tabs.global.tab_selection:label("\f<cloud-question>  Last update: \v9.09.2024"),
    ...
    anti_aim = {
        main = {
            conditions = tabs.anti_aim.upper_builder:combo("Current Condition", {"Shared", "Standing", "Moving", "Air", "Air-Crouching", "Crouching", "Crouching Moving", "Slow-Walking", "On Use", "Fake Lag"}),
            manual_yaw = tabs.anti_aim.settings:combo("Manual Yaw", {"Disabled", "Forward", "Left", "Right"}, function(gear)
                return 
                {
                    static_manuals = gear:switch("Static Yaw", false),
                }
            end),
            freestanding = tabs.anti_aim.settings:switch("Freestanding", false),
        },
        other = {
            safe_head = tabs.anti_aim.other:switch("Safe Head", false, function(gear)
                return
                {
                    conditions = gear:selectable("Conditions", {"Knife", "Taser", "Distance", "Height Advantage"}),
                    tip = gear:label("\vKnife & Taser works only on air-crouch!"),
                    e_spam = gear:switch("E-Spam", false)
                }
            end),
            discharge_lag_exploit = tabs.anti_aim.other:switch("Discharge lag exploit", false, function(gear)
                return 
                {
                    ticks = gear:slider("Ticks", 8, 32, 15, 1)
                }
            end),
            smart_teleport = tabs.anti_aim.other:switch("Smart Teleport", false),
            avoid_backstab = tabs.anti_aim.other:switch("Avoid Backstab", false),
            disable_aa_on_warmup = tabs.anti_aim.other:switch("Disable on Warmup", false),
            edge_yaw = tabs.anti_aim.other:switch("Edge Yaw", false),
            animation_breakers = tabs.anti_aim.other:switch("Animation Breakers", false),
        },  
    },
    settings = {
        visuals = {
        ...
        misc = {
           aimlogs = tabs.settings.additionals:switch("Aimbot Logs", false, function(gear)
            return 
            {
                ...
                miss_color = gear:color_picker("Miss Color", color(255, 0, 0, 255)),
            }
           end),
...
local gear = {
    freestanding = menu.anti_aim.main.freestanding:create(),
...
local gear_funcs = {
    freestanding_yawmod = gear.freestanding:switch("Disable Yaw Modifiers", false),
    freestanding_body = gear.freestanding:switch("Body Freestanding", false),
    freestanding_disablers = gear.freestanding:selectable("Disablers", {"Standing", "Moving", "Air", "Air-Crouching", "Crouching", "Crouching Moving", "Slow-Walking"}),
...
menu.anti_aim.other.discharge_lag_exploit.ticks:depend(menu.anti_aim.other.discharge_lag_exploit)
...
local stateenable_i = {[1] = "Shared", [2] = "Standing", [3] = "Moving", [4] = "Air", [5] = "Air-Crouching", [6] = "Crouching", [7] = "Crouching Moving", [8] = "Slow-Walking", [9] = "On Use", [10] = "Fake Lag"}
local custom_aa = { }
for i = 1, #stateenable_i do
    custom_aa[i] = {
        state_enable = tabs.anti_aim.builder:switch("Enable \v" .. stateenable_i[i], false),
        yaw_base = tabs.anti_aim.builder:combo("Yaw Base", {"Local View", "At Target"}),
        yaw_type = tabs.anti_aim.builder:combo("Yaw Type", {"Default", "Switch"}, function(gear)
            return 
            {
                lr_yaw_offset = gear:slider("Degree", -180, 180, 0),
                l_offset = gear:slider("Degree Left", -180, 180, 0),
                r_offset = gear:slider("Degree Right", -180, 180, 0),
            }
        end),
        yaw_modifier = tabs.anti_aim.builder:combo("Yaw Modifier", {"Off", "Center", "Offset", "Random", "Spin", "3-Way", "5-Way"}, function(gear)
            return 
            {
                yaw_modifier_offset = gear:slider("Degree", -180, 180, 0)
            }
        end),
        body_yaw_switch = tabs.anti_aim.builder:switch("Body Yaw", false),
        fake_yaw_limit_left = tabs.anti_aim.builder:slider("Left Limit", 0, 60, 0),
        fake_yaw_limit_right = tabs.anti_aim.builder:slider("Right Limit", 0, 60, 0),
        fake_yaw_randomization = tabs.anti_aim.builder:slider("Randomization", 0, 100, 0, 1, "%"),
        options = tabs.anti_aim.builder:selectable("Options", {"Avoid Overlap", "Jitter", "Randomize Jitter", "Anti Bruteforce"}),
        jitter_type = tabs.anti_aim.builder:combo("Jitter type", {"Default", "Delay"}),
        delay_ticks = tabs.anti_aim.builder:slider("Ticks", 1, 10, 1, 1, "t", function(gear)
            return 
            {
                randomize_delay_ticks = gear:slider(("Randomization"), 0, 100, 0, 1, "%"),
            }
        end),
        defensive_aa = tabs.anti_aim.builder:switch("Defensive AA", false, function(gear)
            return 
            {
                mode = gear:combo("Mode", {"On Peek", "Always On"}),
                allow_with_hide_shots = gear:switch("Allow with Hideshots", false),
                custom_defensive_aa_pitch = gear:combo("Pitch", {"Down", "Zero", "Up", "Random"}),
                random_pitch_first_value = gear:slider("First Value", -89, 89, 0, 1),
                random_pitch_second_value = gear:slider("Second Value", -89, 89, 0, 1),
                custom_defensive_aa_yaw = gear:combo("Yaw", {"Sideways", "Spin", "Switch", "Forward", "Random"}),
...
get_player_state = function(cmd)
    ...
    if cmd.in_use then
        ...
        if not (refs.doubletap:get() or refs.hideshots:get()) and custom_aa[10].state_enable:get() then
            return "FAKE-LAG"
        else
            if on_ground and not move and not crouch then fsstateid = 1 return "STANDING"
            elseif not jump and not crouch and not is_slowwalk then fsstateid = 2 return "MOVING"
            elseif is_slowwalk and on_ground then fsstateid = 7 return "SLOW-WALK"
            elseif crouch and not move and (not jump and on_ground) then fsstateid = 5 return "CROUCH"
            elseif crouch and move and (not jump and on_ground) then fsstateid = 6 return "CROUCH-MOVING"
            elseif jump and not crouch then fsstateid = 3 return "AIR"
            elseif crouch and (jump or not on_ground) then fsstateid = 4 return "AIR-C"
            end
        end
    end
end
...
function anti_aim(cmd)
    ...
    if custom_aa[2].state_enable:get() and get_player_state(cmd) == "STANDING" then stateid = 2
    elseif custom_aa[3].state_enable:get() and get_player_state(cmd) == "MOVING" then stateid = 3
    elseif custom_aa[4].state_enable:get() and get_player_state(cmd) == "AIR" then stateid = 4
    elseif custom_aa[5].state_enable:get() and get_player_state(cmd) == "AIR-C" then stateid = 5
    elseif custom_aa[6].state_enable:get() and get_player_state(cmd) == "CROUCH" then stateid = 6
    elseif custom_aa[7].state_enable:get() and get_player_state(cmd) == "CROUCH-MOVING" then stateid = 7
    elseif custom_aa[8].state_enable:get() and get_player_state(cmd) == "SLOW-WALK" then stateid = 8
    elseif custom_aa[9].state_enable:get() and get_player_state(cmd) == "ON-USE" then stateid = 9
    elseif custom_aa[10].state_enable:get() and get_player_state(cmd) == "FAKE-LAG" then stateid = 10
    else
        stateid = 1
    end
    ...
    if stateid == 9 then
        ...
        if #CPlantedC4 > 0 then
            ...
            dist_to_bomb = lp:get_origin():dist(bomb_origin)
        end
        ...
        if #CHostage > 0 then
            ...
            dist_to_hostage = math.min(lp:get_origin():dist(hostage_origin[1]), lp:get_origin():dist(hostage_origin[2]))
        end

        local CPropDoorRotating = entity.get_entities("CPropDoorRotating")
        local dist_to_door = 999

        if #CPropDoorRotating > 0 then
            local hostage_origin = {CPropDoorRotating[1]:get_origin(), CPropDoorRotating[2]:get_origin()}

            dist_to_door = math.min(lp:get_origin():dist(hostage_origin[1]), lp:get_origin():dist(hostage_origin[2]))
        end
        
        if dist_to_hostage < 65 and lp.m_iTeamNum ~= 2 then
            should_disable = true
        else
            should_disable = false
        end

        if dist_to_door < 65 then
            should_disable = true
        else
            should_disable = false
        end

        if dist_to_bomb < 65 and lp.m_iTeamNum ~= 2 then
            should_disable = true
        else
            should_disable = false
        end
        ...
        if should_disable ~= true then
            ...
            refs.yaw_base:override(custom_aa[9].yaw_base:get())
    
            inverted = not rage.antiaim:inverter()

            if globals.choked_commands == 0 then 
                if custom_aa[9].jitter_type:get() == "Delay" and custom_aa[9].delay_ticks:get() > 1 then
                    ticks = ticks + 1
                        
                    if ticks > (custom_aa[9].delay_ticks.randomize_delay_ticks:get() > 0 and utils.random_int(custom_aa[9].delay_ticks:get() - (custom_aa[9].delay_ticks.randomize_delay_ticks:get()/100)*custom_aa[9].delay_ticks:get(), custom_aa[9].delay_ticks:get()) or custom_aa[9].delay_ticks:get()) then
                        ticks = 0
            ...
            refs.body_yaw:override(custom_aa[9].body_yaw_switch:get())

            if custom_aa[9].body_yaw_switch:get() and custom_aa[9].jitter_type:get() == "Delay" and custom_aa[9].delay_ticks:get() > 1 then
                refs.inverter:override(inverted)
                refs.options:override("")
            else
                refs.inverter:override()
                refs.options:override(custom_aa[9].options:get())
            end  
        
            refs.left_limit:override(custom_aa[9].fake_yaw_randomization:get() > 0 and utils.random_int(custom_aa[9].fake_yaw_limit_left:get() - (custom_aa[9].fake_yaw_randomization:get()/100)*custom_aa[9].fake_yaw_limit_left:get(), custom_aa[9].fake_yaw_limit_left:get()) or custom_aa[9].fake_yaw_limit_left:get())
            refs.right_limit:override(custom_aa[9].fake_yaw_randomization:get() > 0 and utils.random_int(custom_aa[9].fake_yaw_limit_right:get() - (custom_aa[9].fake_yaw_randomization:get()/100)*custom_aa[9].fake_yaw_limit_right:get(), custom_aa[9].fake_yaw_limit_right:get()) or custom_aa[9].fake_yaw_limit_right:get())
    
            refs.freestanding_body:override("Off")  
    
            if custom_aa[9].yaw_type:get() == "Switch" then
                refs.yaw_offset:override(inverted and custom_aa[9].yaw_type.l_offset:get() or custom_aa[9].yaw_type.r_offset:get())
            elseif custom_aa[9].yaw_type:get() == "Default" then
                refs.yaw_offset:override(custom_aa[9].yaw_type.lr_yaw_offset:get())
            end
            
            if custom_aa[9].yaw_modifier:get() ~= "Off" then
                refs.yaw_modifier:override(custom_aa[9].yaw_modifier:get())
                refs.modifier_degree:override(custom_aa[9].yaw_modifier.yaw_modifier_offset:get())
            else
        ...
        if globals.choked_commands == 0 then
            if gear_funcs.freestanding_disablers:get(freestanding_mass[fsstateid]) then
                refs.freestanding:override(false)
                refs.yaw_mod:override(false)
                refs.body_fs:override(false)
            else
                if menu.anti_aim.main.freestanding:get() then
                    refs.freestanding:override(true)
                else
                    refs.freestanding:override(false)
                end
            
                if gear_funcs.freestanding_yawmod:get() then
                    refs.yaw_mod:override(true)
                else
                    refs.yaw_mod:override(false)
                end
            
                if gear_funcs.freestanding_body:get() then
            ...
            refs.yaw_base:override(custom_aa[stateid].yaw_base:get())
    
            inverted = not rage.antiaim:inverter()
            
            if custom_aa[stateid].jitter_type:get() == "Delay" and custom_aa[stateid].delay_ticks:get() > 1 then
                ticks = ticks + 1
                    
                if ticks > (custom_aa[stateid].delay_ticks.randomize_delay_ticks:get() > 0 and utils.random_int(custom_aa[stateid].delay_ticks:get() - (custom_aa[stateid].delay_ticks.randomize_delay_ticks:get()/100)*custom_aa[stateid].delay_ticks:get(), custom_aa[stateid].delay_ticks:get()) or custom_aa[stateid].delay_ticks:get()) then
                    ticks = 0
                    if_inverted = not if_inverted
                end
            ...
            refs.body_yaw:override(custom_aa[stateid].body_yaw_switch:get())
    
            if custom_aa[stateid].body_yaw_switch:get() and custom_aa[stateid].jitter_type:get() == "Delay" and custom_aa[stateid].delay_ticks:get() > 1 then
                refs.inverter:override(inverted)
                refs.options:override("")
            else
                refs.inverter:override()
                refs.options:override(custom_aa[stateid].options:get())
            end  
        
            refs.left_limit:override(custom_aa[stateid].fake_yaw_randomization:get() > 0 and utils.random_int(custom_aa[stateid].fake_yaw_limit_left:get() - (custom_aa[stateid].fake_yaw_randomization:get()/100)*custom_aa[stateid].fake_yaw_limit_left:get(), custom_aa[stateid].fake_yaw_limit_left:get()) or custom_aa[stateid].fake_yaw_limit_left:get())
            refs.right_limit:override(custom_aa[stateid].fake_yaw_randomization:get() > 0 and utils.random_int(custom_aa[stateid].fake_yaw_limit_right:get() - (custom_aa[stateid].fake_yaw_randomization:get()/100)*custom_aa[stateid].fake_yaw_limit_right:get(), custom_aa[stateid].fake_yaw_limit_right:get()) or custom_aa[stateid].fake_yaw_limit_right:get())
    
            refs.freestanding_body:override("Off")  
    
            if custom_aa[stateid].yaw_type:get() == "Switch" then
                refs.yaw_offset:override(inverted and custom_aa[stateid].yaw_type.l_offset:get() or custom_aa[stateid].yaw_type.r_offset:get())
            elseif custom_aa[stateid].yaw_type:get() == "Default" then
                refs.yaw_offset:override(custom_aa[stateid].yaw_type.lr_yaw_offset:get())
            end
            
            if custom_aa[stateid].yaw_modifier:get() ~= "Off" then
                refs.yaw_modifier:override(custom_aa[stateid].yaw_modifier:get())
                refs.modifier_degree:override(custom_aa[stateid].yaw_modifier.yaw_modifier_offset:get())
            else
                refs.yaw_modifier:override("Disabled")
            end
        end
        ...
        if threat ~= nil then
            ...
            if menu.anti_aim.other.safe_head:get() then
                ...
                elseif menu.anti_aim.other.safe_head.conditions:get("Taser") and is_taser and get_player_state(cmd) == "AIR-C" then
                    values()
                    safe_head_trigger = menu.anti_aim.other.safe_head.e_spam:get() and true or false
                elseif menu.anti_aim.other.safe_head.conditions:get("Distance") and distancesqr > (1000 * 1000) then
                    values()
                elseif menu.anti_aim.other.safe_head.conditions:get("Height Advantage") and lp:get_origin().z - entity.get_threat():get_origin().z > 55 then
                    values()
                end
            end
        end
    ...
    local manual_aa do
        ...
        if menu.anti_aim.main.manual_yaw.static_manuals:get() then
    ...
    local defensive_aa do
        if safe_head_trigger then
            ...
            if custom_aa[stateid].defensive_aa.allow_with_hide_shots:get() then
            ...
            if custom_aa[stateid].defensive_aa:get() then
                if custom_aa[stateid].defensive_aa.mode:get() == "On Peek" then
                    refs.lag_options:override("On Peek")
                elseif custom_aa[stateid].defensive_aa.mode:get() == "Always On" then
                    refs.lag_options:override("Always On")
                else
                ...
                if custom_aa[stateid].defensive_aa.custom_defensive_aa_pitch:get() == "Down" then
                    rage.antiaim:override_hidden_pitch(89)
                elseif custom_aa[stateid].defensive_aa.custom_defensive_aa_pitch:get() == "Zero" then
                    rage.antiaim:override_hidden_pitch(0)
                elseif custom_aa[stateid].defensive_aa.custom_defensive_aa_pitch:get() == "Up" then
                    rage.antiaim:override_hidden_pitch(-89)
                elseif custom_aa[stateid].defensive_aa.custom_defensive_aa_pitch:get() == "Random" then
                    rage.antiaim:override_hidden_pitch(math.random(custom_aa[stateid].defensive_aa.random_pitch_first_value:get(), custom_aa[stateid].defensive_aa.random_pitch_second_value:get()))
                end
    
                if custom_aa[stateid].defensive_aa.custom_defensive_aa_yaw:get() == "Sideways" then
                    rage.antiaim:override_hidden_yaw_offset(inverted and -120 or 120)
                elseif custom_aa[stateid].defensive_aa.custom_defensive_aa_yaw:get() == "Spin" then
                    if yawadd >= 180 then
                        yawadd = -180
                    else
                        yawadd = yawadd + custom_aa[stateid].defensive_aa.spin_speed:get()
                    end
                    rage.antiaim:override_hidden_yaw_offset(yawadd)    
                elseif custom_aa[stateid].defensive_aa.custom_defensive_aa_yaw:get() == "Switch" then
                    rage.antiaim:override_hidden_yaw_offset(inverted and custom_aa[stateid].defensive_aa.switch_yaw_first_value:get() or custom_aa[stateid].defensive_aa.switch_yaw_second_value:get())
                elseif custom_aa[stateid].defensive_aa.custom_defensive_aa_yaw:get() == "Forward" then
                    rage.antiaim:override_hidden_yaw_offset(inverted and 180 or -180)
                elseif custom_aa[stateid].defensive_aa.custom_defensive_aa_yaw:get() == "Random" then
                    rage.antiaim:override_hidden_yaw_offset(math.random(custom_aa[stateid].defensive_aa.random_yaw_first_value:get(), custom_aa[stateid].defensive_aa.random_yaw_second_value:get()))
                end
            else
                refs.lag_options:override("On Peek")
                refs.hideshots_options:override("Favor Fire Rate")
    ...
    local edge_yaw do
        if menu.anti_aim.other.edge_yaw:get() then
            is_edging = false
    ...
    local warmup_aa do
        if menu.anti_aim.other.disable_aa_on_warmup:get() and entity.get_game_rules().m_bWarmupPeriod then
            cmd.in_use = 1
        end
    end

    local discharge_lag_exploit do
        if menu.anti_aim.other.discharge_lag_exploit:get() then
            if globals.tickcount % (menu.anti_aim.other.discharge_lag_exploit.ticks:get()) == 0 then
    ...
    local smart_teleport do
        if menu.anti_aim.other.smart_teleport:get() then
...
function set_visible_aa()
    for i = 1, #stateenable_i do
        condit_fn2 = function()
            if i ~= 1 then
                return custom_aa[i].state_enable:get()
            else
                return true
            end
        end

        custom_aa[i].state_enable:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and i ~= 1)
        custom_aa[i].yaw_type:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and condit_fn2())
        custom_aa[i].yaw_base:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and condit_fn2())
        custom_aa[i].yaw_type.lr_yaw_offset:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].yaw_type:get() == "Default" and condit_fn2())
        custom_aa[i].yaw_type.l_offset:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].yaw_type:get() ~= "Default" and condit_fn2())
        custom_aa[i].yaw_type.r_offset:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].yaw_type:get() ~= "Default" and condit_fn2())
        custom_aa[i].yaw_modifier:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and condit_fn2())
        custom_aa[i].yaw_modifier.yaw_modifier_offset:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].yaw_modifier:get() ~= "Off" and custom_aa[i].yaw_modifier:get() ~= "Custom" and condit_fn2())
        custom_aa[i].body_yaw_switch:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and condit_fn2())
        custom_aa[i].options:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].body_yaw_switch:get() and condit_fn2())
        custom_aa[i].fake_yaw_limit_left:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].body_yaw_switch:get() and condit_fn2())
        custom_aa[i].fake_yaw_limit_right:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].body_yaw_switch:get() and condit_fn2())
        custom_aa[i].fake_yaw_randomization:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].body_yaw_switch:get() and condit_fn2())
        custom_aa[i].jitter_type:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].options:get("Jitter") and condit_fn2())
        custom_aa[i].delay_ticks:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].jitter_type:get() == "Delay" and custom_aa[i].options:get("Jitter") and condit_fn2())

        custom_aa[i].defensive_aa:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.mode:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.allow_with_hide_shots:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.custom_defensive_aa_pitch:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10) 
        custom_aa[i].defensive_aa.random_pitch_first_value:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa.custom_defensive_aa_pitch:get() == "Random" and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.random_pitch_second_value:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa.custom_defensive_aa_pitch:get() == "Random" and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.custom_defensive_aa_yaw:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.spin_speed:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa.custom_defensive_aa_yaw:get() == "Spin" and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.switch_yaw_first_value:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa.custom_defensive_aa_yaw:get() == "Switch" and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.switch_yaw_second_value:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa.custom_defensive_aa_yaw:get() == "Switch" and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.random_yaw_first_value:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa.custom_defensive_aa_yaw:get() == "Random" and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
        custom_aa[i].defensive_aa.random_yaw_second_value:visibility(menu.anti_aim.main.conditions:get() == stateenable_i[i] and custom_aa[i].defensive_aa.custom_defensive_aa_yaw:get() == "Random" and custom_aa[i].defensive_aa:get() and condit_fn2() and i ~= 10)
    end
end
...
local indicators = function() 
    ...
    local gradient_text = gradient.text_animate("FROST LIVE", -1, {color(first_color.r, first_color.g, first_color.b, 255), color(second_color.r, second_color.g, second_color.b, 255)})
    gradient_text:animate()

    local frost_size = render.measure_text(2, "c", "FROST LIVE")
...
local watermark = function() 
    ...
    if gear_funcs.watermark_style:get() == "Frost" then
        ...
        local gradient_animation_watermark_modern = gradient.text_animate(">/ frost anti aim - " .. common.get_username() .. " - [live] /<", -2, {color(frost_color.r, frost_color.g, frost_color.b, 255), color(50, 50, 50, 255)})
        ...
        render.gradient(vector(x - 180, 4), vector(x - 7, 25), color(0, 0, 0, 0), frost_color, color(0, 0, 0, 0), frost_color, 5)
        render.text(1, vector(x - 122, 8), color(255, 255, 255, 255), nil, "discord.gg/frostlua " .. ui.get_icon("snowflake"))
    end
end
...
local function math_lerp(a, b_, t) 
    local t = math_clamp(globals.frametime * (0.045 * 175), 0, 1) 
    if type(a) == "userdata" then 
        r, g, b, a = a.r, a.g, a.b, a.a e_r, e_g, e_b, e_a = b_.r, b_.g, b_.b, b_.a 
        r = math_clamp(r, e_r, t) g = math_clamp(g, e_g, t) 
        b = math_clamp(b, e_b, t) a = math_clamp(a, e_a, t) 
        return color(r, g, b, a) 
    end 
    local d = b_ - a d = d * t d = d + a 
...
local notifications = function()
    ...
    for i, logs in ipairs(notify_data) do
        if logs.time + 1 > globals.realtime then
            logs.alpha = math.lerp(logs.alpha, 1, 0.1)
            logs.alpha1 = math.lerp(logs.alpha1, 1, 0.1)
        end
        ...
        if(math_hundred_floor(logs.alpha1) >= 1) then
            main_alpha = logs.alpha
        else
            main_alpha = logs.alpha1
        end

        render.rect(vector(x / 2 - size - 10, y - 125 * main_alpha - y_add - 6), vector(x / 2 + size + 10, y - 125 * main_alpha - y_add + 20), color(0, 0, 0, 150 * logs.alpha), 5)
        render.shadow(vector(x / 2 - size - 10, y - 125 * main_alpha - y_add - 6), vector(x / 2 + size + 10, y - 125 * main_alpha - y_add + 20), color(accent_color.r, accent_color.g, accent_color.b, accent_color.a * logs.alpha), 20, 0, 5)
        render.text(1, vector(x / 2 - size, y - 125 * main_alpha - y_add), color(255, 255, 255, 255 * main_alpha), nil, string)

        if logs.time + (5 - 0.1) < globals.realtime then
            logs.alpha1 = math.lerp(logs.alpha1, 2, 0.1)
        end

        if logs.time + 5 < globals.realtime and logs.alpha1 > 1.5 then
            logs.alpha = math.lerp(logs.alpha, 0, 0.1)
        end

        if logs.alpha < 0.02 and (logs.time + 5 < globals.realtime) or #notify_data > 5 then
            table.remove(notify_data, i)
        end

        y_add = y_add + 40 * logs.alpha
    end
end
    ...
    if state == nil then
        if menu.settings.misc.aimlogs.select:get("Notify") then
            table.insert(notify_data, {
                text = "\a" .. menu.settings.misc.aimlogs.hit_color:get():to_hex() .. "" .. ui.get_icon("snowflake") .. "\aDEFAULT  Hit ".. target:get_name() .." in the ".. hitgroup .." for ".. e.damage .."" , 
                alpha = 0, alpha1 = 0, time = globals.realtime, color = menu.settings.misc.aimlogs.hit_color:get()})
        else
            notify_data = {}
        end
        ...
        if menu.settings.misc.aimlogs.select:get("Notify") then
            table.insert(notify_data, {
                text = "\a" .. menu.settings.misc.aimlogs.miss_color:get():to_hex() .. "" .. ui.get_icon("snowflake") .. "\aDEFAULT  Missed shot due to ".. state .."", 
                alpha = 0, alpha1 = 0, time = globals.realtime, color = menu.settings.misc.aimlogs.miss_color:get()})
        else
            notify_data = {}
        end
...
local perfomance_boost = function()
    if menu.settings.misc.perfomance_boost:get() then
        table.insert(notify_data, {
            ...
            alpha = 0, alpha1 = 0, time = globals.realtime, color = color(255, 255, 255, 255)})
...
configdata.cfg_list = configdata.cfg_list or {{'Default', ''}}
configdata.menu_list = configdata.menu_list or {'Default'}

configdata.cfg_list[1][2] = ""
