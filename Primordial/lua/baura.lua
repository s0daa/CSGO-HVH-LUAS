...
menu.add_text(groups.main, "Baura.lua")
...
local menu_vars = {
    ...
    custom_clantag_mode = menu.add_selection(groups.misc, "Clantag Select", {"baura.lua (static)", "baura.lua", "gamesense", "primordial with uid", "primordial uid (static)", "aimware"}),
...
callbacks.add(e_callbacks.PAINT, function()
    table.foreach(baura.notification_array, function(i, notification_array_pointer)
        ...
        if notification_array_pointer.duration > text_size.x then 
            ...
            if notification_array_pointer.alpha == 0 then
                table.remove(baura.notification_array, i)
            end
        end
...
function baura:add_notify(text, speed, color)
    local duration = 0
    local alpha = 255
    local animy = 0
    local animx = 0
...
local function welcome_notify()
    if not do_welcome_noti_once then
        baura:add_notify("welcome, " .. user.name .. "!", 0.4, menu.find("Misc", "Main", "Personalization", "Accent Color")[2]:get())
        baura:add_notify("baura.lua, currently on version " .. lua_ver, 0.6, menu.find("Misc", "Main", "Personalization", "Accent Color")[2]:get())
...
local function on_aimbot_hit(hit)
    ...
    if menu_vars.aimbot_notifications_select:get(1) then
        ...
        baura:add_notify("hit [" .. hit.player:get_name() .. "] -> [" .. hitgroups[hit.hitgroup] .. " | " .. hit.damage .. " dmg]" .. " -> [" .. hit.backtrack_ticks .. " bt] -> [safe: " .. tostring(hit.safepoint) .. "]", 0.7, color_t(menu_colors.aimbot_notifications_col_hit:get()[1], menu_colors.aimbot_notifications_col_hit:get()[2], menu_colors.aimbot_notifications_col_hit:get()[3]))
    end
end
...
local function on_aimbot_miss(miss)
    ...
    if menu_vars.aimbot_notifications_select:get(2) then
        ...
        baura:add_notify("missed [" .. miss.player:get_name() .. "] -> [" .. miss.reason_string .. "]", 0.7, color_t(menu_colors.aimbot_notifications_col_miss:get()[1], menu_colors.aimbot_notifications_col_miss:get()[2], menu_colors.aimbot_notifications_col_miss:get()[3]))
    end
end
    ...
    if event.name == "player_death" then
        ...
        if menu_vars.aimbot_notifications_select:get(1) then
            if attacker == entity_list.get_local_player() then
                ...
                baura:add_notify("killed [" .. victim:get_name() .. "] -> [" .. event.weapon .. "]"  , 0.2, color_t(menu_colors.aimbot_notifications_col_hit:get()[1], menu_colors.aimbot_notifications_col_hit:get()[2], menu_colors.aimbot_notifications_col_hit:get()[3]))
            end
        end
    end
end
...
local desirelua_ct = {
    ...
    [12] = "       baura.l",
    [13] = "      Baura.lu",
    [14] = "      baura.lua primordial ",
    [15] = "     ura.lua primordial ",
    [16] = "     ra.lua primordial ",
    [17] = "    a.lua  primordial ",
    [18] = "    b.lua primordial ",
    [19] = "   b.lua primordial ",
    [20] = "   .lua primordial ",
...
local function watermark()
    ...
    local text = "baura.lua / " .. username .. " / " .. user.uid
