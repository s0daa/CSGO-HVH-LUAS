...
function note_c.new(name)
    if (package.ReleaseNotes == nil) then 
...
function note_c:get(func)
    ...
    for key, value in note_c.sort(package.ReleaseNotes) do
        if (value == true) then
            index = index + 1
...
function note_c:set_state(value)
    package.ReleaseNotes[self.name] = value

    table.sort(package.ReleaseNotes)
end

--- @return: void
function note_c:unload()
    if (package.ReleaseNotes[self.name] ~= nil) then 
        package.ReleaseNotes[self.name] = nil 
    end
end
--- @endregion
...
--- @region: unnamed
--- @element: script vars
local script_name_1 = "Rele"
local script_name_2 = "ase"
local script_name = script_name_1..script_name_2
local script_type = "Live"
local script_color = color_t(146, 112, 219, 1)
...
ui_global.last_update = menu_manager_c.new("Global", "Main", ("Lastest Update: %s/%s/%s."):format("18", "09", "2022")):text()
ui_global.discord_link = menu_manager_c.new("Global", "Main", ("Discord.gg/release")):text()

--- @group: Settings
local ui_settings = {}

ui_settings.notify_offset = menu_manager_c.new("Settings", "Placement", "Notify Logs Offset"):slider(100, 500, 300)
ui_settings.indicators_offset = menu_manager_c.new("Settings", "Placement", "Indicators Offset"):slider(0, 150, 40)
ui_settings.hit_log_offset = menu_manager_c.new("Settings", "Placement", "Hit Log Offset"):slider(100, 500, 200)

ui_settings.solus_animation_speed = menu_manager_c.new("Settings", "Placement", "Solus Animation Speed"):slider(0, 25, 12)
ui_settings.hit_log_animation_speed = menu_manager_c.new("Settings", "Placement", "Hit Log Animation Speed"):slider(0, 25, 4)
ui_settings.manual_arrows_animation_speed = menu_manager_c.new("Settings", "Placement", "Manual Arrows Animation Speed"):slider(0, 25, 12)
...
ui_ragebot.doubletap_enable = function() return ui_ragebot.switch_condition ()end
ui_ragebot.doubletap_modifications = menu_manager_c.new("Ragebot", "Ragebot", "Enable DoubleTap Modifications", true, ui_ragebot.doubletap_enable):checkbox()
ui_ragebot.doubletap_modifications_condition = function() return ui_ragebot.doubletap_modifications:get()end
ui_ragebot.doubletap_shift = menu_manager_c.new("Ragebot", "Ragebot", "DoubleTap Tick Slider", true, ui_ragebot.doubletap_modifications_condition):slider(1, 20, 15)
ui_ragebot.doubletap_warn = menu_manager_c.new("Ragebot", "Ragebot",("Some servers might reject ticks that are too high"), true, ui_ragebot.doubletap_modifications_condition):text()
...
ui_widgets.widgets_list = {"Watermark", "Keybinds", "Fake Indication", "Fake Lag Indication", "Exploit Indication", "Frequency Update Indication", "Crosshair Indicators", "Script Info Box"}
...
ui_widgets.custom_watermark_pos = menu_manager_c.new("Widgets", "Widgets", "Drag Watermark Placement", true, function()
	return ui_widgets.switch_condition() and ui_widgets.widgets:get(1)
end):checkbox()

ui_widgets.hit_log = menu_manager_c.new("Widgets", "Widgets", "Enable Hit Logs", true, ui_widgets.switch_condition):checkbox()
...
ui_widgets.miss_color = menu_manager_c.new("Widgets", "Widgets", "Miss Color", true, ui_widgets.manage_color_condition):color_picker(146, 112, 219, 1)

ui_widgets.accent = menu_manager_c.new("Widgets", "Widgets", "Accent", true, function()
	return ui_widgets.switch_condition() and (ui_widgets.notify:get() or ui_widgets.widgets:get(1) or ui_widgets.widgets:get(2) or ui_widgets.widgets:get(3) or ui_widgets.widgets:get(4) or ui_widgets.widgets:get(5) or ui_widgets.widgets:get(6) or ui_widgets.widgets:get(7))
...
end):selection("Solus V1", "Solus V2", "Round")
...
end):selection("Default", "V2")

ui_widgets.disable_default_watermark = menu_manager_c.new("Widgets", "Widgets", "Disable Default Primordial Watermark", true, ui_widgets.switch_condition):checkbox()



ui_widgets.custom_watermark_username = menu_manager_c.new("Widgets", "Widgets", "Custom Watermark Username", true, ui_widgets.switch_condition):checkbox()
ui_widgets.custom_watermark_text = menu.add_text_input("Widgets", "Custom Watermark Input")
ui_widgets.custom_watermark_text:set_visible(false)
...
ui_widgets.manual_arrows_scope_anim = menu_manager_c.new("Widgets", "Widgets", "Scope Animation", true, ui_widgets.manual_arrows_condition):checkbox()
ui_widgets.manual_arrows_hide = menu_manager_c.new("Widgets", "Widgets", "Fade Arrows On Scope", true, ui_widgets.manual_arrows_condition):checkbox()
...
ui_other.clan_tag = menu_manager_c.new("Widgets", "Other", "Enable Clantag", true, ui_other.switch_condition):checkbox()
...
function doubletap()
	if (not ui_ragebot.switch:get() or ui_ragebot.doubletap_modifications:get()) then
		return
	end
	ui_ragebot.doubletap_warn:set_visible(true)
	cvars.sv_maxusrcmdprocessticks:set_int(ui_ragebot.doubletap_shift:get())
end
...
function condition_anti_aim.create_items()
	...
	for key, value in ipairs(items) do
		...
		setup.disable_jitter = menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Jitter Disablers"), true, visible_condition):multi_selection("On Manual", "On Auto Direction")
		...
		setup.desync_amount_type = menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Desync Amount Type"), true, visible_condition):selection("Default", "Custom")
...
function watermark_c.handle()
	...
	watermark_c.note:get(function(id)
		...
		if not ui_widgets.custom_watermark_username:get() then
			ui_widgets.custom_watermark_text:set_visible(false)
        	local nickname = user.name
		else 
			ui_widgets.custom_watermark_text:set_visible(true)
			nickname = ui_widgets.custom_watermark_text:get()
		end
        local prefix = {
            first = script_name_1,
            second = script_name_2,
        }
...
function info_bar_c.handle()
	...
	if menu.is_open() then
        ...
        local text = {
			{text = ("Build: [ %s ] "):format(script_type)},
			{text = ("	Latest Update: [ %s / %s - %s ]"	):format("18", "09", "22")},
            {text = ("	User: [ %s ]"      ):format(nickname)},
			{text = ("	 Script: [ %s ] "):format(script_name)},
			{text = ("	Time Of Day: [%02d:%02d:%02d]"):format(h, m, s)}
        }
		
		local text_sizex, text_sizey = render_c.get_multitext_width(fonts.verdana.shadow, text), 12
		local animsizex, animsizey = animation.create("infobar [width]", text_sizex + 10, 8), 22

		render_c.container(vec2_t(menu.get_pos().x+menu.get_size().x/2-text_sizex/2+3, menu.get_pos().y - 30), vec2_t(animsizex, animsizey), color_t(r, g, b, 255), ui_widgets.style:get())
		render_c.multitext(fonts.verdana.shadow, vec2_t(menu.get_pos().x+menu.get_size().x/2-text_sizex/2+8, menu.get_pos().y-25), text)
	end
end
...
local  custom_watermark_username_c = {}

function  custom_watermark_username_c.handle()
	if (not ui_widgets.custom_watermark_username:get() or not ui_widgets.switch:get()) then
		ui_widgets.custom_watermark_text:set_visible(false)
		nickname = user.name
	else
		ui_widgets.custom_watermark_text:set_visible(true)
		text = ui_widgets.custom_watermark_text:get()
	end
...
clan_tag.tag = clan_tag.create("Release By Anonimous")
...
ui_global.import_default_button = menu_manager_c.new("Global", "Main", "Load Default Config", false):button(function()
	--- pizdec
	local data = ""

	config_c.import(data)
end)

notify.push(("Welcome to Release, %s!"):format(user.name))
--- @endregion

--- @region: callbacks
--- @callback: paint
callbacks.add(e_callbacks.PAINT, info_bar_c.handle)
...
callbacks.add(e_callbacks.PAINT, custom_watermark_username_c.handle)
...
callbacks.add(e_callbacks.SETUP_COMMAND, doubletap)
callbacks.add(e_callbacks.DRAW_WATERMARK, function() 
	if ui_widgets.disable_default_watermark:get() then
		return "" 
	end
