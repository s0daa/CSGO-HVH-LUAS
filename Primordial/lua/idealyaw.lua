...
UPDATE LOG:
- added nickname generator
- * этот апдейт даже не вышел лол
-
...
gui.add_text(group, "Nickname editor")
gui.add_text(group, "Your nickname: -")
gui.add_text(group, "Generated name: -")
gui.add_button(group, "Generate nickname", function() functions.misc.nickname_generate() end)
gui.add_button(group, "Copy generated nickname", function() functions.misc.copy_generated_name() end)
...
gui.add_slider(group, "Rank", 0, 100, 1)
...
functions.visuals.logs = function()
    ...
    local render_log = function(info)
        ...
        if not menu.is_open() then
            ...
            if info[3] == "ping (local death)" or info[3] == "ping (target death)" then
                info[3] = "death"
            end
            if style == 1 then
                local text = ""
...
functions.misc.generated_nickname = "-"
functions.misc.nickname_generate = function()
    local database = {
        "top player", "ez 1", "1", "(1) 1", "rombit huy", "Александра Розоткина", "Андрей Васильков", "олег дуб", "дерево онлайн",
        "download", "ffi player", "function opai() { }", "airflow top1?", "weave top1?", "airflow and weave", "ez owned by airflow",
        "ahhaha", "хахахах", "ок", "ok", "уже тошнит", "смотрите товарищи", "хватит хулеганить", "что вы делаете?", "это кошмар",
        "это ваще", "я не могу", "poor", "godbless", "godeles owned by hackstar", "hackstar n1?", "small db", "ev0lve russia",
        "fatality russia", "clownfish", "вы портите товар", "artem wolf", "KucJloTa1337", "Fipp1337", "Адмирал Абудлов",
        "нл дал вилке денег за скит", "окей"
    }

    functions.misc.generated_nickname = database[client.random_int(1, #database)]
    gui.get("Misc - other", "Generated name: -"):set("Generated name: " .. functions.misc.generated_nickname)
end
functions.misc.copy_generated_name = function()
    clipboard.set(functions.misc.generated_nickname)
    print(functions.misc.generated_nickname)
    chat_print("{red}Nickname duplicated in console! If it russian nickname copy from console!")
end
functions.misc.nickname_core = function()
    local local_player = entity_list.get_local_player()
    if local_player then
        gui.get("Misc - other", "Your nickname: -"):set("Your nickname: " .. local_player:get_name())
    end
end
