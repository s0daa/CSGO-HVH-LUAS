...
if ind:get() then
    ...
    if weaponname == "taser" then
        render.text(icon_font, "taser", vec2_t(bbox.x - 60, bbox.y + 0), ind_col()) --the bboxsize.x/5 makes no sense ik but idc will fix somewhen
    else
      render.text(icon_font, "taser", vec2_t(bbox.x - 60, bbox.y + 0), ind_col2())
    end
end
end
end
