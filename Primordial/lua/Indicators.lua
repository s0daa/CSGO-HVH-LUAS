local pixel = render.create_font("Cascadia Mono", 15, 0, e_font_flags.OUTLINE)
...
local function indicators2()
    ...
    if local_player:is_alive() then -- check if player is alive
        --render
        local eternal_ts = render.get_text_size(pixel, "big ")
        render.text(pixel, "big ", vec2_t(x / 2, y / 2 + ay), color_t(220, 135, 49, 255), 12, true)
        render.text(pixel, "boss", vec2_t(x / 2 + eternal_ts.x - 2, y / 2 + ay), color_t(220, 135, 49, alpha), 10, true)
        ay = ay + 10
