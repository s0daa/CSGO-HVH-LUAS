...
render.outline_text = function(font,text,vec,col,active)
    if active == nil then active = true end
    local x,y = vec.x, vec.y
    render.push_alpha_modifier(active and col.a/255 or 0.2*col.a/255)
    render.text(font, text, vec2_t(x+1, y+1), color_t(0,0,0,255))
    render.text(font, text, vec2_t(x+1, y-1), color_t(0,0,0,255))
    render.text(font, text, vec2_t(x-1, y+1), color_t(0,0,0,255))
    render.text(font, text, vec2_t(x-1, y-1), color_t(0,0,0,255))
    render.text(font, text, vec2_t(x, y-1), color_t(0,0,0,255))
    render.text(font, text, vec2_t(x, y+1), color_t(0,0,0,255))
    render.text(font, text, vec2_t(x+1, y), color_t(0,0,0,255))
    render.text(font, text, vec2_t(x-1, y), color_t(0,0,0,255))
    render.pop_alpha_modifier()

    render.text(font, text, vec2_t(x, y), active and col or color_t(col.r,col.b,col.g,145*col.a/255))
end
...
local fonts = {
    pixel = render.create_font("Small Fonts", 10, 0),
    verdana = render.create_font("Verdana", 11, 0, e_font_flags.DROPSHADOW),
    verdana_flag = render.create_font("Verdana", 10, 0, e_font_flags.DROPSHADOW),
...
local function esp_draw(ctx)
    ...
    if ui.healthbar:get() ~= 1 then
        ...
        if enemy_hp < 100 or (local_player:is_alive() and (weapon_local ~= nil and (weapon_local:get_weapon_data().type == e_weapon_types.PISTOL or weapon_local:get_weapon_data().type == e_weapon_types.GRENADE or weapon_local:get_weapon_data().type == e_weapon_types.KNIFE)) and armor == 0 and dist_to(local_player:get_render_origin(), ctx.player:get_render_origin()) < 684) then
            local textsize = render.get_text_size(fonts.pixel, tostring(enemy_hp))
            render.outline_text(fonts.pixel, tostring(enemy_hp), vec2_t(x1-(ui.dpi_scale:get() >= 2 and (ui.dpi_scale:get() >= 4 and 6 or 8) or 5)-textsize.x/2, y1+offset-3*esp.size2), color_t(255, 255, 255, math.floor(a*250)))
        end
    end
    ...
    if ui.ammo_bar:get() and weapon then
        ...
        if max_clip ~= 0 then
            ...
            if clip <= 3 and clip > 0 then
                render.outline_text(fonts.pixel, tostring(clip), vec2_t(x1+1+y2*ammo_percentage - 1, y1 + y2 + 1+y__), ctx.player:is_dormant() and color_t(255, 255, 255, math.floor(a*255)) or color_t(255, 255, 255, 230))
            end
            y__ = y__ + 5/esp.size2
        end
    end
    if ui.flags:get(17) then
        ...
        local textsize = render.get_text_size(fonts.pixel, dist)

        render.outline_text(fonts.pixel, dist, vec2_t(x1 + x2 / 2-textsize.x/2, y1+y2+y__), ctx.player:is_dormant() and color_t(255, 255, 255, math.floor(a*255)) or color_t(255, 255, 255, 230))
        y__ = y__ + 7*esp.size2
    end
    ...
    if ui.flags:get(1) and wpn ~= nil then
        ...
        local textsize = render.get_text_size(fonts.pixel, weapon_name:upper())
        render.outline_text(fonts.pixel, weapon_name:upper(), vec2_t(x1 + x2 / 2-textsize.x/2, y1+y2+y__), ctx.player:is_dormant() and color_t(255, 255, 255, math.floor(a*255)) or color_t(255, 255, 255, 230))
        y__ = y__ + 7*esp.size2
    end
    ...
    for i, v in pairs(flags) do
        y_ = y_ + 10
        render.outline_text(ui.dpi_scale:get() >= 2 and fonts.verdana_flag or fonts.pixel, v.text, vec2_t(x_, y_), ctx.player:is_dormant() and color_t(255, 255, 255, math.floor(a*255)) or color_t(v.r, v.g, v.b, v.a))
