...
menu.add_text(groups.main, "Chloride.lua")
...
local menu_vars = {
    ...
    custom_clantag_mode = menu.add_selection(groups.misc, "Clantag Select", {"chloride.lua (static)", "chloride.lua", "gamesense", "primordial with uid", "primordial uid (static)", "aimware"}),
...
callbacks.add(e_callbacks.PAINT, function()
    table.foreach(chloride.notification_array, function(i, notification_array_pointer)
        ...
        if notification_array_pointer.duration > text_size.x then 
            ...
            if notification_array_pointer.alpha == 0 then
                table.remove(chloride.notification_array, i)
            end
        end
...
function chloride:add_notify(text, speed, color)
    local duration = 0
    local alpha = 255
    local animy = 0
    local animx = 0
...
local function welcome_notify()
    if not do_welcome_noti_once then
        chloride:add_notify("welcome, " .. user.name .. "!", 0.4, menu.find("Misc", "Main", "Personalization", "Accent Color")[2]:get())
        chloride:add_notify("chloride.lua, currently on version " .. lua_ver, 0.6, menu.find("Misc", "Main", "Personalization", "Accent Color")[2]:get())
...
local function on_aimbot_hit(hit)
    ...
    if menu_vars.aimbot_notifications_select:get(1) then
        ...
        chloride:add_notify("hit [" .. hit.player:get_name() .. "] -> [" .. hitgroups[hit.hitgroup] .. " | " .. hit.damage .. " dmg]" .. " -> [" .. hit.backtrack_ticks .. " bt] -> [safe: " .. tostring(hit.safepoint) .. "]", 0.7, color_t(menu_colors.aimbot_notifications_col_hit:get()[1], menu_colors.aimbot_notifications_col_hit:get()[2], menu_colors.aimbot_notifications_col_hit:get()[3]))
    end
end
...
local function on_aimbot_miss(miss)
    ...
    if menu_vars.aimbot_notifications_select:get(2) then
        ...
        chloride:add_notify("missed [" .. miss.player:get_name() .. "] -> [" .. miss.reason_string .. "]", 0.7, color_t(menu_colors.aimbot_notifications_col_miss:get()[1], menu_colors.aimbot_notifications_col_miss:get()[2], menu_colors.aimbot_notifications_col_miss:get()[3]))
    end
end
    ...
    if event.name == "player_death" then
        ...
        if menu_vars.aimbot_notifications_select:get(1) then
            if attacker == entity_list.get_local_player() then
                ...
                chloride:add_notify("killed [" .. victim:get_name() .. "] -> [" .. event.weapon .. "]"  , 0.2, color_t(menu_colors.aimbot_notifications_col_hit:get()[1], menu_colors.aimbot_notifications_col_hit:get()[2], menu_colors.aimbot_notifications_col_hit:get()[3]))
            end
        end
    end
end
...
local desirelua_ct = {
    ...
    [12] = "       chloride.l",
    [13] = "      chloride.lu",
    [14] = "      chloride.lua primordial ",
    [15] = "     loride.lua primordial ",
    [16] = "     lor.lua primordial ",
    [17] = "    ide.lua  primordial ",
    [18] = "    de.lua primordial ",
    [19] = "   e.lua primordial ",
    [20] = "   .lua primordial ",
    [21] = "  lua primordial ",
    [22] = "  ua primordial   ",
...
local function do_clantag()
    ...
    if menu_vars.custom_clantag_mode:get() == 1 then
        if globals.real_time() - last_tick >= 0.15 then
            ...
            client.set_clantag("chloride.lua")
            last_tick = globals.real_time()
        end
    end
...
local function watermark()
    ...
    local text = "chloride.lua / " .. username .. " / " .. user.uid
                ...
                if local_player:is_point_visible(target_origin) or distance(local_origin, target_origin) < 500 then        
                    if bit.band(local_player:get_prop("m_fFlags"), 1) == 0 then    
                        if menu_vars.custom_antiaim_anti_prediction_pitch_triggers:get(1) then
                            ...
                            if globals.tick_count() % 10 < 2 then
