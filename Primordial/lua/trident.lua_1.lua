...
static_sizes.kill_static = function()
    ...
    static_sizes.pre_text_data = render.get_text_size(fonts.verdana_s, "Trident.lua")
...
ui_handler.tab_list = {}
menu.add_text("Trident.lua", "Welcome to Trident.lua " .. alpha_v_text .. " " .. lua_version)
ui_handler.combo_controller = menu.add_selection("Trident.lua", "Tab Selection", ui_handler.tab_list)
...
function trident_fs.write(file, buffer)
...
function trident_fs.exists(file)
    return raw_funcs.file_exist(filesys_class, file, nil)
end

function trident_fs.directory_exists(dir)
    return raw_funcs.directory_exist(full_filesys_class, dir, nil)
end

function trident_fs.create_directory(dir)
...
configs.load = function(text)
    ...
    client.log_screen(colors.white,"Trident.lua", colors.main[2]:get(), "[Configs]", colors.white, "Config successfully loaded!")
end
...
local function add_notification(type, data)
    ...
    notification_logs[#notification_logs + 1] = {
        "[", "Trident.lua", "] ", types, data, global_vars.tick_count() + 300
    }
end
...
local events_fun = function(event)
    ...
    if event.name == "round_start" then
        ...
        if total_brute> 0 then 
            ...
            if ui_handler.refs["Visuals"]["notifications_console"].ref:get(3) then 
                client.log("[", colors.aa_info_color:get(), "Trident.lua",
    ...
    if event.name == "player_death" then
        ...
        if ui_handler.refs["Misc"]["killsay"].ref:get() and attacker == localplayer and not victim == nil then
            local kill_says = {
                "1'ed by Trident.lua",
                "nn " .. tostring(victim:get_name()) .. " deleted by Trident.lua",
                "You wouldnt have died with Trident.lua",
                "Thanks for increasing my kd", "Trident.lua - just the best",
                "Get good get Trident.lua",
                "Another kill for me, and another death for " ..
                    tostring(victim:get_name()), "I let Trident.lua carry me"
            }
         ...
         --             if anti_brute_total > 0 then
         --                 client.log("[", accent_color_color:get(), "Trident.lua",
...
if not ui_handler.refs["Visuals"]["notifications"].ref:get() then return end
    ...
    if shot.aim_damage > shot.damage then
        ...
        if ui_handler.refs["Visuals"]["notifications_console"].ref:get(1) then
            client.log(colors.white, "[", colors.hit_color:get(), "Trident.lua", colors.white, "]",
            colors.white, "Hit",
...
if not ui_handler.refs["Visuals"]["notifications"].ref:get() then return end
    ...
    local Name = entity:get_name()
    -- if lua_menu.visuals.logs_in:get(1) then
    --     notification_logs[#notification_logs + 1] = {"[", "Trident.lua","] " .. "Missed " .. 
    ...
    if ui_handler.refs["Visuals"]["notifications_console"].ref:get(2) then
        ...
        client.log(colors.white, "[", colors.miss_color:get(), "Trident.lua", colors.white, "]",
...
-- local tag_s = {
--     "t", "tr", "tri", "trid", "trident", "trident⛦", "trident⛦l", "trident⛦lu",
--     "trident⛦lua", "trident⛦lua", "trident⛦lua", "trident⛦lua", "trident⛦lu",
--     "trident⛦l", "trident⛦", "trident", "triden", "tride", "trid", "tri", "tr", "t"
-- }
local last_tag_iter
...
local water_func = function(localplayer, menu_vis, in_game, frametime) 
    ...
    local name_text = ui_handler.refs["Visuals"]["watermark_options"].ref:get(1) and "Primordial" or "Trident.lua " .. alpha_v_text .. " " .. version_text
...
anti_brute.brute_add = function(euid)
    if g_cont_time < global_vars.tick_count() and gothit == false then
        ...
        if ui_handler.refs["Visuals"]["notifications"].ref:get() then
            if ui_handler.refs["Visuals"]["notifications_console"].ref:get(3) then
                client.log(colors.white, "[", colors.aa_info_color:get(), "Trident.lua", colors.white, "]", colors.white, "Anti Aim switched due to brute force")
            end
...
add_notification("", "Welcome to Trident.lua" .. alpha_v_text .. " ".. forumInfo.name .. ". The current script version is " .. lua_version)
...
if trident_fs.directory_exists("/trident") then -- returns true if the directory exists and false if it doesn't
    if trident_fs.exists("trident\\load.txt") then
        client.log_screen(colors.white,"Trident.lua", colors.main[2]:get(), "[Configs]", colors.white, "Local config found, loading...")
        local cfg_string = trident_fs.read("trident\\load.txt")
        configs.load(cfg_string)
    end
else
    trident_fs.create_directory("/trident") -- creates the directory if it doesn't exist
end
menu.add_button("Configs", "Save local config", function()
    local config_code = configs.make()
    trident_fs.write("trident\\load.txt", config_code) 
    client.log_screen(colors.white,"Trident.lua", colors.main[2]:get(), "[Configs]", colors.white, "Local config saved!")
end)
...
menu.add_button("Configs", "Export config to clipboard", function()
    ...
    client.log_screen(colors.white,"Trident.lua", colors.main[2]:get(), "[Configs]", colors.white, "Config code exported to clipboard")
end)
