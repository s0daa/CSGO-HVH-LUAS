...
local watermarkValue = string.format("RotN BETA 2.017.1 | %s | %02d:%02d:%02d", "ezcracked", h, m, s)
...
local aboutScript = menu.add_text("RotN ", "Welcome back, ezcracked")
...
local function missLogs(shot)
    ...
    if missValue == true then
        PrintInChat('\x01[\x07RotN\x01] \x08Missed \x07' ..shot.player:get_name().. '\x08 due to \x03' ..shot.reason_string.. '\x08 with \x10' ..shot.backtrack_ticks.. '\x08 ms backtrack. Predicted damage \x0B' ..shot.aim_damage.. '\x08 and hitchance \x05' ..shot.aim_hitchance.. '\x08.')
...
local function hitLogs(shot)
    ...
    if hitValue == true then
        PrintInChat('\x01[\x10RotN\x01] \x08Hit \x07'..shot.player:get_name()..'\x08 in the \x10'..shot.hitgroup..'\x08 for \x0B'..shot.damage..'\08 damage with \x05'..shot.aim_hitchance..'\x08 hitchance while predicted damage \x03' ..shot.aim_damage.. ' \x08 and \x04'..shot.backtrack_ticks..'\x08 ms backtrack.')
	    print('\x01[\x10RotN\x01] \x08Hit \x07'..shot.player:get_name()..'\x08 in the \x10'..shot.hitgroup..'\x08 for \x0B'..shot.damage..'\08 damage with \x05'..shot.aim_hitchance..'\x08 hitchance while predicted damage \x03' ..shot.aim_damage.. ' \x08 and \x04'..shot.backtrack_ticks..'\x08 ms backtrack.')
    end
end
...
client.log_screen(color_t(255, 0, 0), "Welcome back, " .. "ezcracked")
client.log(color_t(255, 255, 255), "Join our discord server: idi naxyi nety ego")
client.log(color_t(255, 255, 255), "Here you can recieve support or suggest features")
