...
local fonts = {
    normal = render.create_font("Tahoma", 15, 150, e_font_flags.ANTIALIAS),
    bold = render.create_font("Tahoma Bold", 15, 550, e_font_flags.ANTIALIAS)
}
...
function drawLog()
    ...
    for i = 1, #logs do
        if logrender[i] then
            ...
            render.text(fontstyle:get() == 1 and fonts.normal or fonts.bold, logs[i], vec2_t(screen_size.x/2, screen_size.y/(1.25+i/45)), color_t(255, 250, 255, logtime[i]), true)
        end
    end
end
