...
local watermarkValue = string.format("Akatsuki.sus| %s | %02d:%02d:%02d", "Love Miss", h, m, s)
...
local aboutScript = menu.add_text("Akatsuki.sus ", "Welcome to Akatsuki.sus")
...
if enableCrosshairIndicators:get() and entity_list.get_local_player() ~= nil and entity_list.get_local_player():is_alive() and engine.is_in_game() then
    ...
    render.text(font, "Akatsuki", vec2_t(render.get_screen_size()[0] / 2 + moveOffset, render.get_screen_size()[1] / 1.89), color_t(255,255, 255, wa), true)
    render.text(fontCrosshairIndic1, "Miss", vec2_t(render.get_screen_size()[0] / 2 + moveOffset, render.get_screen_size()[1] / 1.85), color_t(255,255, 255, 255), true)
...
local function missLogs(shot)
    ...
    if missValue == true then
        PrintInChat('\x01[\x07Akatsuki.tech\x01] \x08Missed \x07' ..shot.player:get_name().. '\x08 due to \x03' ..shot.reason_string.. '\x08 with \x10' ..shot.backtrack_ticks.. '\x08 ms backtrack. Predicted damage \x0B' ..shot.aim_damage.. '\x08 and hitchance \x05' ..shot.aim_hitchance.. '\x08.')
...
local function hitLogs(shot)
    ...
    if hitValue == true then
        PrintInChat('\x01[\x10Akatsuki.tech\x01] \x08Hit \x07'..shot.player:get_name()..'\x08 in the \x10'..shot.hitgroup..'\x08 for \x0B'..shot.damage..'\08 damage with \x05'..shot.aim_hitchance..'\x08 hitchance while predicted damage \x03' ..shot.aim_damage.. ' \x08 and \x04'..shot.backtrack_ticks..'\x08 ms backtrack.')
	    print('\x01[\x10Akatsuki.tech\x01] \x08Hit \x07'..shot.player:get_name()..'\x08 in the \x10'..shot.hitgroup..'\x08 for \x0B'..shot.damage..'\08 damage with \x05'..shot.aim_hitchance..'\x08 hitchance while predicted damage \x03' ..shot.aim_damage.. ' \x08 and \x04'..shot.backtrack_ticks..'\x08 ms backtrack.')
    end
end
