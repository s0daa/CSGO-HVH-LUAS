print("-> Initializing Vendetta Live...")
print("-> ...")
print("-> ..")
print("-> .")
print(" _____________________________________")
print("| Vendetta Live Build Loaded          |")
print("| Version: 1.3                        |")
print("| Dev: dominic-#0001                  |")
print("| Get the latest version from:        |")
print("| https://discord.gg/PJD4akBd85       |")
print("|_____________________________________|")
print("-> Using an outdated version could heavily impact your Performance!")
...
local ui = {
    ...
    fpamount  = menu.add_slider("Vendetta | Ragebot", "Custom backtrack amount", 0, 1000),
    ...
    indicatorsmain = menu.add_selection("Vendetta | Visuals", "Indicators:", {"-", "Modern", "Classic", "Acatel"}),
    ...
    --misc
    clientside_animations = menu.add_multi_selection("Vendetta | Misc", "Clientside animations", {"Reversed legs", "Static legs in air", "Lean in-air", "Pitch 0 on land"}),
...
local function Visuals_indic()
    ...
    if ui.indicatorsmain:get() == 2 and local_player:is_alive() then
        ...
        render.text(pixel, "Yaw", vec2_t(x + offset_scope + 45, y+ay-15), color_t(255, 255, 255, 255), true, true)
        render.text(pixel, "ULTIMATE", vec2_t(x+offset_scope+25, y+ay-25),color_t(255, 255, 255, alpha), false, true)
        ay = ay + 10

        render.rect_fade(vec2_t(x + offset_scope + 0, y+ay-20), vec2_t(50, 4), color_t(118,161,219,255), color_t(20,33,49,255), true)
        render.rect(vec2_t(x + offset_scope + 0, y+ay-20), vec2_t(50, 5), color_t(0,0,0, 255))
        ...
        if exploits.get_charge() >= 1 then
            render.text(pixel, "Rapid", vec2_t(x + 25 + offset_scope, y+ay-10), color_t(255, 255, 255, 255), true, true)
            ay = ay + 9
        end
        if exploits.get_charge() < 1 then
            render.text(pixel, "Rapid", vec2_t(x + 25 + offset_scope, y+ay-10), color_t(255, 0, 0, 255), true, true)
            ay = ay + 9
        end
    end
        ...
        if primordial.isHS[2]:get() then
            render.text(pixel, "OS-AA", vec2_t(x + 25 + offset_scope, y+ay-10), color_t(250, 173, 181, 255), true, true)
            ay = ay + 10
        end
    
        if primordial.ping_spike:get() then
            render.text(pixel, "PING", vec2_t(x + 25 + offset_scope, y+ay-10), color_t(115, 206, 108, 255), true, true)
            ay = ay + 10
        end
    end
    ...
    if ui.indicatorsmain:get() == 3 and local_player:is_alive() then
        ...
        local eternal_ts = render.get_text_size(pixel, "Vendetta")
        render.text(pixel, "Vendetta", vec2_t(x + 13 + offset_scope, y+ay-25), color_t(255, 255, 255, 255), true, true)
        render.text(pixel, "Debug", vec2_t(x+eternal_ts.x-2 + 10 + offset_scope, y+ay-25),color_t(color_acc:get().r, color_acc:get().g, color_acc:get().b, alpha), false, true)
        ay = ay + 10
        ...
        render.text(pixel, current_state, vec2_t(x + offset_scope + 25, y+ay-25), clr0, true, true)
        ay = ay + 9
   
        local asadsa = math.min(math.floor(math.sin((exploits.get_charge()%2) * 1) * 122), 100)
        if primordial.isDT[2]:get() then
        if exploits.get_charge() >= 1 then
            render.text(pixel, "DT", vec2_t(x + offset_scope + 25, y+ay-25), color_t(0, 255, 0, 255), true, true)
            ay = ay + 9
        end
        if exploits.get_charge() < 1 then
            render.text(pixel, "DT", vec2_t(x + offset_scope + 25, y+ay-25), color_t(255, 0, 0, 255), true, true)
            ay = ay + 9
        end
        end
        ...
        local ax = 0
        if primordial.isHS[2]:get() then
            render.text(pixel, "OS", vec2_t(x + offset_scope + 25, y+ay-25), color_t(250, 173, 181, 255), true, true)
            ay = ay + 10
        end
    end
...
local function getweapon()
    ...
    render.text(pixel, "Vendetta", vec2_t(x/2+5, y/2+ay-25), color_t(255, 255, 255, 255), 10, true)
    render.text(pixel, "Debug", vec2_t(x/2+eternal_ts.x-16, y/2+ay-9), color_t(color_acc:get().r, color_acc:get().g, color_acc:get().b, alpha1), false, true)
    ay = ay + 10.5
    
        if isDT[2]:get() then
        if exploits.get_charge() >= 1 then
            render.text(pixel, "DT", vec2_t(x/2+5, y/2+ay-15), color_t(255, 255, 255, 255), 10, true)
            ay = ay + 10.5
        end
        if exploits.get_charge() < 1 then
            render.text(pixel, "DT", vec2_t(x/2+5, y/2+ay-15), color_t(255, 255, 255, 255), 10, true)
    ...
    render.text(pixel, "DMG", vec2_t(x/2+5, y/2+ay-15), color_t(255, 255, 255, 128), 10, true)

    render.text(pixel, "SP", vec2_t(x/2+23, y/2+ay-15), isSP[2]:get() == 2 and color_t(255, 255, 255, 255) or color_t(255, 255, 255, 128), 10, true)
    ax = ax + render.get_text_size(pixel, "SP ").x

    render.text(pixel, "BA", vec2_t(x/2+ax+20, y/2+ay-15), isBA[2]:get() == 5 and color_t(255, 255, 255, 255) or color_t(255, 255, 255, 128), 10, true)
end
end
end
...
local get_phrase = function()
    local phrases = {"Vendetta makes me luck boosted", "I wish I had Vendetta Yaw", "Choppa in my hand but allah is in the sand", "Vendetta > Pastenobyl", "Allah live, allah die. all you do is die. Choose allah", "hear that? sounds like copium", "timehacked back to narnia", "eat ass then touch grass - ghandi", "DUDE ALL YOU SHOOT IS ONSHOT!! NOONE LOVES YOU - geico 2021", "IM GAYYYYY!!! i think i just broke my hand :( - geico 2021", "VAC ATTACK! vac ban successful"}
