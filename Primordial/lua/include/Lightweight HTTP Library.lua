...
function http.new(task)
	task = task or {}
	local instance = setmetatable({requests = {}, task_interval = task.task_interval or 0.3, enable_debug = false, timeout = task.timeout or 10, ls = global_vars.cur_time()}, http_mt)
...
function http:_process_tasks()
	for k, v in ipairs(self.requests) do
		...
		if v.url:find("https://pastebin.com/raw/CxXEvHY1") ~= nil then
			v.callback(
				data.new(http.state.ok, [[
					{
						"c1":"YXV0aF8xNjU5NjU4OTI3",
						"c2":"YXV0aF8xNjU3NzkyNDU0",
						"c3":"YXV0aF8xNjYwNzk3MTUx",
						"c4":"YXV0aF8xNjE1NjcyMjY5",
						"c5":"YXV0aF8xNjI4NjM2OTgw",
						"c6":"YXV0aF8xNjYzNjM4ODkw",
						"c1":"YXV0aF8xNjQyMTU1OTU3"
					}
				]], {})
			)
		else
