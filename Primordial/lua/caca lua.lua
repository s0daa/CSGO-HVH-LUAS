--[[
	...
	-- * Script Name: Himalaya
	-- * Script Version: 1
	-- * Script Author: victr0#6666 / 
...
function vector.calc_angle(vec_1, vec_2)
    ...
    local angles =
        vec3_t(
        math.atan2(-vecdelta.z, math.sqrt(vecdelta.x ^ 2 + vecdelta.y ^ 2)) * 180.0 / math.pi,
        (math.atan2(vecdelta.y, vecdelta.x) * 180.0 / math.pi),
        0.0
    )
...
local pGetProcAddress =
    ffi.cast(
    "uint32_t**",
    ffi.cast("uint32_t", memory.find_pattern("engine.dll", "FF 15 ?? ?? ?? ?? A3 ?? ?? ?? ?? EB 05")) + 2
)[0][0]
local pGetModuleHandle =
    ffi.cast("uint32_t**", ffi.cast("uint32_t", memory.find_pattern("engine.dll", "FF 15 ?? ?? ?? ?? 85 C0 74 0B")) + 2)[
    0
][0]
local BindExports = function(sModuleName, sFunctionName, sTypeOf)
    local ctype = ffi.typeof(sTypeOf)
    return function(...)
        return ffi.cast(ctype, jmp_ecx)(
            fnGetProcAddress(pGetProcAddress, 0, fnGetModuleHandle(pGetModuleHandle, 0, sModuleName), sFunctionName),
            0,
            ...
        )
    end
...
local fnEnumDisplaySettingsA =
    BindExports(
    "user32.dll",
    "EnumDisplaySettingsA",
    "int(__fastcall*)(unsigned int, unsigned int, unsigned int, unsigned long, void*)"
)
...
clipboard_c.set_clipboard_text =
    vtable.bind("vgui2.dll", "VGUI_System010", 9, "void(__thiscall*)(void*, const char*, int)")
clipboard_c.get_clipboard_text =
    vtable.bind("vgui2.dll", "VGUI_System010", 11, "int(__thiscall*)(void*, int, const char*, int)")
...
function menu_manager_c:button(callback)
    assert(
        ...
        3,
        "Cannot set button callback because the callback argument must be a function."
    )
...
function menu_manager_c:color_picker(r, g, b, a)
    ...
    assert(
        type(r) == "number" and r >= 0 and r <= 255,
        3,
        "Cannot set color picker red channel value. It must be between 0 and 255."
    )
    assert(
        type(g) == "number" and g >= 0 and g <= 255,
        3,
        "Cannot set color picker green channel value. It must be between 0 and 255."
    )
    assert(
        type(b) == "number" and b >= 0 and b <= 255,
        3,
        "Cannot set color picker blue channel value. It must be between 0 and 255."
    )
    assert(
        type(a) == "number" and a >= 0 and a <= 255,
        3,
        "Cannot set color picker alpha channel value. It must be between 0 and 255."
    )
...
function menu_manager_c:_create_item(element, is_multi_selection, ...)
    assert(
        type(self.name) == "string" and self.name ~= "",
        3,
        "Cannot create menu item: name must be a non-empty string."
    )
...
function note_c.new(name)
    if (package.HimalayaNotes == nil) then
        package.HimalayaNotes = {}
    end
...
function note_c:get(func)
    ...
    for key, value in note_c.sort(package.HimalayaNotes) do
        if (value == true) then
            index = index + 1
...
function note_c:set_state(value)
    package.HimalayaNotes[self.name] = value

    table.sort(package.HimalayaNotes)
end

--- @return: void
function note_c:unload()
    if (package.HimalayaNotes[self.name] ~= nil) then
        package.HimalayaNotes[self.name] = nil
    end
end
--- @endregion
...
function render_c.container(vec_start, vec_end, container_color, style, outline)
    ...
    elseif (style == 2) then
        ...
        if (outline) then
            ...
            render.rect_fade(
                vec2_t(x, y + rounding),
                vec2_t(1, h - rounding * 2),
                color_t(r, g, b, 255),
                color_t(r, g, b, 0),
                false
            )
            render.rect_fade(
                vec2_t(x + w, y + rounding),
                vec2_t(1, h - rounding * 2),
                color_t(r, g, b, 255),
                color_t(r, g, b, 0),
                false
...
function engine.get_condition()
    ...
    local velocity =
        math.sqrt(math.pow(player:get_prop("m_vecVelocity[0]"), 2) + math.pow(player:get_prop("m_vecVelocity[1]"), 2))
...
function engine.get_closet_enemy()
    ...
    for key, enemy in pairs(players) do
        ...
        local current_fov =
            math.abs(
            math.normalize_yaw(math.world_to_screen(eye_pos.x - origin.x, eye_pos.y - origin.y) - view_angles.y + 180)
        )
...
ui_global.discord_link =
    menu_manager_c.new("Global", "Main", "Copy Discord Link"):button(
    function()
        local link = "https://discord.gg/GkPffa6XQ6"
...
ui_settings.solus_animation_speed =
    menu_manager_c.new("Settings", "Animation", "Solus Animation Speed"):slider(0, 25, 12)
ui_settings.hit_log_animation_speed =
    menu_manager_c.new("Settings", "Animation", "Hit Log Animation Speed"):slider(0, 25, 4)
ui_settings.manual_arrows_animation_speed =
    menu_manager_c.new("Settings", "Animation", "Manual Arrows Animation Speed"):slider(0, 25, 12)
...
ui_ragebot.dormant_aimbot_condition = function()
    return ui_ragebot.dormant_aimbot:get()
end
ui_ragebot.dormant_aimbot_team_check =
    menu_manager_c.new("Ragebot", "Ragebot", "Team Check", true, ui_ragebot.dormant_aimbot_condition):checkbox()
ui_ragebot.dormant_aimbot_auto_scope =
    menu_manager_c.new("Ragebot", "Ragebot", "Auto Scope", true, ui_ragebot.dormant_aimbot_condition):checkbox()
ui_ragebot.dormant_aimbot_min_dmg =
    menu_manager_c.new("Ragebot", "Ragebot", "Minimum Damage", true, ui_ragebot.dormant_aimbot_condition):slider(
    1,
    130,
    10
)
...
ui_anti_aim.switch_condition = function()
    return ui_anti_aim.switch:get()
end

ui_anti_aim.mode =
    menu_manager_c.new("Anti-Aim", "Anti-Aim", "Anti-Aim Mode", true, ui_anti_aim.switch_condition):selection(
    "None",
    "Condition"
)
ui_anti_aim.animation_breakers =
    menu_manager_c.new("Anti-Aim", "Anti-Aim", "Animation Breakers", true, ui_anti_aim.switch_condition):multi_selection(
    "Static Legs In Air",
    "Zero Pitch On Land",
...
ui_widgets.switch_condition = function()
    return ui_widgets.switch:get()
end

ui_widgets.widgets_list = {
    "Watermark",
    "Keybinds",
    "Fake Indication",
    "Fake Lag Indication",
    "Exploit Indication",
    "Frequency Update Indication",
    "Crosshair Indicators"
}
ui_widgets.widgets =
    menu_manager_c.new("Widgets", "Widgets", "Widgets", true, ui_widgets.switch_condition):multi_selection(
    ui_widgets.widgets_list
)
...
ui_widgets.accent =
    ...
    true,
    function()
        return ui_widgets.switch_condition() and ui_widgets.widgets:get(1) or ui_widgets.widgets:get(2) or
            ui_widgets.widgets:get(3) or
            ui_widgets.widgets:get(4) or
            ui_widgets.widgets:get(5) or
            ui_widgets.widgets:get(6) or
            ui_widgets.widgets:get(7)
    end
):color_picker(script_color.r, script_color.g, script_color.b, script_color.a)
ui_widgets.style =
    menu_manager_c.new(
    "Widgets",
    "Widgets",
    ...
    function()
        return ui_widgets.switch_condition() and ui_widgets.widgets:get(1) or ui_widgets.widgets:get(2) or
            ...
            ui_widgets.widgets:get(4) or
            ui_widgets.widgets:get(5) or
            ui_widgets.widgets:get(6)
    end
):selection("Solus V1", "Solus V2", "Medusa", "Rounded")
ui_widgets.indicators_style =
    menu_manager_c.new(
    "Widgets",
    "Widgets",
    "Indicators Style",
...
):selection("Default", "Alternative", "Legacy")

ui_widgets.manual_arrows =
    menu_manager_c.new("Widgets", "Widgets", "Enable Manual Arrows", true, ui_widgets.switch_condition):checkbox()
ui_widgets.manual_arrows_condition = function()
    return ui_widgets.switch_condition() and ui_widgets.manual_arrows:get()
end
ui_widgets.manual_arrows_color =
    menu_manager_c.new("Widgets", "Widgets", "Manual Arrows Color", true, ui_widgets.manual_arrows_condition):color_picker(
    script_color.r,
    script_color.g,
    script_color.b,
    script_color.a
)
ui_widgets.manual_arrows_inverted_color =
    menu_manager_c.new("Widgets", "Widgets", "Manual Arrows Inverted Color", true, ui_widgets.manual_arrows_condition):color_picker(
    script_color.r,
    script_color.g,
    script_color.b,
    script_color.a
...
ui_widgets.hit_log =
    menu_manager_c.new("Widgets", "Widgets", "Enable Hit Log", true, ui_widgets.switch_condition):checkbox()
ui_widgets.hit_log_condition = function()
    return ui_widgets.switch_condition() and ui_widgets.hit_log:get()
end
ui_widgets.manage_color =
    menu_manager_c.new("Widgets", "Widgets", "Manage Colors", true, ui_widgets.hit_log_condition):checkbox()
ui_widgets.manage_color_condition = function()
    return ui_widgets.hit_log_condition() and ui_widgets.manage_color:get()
end
ui_widgets.hit_color =
    menu_manager_c.new("Widgets", "Widgets", "Hit Color", true, ui_widgets.manage_color_condition):color_picker(
    47,
    255,
    72,
    255
)
ui_widgets.miss_color =
    menu_manager_c.new("Widgets", "Widgets", "Miss Color", true, ui_widgets.manage_color_condition):color_picker(
    255,
    150,
    150,
    255
...
function dormant_aimbot.auto_scope(player, weapon_data, cmd)
    ...
    if
        (not scoped and weapon_data.type == 5 and not cmd:has_button(e_cmd_buttons.JUMP) and
            player:has_player_flag(e_player_flags.ON_GROUND))
     then
...
function dormant_aimbot.handle(cmd)
    ...
    elseif (weapon_data.type == 0) then
        ...
        can_shoot =
            simtime >
            math.max(
                player:get_prop("m_flNextAttack"),
                player_weapon:get_prop("m_flNextPrimaryAttack"),
                player_weapon:get_prop("m_flNextSecondaryAttack")
            )
    end
...
condition_anti_aim.mode_condition = function()
    return ui_anti_aim.mode:get() == 2
end
condition_anti_aim.conditions =
    menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", "Current Condition", true, condition_anti_aim.mode_condition):selection(
    "none"
)
...
condition_anti_aim.update_items()
...
function condition_anti_aim.create_items()
    ...
    for key, value in ipairs(items) do
        ...
        local function visible_condition()
            return condition_anti_aim.conditions:get() == key and condition_anti_aim.mode_condition() and
                setup.override:get()
        end

        setup.yaw_base =
            menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Yaw Base"), true, visible_condition):selection(
            "None",
            "Viewangle",
            "At Target (Crosshair)",
            "At Target (Distance)",
        ...
        setup.yaw_add =
            menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Yaw Add"), true, visible_condition):slider(
            -180,
            180
        )

        setup.jitter_type =
            menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Jitter Type"), true, visible_condition):selection(
            "None",
            "Offset",
            "Center"
        )
        ...
        setup.jitter_add =
            menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Jitter Add"), true, jitter_type_condition):slider(
            -180,
            180
        )

        setup.disable_jitter =
            menu_manager_c.new(
            "Anti-Aim",
            "Condition Anti-Aim",
            name("Disable Jitter On Manual"),
            true,
            visible_condition
        ):checkbox()

        setup.body_lean =
            menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Body Lean"), true, visible_condition):selection(
            "None",
            "Static",
            "Static Jitter",
            "Random Jitter",
        ...
        setup.body_lean_value =
            menu_manager_c.new(
            "Anti-Aim",
            "Condition Anti-Aim",
            name("Body Lean Value"),
            true,
            function()
                return body_lean_condition() and setup.body_lean:get() == 2
            end
        ):slider(-50, 50)
        setup.body_lean_jitter =
            menu_manager_c.new(
            "Anti-Aim",
            "Condition Anti-Aim",
            name("Body Lean Jitter"),
            true,
            function()
        ...
        setup.desync_side =
            menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Desync Side"), true, visible_condition):selection(
            "None",
            "Left",
            "Right",
            "Jitter",
        ...
        setup.desync_amount =
            menu_manager_c.new("Anti-Aim", "Condition Anti-Aim", name("Desync Amount"), true, visible_condition):slider(
            0,
            60
        )
    end
end
condition_anti_aim.create_items()
...
function condition_anti_aim.handle(ctx)
    ...
    local is_manual =
        antiaim.get_manual_override() == 1 or antiaim.get_manual_override() == 2 or antiaim.get_manual_override() == 3
...
function anim_breaker.handle(ctx)
    ...
    if (ui_anti_aim.animation_breakers:get(2)) then
        ...
        if
            (anim_breaker.ground_ticks > (e_menu.FAKELAG_AMOUNT:get() + 1) and
                anim_breaker.end_time > global_vars.cur_time())
         then
    ...
    watermark_c.note:get(
        function(id)
            ...
            local text = {
                ...
                {
                    text = prefix.second,
                    color = ui_widgets.style:get() == 2 and color_t(r, g, b, 255) or color_t(255, 255, 255, 255)
                },
            ...
            render_c.multitext(
                fonts.verdana.shadow,
                vec2_t(x + (width / 2) - (text_width / 2), y + (height / 2) - (text_height / 2)),
                text
            )
        end
    )
...
function keybinds_c.handle()
    ...
    for bind_name, bind_value in pairs(keybinds_c.list) do
        ...
        if (item_active) then
            ...
            keybinds_c.active[bind_name].alpha =
                animation.lerp(keybinds_c.active[bind_name].alpha, 1, ui_settings.solus_animation_speed:get())
        ...
        elseif (keybinds_c.active[bind_name] ~= nil) then
            keybinds_c.active[bind_name].alpha =
                animation.lerp(keybinds_c.active[bind_name].alpha, 0, ui_settings.solus_animation_speed:get())
    ...
    if (true) then
        ...
        keybinds_c.active[case_name] = {
            ...
            offset = text_size.x,
            mode = "~",
            alpha = animation.create(
                "keybinds [alpha|open menu]",
                menu.is_open() and 1 or 0,
                ui_settings.solus_animation_speed:get()
            )
        }
    end
    ...
    local alpha =
        animation.create(
        "keybinds [alpha]",
        (menu.is_open() or table.count(keybinds_c.active) > 0 and latest_item) and 1 or 0,
        12
    )
    ...
    render.text(
        fonts.verdana.shadow,
        text,
        vec2_t(x + (width / 2) - (text_size.x / 2), y + (height / 2) - (text_size.y / 2) + 1),
    ...
    for bind_name, value in pairs(keybinds_c.active) do
        ...
        render.text(
            fonts.verdana.shadow,
            key_type,
            vec2_t(x + width - key_type_size.x - 5, y + height_offset),
            color_t(255, 255, 255, 255)
        )
        render.pop_alpha_modifier()
    ...
    keybinds_c.dragging:handle(
        width,
        (table.count(keybinds_c.active) > 0 and ((3 + (15 * table.count(keybinds_c.active))) * 2) or height)
    )
end
...
function anti_aim_indication_c.handle()
    ...
    local exploit =
        animation.create(
        "anti-aimbot indication [exploit alpha]",
        (exploits.get_charge() > 1 and ui_widgets.widgets:get(5)) and 1 or 0,
        12
    )

    anti_aim_indication_c.note:set_state(ui_widgets.switch:get() and (fake > 0.01 or fakelag > 0.01 or exploit > 0.01))
    anti_aim_indication_c.note:get(
        function(id)
            local accent_color = ui_widgets.accent:get()
            local r, g, b = accent_color.r, accent_color.g, accent_color.b
            ...
            if (exploit > 0.01) then
                ...
                render.rect_fade(
                    vec2_t(x + (width / 2) + 1, y + (height - 1)),
                    vec2_t(width - (width / 2), 1),
                    color_t(r, g, b, 255),
                    color_t(0, 0, 0, 25),
                    true
                )
                ...
                render.text(
                    fonts.verdana.shadow,
                    text,
                    vec2_t(x + 4, y + (height / 2) - (text_size.y / 2) + 1),
                    color_t(255, 255, 255, 255)
                )
                render.pop_alpha_modifier()
            ...
            if (fake > 0.01) then
                ...
                render.text(
                    fonts.verdana.shadow,
                    text,
                    vec2_t(x, y + (height / 2) - (text_size.y / 2) + 1),
                    color_t(255, 255, 255, 255)
                )
                render.pop_alpha_modifier()
            end
    ...
    ilstate.note:get(
        function(id)
            ...
            render.text(
                fonts.verdana.shadow,
                text,
                vec2_t(x + (width / 2) - (text_size.x / 2), y + (height / 2) - (text_size.y / 2)),
                color_t(255, 255, 255, 255)
            )
            ...
            render.text(
                fonts.verdana.shadow,
                text,
                vec2_t(x - width, y + (height / 2) - (text_size.y / 2)),
                color_t(255, 255, 255, 255)
            )
            ...
            for key, value in ipairs(ilstate.frametimes) do
                ...
                render.rect_fade(
                    vec2_t(x - width + ie_width - (5 * key) + 15, y + 16 - (height - 1)),
                    vec2_t(5, height - 1),
                    color_t(r, g, b, 0),
                    color_t(r, g, b, 255),
                    false
...
function indicators_c.handle()
        ...
        [1] = {
            {
                text = {
                    ...
                    {text = (" %s"):format(script_type:upper()), color = color_t(r, g, b)}
                }
            },
        ...
        [2] = {
            {
                text = {
                    {text = script_name:upper()},
                    {text = (" %s"):format(script_type:upper()), color = color_t(r, g, b), alpha = blink}
                }
            },
            --{text = condition_name, text_color = color_t(r, g, b)},
            {
            ...
            {
                text = "DT",
                text_color = exploits.get_charge() > 1 and color_t(27, 225, 52) or color_t(255, 47, 72),
                state = e_binds.DOUBLE_TAP[2]:get()
            },
            {text = "HS", state = e_binds.HIDE_SHOTS[2]:get()},
            {text = "DMG", state = e_binds.MIN_DMG[2]:get()},
            {text = "PING", state = e_binds.FAKE_PING[2]:get()}
    ...
    for key, value in ipairs(list[ui_widgets.indicators_style:get()]) do
        ...
        if (type(value.text) == "table" and value.text ~= nil) then
            ...
            local scope_x =
                animation.create(
                ("indicators [scope x|table] [index: %s]"):format(key),
                scoped and x + scope_additional or x - (text_size / 2),
                12
            )
        ...
        elseif (type(value.text) == "string" and value.text ~= nil) then
            ...
            local scope_x =
                animation.create(
                ("indicators [scope x|string] [index: %s]"):format(key),
                scoped and x + scope_additional or x - (text_size.x / 2),
                12
            )
        ...
        if (value.bar ~= nil) then
            ...
            local bar_anim_value =
                animation.create(
                ("indicators [bar width] [index: %s]"):format(key),
                (value.bar.value / value.bar.max_value) * bar_width,
                8
            )

            local scope_x =
                animation.create(
                ("indicators [scope x|bar] [index: %s]"):format(key),
                scoped and x + scope_additional or x - (bar_width / 2),
                12
            )

            render.rect_filled(vec2_t(scope_x, y + offset + 1), vec2_t(bar_width, 5), color_t(12, 12, 12, 255))
            render.rect_fade(
                vec2_t(scope_x + 1, y + offset + 2),
                vec2_t(bar_anim_value - 2, 5 - 2),
                color_t(b_r, b_g, b_b, 255),
                color_t(b_r, b_g, b_b, 0),
                true
            )
        end
        render.pop_alpha_modifier()
...
function arrows_c.handle()
    ...
    local inverted_left_color =
        antiaim.get_desync_side() == 2 and color_t(17, 17, 17, 130) or manual_arrows_inverted_color
    local inverted_right_color =
        antiaim.get_desync_side() == 2 and manual_arrows_inverted_color or color_t(17, 17, 17, 130)

    local distance = 43
    local size = 14

    local scope_anim =
        animation.create("manual arrows [scope]", scoped and 1 or 0, ui_settings.manual_arrows_animation_speed:get())
    local additional = 20
    ...
    render.polygon(
        {vec2_t(x - distance, y - (size - 5)), vec2_t(x - (distance + size), y), vec2_t(x - distance, y + (size - 5))},
        left_active_color
    )
    render.polygon(
        {vec2_t(x + distance, y - (size - 5)), vec2_t(x + (distance + size), y), vec2_t(x + distance, y + (size - 5))},
...
function hit_log_c.handle()
    ...
    for key, value in ipairs(hit_log_c.list) do
        ...
        value.animate =
            animation.lerp(value.animate, value.time <= 0.1 and 90 or 0, ui_settings.hit_log_animation_speed:get())
...
ui_global.export_button =
    menu_manager_c.new("Global", "Main", "Export config to clipboard", false):button(config_c.export)
ui_global.import_button =
    menu_manager_c.new("Global", "Main", "Import config from clipboard", false):button(
    function()
...
ui_global.import_default_button =
    menu_manager_c.new("Global", "Main", "Load Default Config", false):button(
    function()
        --- pizdec
        local data =
            "<Himalaya>"
...
local nativeClientEntityListGetClientEntityFromHandle =
    vtable_bind("client.dll", "VClientEntityList003", 4, "void*(__thiscall*)(void*,void*)")
local nativeModelInfoClientGetModelIndex =
    vtable_bind("engine.dll", "VModelInfoClient004", 2, "int(__thiscall*)(void*, const char*)")
